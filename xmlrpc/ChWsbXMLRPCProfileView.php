...
require_once( CH_DIRECTORY_PATH_BASE . 'scripts/ChBaseProfileGenerator.php' );

define('CH_BLOCK_GENERALINFO', 17);
define('CH_BLOCK_ADDITIONALINFO', 20);

class ChWsbXMLRPCProfileView extends ChBaseProfileGenerator
{
    var $_iViewerId;

    function __construct($iProfileId, $iViewerId = 0)
    {
        ChBaseProfileGenerator::__construct ((int)$iProfileId);
    ...
    function getProfileInfoExtra()
    {
        ...
        $r = db_res ("SELECT `pc`.`Caption`, `pc`.`Content`, `pc`.`Func`, `pc`.`ID` AS `BlockID`
            ...
            ON ((`pc`.`Func` = 'PFBlock' AND `pc`.`Content` = `pf`.`ID`) OR (`pc`.`Func` = 'GeneralInfo' AND " . CH_BLOCK_GENERALINFO . " = `pf`.`ID`) OR (`pc`.`Func` = 'AdditionalInfo' AND " . CH_BLOCK_ADDITIONALINFO . " = `pf`.`ID`))
        ...
        while ($a = $r->fetch()) {
            ...
            switch ($a['Func']) {
                case 'GeneralInfo': $i = CH_BLOCK_GENERALINFO; break;
                case 'AdditionalInfo': $i = CH_BLOCK_ADDITIONALINFO; break;
                default: $i = $a['Content'];
            }
