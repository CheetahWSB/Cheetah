...
class ChWsbXMLRPCUtil
{
    function getContacts($sUser, $sPwd)
    {
        ...
        $oZ = new ChWsbAlerts('mobile', 'contacts', $iId, 0, array('contacts_data' => $aAll));
        $oZ->alert();
    ...
    public static function getUserInfo($iId, $iIdViewer = 0, $isCountData = true)
    {
        ...
        $aRet['user_location'] = new xmlrpcval(ChWsbXMLRPCUtil::getUserLocation($aSexSql) . ' ');
        ...
        $aRet['thumb'] = new xmlrpcval(ChWsbXMLRPCUtil::getThumbLink($iId));
        ...
        if ($isCountData) {
            ...
            $oChWsbXMLRPCMedia = new ChWsbXMLRPCMedia;
            $aRet['countPhotos'] = new xmlrpcval($oChWsbXMLRPCMedia->_getMediaCount('photo', $iId, $iIdViewer));
            $aRet['countVideos'] = new xmlrpcval($oChWsbXMLRPCMedia->_getMediaCount('video', $iId, $iIdViewer));
            $aRet['countSounds'] = new xmlrpcval($oChWsbXMLRPCMedia->_getMediaCount('music', $iId, $iIdViewer));
        }

        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('mobile', 'user_info', $iId, $iIdViewer, array('profile' => &$aSexSql, 'return_data' => &$aRet));
        $oZ->alert();
    ...
    public static function fillProfileArray ($a, $sImage = 'icon', $iIdViewer = 0)
    {
        ...
        $sImage = ChWsbXMLRPCUtil::getThumbLink($a['ID'], $sImage);
        ...
        $aRet = array (
               ...
               'user_location' => new xmlrpcval(ChWsbXMLRPCUtil::getUserLocation($a) . ' '),
               ...
               'CountPhotos' => new xmlrpcval($oChWsbXMLRPCMedia->_getMediaCount('photo', $iId, $iIdViewer)),
               'CountVideos' => new xmlrpcval($oChWsbXMLRPCMedia->_getMediaCount('video', $iId, $iIdViewer)),
               'CountSounds' => new xmlrpcval($oChWsbXMLRPCMedia->_getMediaCount('music', $iId, $iIdViewer)),
               'CountFriends' => new xmlrpcval(getFriendNumber($a['ID'])),
               $sImageKey => new xmlrpcval($sImage),
            );

        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('mobile', 'user_info2', $a['ID'], $iIdViewer, array('profile' => &$a, 'return_data' => &$aRet));
        $oZ->alert();
    ...
    public static function getMenu ($sMenu, $aMarkersReplace = array ())
    {
        $aDefaultMarkers = array (
            ...
            'xmlrpc_url' => CH_WSB_URL_ROOT . 'xmlrpc/',
        );
        ...
        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('mobile', 'menu', 0, 0, array('menu' => $sMenu, 'data' => &$aRecords, 'markers_replace' => &$aMarkersReplace));
        $oZ->alert();
    ...
    {
        $sUser = process_db_input($sUser, CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION);
        return (int)db_value("SELECT `ID` FROM `Profiles` WHERE `NickName` = '$sUser' LIMIT 1");
    }

    public static function checkLogin ($sUser, $sPwd)
    {
        $iId = (int)ChWsbXMLRPCUtil::getIdByNickname ($sUser);
        $aProfile = getProfileInfo((int)$iId);
        if (!$aProfile || getParam('enable_cheetah_footer') == 'on')
            return 0;
        ...
        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('mobile', 'check_login', $iId, 0, array('password' => $sPwd, 'return_data' => &$iRet));
        $oZ->alert();
    ...
    public static function setLanguage ($sLang)
    {
        ...
        global $LANG;
        require_once( CH_DIRECTORY_PATH_ROOT . "langs/lang-{$sCurrentLanguage}.php" );
    }
    ...
    function service ($sModuleUri, $sMethod, $aParams = array(), $sClass = 'Module')
    {
        ...
        $mixed = ChWsbService::call($sModuleUri, $sMethod, $aParams, $sClass);
