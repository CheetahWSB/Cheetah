...
require_once(CH_DIRECTORY_PATH_MODULES . 'cheetah/videos/classes/ChVideosSearch.php');
...
function video_getList($sId)
{
    ...
    for($i=0; $i<count($aData); $i++) {
        ...
        $sResult .= parseXml($aXmlTemplates['file'], $sThumbFile, $aData[$i]['size'], $aData[$i]['ownerName'], $aData[$i]['view'], $aData[$i]['voting_rate'], $aData[$i]['date'], $aData[$i]['title'], CH_WSB_URL_ROOT . $aData[$i]['uri']);
    }
    return $sResult;
}
