...
$_page['name_index'] = ch_get('fullpage') == 1 ? 7 : 44;
...
function getMainCode()
{
    ...
    if (false != ch_get('ConfCode') && false != ch_get('sendto')) {
        $recipientID = (int)ch_get('sendto');
    } else {
        //check post value
...
function MemberSendVKiss($member, $recipient, $isCheckVisitorGreeting = true)
{
    ...
    $sKissLink = $sender
        ? '<a href="' . CH_WSB_URL_ROOT . 'greet.php?fullpage=1&sendto=' . $member['ID'] . '&from=' . $recipient['ID'] . '&ConfCode=' . $sConfCode . '">' . CH_WSB_URL_ROOT . 'greet.php?sendto=' . $member['ID'] . '&from=' . $recipient['ID'] . '&ConfCode=' . $sConfCode . '</a>'
        : '<a href="' . CH_WSB_URL_ROOT . 'communicator.php">' . CH_WSB_URL_ROOT . 'communicator.php</a>';
    ...
    $aTemplate['Subject'] = process_db_input($aTemplate['Subject'], CH_TAGS_NO_ACTION);
    $aTemplate['Body']    = process_db_input($aTemplate['Body'], CH_TAGS_NO_ACTION);
	...
	$oAlert = new ChWsbAlerts('greeting', 'add', 0, $member['ID'], array('Recipient' => $recipient['ID']));
	$oAlert->alert();
