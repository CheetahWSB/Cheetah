...
require_once(CH_DIRECTORY_PATH_CLASSES . 'ChWsbSearch.php');
require_once(CH_DIRECTORY_PATH_CLASSES . 'ChWsbPaginate.php');

class ChBaseSearchResult extends ChWsbSearchResult
{
    var $aPermalinks;
    var $aConstants;
    ...
    function displaySearchBox ($sCode, $sPaginate = '', $bAdminBox = false)
    {
        ...
        if (isset($this->aCurrent['rss']) && $this->aCurrent['rss']['link']) {
            ...
            $sMenu = ChWsbPageView::getBlockCaptionItemCode(time(), array(_t('RSS') => array('href' => $this->aCurrent['rss']['link'] . (false === strpos($this->aCurrent['rss']['link'], '?') ? '?' : '&') . 'rss=1', 'icon' => 'rss')));
        }
    ...
    public static function showAdminFilterPanel($sFilterValue, $sInputId = 'filter_input_id', $sCheckboxId = 'filter_checkbox_id', $sFilterName = 'filter', $sOnApply = '')
    {
        ...
        $sFilterValue = ch_html_attribute($sFilterValue);
        ...
        $sContent = <<<EOF
                <table>
                    <tr>
                        <td>{$sFilter}</td>
                        <td>
                            <div class="input_wrapper input_wrapper_text ch-def-round-corners-with-border">
                                <input type="text" id="{$sInputId}" value="{$sFilterValue}" class="form_input_text ch-def-font" onkeypress="return on_filter_key_up(event, '{$sCheckboxId}')" />
                            </div>
                        </td>
