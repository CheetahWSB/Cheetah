 ...
 * @see ChWsbVoting
 */
class ChBaseVotingView extends ChWsbVoting
{
    ...
    function getVoting($sType, $iObjectId = 0, $aParams = array())
    {
        ...
        $sJsObject = ch_gen_method_name($sHtmlId);
        ...
        if ($iCanRate) {
            ...
            $sRet .= $GLOBALS['oSysTemplate']->_wrapInTagJsCode('var ' . $sJsObject . ' = new ChWsbVoting(' . json_encode($aJsParams) . ');');
        }
    ...
    function getExtraJs ()
    {
        $GLOBALS['oSysTemplate']->addJs('ChWsbVoting.js');
    }
