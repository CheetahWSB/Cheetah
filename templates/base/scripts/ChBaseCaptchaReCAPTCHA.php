 ...
 * @see ChWsbCaptcha
 */
class ChBaseCaptchaReCAPTCHA extends ChWsbCaptcha
{
    ...
    public function display ($bDynamicMode = false)
    {
        ...
        $sCodeJs = $this->_oTemplate->addJs(ch_append_url_params($this->_sApiUrl, $aApiParams), $bDynamicMode);
    ...
    {
    	$mixedResponce = ch_file_get_contents($this->_sVerifyUrl, array(
    		'secret' => $this->_sKeyPrivate,
    		'response' => process_pass_data(ch_get('g-recaptcha-response')),
    ...
    {
        return process_pass_data(ch_get('g-recaptcha-response'));
    }
