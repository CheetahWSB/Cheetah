...
require_once( CH_DIRECTORY_PATH_CLASSES . 'ChWsbForm.php' );

class ChBaseFormView extends ChWsbForm
{
    ...
    function genTable()
    {
        ...
        if($GLOBALS['MySQL']->getParam('sys_security_form_token_enable') == 'on' && !defined('CH_WSB_CRON_EXECUTE') && (!isset($this->aParams['csrf']['disable']) || (isset($this->aParams['csrf']['disable']) && $this->aParams['csrf']['disable'] !== true)) && ($mixedCsrfToken = ChWsbForm::getCsrfToken()) !== false)
    ...
    function genRowStandard(&$aInput)
    {
        ...
        $sRequired  = !empty($aInput['required']) ? '<span class="ch-form-required">*</span>' : '';
        $sClassAdd = '';
        ...
        if($bMultiplyable)
        	$sElementClass .= ' ch-form-element-multiplyable';

        if(!empty($aInput['error']))
        	$sElementClass .= ' ch-form-element-error';
		...
		$sCode .= $GLOBALS['oSysTemplate']->parseHtmlByName('form_row_standard_' . (!empty($aInput['colspan']) ? 'colspan' : 'simple') . '.html', array(
			'element_type' => 'ch-form-element-' . $aInput['type'],
    ...
    function genInputStandard(&$aInput)
    {
        ...
        // add default className to attributes
        $aAttrs['class'] = "form_input_{$aInput['type']} ch-def-font-inputs" . (isset($aAttrs['class']) ? (' ' . $aAttrs['class']) : '');
    ...
    function genInputButton(&$aInput)
    {
        ...
        // add default className to attributes
        $aAttrs['class'] = "form_input_{$aInput['type']} ch-btn" . (isset($aAttrs['class']) ? (' ' . $aAttrs['class']) : '');
        $aAttrs['type']  = $aInput['type'];
    ...
    function genInputTextarea(&$aInput)
    {
        ...
        // add default className to attributes
        $aAttrs['class'] =
            "form_input_{$aInput['type']} ch-def-font-inputs" .
    ...
    function getOpenTbody($aAttrsAdd = false)
    {
		...
		$aAttrsAdd['class'] = 'ch-form-fields-wrapper' . (!empty($aAttrsAdd['class']) ? ' ' . $aAttrsAdd['class'] : '');
    ...
    function getInput($sType, $sAttrs, $sContent = '')
    {
    	...
    	switch($sType) {
    		case 'input':
    			...
    			break;
    		case 'file':
    			$sResult = '<label class="ch-btn form_input_multiply_select"><input ' . $sAttrs . ' /><span>' . _t('_Select file') . '</span></label>';
    			break;
    		case 'button':
    			$sResult = '<input ' . $sAttrs . ' />';
    			break;
