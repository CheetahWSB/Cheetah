...
ch_import('ChWsbBrowse');
ch_import('ChWsbPaginate');
ch_import('ChTemplSearchProfile');

class ChBaseBrowse extends ChWsbBrowse
{
    ...
    function __construct( &$aFilteredSettings, &$aDisplaySettings, $sPageName )
    {
        ...
        (
            'sex'         => process_db_input($aFilteredSettings['sex'], CH_TAGS_STRIP),
            'age'         => process_db_input($aFilteredSettings['age'], CH_TAGS_STRIP),
            'country'     => process_db_input($aFilteredSettings['country'], CH_TAGS_STRIP),
        );

        $this -> aAdditionalParameters = array
        (
            'photos_only' => process_db_input($aFilteredSettings['photos_only'], CH_TAGS_STRIP),
            'online_only' => process_db_input($aFilteredSettings['online_only'], CH_TAGS_STRIP),
        );
        ...
        $this -> oSearchProfileTmpl = new ChTemplSearchProfile();
    }
    ...
    function getBlockCode_SettingsBlock()
    {
        ...
        */
        ...
        $sContent =
        '
            <div class="main_settings ch-def-bc-margin">
                <div class="devider"></div>
                ' . $sSexSection . '
                ' . $sAgeSection . '
                ' . $sCountrySection . '
    ...
    */
    function genLinkLocation( $sType, $sTypeValue )
    {
        ...
        $sLocation = ($this->bPermalinkMode) ? CH_WSB_URL_ROOT . 'browse' . $sLocation : 'browse.php?' . $sLocation;
    ...
    */
    function getBlockCode_SearchedMembersBlock()
    {
        ...
        if ( $this -> bPermalinkMode ) {
            ...
            $sRequest = CH_WSB_URL_ROOT . 'browse.php?';
        ...
        $sOutputHtml = $GLOBALS['oSysTemplate'] -> parseHtmlByName( $aUsedTemplates[0], array(
            ...
            'ch_if:show_sim_css' => array (
                'condition' => $this->aDisplaySettings['mode'] != 'extended',
                'content' => array()
            ),
            'ch_if:show_ext_css' => array (
                'condition' => $this->aDisplaySettings['mode'] == 'extended',
                'content' => array()
            ),
