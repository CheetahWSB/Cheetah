...
class ChBaseIndexPageView extends ChWsbPageView
{
    function __construct()
    {
        ChWsbPageView::__construct( 'index' );
    }
    ...
    {
        $oCategories = new ChTemplCategories();
    ...
    function getMembers ($sBlockName, $aParams = array(), $iLimit = 16, $sMode = 'last')
    {
        ...
        foreach( $aModes as $sMyMode ) {
            ...
            $aDBTopMenu[$sModeTitle] = array('href' => CH_WSB_URL_ROOT . "index.php?{$sBlockName}Mode=$sMyMode", 'dynamic' => true, 'active' => ( $sMyMode == $sMode ));
        }
        ...
        if ($iCount) {
            ...
            $sCode = $GLOBALS['oSysTemplate']->parseHtmlByName('members_list.html', array(
                'ch_repeat:list' => $aTmplVars
            ));

            if ($iPages > 1) {
                $oPaginate = new ChWsbPaginate(array(
                    'page_url' => CH_WSB_URL_ROOT . 'index.php',
                    'count' => $iCount,
                    'per_page' => $iLimit,
                    'page' => $iPage,
                ...
                $sPaginate = $oPaginate->getSimplePaginate(CH_WSB_URL_ROOT . $sViewAllUrl . $sViewAllQuery);
            }
        } else {
