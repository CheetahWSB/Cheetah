 ...
 */
    ...
    class ChBaseMailBox extends ChWsbMailBox
    {
        ...
        function getBlockCode_ComposeMessage()
        {
            ...
            $aTemplateKeys = array
            (
                'plugins_dir'        => CH_WSB_URL_PLUGINS,
            ...
            if ($oEditor) {
                ...
                $sEditor = $oEditor->attachEditor ('#compose_message', CH_EDITOR_FULL);
            }
        ...
        function getBlockCode_ViewMessage()
        {
            ...
            "
                SELECT
                    *,
                    DATE_FORMAT(`Date`, '" . getLocaleFormat(CH_WSB_LOCALE_DATE, CH_WSB_LOCALE_DB) . "') AS `Date`
                FROM
                    `sys_messages`
                WHERE
                    `ID` = {$this -> aMailBoxSettings['messageID']}
                    ...
                    case 'inbox' :
                            ...
                            'inputs' => array(
                                'actions' => array(
                                    ...
                                    array (
                                        ...
                                        'attrs'     => array('onclick' => 'if ( typeof oMailBoxViewMessage != \'undefined\') oMailBoxViewMessage.deleteMessages(' . $aRow['ID'] . ')', 'class' => 'ch-btn-small'),
                                    ),
                                    array (
                                        'type'      => 'button',
                                        'value'     => $aLanguageKeys['spam_message'],
                                        'attrs'     => array('onclick' => 'if ( typeof oMailBoxViewMessage != \'undefined\') oMailBoxViewMessage.spamMessages(\'\', ' . $aRow['Sender'] . ')', 'class' => 'ch-btn-small'),
                                    ),
                                    array (
                                        'type'      => 'button',
                                        'value'     => $aLanguageKeys['mark_read'],
                                        'attrs'     => array('onclick' => 'if ( typeof oMailBoxViewMessage != \'undefined\') oMailBoxViewMessage.markMessages(\'read\', ' . $aRow['ID'] . ')', 'class' => 'ch-btn-small'),
                                    ),
                                    array (
                                        'type'      => 'button',
                                        'value'     => $aLanguageKeys['mark_unread'],
                                        'attrs'     => array('onclick' => 'if ( typeof oMailBoxViewMessage != \'undefined\') oMailBoxViewMessage.markMessages(\'unread\', ' . $aRow['ID'] . ')', 'class' => 'ch-btn-small'),
                                    )
                                )
                            )
                        );
                        ...
                        if($aRow['Sender'] != $this -> aMailBoxSettings['member_id']){
                            $aForm['inputs']['actions'][0] = array(
                                ...
                                'attrs'     => array('onclick' => 'oMailBoxViewMessage.replyMessage(' . $this -> aMailBoxSettings['messageID'] . ', ' . $aRow['Sender'] . ');', 'class' => 'ch-btn-small'),
                            );
                        }
                        ...
                        'inputs' => array(
                            'actions' => array(
                                ...
                                1 => array (
                                    ...
                                    'attrs'     => array('onclick' => 'if (typeof oMailBoxViewMessage != \'undefined\') oMailBoxViewMessage.restoreMessages(' . $aRow['ID'] . ');', 'class' => 'ch-btn-small'),
                                ),
                            )
                        )
                    );
        ...
        function getBlockCode_NewMessages($bShowEmpty = true)
        {
            ...
            foreach($aMessages as $aMessage) {
                $aTmplVars['ch_repeat:messages'][] = array(
            ...
            if ( method_exists($this, $sFunctionName) ) {
                ...
                if ( is_array($aMessagesList) and !empty($aMessagesList) ) {
                    ...
                    (
                        ...
                        'ch_repeat:messages' => $aMessageKeys,
                    );
        ...
        function _genMessagesRows()
        {
            ...
            if ( is_array($aMessages) and !empty($aMessages) ) {
                    ...
                    foreach( $aMessages as $iKey => $aItems ) {
                        ...
                        $aMessageRows[] = array
                         (
                            ...
                            'message_contact_url' => CH_WSB_URL_ROOT . "mail.php?mode=compose&recipient_id=" . $aProfileInfo['ID'],
                ...
                $sRequest = CH_WSB_URL_ROOT . 'mail.php?';
                                    ...
                                    'inputs' => array(
                                        'actions' => array(
                                            ...
                                            array (
                                                ...
                                                'attrs'     => array('onclick' => 'if ( typeof oMailBoxMessages != \'undefined\' ) oMailBoxMessages.deleteMessages(\'messages_container\', \'genMessagesRows\')', 'class' => 'ch-btn-small'),
                                            ),
                                            array (
                                                'type'      => 'button',
                                                'value'     => $aLanguageKeys['spam'],
                                                'attrs'     => array('onclick' => 'if (typeof oMailBoxMessages != \'undefined\') oMailBoxMessages.spamMessages(\'messages_container\')', 'class' => 'ch-btn-small'),
                                            ),
                                            array (
                                                'type'      => 'button',
                                                'value'     => $aLanguageKeys['mark_read'],
                                                'attrs'     => array('onclick' => 'if ( typeof oMailBoxMessages != \'undefined\' ) oMailBoxMessages.markMessages(\'read\', \'genMessagesRows\')', 'class' => 'ch-btn-small'),
                                            ),
                                            array (
                                                'type'      => 'button',
                                                'value'     => $aLanguageKeys['mark_unread'],
                                                'attrs'     => array('onclick' => 'if ( typeof oMailBoxMessages != \'undefined\' ) oMailBoxMessages.markMessages(\'unread\', \'genMessagesRows\')', 'class' => 'ch-btn-small'),
                                            )
                                        )
                                    )
                                );
                                    ...
                                    'inputs' => array(
                                        'actions' => array(
                                            ...
                                            0 => array (
                                                ...
                                                'attrs'     => array('onclick' => 'if ( typeof oMailBoxMessages != \'undefined\' ) oMailBoxMessages.restoreMessages(\'messages_container\', \'genMessagesRows\')', 'class' => 'ch-btn-small'),
                                            ),
                                            1 => array (
                                                'type'      => 'button',
                                                'value'     => $aLanguageKeys['delete'],
                                                'attrs'     => array('onclick' => 'if ( typeof oMailBoxMessages != \'undefined\' ) oMailBoxMessages.hideDeletedMessages(\'messages_container\', \'genMessagesRows\')', 'class' => 'ch-btn-small'),
                                            ),
                                        ),
                                    )
                                );
        ...
        function genReplayMessage($iRecipientID, $iMessageID = 0)
        {
            ...
            (
                ...
                'cancel'        => ch_js_string( _t('_Cancel') ),
                'send'          => ch_js_string( _t('_Send') ),
                'send_copy'     => _t( '_Send copy to personal email', $aRecipientInfo['NickName'] ),
                'send_copy_my'  => _t( '_Send copy to my personal email' ),
                'notify'        => _t( '_Notify by e-mail', $aRecipientInfo['NickName'] ),
                'error_message' => ch_js_string( _t('_please_fill_next_fields_first') ),
            );
            ...
            if ($oEditor) {
                $oEditor->setCustomConf ("remove_script_host: false,\nrelative_urls: false,\n");
                $sEditor = $oEditor->attachEditor ('#compose_message', CH_EDITOR_FULL, $this->bAjaxMode);
            }
        ...
        */
        ...
        function getAutoCompleteList($sQuery, $iLimit = 10 )
        {
            ...
            // init some needed variables ;
            $iLimit = (int) $iLimit;
            $sQuery = process_db_input($sQuery, CH_TAGS_STRIP);
        ...
        */
        static function get_member_menu_messages_list( $iMemberId = 0 )
        {
            ...
            $iInboxCount = ChWsbMailBox::getCountInboxMessages($iMemberId);

            // get count of sent messages ;
            $iSentCount = ChWsbMailBox::getCountSentMessages($iMemberId);

            // get count of trashed messages ;
            $iTrashCount = ChWsbMailBox::getCountTrashedMessages($iMemberId);
