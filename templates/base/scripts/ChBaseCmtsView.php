 ...
 * @see ChWsbCmts
 */
class ChBaseCmtsView extends ChWsbCmts
{
    var $_oPaginate;
    var $_sStylePrefix;
    ...
    {
        ChWsbCmts::__construct( $sSystem, $iId, $iInit );
        if (empty($sSystem) || !$this->_aSystem)
            return;

        $this->_sJsObjName = 'oCmts' . ucfirst($sSystem) . $iId;
        $this->_oPaginate = new ChWsbPaginate(array(
    ...
    function getCommentsFirst ()
    {
        ...
        return $GLOBALS['oSysTemplate']->parseHtmlByName('cmts_main.html', array(
            ...
            'ch_if:show_paginate' => array(
                'condition' => $sPaginate !== "",
                'content' => array(
                    'content' => $sPaginate
                )
            ),
            'ch_if:show_post' => array(
    ...
    function getComments ($iCmtsParentId = 0, $sCmtOrder = 'asc', $iStart = 0, $iPerPage = -1)
    {
        ...
        if (!$aCmts)
            ...
            foreach ($aCmts as $k => $r) {
                ...
                $sRet .= '<table class="cmt-balloon ch-def-round-corners-with-border">';
    ...
    function getExtraJs ()
    {
        $GLOBALS['oSysTemplate']->addJs('ChWsbCmts.js');
    ...
    function getCmtsInit ()
    {
        ...
        return $GLOBALS['oSysTemplate']->_wrapInTagJsCode("var " . $this->_sJsObjName . " = new ChWsbCmts(" . json_encode($aParams) . ");");
    }
    ...
    function _getCommentHeadBox (&$a)
    {
        ...
        $sRet = '<tr class="cmt-head ch-def-font-small"><td class="' . $this->_sStylePrefix . '-head-l">&nbsp;</td><td class="' . $this->_sStylePrefix . '-head-m">' . $sAuthor . ':</td><td class="' . $this->_sStylePrefix . '-head-r">&nbsp;</td></tr>';
    ...
    function _getActionsBox(&$a, $isJustPosted)
    {
        ...
        $sRet = '<tr id="cmt-jp-'.$a['cmt_id'].'" class="cmt-foot ch-def-font-small"><td class="' . $this->_sStylePrefix . '-cont-l">&nbsp;</td><td class="' . $this->_sStylePrefix . '-cont-m">';
    ...
    function _getFormBox($sType, $aCmt = array())
    {
		...
		$sContent = '
			<form name="cmt-post-reply" onsubmit="' . $this->_sJsObjName . '.' . $sFunction . '; return false;">
				...
				<div class="cmt-post-reply-text">
					<textarea class="cmt-text-' . $sType . ' ch-def-round-corners-with-border" id="' . $sTextareaId . '" name="CmtText">' . $sCmtText . '</textarea>
				</div>
				...
				<div class="cmt-post-reply-post"><button class="ch-btn ch-btn-small" type="submit">' . _t('_Submit Comment') . '</button></div>
				' . $sMood . '
			</form>';
		...
		if($this->iGlobAllowHtml == 1) {
            ...
            $sContent .= $oEditor ? $oEditor->attachEditor ('#' . $sTextareaId, CH_EDITOR_MINI, $this->bDynamic) : '';
        }
    ...
    function _getAuthorIcon ($a)
    {
        ...
        if (!$a['cmt_author_id'] || !getProfileInfo($a['cmt_author_id'])) {
            if (!@include_once (CH_DIRECTORY_PATH_MODULES . 'cheetah/avatar/include.php'))
                return '';
            return '<div class="thumbnail_block thumbnail_block_icon" style="float:none;"><div class="thumbnail_image"><img class="thumbnail_image_file ch-def-thumbnail ch-def-shadow ch-img-retina" src="' . $oFunctions->getSexPic('', 'small') . '" /></div></div>';
        } else {
