...
class ChBaseMenu extends ChWsbMenu
{
	var $bGroupInMore;
...
EOF;
        ...
        if ($this->sCustomSubHeader == '' && !empty($this->aMenuInfo['profileID'])) {
            ...
            if($this->aMenuInfo['memberID'] == $this->aMenuInfo['profileID']) {
                $aTmplVars = array(
                    'ch_if:show_script' => array(
                        'condition' => true,
                        'content' => array()
                    ),
                    'ch_if:show_when' => array(
                        'condition' => false && $sProfStatusMessage != '',
                        'content' => array(
                            'when' => $sProfStatusMessageWhen
                        )
                    ),
                    'ch_if:show_update' => array(
                        'condition' => true,
                        'content' => array()
                    ),
        ...
        $aTemplateKeys = array (
            ...
            'ch_if:show_submenu' => array(
                'condition' => $sProfStatusMessage == '' && $sSubmenu != '',
                'content' => array(
                    'submenu' => $sSubmenu
                )
            ),
            'ch_if:show_empty' => array(
                'condition' => $sProfStatusMessage == '' && $sSubmenu == '',
                'content' => array(
                    'content' => ''
                )
            ),
            'ch_if:show_submenu_bottom' => array(
                'condition' => $sProfStatusMessage != '' && $sSubmenu != '',
                'content' => array(
                    'submenu' => $sSubmenu
                )
...
<td class="top">
    <a href="javascript: void(0);" onclick="void(0);" class="top_link">
        <span class="down ch-def-padding-sec-leftright">{$sMoreMainCaption}</span>
        <!--[if gte IE 7]><!--></a><!--<![endif]-->
...
EOF;
    ...
    function genBreadcrumb($aPath = array())
    {
        ...
        $sDivider = '<div class="bc_divider ch-def-margin-sec-left">&#8250;</div>';
        $aPathLinks = array();
        foreach($aPath as $sLink)
            $aPathLinks[] = '<div class="bc_unit ch-def-margin-sec-left">' . $sLink . '</div>';
        $sPathLinks = implode($sDivider, $aPathLinks);

        //--- Get additional links(right side) ---//
        $sAddons = "";
        return '<div class="sys_bc ch-def-margin-leftright">' . $sPathLinks . '<div class="bc_addons">' . $sAddons . '</div></div>';
    }
    ...
    function getUrlProfileMessage($iId)
    {
        ...
        return CH_WSB_URL_ROOT . 'mail.php?mode=compose&recipient_id=' . $iId;
    }
    ...
    function getUrlAccountPage($iId)
    {
        ...
        return CH_WSB_URL_ROOT . 'member.php';
    }
