...
ch_import('ChBaseSearchResult');
ch_import('ChWsbAlbums');
ch_import('ChTemplVotingView');

class ChBaseSearchResultSharedMedia extends ChBaseSearchResult
{
	var $bDynamic = false;
    ...
    function __construct ($sModuleClass = '')
    {
        ...
        */
        ...
        $this->oModule = ChWsbModule::getInstance($this->sModuleClass);
        $this->oTemplate = $GLOBALS['oSysTemplate'];

        $sClassName = $this->oModule->_oConfig->getClassPrefix() . 'Privacy';
        ch_import('Privacy', $this->oModule->_aModule);
    ...
    function getCurrentUrl ($sType, $iId = 0, $sUri = '', $aOwner = '')
    {
        ...
        return CH_WSB_URL_ROOT . $this->oModule->_oConfig->getBaseUri() . str_replace('{uri}', $sUri, $sLink);
    }
    ...
    function getSearchUnit ($aData)
    {
        ...
        if (!is_null($this->oRate) && $this->oRate->isEnabled()) {
            if ($this->sTemplUnit == 'browse_unit')
                ...
                $oRate = new ChTemplVotingView($this->aCurrent['name'], $aData['id']);
    ...
    function getTopMenu ($aExclude = array())
    {
        ...
        foreach (array('last', 'top') as $sMyMode) {
         ...
         if (basename( $_SERVER['PHP_SELF'] ) == 'rewrite_name.php' || basename( $_SERVER['PHP_SELF'] ) == 'profile.php')
                $sLink = CH_WSB_URL_ROOT . "profile.php?ID={$this->aCurrent['restriction']['owner']['value']}&";
            else
    ...
    function getLatestFile ()
    {
                ...
                switch ($sKey) {
                    ...
                    case 'tag':
                        if (strlen($aValue['value']) > 0)
                            $aWhere[] = "MATCH(`{$this->aCurrent['table']}`.`{$aValue['field']}`) AGAINST ('" . trim(process_db_input($aValue['value'], CH_TAGS_STRIP)) . "')";
                        break;
                    case 'allow_view':
        ...
        if ($iFileId != 0) {
            ...
            $oRate = new ChTemplVotingView($this->aCurrent['name'], $aInfo['medID']);
            $aDraw = array(
    ...
    function addCustomParts ()
    {
        if (!$this->bCustomParts) {
            ...
            $this->oTemplate->addLocation($this->aCurrent['name'], CH_DIRECTORY_PATH_MODULES . $sModulePart, CH_WSB_URL_MODULES . $sModulePart);
    ...
    {
        return 'cheetah/'.$this->aCurrent['name'];
    }
    ...
    {
        $oSet = new ChWsbAlbums($this->aCurrent['name']);
    ...
    function getAlbumsBlock ($aSectionParams = array(), $aAlbumParams = array(), $aCustom = array())
    {
        $aCustomTmpl = array(
            ...
            'menu_bottom' => '',
            'paginate_url' => '',
            'simple_paginate_url' => CH_WSB_URL_ROOT . $this->oModule->_oConfig->getUri() . '/albums/browse',
    ...
    function displayAlbumUnit ($aData, $bCheckPrivacy = true)
    {
        ...
        $aUnit['ch_repeat:units'] = array();
        for($i = 0; $i < $iPics; $i++)
            $aUnit['ch_repeat:units'][] = $this->_getAlbumUnitItem($i, array_shift($aPics), array('album_url' => $sLink));
    ...
    {
        return CH_WSB_URL_ROOT . $this->oModule->_oConfig->getBaseUri() . 'view/' . $a['uri'];
    }
    ...
    function serviceGetWallPostComment($aEvent, $aParams = array())
    {
        ...
        $oCmts = new ChTemplCmtsView($this->oModule->_oConfig->getMainPrefix(), $iId);
        if(!$oCmts->isEnabled())
            return '';
        ...
        $sTextAddedNew = _t('_ch_' . $sUri . '_wall_added_new_comment');
