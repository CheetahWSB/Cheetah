...
require_once(CH_DIRECTORY_PATH_ROOT . 'templates/tmpl_'.$GLOBALS['tmpl'].'/scripts/ChTemplFormView.php');

class ChBaseAlbumForm
{
    var $iOwnerId;
    var $sType;
    var $iAlbumId;
    ...
    function __construct ($sType, $iAlbum = 0)
    {
        ...
        $oPrivacy = new ChWsbPrivacy('sys_albums', 'ID', 'Owner');
        ...
        $iAlbum = (int)$iAlbum;
        if ($this->iAlbumId > 0) {
            $oAlbum = new ChWsbAlbums($this->sType);
    ...
    {
        $oForm = new ChTemplFormView($this->aForm);
        $oForm->initChecker();
