...
ch_import('ChWsbPrivacyView');
ch_import('ChWsbPrivacySearch');
ch_import('ChTemplFormView');

class ChBasePrivacyView extends ChWsbPrivacyView
{
    /**
     * constructor
     */
    ...
    function getBlockCode_AddMembers()
    {
         $aForm = array(
            'form_attrs' => array(
                ...
                'action' => CH_WSB_URL_ROOT . 'member_privacy.php',
                'method' => 'post'
            ),
            'params' => array (
                'db' => array(),
        ...
        $sControl = ChWsbPrivacySearch::showAdminActionsPanel('ps-add-member-form', $aButtons, 'ps-add-member-ids');

        return PopupBox('ps-add-members', _t('_ps_bcpt_add_members'), $this->parseHtmlByName('ps_group_members.html', array(
            'wnd_action' => 'add',
            'ch_if:search_form' => array(
                'condition' => true,
                'content' => array(
                    'form' => $oForm->getCode(),
                )
            ),
            'js_site_url' => CH_WSB_URL_ROOT,
            'results' => '',
            'control' => $sControl,
    ...
    function getBlockCode_DeleteMembers()
    {
        ...
        $sControl = ChWsbPrivacySearch::showAdminActionsPanel('ps-del-member-form', $aButtons, 'ps-del-member-ids');
        ...
        foreach($aActions as $aAction) {
            if($sModuleUri != $aAction['module_uri']) {
                ...
                $aForm['inputs'][$aAction['module_uri'] . '_begin'] = array(
                    'type' => 'block_header',
                    'caption' => ChWsbModule::getTitle($aAction['module_uri']),
                    'collapsable' => true,
    ...
    function getBlockCode_MyGroups()
    {
        ...
        foreach($aGroups as $aGroup) {
            if(!empty($aGroup['parent_id'])) {
                ...
                $sExtend .= !empty($aParentGroup['home_url']) ? '<a href="' . CH_WSB_URL_ROOT . $aParentGroup['home_url'] . '" target="_blank">' . $sTitle . '</a>' : $sTitle;
            }
            $aContent[] = array(
                'group_id' => $aGroup['id'],
                'group_title' => $aGroup['title'],
                'group_members' => $aGroup['members_count'],
                'ch_if:extended' => array(
        ...
        $sControl = ChWsbPrivacySearch::showAdminActionsPanel('ps-my-groups-form', $aButtons, 'ps-my-groups-ids');

        $sContent = $this->parseHtmlByName('ps_my_groups.html', array(
            'ch_repeat:groups' => !empty($aContent) ? $aContent : MsgBox(_t('_Empty')),
            'control' => $sControl
        ));
