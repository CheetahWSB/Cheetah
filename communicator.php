 ...
 */
    ...
    require_once( CH_DIRECTORY_PATH_ROOT . 'templates/tmpl_' . $tmpl . '/scripts/ChTemplCommunicator.php');
    ...
    $aCommunicatorSettings = array (
        ...
        // page mode ;
        'communicator_mode' => ( false !== ch_get('communicator_mode') )
            ? ch_get('communicator_mode')
            : '',

        // switch the person mode - from me or to me ;
        'person_switcher' => ( false !== ch_get('person_switcher') )
            ? ch_get('person_switcher')
            : 'to',

        // type of message's sort ;
        'sorting' => ( false !== ch_get('sorting'))
            ?  ch_get('sorting')
            : 'date_desc',

        // contain number of current page ;
        'page'	=> ( false !== ch_get('page'))
            ? (int) ch_get('page')
            : 1,

        // contain per page number for current page ;
        'per_page' => ( false !== ch_get('per_page'))
            ? (int) ch_get('per_page')
            : 10,

        // contain number of current alert's page ;
        'alert_page'  => ( false !== ch_get('alert_page'))
            ? (int)  ch_get('alert_page')
            : 1,
    );

    // create ChTemplCommunicator object ;
