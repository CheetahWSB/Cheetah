...
$aForm = array(
    'form_attrs' => array(
        'id'       => 'invite_friend',
        'name'     => 'invite_friend',
        'action'   => CH_WSB_URL_ROOT . 'tellfriend.php',
        'method'   => 'post',
...
function SendTellFriend($iSenderID = 0)
{
    ...
    $sSenderEmail = clear_xss(ch_get('sender_email'));
    if(strlen(trim($sSenderEmail)) <= 0)
        return 0;

	$sSenderName = strmaxtextlen(ch_get('sender_name'), 16);
    $sSenderLink = $iSenderID != 0 ? getProfileLink($iSenderID) : CH_WSB_URL_ROOT;

    $sRecipientEmail = clear_xss(ch_get('recipient_email'));
