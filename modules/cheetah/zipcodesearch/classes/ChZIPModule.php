...
class ChZIPModule extends ChWsbModule
{
    ...
    {
        if (!getParam('ch_zip_enabled'))
            return false;

        $sWhere = '';

        // check input fields
        $sZip = process_db_input( strtoupper( trim($sZip) ), CH_TAGS_STRIP);
        $sMetric = process_db_input( $sMetric, CH_TAGS_STRIP);
        $iDistance = (int)$iDistance;
        if (is_array($mixedCountry)) {
            $sCountry = process_db_input( $mixedCountry[0], CH_TAGS_STRIP); // no reason to search by zipcode in different countries
        } else {
            $sCountry = process_db_input( $mixedCountry, CH_TAGS_STRIP);
        }
        ...
        if ('Google' == getParam('ch_zip_mode') && $this->_oDb->isModule('wmap')) {
            if ($sMetric == 'km')
                $iDistance *= 0.62;
            ...
            do {
                ...
                if (!class_exists('RadiusAssistant'))
                    require_once( CH_DIRECTORY_PATH_INC . 'RadiusAssistant.inc.php' );
                ...
                $sWhere = " `Profiles`.`Country` = '$sCountry' AND `ch_wmap_locations`.`country` = '$sCountry' AND `ch_wmap_locations`.`failed` = 0 AND `ch_wmap_locations`.`lat` >= {$minLat} AND `ch_wmap_locations`.`lat` <= {$maxLat} AND `ch_wmap_locations`.`lng` >= {$minLong} AND `ch_wmap_locations`.`lng` <= {$maxLong} ";
                $sJoin .= " INNER JOIN `ch_wmap_locations` ON (`ch_wmap_locations`.`part` = 'profiles' AND  `ch_wmap_locations`.`id` = `Profiles`.`ID`) ";
        ...
        // search using geonames
            ...
            do {
                $s = $this->_readFromUrl($this->_server . "/findNearbyPostalCodes?postalcode={$sZip}&country={$sCountry}&radius={$iDistance}&style={$this->_style}&maxRows={$this->_maxRows}&username=" . trim(getParam('ch_zip_geonames_username')));
                if (!$s) {
    ...
    function actionAdministration ()
    {
        ...
        if(empty($iId)) {
            ...
            $this->_oTemplate->pageCodeAdmin (_t('_ch_zip_administration'));
            return;
        }
        ...
        echo $this->_oTemplate->adminBlock ($this->_oTemplate->parseHtmlByName('default_padding', $aVars), _t('_ch_zip_administration'));

        $aVars = array (
            'content' => _t('_ch_zip_help_text'),
        );
        echo $this->_oTemplate->adminBlock ($this->_oTemplate->parseHtmlByName('default_padding', $aVars), _t('_ch_zip_help'));
