...
CREATE TABLE `ch_forum_user_activity` (
  `user` varchar(32) NOT NULL default '',
...
CREATE TABLE `ch_forum_user_stat` (
  `user` varchar(32) NOT NULL default '',
...
CREATE TABLE `ch_forum_actions_log` (
  `user_name` varchar(32) NOT NULL default '',
...
CREATE TABLE `ch_forum_attachments` (
...
CREATE TABLE IF NOT EXISTS `ch_forum_signatures` (
...
INSERT INTO `ch_forum` (`forum_id`, `forum_uri`, `cat_id`, `forum_title`, `forum_desc`, `forum_posts`, `forum_topics`, `forum_last`, `forum_type`, `forum_order`) VALUES
(1, 'General-discussions', 1, 'General discussions', 'General discussions', 0, 0, 0, 'public', 2);

INSERT INTO `ch_forum_cat` (`cat_id`, `cat_uri`, `cat_name`, `cat_icon`, `cat_order`, `cat_expanded`) VALUES
(1, 'General', 'General', '', 2, 0);


-- stat member
INSERT INTO `sys_stat_member` VALUES('mop', 'SELECT COUNT(*) FROM `ch_forum_post` WHERE `user` = ''__member_nick__''');
INSERT INTO `sys_stat_member` VALUES('mot', 'SELECT COUNT(*) FROM `ch_forum_topic` WHERE `first_post_user` = ''__member_nick__''');

-- stat site
SET @iStatSiteOrder := (SELECT `StatOrder` + 1 FROM `sys_stat_site` WHERE 1 ORDER BY `StatOrder` DESC LIMIT 1);
INSERT INTO `sys_stat_site` VALUES(NULL, 'tps', 'ch_forum_discussions', 'forum/', 'SELECT IF( NOT ISNULL( SUM(`forum_topics`)), SUM(`forum_posts`), 0) AS `Num` FROM `ch_forum`', '', '', 'comments', @iStatSiteOrder);

-- pages
INSERT INTO `sys_page_compose` (`Page`, `PageWidth`, `Desc`, `Caption`, `Column`, `Order`, `Func`, `Content`, `DesignBox`, `ColWidth`, `Visible`, `MinWidth`) VALUES
('index', '1140px', 'Forum Posts', '_ch_forum_forum_posts', 0, 0, 'RSS', '{SiteUrl}forum/?action=rss_all#4', 1, 71.9, 'non,memb', 0),
('member', '1140px', 'Forum Posts', '_ch_forum_forum_posts', 0, 0, 'RSS', '{SiteUrl}forum/?action=rss_user&user={NickName}#4', 1, 71.9, 'non,memb', 0),
('profile', '1140px', 'Last posts of a member in the forum', '_ch_forum_forum_posts', 0, 0, 'RSS', '{SiteUrl}forum/?action=rss_user&user={NickName}#4', 1, 71.9, 'non,memb', 0);
...
INSERT INTO `sys_page_compose` (`Page`, `PageWidth`, `Desc`, `Caption`, `Column`, `Order`, `Func`, `Content`, `DesignBox`, `ColWidth`, `Visible`, `MinWidth`) VALUES
('forums_index', '1140px', 'Full Index', '_ch_forums_index', 1, 0, 'FullIndex', '', 0, 100, 'non,memb', 0);

SET @iMaxOrder = (SELECT `Order` + 1 FROM `sys_page_compose_pages` ORDER BY `Order` DESC LIMIT 1);
INSERT INTO `sys_page_compose_pages` (`Name`, `Title`, `Order`) VALUES ('forums_home', 'Forums Home', @iMaxOrder);
INSERT INTO `sys_page_compose` (`Page`, `PageWidth`, `Desc`, `Caption`, `Column`, `Order`, `Func`, `Content`, `DesignBox`, `ColWidth`, `Visible`, `MinWidth`) VALUES
('forums_home', '1140px', 'Quick Search', '_ch_forums_quick_search', 2, 0, 'Search', '', 1, 28.1, 'non,memb', 0),
('forums_home', '1140px', 'Short Index', '_ch_forums_index', 2, 1, 'ShortIndex', '', 1, 28.1, 'non,memb', 0),
('forums_home', '1140px', 'Recent Topics', '_ch_forums_recent_topics', 1, 0, 'RecentTopics', '', 0, 71.9, 'non,memb', 0);

-- actions
INSERT INTO `sys_objects_actions` (`Caption`, `Icon`, `Url`, `Script`, `Eval`, `Order`, `Type`) VALUES
('{evalResult}', 'plus', 'javascript:void(0);', 'return f.newTopic(''0'')', 'return $GLOBALS[''logged''][''member''] || $GLOBALS[''logged''][''admin''] ? _t(''_ch_forums_new_topic'') : '''';', '1', 'ch_forum_title');
...
(NULL, 0, 'Forums', '_ch_forums', 'forum/', @iMaxMenuOrder, 'non,memb', '', '', '', 1, 1, 1, 'top', 'comments', 'comments', 1, '');
SET @iId = (SELECT LAST_INSERT_ID());

INSERT INTO `sys_menu_top`(`ID`, `Parent`, `Name`, `Caption`, `Link`, `Order`, `Visible`, `Target`, `Onclick`, `Check`, `Editable`, `Deletable`, `Active`, `Type`, `Picture`, `Icon`, `BQuickLink`, `Statistics`) VALUES
(NULL, @iId, 'Forums Home', '_ch_forum_menu_home', 'forum/', 1, 'non,memb', '', '', '', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Forums Index', '_ch_forum_menu_forum_index', 'forum/?action=goto&index=1', 2, 'non,memb', '', '', '', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Flagged Topics', '_ch_forum_menu_flagged_topics', 'forum/?action=goto&my_flags=1', 5, 'memb', '', '', '', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'My Topics', '_ch_forum_menu_my_topics', 'forum/?action=goto&my_threads=1', 6, 'memb', '', '', '', 1, 1, 1, 'custom', 'comments', 'comments', 1, ''),
(NULL, @iId, 'Spy', '_ch_forum_menu_spy', 'forum/?action=live_tracker', 7, 'non,memb', '', '', '', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Forum Search', '_ch_forum_menu_search', 'forum/?action=goto&search=1', 8, 'non,memb', '', '', '', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Manage Forum', '_ch_forum_menu_manage_forum', 'forum/?action=goto&manage_forum=1', 20, 'memb', '', '', 'return isAdmin();', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Reported Posts', '_ch_forum_menu_reported_posts', 'forum/?action=goto&reported_posts=1', 22, 'memb', '', '', 'return isAdmin();', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Hidden Posts', '_ch_forum_menu_hidden_posts', 'forum/?action=goto&hidden_posts=1', 24, 'memb', '', '', 'return isAdmin();', 1, 1, 1, 'custom', '', '', 0, ''),
(NULL, @iId, 'Hidden Topics', '_ch_forum_menu_hidden_topics', 'forum/?action=goto&hidden_topics=1', 26, 'memb', '', '', 'return isAdmin();', 1, 1, 1, 'custom', '', '', 0, '');
...
INSERT INTO `sys_menu_member` SET `Name` = 'ch_forum', `Eval` = '$oMemberMenu = ch_instance(''ChWsbMemberMenu''); $a = array(''item_img_src'' => ''comments'', ''item_link'' => CH_WSB_URL_ROOT . ''forum/#action=goto&new_topic=0'', ''item_title'' => _t(''_ch_forum_forum_topic'')); return $oMemberMenu->getGetExtraMenuLink($a);', `Position`='top_extra', `Type` = 'linked_item', `Parent` = IFNULL(@iMemberMenuParent, 0), `Order` = IFNULL(@iMemberMenuOrder, 1);

-- menu admin
SET @iMax = (SELECT MAX(`order`) FROM `sys_menu_admin` WHERE `parent_id` = '2');
INSERT IGNORE INTO `sys_menu_admin` (`parent_id`, `name`, `title`, `url`, `description`, `icon`, `order`) VALUES
(2, 'ch_forum', '_ch_forum', '{siteUrl}forum/?action=goto&manage_forum=1', 'Forum Administration Panel', 'comments', @iMax+1);

-- stat member
INSERT INTO `sys_account_custom_stat_elements` VALUES (NULL, '_ch_forums', '__mop__ (<a href="__site_url__forum/">__l_add__</a>)');
...
('ch_forum_profile', 'ChForumProfileResponse', 'modules/cheetah/forum/profile_response.php');
...
('ch_forum_notifier', 'New Post In: <TopicTitle>', '<ch_include_auto:_email_header.html />\r\n\r\n    <p>Hello <Recipient>,</p> \r\n    <p><a href="<PosterUrl>"><PosterNickName></a> has posted a new reply in "<TopicTitle>" topic:</p> \r\n    <hr> <ReplyText> <hr> \r\n    \r\n <ch_include_auto:_email_footer.html />', 'Notification about new post in flagged topic', 0);
...
('ch_forum', '_ch_forums_sitemap', '0.8', 'auto', 'ChForumSiteMaps', 'modules/cheetah/forum/classes/ChForumSiteMaps.php', @iMaxOrderSiteMaps, 1);

-- chart
SET @iMaxOrderCharts = (SELECT MAX(`order`)+1 FROM `sys_objects_charts`);
INSERT INTO `sys_objects_charts` (`object`, `title`, `table`, `field_date_ts`, `field_date_dt`, `query`, `active`, `order`) VALUES
('ch_forum', '_ch_forum_forum_posts', 'ch_forum_post', 'when', '', '', 1, @iMaxOrderCharts);
