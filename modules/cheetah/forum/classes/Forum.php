        ...
        switch ($type) {
            case 'tlts':
                foreach ($a as $r) {
                    ...
                    $r['date'] = ch_html_attribute(orca_format_date($r['date']));
...
EOF;
        ...
        $textEncoded = ch_js_string($text, CH_ESCAPE_STR_APOS);
        $userEncoded = ch_js_string($u, CH_ESCAPE_STR_APOS);
...
EOF;
    ...
    function postReplyXML (&$p)
    {
        ...
        if (!$this->_checkUserPerm ('', $f['forum_type'], 'post', (int)$p['forum_id']) || $t['topic_locked'] || ch_is_spam($p['topic_text'])) {
            return <<<EOF
<html>
<body>
...
EOF;
        ...
        if (!$this->_checkUserPerm ('', $f['forum_type'], 'post', (int)$p['forum']) || ch_is_spam($p['topic_text']))
