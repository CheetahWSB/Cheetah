...
<xsl:template match="topic">
    ...
    <tr>
        <td class="forum_table_column_first forum_table_fixed_height">
            ...
            <div class="forum_icon_title_desc forum_icon_medium">
                ...
                <xsl:if test="string-length(first_u/avatar_medium) &gt; 0">
                    <img class="forum_user_icon forum_user_icon_medium thumbnail_image_file ch-def-thumbnail ch-def-shadow" src="{first_u/avatar_medium}" />
                </xsl:if>
                <xsl:if test="string-length(first_u/avatar_medium) = 0">
                    <div class="thumbnail_image">
                        <p class="thumbnail_image_letter ch-def-border ch-def-thumbnail ch-def-shadow">
                            <xsl:value-of select="substring(first_u/profile_title,1,1)" />
                        </p>
                    </div>
                </xsl:if>

                <a class="forum_topic_title ch-def-font-h2" href="{$rw_topic}{uri}{$rw_topic_ext}" onclick="return f.selectTopic('{uri}');">
                ...
                <span class="ch-def-font-small ch-def-font-grayed">
        ...
        <td class="forum_table_column_stat ch-def-font-h2">
