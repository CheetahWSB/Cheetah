...
<xsl:template match="live_tracker">
    ...
    <xsl:call-template name="box">
        ...
        <xsl:with-param name="content">
            ...
            <div id="live_tracker" class="live_tracker ch-def-bc-padding">
...
<xsl:template match="post">
    <div id="live_post_{@id}" class="live_post ch-def-margin-sec-top-auto">		

        <xsl:if test="string-length(avatar_medium) &gt; 0">
            <img class="lp_img thumbnail_image_file ch-def-thumbnail ch-def-shadow" src="{avatar_medium}" />
        </xsl:if>
        <xsl:if test="string-length(avatar_medium) = 0">
        ...
        <div class="lp_date ch-def-font-large"><xsl:copy-of select="date" /></div>
	</div>
</xsl:template>
