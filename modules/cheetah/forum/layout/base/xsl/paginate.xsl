<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <xsl:template name="paginate">
        ...
        <xsl:if test="$num &gt; $pages/@per_page">
            <div class="paginate paginate_simple ch-def-padding-right ch-def-padding-left clearfix">
    ...
    <xsl:template name="paginate_number">
        ...
        <xsl:if test="$if_first">
            ...
            <xsl:if test="0 = $start">
                <div class="paginate_btn ch-def-border ch-def-round-corners">
                    <span>
...
-->
    	...
    	<xsl:if test="$c = 0">
            <div class="paginate_page not_active ch-def-border ch-def-round-corners">
                <a href="{$link_curr}" onclick="{$onclick_curr}"><xsl:value-of select="$title" /></a>
            </div>
	    </xsl:if>
    	<xsl:if test="$c = 1">
            <div class="paginate_page active ch-def-border ch-def-round-corners">
