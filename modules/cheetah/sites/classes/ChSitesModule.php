...
function ch_sites_import ($sClassPostfix, $aModuleOverwright = array())
{
    global $aModule;
    $a = $aModuleOverwright ? $aModuleOverwright : $aModule;
    if (!$a || $a['uri'] != 'sites') {
        $oMain = ChWsbModule::getInstance('ChSitesModule');
        $a = $oMain->_aModule;
    }
...
require_once('ChSitesPrivacy.php');
 ...
 *
 * Memberships/ACL:
 * sites view - CH_SITES_VIEW
 * sites browse - CH_SITES_BROWSE
 * sites edit any site - CH_SITES_EDIT_ANY_SITE
 * sites delete any site - CH_SITES_DELETE_ANY_SITE
 * sites mark as featured - CH_SITES_MARK_AS_FEATURED
 *
 *
 *
 * Alerts:
 * Alerts type/unit - 'ch_sites'
 ...
 *
 */
class ChSitesModule extends ChWsbTwigModule
{
    var $_sPrefix = 'ch_sites';
    var $oPrivacy;
    var $iOwnerId;
    ...
    function __construct($aModule)
    {
        ...
        $this->oPrivacy = new ChSitesPrivacy($this);
        $this->iOwnerId = isLogged() ? getLoggedId() : 0;
        $GLOBALS['oChSitesModule'] = &$this;

        // BEGIN STW INTEGRATION
        $this->sHomeUrl = $this->_oConfig->getHomeUrl();
        $this->sHomePath = $this->_oConfig->getHomePath();
        $this->sModuleUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri();
    ...
    function actionHome()
    {
        ch_sites_import ('PageMain');
        $oPage = new ChSitesPageMain ($this);
        $this->_oTemplate->addCss(array('main.css', 'block_percent.css'));
        $this->_oTemplate->pageStart();
        echo $oPage->getCode();
        $this->_oTemplate->pageCode(_t('_ch_sites_caption_home'), false, false);
    }

    function actionCalendar($iYear = '', $iMonth = '')
    {
        ch_sites_import('Calendar');
        $oCalendar = new ChSitesCalendar($iYear, $iMonth, $this);
        $this->_oTemplate->pageStart();
        echo $oCalendar->display();
        $this->_oTemplate->pageCode(_t('_ch_sites_caption_browse_calendar'), true, false);
    }
    ...
    function actionDelete($iSiteId)
    {
        ...
        if (!($aSite = $this->_oDb->getSiteById($iSiteId))) {
            $this->_oTemplate->displayPageNotFoundExt (_t('_ch_sites_action_title_delete'));
            return;
        }

        header('Content-Type: text/html; charset=utf-8');
        if (!$this->isAllowedDelete($aSite)) {
            echo MsgBox(_t('_ch_events_msg_access_denied')) . genAjaxyPopupJS($iSiteId, 'ajaxy_popup_result_div');
            exit;
        }

        if ($this->deleteSite($iSiteId)) {
            $sRedirect = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'browse/my';
            $sJQueryJS = genAjaxyPopupJS($iSiteId, 'ajaxy_popup_result_div', $sRedirect);
            echo MsgBox(_t('_ch_sites_site_was_deleted')) . $sJQueryJS;
            exit;
        }

        echo MsgBox(_t('_ch_sites_error_occured')) . genAjaxyPopupJS($iSiteId, 'ajaxy_popup_result_div');
        exit;
    }
    ...
    function actionEdit($iSiteId)
    {
        ...
        if (!($aSite = $this->_oDb->getSiteById($iSiteId))) {
            $this->_oTemplate->displayPageNotFoundExt (_t('_ch_site_caption_edit'));
            return;
        }

        if (!$this->isAllowedEdit($aSite)) {
            $this->_oTemplate->displayAccessDeniedExt (_t('_ch_site_caption_edit'));
            return;
        }

        ch_sites_import('FormEdit');
        $oForm = new ChSitesFormEdit($this, $aSite);
        $oForm->initChecker($aSite);

        $this->_oTemplate->addCss(array('main.css'));

        if ($oForm->isSubmittedAndValid ()) {
            $sStatus = $this->_oDb->getParam('ch_sites_autoapproval') == 'on' || $this->isAdmin() ? 'approved' : 'pending';
            ...
            if ($oForm->update($iSiteId, $aValsAdd)) {
                ...
                if ($sStatus == 'approved')
                    header('Location:' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aSite['entryUri']);
                else
                    header('Location:' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri());
            } else {
                $this->_oTemplate->pageStart();
                echo MsgBox(_t('_ch_sites_err_edit_site'));
            }
        } else {
            $this->_oTemplate->pageStart();
            echo $oForm->getCode();
        ...
        $this->_oTemplate->pageCode(_t('_ch_site_caption_edit'));
    }

    function actionView($mixedVar)
    {
        $GLOBALS['oTopMenu']->setCustomSubHeader(_t('_ch_sites'));

        $aSite = is_numeric($mixedVar) ? $this->_oDb->getSiteById((int)$mixedVar) : $this->_oDb->getSiteByEntryUri(process_db_input($mixedVar));

        if (empty($aSite)) {
            $this->_oTemplate->displayPageNotFoundExt (_t('_ch_sites'));
            return;
        }
        ...
        ch_sites_import ('PageView');
        $oPage = new ChSitesPageView ($this, $aSite);
        ...
        $GLOBALS['oTopMenu']->setCustomSubHeaderUrl(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aSite['entryUri']);
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_ch_sites') => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
            $aSite['title'] => '',
        ));
        $this->_oTemplate->pageCode($aSite['title'], false, false);

        ch_import ('ChWsbViews');
        new ChWsbViews('ch_sites', $aSite['id']);
    ...
    function actionFeatured($iSiteId)
    {
        ...
        if (!($aSite = $this->_oDb->getSiteById($iSiteId))) {
            $this->_oTemplate->displayPageNotFoundExt (_t('_ch_sites_featured_top_menu_sitem'));
            return;
        }
        ...
        if ($this->_oDb->markFeatured($iSiteId)) {
            ...
            $sRedirect = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aSite['entryUri'];
            $sJQueryJS = genAjaxyPopupJS($iSiteId, 'ajaxy_popup_result_div', $sRedirect);
            echo MsgBox($aSite['featured'] ? _t('_ch_sites_msg_removed_from_featured') : _t('_ch_sites_msg_added_to_featured')) . $sJQueryJS;
            exit;
        }
    ...
    function actionHon()
    {
        ch_sites_import('PageHon');
        $oPage = new ChSitesPageHon($this);
        $this->_oTemplate->addCss(array('main.css', 'block_percent.css'));
        $this->_oTemplate->pageStart();
        echo $oPage->getCode();
        $this->_oTemplate->pageCode(_t('_ch_sites_hon'), false, false);
    }

    function actionSearch()
    {
        if (!$this->isAllowedSearch()) {
            $this->_oTemplate->displayAccessDeniedExt(_t('_ch_sites_caption_browse_search'), false);
            return;
        }

        ch_sites_import ('FormSearch');
        $oForm = new ChSitesFormSearch($this->_oConfig);
        $oForm->initChecker();
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            $o = new ChSitesSearchResult('search', $oForm->getCleanValue('Keyword'));

            if ($o->isError) {
                $this->_oTemplate->displayPageNotFoundExt (_t('_ch_sites_caption_browse_search'));
                return;
            }
            ...
            if ($s = $o->processing()) {
                ...
                $this->_oTemplate->displayNoDataExt (_t('_ch_sites_caption_browse_search'));
                return;
            }
            ...
            $this->_oTemplate->pageStart();
            echo $oForm->getCode ();
            $this->_oTemplate->pageCode(_t('_ch_sites_caption_browse_search'));
        }
    }
    ...
    function actionBrowse($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        ...
        if (!$this->isAllowedBrowse() || ('my' == $sMode && $this->iOwnerId == 0)) {
            $this->_oTemplate->displayAccessDeniedExt(_t('_ch_sites'), $bAjaxMode);
            return;
        }

        ch_sites_import ('SearchResult');
        $o = new ChSitesSearchResult(
        ...
        if(ch_get('rss') !== false && ch_get('rss')) {
            echo $o->rss();
            exit;
        }
    ...
    function actionDeleteProfileSites ($iProfileId)
    {
        ...
        if (!$iProfileId || !defined('CH_SITES_ON_PROFILE_DELETE'))
            return;
    ...
    {
        parent::_actionSharePopup ($iSiteId, _t('_ch_sites_caption_share_site'), true);
    }
    ...
    {
        if (!$this->isAdmin()) {
            $this->_oTemplate->displayAccessDeniedExt (_t('_ch_sites'));
            return;
        }

        $aMenu = array(
            'home' => array(
                'title' => _t('_ch_sites_pending_approval'),
            ...
            'admin_entries' => array(
                'title' => _t('_ch_sites_administration_admin_sites'),
            ...
            'add' => array(
                'title' => _t('_ch_sites_administration_add_site'),
                'href' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'administration/add',
                '_func' => array ('name' => '_actionAdministrationAdd', 'params' => array()),
            ),
            'settings' => array(
                'title' => _t('_ch_sites_administration_settings'),
        ...
        $this->_oTemplate->pageStart();
        echo $this->_oTemplate->adminBlock ($sContent, _t('_ch_sites_administration'), $aMenu);
        $this->_oTemplate->addCssAdmin(array('forms_adv.css', 'main.css', 'twig.css'));
        $this->_oTemplate->pageCodeAdmin (_t('_ch_sites_administration'));
    }
    ...
    function actionAdd()
    {
        ...
        $this->_oTemplate->pageCode(_t('_ch_sites_bcaption_site_add'), true, false);
    }
    ...
    function actionTags()
    {
        ...
        $aParam = array(
            ...
            $this->_oTemplate->pageStart();
            echo $oTags->getCode();
            $this->_oTemplate->pageCode(_t('_ch_sites_caption_browse_tags'), false, false);
    }
    ...
    function actionCategories()
    {
        ...
        $aParam = array(
            ...
            $this->_oTemplate->pageStart();
            echo $oCateg->getCode();
            $this->_oTemplate->pageCode(_t('_ch_sites_caption_browse_categories'), false, false);
    }
    ...
    function serviceGetSubscriptionParams ($sAction, $iEntryId)
    {
        ...
        $aActionList = array(
            'commentPost' => '_ch_sites_sbs_comment'
        );
        ...
        return array (
            ...
            'template' => array (
                ...
                'ViewLink' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry['entryUri'],
            ),
        );
    }
    ...
    function serviceGetWallPost ($aEvent)
    {
        ...
        $oVoting = new ChTemplVotingView ('ch_sites', 0, 0);
        ...
        if($iItems > 1) {
            ...
            return array(
                'owner_id' => $iOwner,
                'title' => _t('_ch_sites_wall_added_new_title_items', $sOwner, $iItems),
                'description' => '',
                'content' => $sCss . $this->_oTemplate->parseHtmlByName('modules/cheetah/wall/|timeline_post_twig_grouped.html', array(
	            	'mod_prefix' => $sCssPrefix,
					'mod_icon' => 'link',
	                'cpt_user_name' => $sOwner,
	                'cpt_added_new' => _t('_ch_sites_wall_added_new_items', $iItems),
        ...
        //--- Single public event
        $sTxtWallObject = _t('_ch_sites_wall_object');

        $aItem = $aItems[0];
        return array(
        	'owner_id' => $iOwner,
            'title' => _t('_ch_sites_wall_added_new_title', $sOwner, $sTxtWallObject),
            'description' => $aItem['description'],
            'content' => $sCss . $this->_oTemplate->parseHtmlByName('modules/cheetah/wall/|timeline_post_twig.html', array(
        		'mod_prefix' => $sCssPrefix,
				'mod_icon' => 'link',
                'cpt_user_name' => $sOwner,
                'cpt_added_new' => _t('_ch_sites_wall_added_new'),
        ...
        foreach($aObjectIds as $iId) {
            ...
            if(empty($aItem))
                ...
                if($aItem[$this->_oDb->_sFieldThumb]) {
                    $aImage = ChWsbService::call('photos', 'get_entry', array($aItem[$this->_oDb->_sFieldThumb], 'browse'), 'Search');
                ...
                // BEGIN STW INTEGRATION
                if ($aItem['thumb_file'] == '' && getParam('ch_sites_account_type') != 'No Automated Screenshots') {
                    ch_sites_import('STW');
	...
	function serviceGetWallAddComment($aEvent)
    {
        ...
        $oCmts = new ChSitesCmts('ch_sites', $iItem);
        if(!$oCmts->isEnabled())
            return '';
        ...
        $sTextWallObject = _t('_ch_sites_wall_object');
        return array(
            'title' => _t('_ch_sites_wall_added_new_title_comment', $sOwner, $sTextWallObject),
            'description' => $aComment['cmt_text'],
            'content' => $sCss . $this->_oTemplate->parseHtmlByName('modules/cheetah/wall/|timeline_comment.html', array(
        		'mod_prefix' => $sCssPrefix,
	            'cpt_user_name' => $sOwner,
	            'cpt_added_new' => _t('_ch_sites_wall_added_new_comment'),
    ...
    function serviceGetWallPostComment($aEvent)
    {
        ...
        ch_import('Cmts', $this->_aModule);
        $oCmts = new ChSitesCmts('ch_sites', $iId);
        if(!$oCmts->isEnabled())
            return '';
    ...
    function serviceGetSpyData ()
    {
        return array(
            'handlers' => array(
                array('alert_unit' => 'ch_sites', 'alert_action' => 'add', 'module_uri' => 'sites', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                array('alert_unit' => 'ch_sites', 'alert_action' => 'change', 'module_uri' => 'sites', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                array('alert_unit' => 'ch_sites', 'alert_action' => 'rate', 'module_uri' => 'sites', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                array('alert_unit' => 'ch_sites', 'alert_action' => 'commentPost', 'module_uri' => 'sites', 'module_class' => 'Module', 'module_method' => 'get_spy_post')
            ),
            'alerts' => array(
                array('unit' => 'ch_sites', 'action' => 'add'),
                array('unit' => 'ch_sites', 'action' => 'change'),
                array('unit' => 'ch_sites', 'action' => 'rate'),
                array('unit' => 'ch_sites', 'action' => 'delete'),
                array('unit' => 'ch_sites', 'action' => 'commentPost'),
                array('unit' => 'ch_sites', 'action' => 'commentRemoved')
            )
        );
    }
    ...
    function serviceGetSpyPost($sAction, $iObjectId, $iSenderId, $aExtraParams = array())
    {
        ...
        switch($sAction) {
            ...
            case 'commentPost' :
                ...
                if (!empty($aSite))
                    $aRet = array(
                        'lang_key'  => '_ch_sites_poll_' . $sAction,
                        'params'    => array(
    ...
    {
        return parent::_serviceGetMemberMenuItem (_t('_ch_sites'), _t('_ch_sites'), 'link');
    }
    ...
    function serviceGetMemberMenuItemAddContent ()
    {
        if (!$this->isAllowedAdd())
            return '';
        return parent::_serviceGetMemberMenuItem (_t('_ch_sites_site'), _t('_ch_sites_site'), 'link', false, 'add');
    }

    function browseMy($aProfile, $sValue = '')
    {
        ch_sites_import ('PageProfile');
        if (strlen($sValue))
            $sTitle = _t('_ch_sites_caption_browse_' . $sValue);
        else
            $sTitle = _t('_ch_sites_caption_browse_my');
        $oPage = new ChSitesPageProfile($this, $aProfile, $sValue);
        $this->_oTemplate->addCss(array('main.css'));
        $this->_oTemplate->pageStart();
        echo $oPage->getCode();
    ...
    function isAllowedEdit($aSite, $isPerformAction = false)
    {
        if ($this->isAdmin() || ($GLOBALS['logged']['member'] && $aSite['ownerid'] == $this->iOwnerId && isProfileActive($this->iOwnerId)))
            return true;
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_EDIT_ANY_SITE, $isPerformAction);
    ...
    function isAllowedAdd ($isPerformAction = false)
    {
        ...
        if (!$GLOBALS['logged']['member'])
            return false;
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_ADD, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedMarkAsFeatured($aSite, $isPerformAction = false)
    {
        if ($this->isAdmin())
            return true;
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_MARK_AS_FEATURED, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedDelete(&$aSite, $isPerformAction = false)
    {
        if ($this->isAdmin() || ($GLOBALS['logged']['member'] && $aSite['ownerid'] == $this->iOwnerId && isProfileActive($this->iOwnerId)))
            return true;
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_DELETE_ANY_SITE, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedView ($aSite, $isPerformAction = false)
    {
        ...
        // check admin acl
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_VIEW, $isPerformAction);
    ...
    function isAllowedBrowse ($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_BROWSE, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedSearch ($isPerformAction = false)
    {
        if ($this->isAdmin())
            return true;
        $this->_defineActions();
        $aCheck = checkAction($this->iOwnerId, CH_SITES_SEARCH, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function deleteSite($iSiteId)
    {
        ...
        if (count($aSite) > 0 && $this->_oDb->deleteSiteById($iSiteId)) {
            if ($aSite['photo'] != 0)
                ChWsbService::call('photos', 'remove_object', array($aSite['photo']), 'Module');
    ...
    function onSiteCreate ($iSiteId, $sStatus)
    {
        ...
        $oAlert = new ChWsbAlerts('ch_sites', 'add', $iSiteId, $this->iOwnerId, array('Status' => $sStatus));
        $oAlert->alert();
    }
    ...
    function onSiteChanged ($iSiteId, $sStatus)
    {
        ...
        ch_import('ChWsbAlerts');
        $oAlert = new ChWsbAlerts('ch_sites', 'change', $iSiteId, $this->iOwnerId, array('Status' => $sStatus));
        $oAlert->alert();
    }
    ...
    function onSiteDeleted ($iSiteId)
    {
        ...
        $oVotingProfile = new ChWsbVoting ('ch_sites', 0, 0);
        $oVotingProfile->deleteVotings ($iSiteId);

        // delete sites comments
        ch_import('ChWsbCmts');
        $oCmts = new ChWsbCmts ('ch_sites', $iSiteId);
        $oCmts->onObjectDelete ();

        // delete views
        ch_import ('ChWsbViews');
        $oViews = new ChWsbViews('ch_sites', $iSiteId, false);
        $oViews->onObjectDelete($iSiteId);

        //delete all subscriptions
		$oSubscription = ChWsbSubscription::getInstance();
		$oSubscription->unsubscribe(array('type' => 'object_id', 'unit' => 'ch_sites', 'object_id' => $iSiteId));

        // arise alert
        ch_import('ChWsbAlerts');
        $oAlert = new ChWsbAlerts('ch_sites', 'delete', $iSiteId, $this->iOwnerId);
        $oAlert->alert();
    }
    ...
    function onSiteMarkAsFeatured ($aSite)
    {
        // arise alert
        ch_import('ChWsbAlerts');
        $oAlert = new ChWsbAlerts('ch_sites', 'mark_as_featured', $aSite['id'], $aSite['Featured']);
        $oAlert->alert();
    }
    ...
    function _actionAdministrationManage($isAdminEntries, $sKeyBtnDelete = '', $sKeyBtnActivate = '', $sUrl = false)
    {
        ...
        if (getParam('ch_sites_redo') == 'on' && getParam('ch_sites_account_type') == 'Enabled') {
            ...
            if (!$isAdminEntries)
            $aButtons['action_activate'] = '_ch_sites_admin_activate';
    ...
    function _actionAdministrationSettings($sSettingsCatName = 'Sites')
    {
        ...
        if (getParam('ch_sites_key_id') != '' && getParam('ch_sites_secret_key') != '') {
            $aResponse = saveAccountInfo();
            if ($aResponse['stw_response_status'] == 'Success') {
                $sCodeSTW = MsgBox(_t('_ch_sites_administration_stw_acc_success'), 5);
            } else {
                $sCodeSTW = MsgBox(_t('_ch_sites_administration_stw_acc_failed'), 5);
            }
        } else {
            $sCodeSTW = MsgBox(_t('_ch_sites_administration_stw_acc_no_data'), 5);
        }

        $oSettings = new ChWsbAdminSettings($iId);
        $sForm = $oSettings->getForm();

        $aAccInfo = $this->_oDb->getAccountInfo(getParam('ch_sites_key_id'));
        $aVars = array (
            'actual_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'administration/settings',
    ...
    function _addSiteForm()
    {
        ...
        ch_sites_import('FormAdd');
        $oForm = new ChSitesFormAdd($this);
        $sMsgBox = '';
        ...
        if (isset($_POST['url'])) {
            if (isset($_POST['title'])) {
                ...
                $oForm = new ChSitesFormAdd($this, $aParam);
                $oForm->initChecker();
                if ($oForm->isSubmittedAndValid()) {
                    $sCategories = implode(';', array_unique(explode(';', $oForm->getCleanValue('categories'))));
                    ...
                    if ($iSiteId = $oForm->insert($aValsAdd)) {
                        ...
                        header('Location:' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'browse/my');
                    } else
                        $sMsgBox = MsgBox(_t('_ch_sites_error_occured'));
                }
            } else {
                $oForm->initChecker();
                if ($oForm->isSubmittedAndValid()) {
                    ...
                    $aSite = $this->_oDb->getSiteByUrl(process_db_input($sUrl, CH_TAGS_STRIP));
                    ...
                    if (empty($aSite) || !is_array($aSite)) {
                        ...
                        if (!empty($aInfo)) {
                            ...
                            $sMsgBox = MsgBox(_t('_ch_sites_site_link_error'));
    ...
    {
        require_once(CH_DIRECTORY_PATH_MODULES . '/cheetah/sites/classes/ChSitesSearchResult.php');
        $this->_oTemplate->addCss(array('main.css'));
        $o = new ChSitesSearchResult('index');
    ...
    function _getSitesProfile($sNickName)
    {
        require_once(CH_DIRECTORY_PATH_MODULES . '/cheetah/sites/classes/ChSitesSearchResult.php');
        $this->_oTemplate->addCss(array('main.css'));
        $o = new ChSitesSearchResult('profile', $sNickName);
    ...
    {
        ch_sites_import('SearchResult');
        $oSearchResult = new ChSitesSearchResult($sMode, $sValue);
