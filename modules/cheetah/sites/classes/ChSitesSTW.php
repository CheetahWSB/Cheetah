 ...
 */
    ...
    define('ACCESS_KEY', getParam('ch_sites_key_id'));
    define('SECRET_KEY', getParam('ch_sites_secret_key'));
    define('ACCOUNT_TYPE', getParam('ch_sites_account_type')); // 'No Automated Screenshots' or 'Enabled'
    define('THUMBNAIL_URI', $GLOBALS['oChSitesModule']->sThumbUrl);
    define('THUMBNAIL_DIR', $GLOBALS['oChSitesModule']->sThumbPath);
    define('INSIDE_PAGES', getParam('ch_sites_inside_pages') == 'on' ? true : false); // set to true if inside capturing should be allowed
    define('CUSTOM_MSG_URL', getParam('ch_sites_custom_msg_url')); // i.e. 'http://yourdomain.com/path/to/your/custom/msgs'
    define('CACHE_DAYS', getParam('ch_sites_cache_days')); // how many days should the local copy be valid?
    ...
    define('DEBUG', getParam('ch_sites_debug') == 'on' ? true : false);
    ...
    ********************************************/
    ...
    function saveAccountInfo()
    {
        ...
        $sRemoteData = ch_file_get_contents($sRequestUrl, $aArgs);
        $aResponse = _getAccXMLResponse($sRemoteData);

        if ($aResponse['stw_response_status'] == 'Success') {
            $GLOBALS['oChSitesModule']->_oDb->addAccountInfo(ACCESS_KEY, $aResponse);
        }
    ...
    function _getThumbnailPaid($sUrl, $aOptions, $sAttribAlt, $sAttribClass, $sAttribStyle)
    {
        ...
        $sTags = false;
        if ($sAttribStyle) {
            $sTags .= ' style="' . ch_html_attribute($sAttribStyle) . '"';
        }
        if ($sAttribAlt) {
            $sTags .= ' alt="' . ch_html_attribute($sAttribAlt) . '"';
        }
        if ($sAttribClass) {
            $sTags .= ' class="' . ch_html_attribute($sAttribClass) . '"';
        }

        return $sImageUrl ? '<img src="' . ch_html_attribute($sImageUrl) . '"'.$sTags.'/>' : false;
    }
    ...
    {
        $aOptions['Size'] = $aOptions['Size'] ? $aOptions['Size'] : getParam('ch_sites_thumb_size');
        $aOptions['SizeCustom'] = $aOptions['SizeCustom'] ? $aOptions['SizeCustom'] : getParam('ch_sites_thumb_size_custom');
        $aOptions['FullSizeCapture'] = $aOptions['FullSizeCapture'] ? $aOptions['FullSizeCapture'] : getParam('ch_sites_full_size') == 'on' ? true : false;
        $aOptions['MaxHeight'] = $aOptions['MaxHeight'] ? $aOptions['MaxHeight'] : getParam('ch_sites_max_height');
        $aOptions['NativeResolution'] = $aOptions['NativeResolution'] ? $aOptions['NativeResolution'] : getParam('ch_sites_native_res');
        $aOptions['WidescreenY'] = $aOptions['WidescreenY'] ? $aOptions['WidescreenY'] : getParam('ch_sites_widescreen_y');
        $aOptions['RefreshOnDemand'] = $aOptions['RefreshOnDemand'] ? $aOptions['RefreshOnDemand'] : false;
        $aOptions['Delay'] = $aOptions['Delay'] ? $aOptions['Delay'] : getParam('ch_sites_delay');
        $aOptions['Quality'] = $aOptions['Quality'] ? $aOptions['Quality'] : getParam('ch_sites_quality');
    ...
    function _checkWebsiteThumbnailCaptured($aArgs)
    {
        ...
        if ($sRemoteData != "") {
            ...
            // otherwise return error because we dont know the situation
              ...
              if (DEBUG) {
                $GLOBALS['oChSitesModule']->_oDb->addRequest($aArgs, $aResponse, _generateHash($aArgs));
           }
        } else {
    ...
    {
        $sRemoteData = ch_file_get_contents($sRemoteUrl, array());
