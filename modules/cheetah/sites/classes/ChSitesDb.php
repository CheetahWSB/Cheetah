...
define('CH_SITES_TABLE_PREFIX', 'ch_sites');

class ChSitesDb extends ChWsbModuleDb
{
    var $_oConfig;
    var $sTablePrefix;
    var $_sFieldId = 'id';
    ...
    {
        return $this->getRow ("SELECT * FROM `" . CH_SITES_TABLE_PREFIX . "_main` WHERE `id` = ? LIMIT 1", [$iSiteId]);
    }
    ...
    {
        return $this->getRow ("SELECT * FROM `" . CH_SITES_TABLE_PREFIX . "_main` WHERE `entryUri` = ? LIMIT 1", [$sEntryUri]);
    }

    function getSiteLatest()
    {
        return $this->getRow ("SELECT * FROM `" . CH_SITES_TABLE_PREFIX . "_main` ORDER BY `date` DESC LIMIT 1");
    }

    function getSiteByUrl($sUrl)
    {
        return $this->getRow ("SELECT * FROM `" . CH_SITES_TABLE_PREFIX . "_main` WHERE `url` = ? LIMIT 1", [$sUrl]);
    }

    function getSites()
    {
        return $this->getAll("SELECT * FROM `" . CH_SITES_TABLE_PREFIX . "_main`");
    }

    function getSitesByAuthor($iProfileId)
    {
        return $this->getAll("SELECT * FROM `" . CH_SITES_TABLE_PREFIX . "_main` WHERE `ownerid` = $iProfileId");
    }

    function markFeatured($iSiteId)
    {
        return $this->query ("UPDATE `" . CH_SITES_TABLE_PREFIX . "_main` SET `featured` = (`featured` - 1)*(`featured` - 1) WHERE `id` = $iSiteId LIMIT 1");
    }

    function deleteSiteById($iSiteId)
    {
        return $this->query("DELETE FROM `" . CH_SITES_TABLE_PREFIX . "_main` WHERE `id` = $iSiteId");
    }
    ...
    function getSitesByMonth($iYear, $iMonth, $iNextYear, $iNextMonth)
    {
        return $this->getAll("SELECT *, DAYOFMONTH(FROM_UNIXTIME(`date`)) AS `Day`
            FROM `" . CH_SITES_TABLE_PREFIX . "_main`
    ...
    {
        $this->query("UPDATE `" . CH_SITES_TABLE_PREFIX . "_main` SET `status` = '$sStatus' WHERE `id` = $iSiteId");
    }

    function getCountByOwnerAndStatus($iOwnerId, $sStatus)
    {
        return $this->getOne ("SELECT count(*) FROM `" . CH_SITES_TABLE_PREFIX . "_main` WHERE `status` = '$sStatus' AND `ownerid` = $iOwnerId");
    }
