...
class ChSitesFormAdd extends ChTemplFormView
{
    var $_oModule;
    var $_aParam;
    var $_aCustomForm;
    ...
    function getUrlForm()
    {
        return array(
            ...
            'form_attrs' => array(
                'name'    => 'form_site',
                'action'  => CH_WSB_URL_ROOT . $this->_oModule->_oConfig->getBaseUri() . 'browse/my/add',
                'method'  => 'post',
            ...
            'inputs' => array(
                'url' => array(
                    'type'     => 'text',
                    'name'     => 'url',
                    'caption'  => _t('_ch_sites_form_url'),
                    'required' => true,
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(1, 255),
                        'error'  => _t('_ch_sites_form_field_err'),
                    ),
                    'db'       => array(
                        'pass' => 'Xss'
                    ),
        ...
        $aForm = array(
            ...
            'inputs' => array(
                ...
                'title'         => array(
                    ...
                    'caption'  => _t('_ch_sites_form_title'),
                    'required' => true,
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(1, 100),
                ...
                'description'   => array(
                    ...
                    'caption'  => _t('_ch_sites_form_description'),
                    'required' => true,
                    'html'     => 1,
                    'checker'  => array(
                        'func'   => 'length',
                ...
                'photo' => array(
                    'type' => 'file',
                    'name' => 'photo',
                    'caption' => _t('_ch_sites_form_photo'),
                    'info' => _t('_ch_sites_form_photo_inf')
                ),
                'tags'          => array(
                    'type'     => 'text',
                    'name'     => 'tags',
                ...
                'categories'    => $oCategories->getGroupChooser('ch_sites', (int)$this->_oModule->iOwnerId, true),
                'allowView'     => $this->_oModule->oPrivacy->getGroupChooser($this->_oModule->iOwnerId,
                    'ch_sites', 'view', array(), _t('_ch_sites_caption_allow_view')),
                'allowComments' => $this->_oModule->oPrivacy->getGroupChooser($this->_oModule->iOwnerId,
                    'ch_sites', 'comments', array(), _t('_ch_sites_caption_allow_comments')),
                'allowRate'     => $this->_oModule->oPrivacy->getGroupChooser($this->_oModule->iOwnerId,
                    'ch_sites', 'rate', array(), _t('_ch_sites_caption_allow_rate')),
            ),
        );

        if (isset($this->_aParam['thumbnail']) && getParam('ch_sites_account_type') != 'No Automated Screenshots') {
            $aForm['inputs']['thumbnail']['content'] = $this->_aParam['thumbnail'];
            $aForm['inputs']['photo']['caption']     = _t('_ch_sites_form_other_thumbnail');
    ...
    function uploadPhoto($sPath, $isRemote = false)
    {
        ...
        $sTmpFile  = CH_DIRECTORY_PATH_ROOT . 'tmp/' . time() . (isset($_COOKIE['memberID']) ? (int)$_COOKIE['memberID'] : '');
        $bResult   = $isRemote ? copy($sPath, $sTmpFile) : move_uploaded_file($_FILES['photo']['tmp_name'], $sTmpFile);
        if ($bResult) {
            $iRet = ChWsbService::call('photos', 'perform_photo_upload', array($sTmpFile, $aFileInfo, false),
                'Uploader');
            if (!$iRet) {
                @unlink($sTmpFile);
            } else {
