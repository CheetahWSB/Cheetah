...
class ChSitesTemplate extends ChWsbModuleTemplate
{
    var $_iPageIndex = 600;
    var $_aCss = array ();
    var $_aJs = array ();
    ...
    {
        if (null == $this->_oMain)
            $this->_oMain = ChWsbModule::getInstance('ChSitesModule');
    ...
    {
        $oVoting = new ChTemplVotingView('ch_sites', $aData['id']);
    ...
    function blockInformation($aSite)
    {
        ...
        $aResult = array(
            ...
            'date' => getLocaleDate($aSite['date'], CH_WSB_LOCALE_DATE_SHORT),
    ...
    function pageCode($sTitle, $isDesignBox = true, $isWrap = true, $isSubActions = true)
    {
        ...
        if ($isSubActions) {
            ...
            $GLOBALS['oTopMenu']->setCustomSubActions($aVars, 'ch_sites_title', false);
        }
    ...
    function _checkOwner($aData, &$aResult)
    {
        if ($aData['ownerid']) {
            ...
            $aResult['owner'] = _t('_ch_sites_admin');
        }
    }
    ...
    function _getUnit($aData, $sThumbSize = 'browse')
    {
        ...
        $aFile = ChWsbService::call('photos', 'get_photo_array', array($aData['photo'], $sThumbSize), 'Search');
        ...
        if (getParam('ch_sites_account_type') != 'No Automated Screenshots') {
            if ($sImage == '') {
                ...
                ch_sites_import('STW');
                $sThumbHTML = getThumbnailHTML($sUrl, $aSTWOptions, false, 'ch-def-shadow ch-def-round-corners');
            }
        }
        // END STW INTEGRATION

        $sSiteLink = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aData['entryUri'];
        ...
        $aResult = array(
            ...
            // END STW INTEGRATION
            'comments' => $aData['commentsCount'] . ' ' . _t('_ch_sites_unit_comments'),
            'date' => defineTimeInterval($aData['date']),
            'owner_str' => _t('_ch_sites_unit_from') . ' ',
    ...
    {
        $this->_showDisplay($sTitle, _t('_ch_sites_msg_access_denied'), $isAjaxMode);
    }
    ...
    {
        $this->_showDisplay($sTitle, _t('_ch_sites_msg_pending_approval'), $isAjaxMode);
    }

    function displayPageNotFoundExt($sTitle, $isAjaxMode = false)
    {
        header("HTTP/1.0 404 Not Found");
        $this->_showDisplay($sTitle, _t('_ch_sites_msg_page_not_found'), $isAjaxMode);
    }
