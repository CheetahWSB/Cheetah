...
require_once(CH_DIRECTORY_PATH_PLUGINS . 'myadmin/zip.lib.php');
require_once(CH_DIRECTORY_PATH_PLUGINS . 'myadmin/unzip.lib.php');

ch_import('ChWsbModule');

define('CH_PROFILE_CUSTOMIZE_DIR_IMAGES', 'data/images/');
define('CH_PROFILE_CUSTOMIZE_SMALL_PREFIX', 's_');
define('CH_PROFILE_CUSTOMIZE_THEME_PREFIX', 't_');
define('CH_PROFILE_CUSTOMIZE_THUMB_EXT', '.jpg');
define('CH_PROFILE_CUSTOMIZE_THEME_CONF', 'conf.php');
define('CH_PROFILE_CUSTOMIZE_THEME_THUMB', 'thumb.jpg');
define('CH_PROFILE_CUSTOMIZE_IMAGES_DELETE', 0);
define('CH_PROFILE_CUSTOMIZE_IMAGES_COPY', 1);
 ...
 * @see ChProfileCustomizeModule::serviceGetCustomizeBlock
 * ChWsbService::call('profile_customize', 'get_customize_block', array($sPage, $sTarget));
 *
 * Get custom styles for current profile page
 * @see ChProfileCustomizeModule::serviceGetProfileStyle
 * ChWsbService::call('profile_customize', 'get_profile_style', array($iProfileId));
 *
 *
 ...
 *
 */
class ChProfileCustomizeModule extends ChWsbModule
{
    var $_aCssMatch;
    ...
    function actionSave($isReset = '')
    {
        ...
        if (isset($_POST['page']) && isset($_POST['trg'])) {
            $sPage = process_db_input($_POST['page'], CH_TAGS_STRIP);
            $sTarget = process_db_input($_POST['trg'], CH_TAGS_STRIP);
            ...
            if (!$isReset) {
                ...
                foreach ($aVars as $sKey => $sValue) {
                    if ($sValue != '' && $sValue != 'default' && $sValue != '-1')
                        $aTmpStyle[$sPage][$sTarget][$sKey] = process_db_input($sValue, CH_TAGS_STRIP);
                }
            } else if (!empty($aTmpStyle) && isset($aTmpStyle[$sPage][$sTarget])) {
                $this->_parseImages($aTmpStyle[$sPage][$sTarget], CH_PROFILE_CUSTOMIZE_IMAGES_DELETE);
    ...
    function actionProfileBlock($sAction = '', $iTheme = '')
    {
        ...
        switch ($sAction) {
            case 'save':
                ...
                if (!empty($aStyles)) {
                    $this->_parseImages(unserialize($aStyles['css']), CH_PROFILE_CUSTOMIZE_IMAGES_DELETE);
                    $this->_parseImages(unserialize($aStyles['tmp']), CH_PROFILE_CUSTOMIZE_IMAGES_COPY);
    ...
    function actionPublish($isSave = '')
    {
        ...
        if ($isSave && isset($_POST['name_theme']) && $_POST['name_theme']) {
            $sName = process_db_input($_POST['name_theme'], CH_TAGS_STRIP);
                ...
                if ($iThemeId != -1) {
                    ...
                    if (isset($_FILES[$sThumb]) && is_uploaded_file($_FILES[$sThumb]['tmp_name'])) {
                        if (strpos($_FILES[$sThumb]['type'], 'image') !== false) {
                            ...
                            $sThumbName = CH_PROFILE_CUSTOMIZE_THEME_PREFIX . $iThemeId . CH_PROFILE_CUSTOMIZE_THUMB_EXT;
                    ...
                    $sComplete = _t('_ch_profile_customize_complete');
                } else
                    $sComplete = _t('_ch_profile_customize_err_add_theme');
            } else
                $sComplete = sprintf(_t('_ch_profile_customize_err_already_exist'), $sName);
        }
    ...
    function actionSaveTheme($iThemeId)
    {
        ...
        if (!empty($aTheme)) {
            ...
            if (!empty($aStyles)) {
                $this->_parseImages(unserialize($aStyles['css']), CH_PROFILE_CUSTOMIZE_IMAGES_DELETE);
                $this->_parseImages(unserialize($aStyles['tmp']), CH_PROFILE_CUSTOMIZE_IMAGES_DELETE);
            }

            $this->_parseImages($aTheme, CH_PROFILE_CUSTOMIZE_IMAGES_COPY);
            $this->_parseImages($aTheme, CH_PROFILE_CUSTOMIZE_IMAGES_COPY);
    ...
    function actionAdministration($sType = '', $iUnitId = '')
    {
        ...
        $this->_oTemplate->pageCodeAdmin (_t('_ch_profile_customize_administration'), $sType, $iUnitId, $this->_checkActions());
    }
    ...
    {
        if (!$this->iUserId || !getParam('ch_profile_customize_enable'))
            return '';

        $sUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'customizepage/';
        ...
        foreach ($aMenuItems as $sItem) {
            $aTopMenu[_t('_ch_profile_customize_page_' . $sItem)] = array(
                'href' => $sUrl . $sItem,
                'dynamic' => true,
                'active' => $sItem == $sPage
            );
        ...
        if (isset($this->_aCssMatch[$sPage])) {
            ...
            foreach ($aThemesTargets as $sValue) {
                $aTargets[] = array(
                    'name' => _t('_ch_profile_customize_page_themes_' . $sValue),
    ...
    function serviceGetProfileStyle($iUserId)
    {
        ...
        if (!$iUserId || !getParam('ch_profile_customize_enable'))
            return '';
    ...
    {
        return CH_WSB_URL_MODULES . $this->_aModule['path'] . CH_PROFILE_CUSTOMIZE_DIR_IMAGES;
    }

    function _getImagesDir()
    {
        return CH_DIRECTORY_PATH_MODULES . $this->_aModule['path'] . CH_PROFILE_CUSTOMIZE_DIR_IMAGES;
    }
            ...
            if (strpos($_FILES[$sName]['type'], 'image') !== false && in_array($sExt, array ('gif', 'png', 'jpg', 'jpeg'))) {
                ...
                if ($sFileName) {
                    ...
                    if (move_uploaded_file($_FILES[$sName]['tmp_name'],  $sDestDir . $sFileName)) {
                        imageResize($sDestDir . $sFileName, $sDestDir . CH_PROFILE_CUSTOMIZE_SMALL_PREFIX . $sFileName, 64, 64);
                        return $sFileName;
                    }
                }
            } else
    ...
    function _deleteImage($sFileName)
    {
        if ($this->_oDb->deleteImage($sFileName)) {
            ...
            if (file_exists($sDestDir . CH_PROFILE_CUSTOMIZE_SMALL_PREFIX . $sFileName))
                unlink($sDestDir . CH_PROFILE_CUSTOMIZE_SMALL_PREFIX . $sFileName);
        }
    }
    ...
    function _getThemeFromTmp()
    {
        ...
        $this->_parseImages($aTmpStyle, CH_PROFILE_CUSTOMIZE_IMAGES_COPY);
    ...
    function _parseImages($aCss, $iOperation)
    {
        ...
        foreach ($aCss as $sKey => $mixedValue) {
            if (!is_array($mixedValue)) {
                if ($sKey == 'image') {
                    switch ($iOperation) {
                        case CH_PROFILE_CUSTOMIZE_IMAGES_DELETE:
                            $this->_deleteImage($mixedValue);
                            break;

                        case CH_PROFILE_CUSTOMIZE_IMAGES_COPY:
    ...
    {
        $sResult = _t('_ch_profile_customize_err_delete_theme');
        ...
        if (!empty($aTheme) && $this->_oDb->deleteTheme($iThemeId)) {
            $sFile = $this->_getImagesDir() . CH_PROFILE_CUSTOMIZE_THEME_PREFIX . $iThemeId . CH_PROFILE_CUSTOMIZE_THUMB_EXT;
            if (file_exists($sFile))
                unlink($sFile);
            $this->_parseImages(unserialize($aTheme['css']), CH_PROFILE_CUSTOMIZE_IMAGES_DELETE);

            $sResult = _t('_ch_profile_customize_delete_complete');
        }
    ...
    function _exportTheme($iThemeId)
    {
        ...
        $oZipFile->addFile($sConf, CH_PROFILE_CUSTOMIZE_THEME_CONF);
        $sFile = $sImagesPath . CH_PROFILE_CUSTOMIZE_THEME_PREFIX . $iThemeId . CH_PROFILE_CUSTOMIZE_THUMB_EXT;
        if (file_exists($sFile)) {
            $oData = implode("", file($sFile));
            $oZipFile->addFile($oData, CH_PROFILE_CUSTOMIZE_THEME_THUMB);
        }
    ...
    function _importTheme($sFileImport)
    {
        ...
        if (pathinfo($_FILES[$sFileImport]['name'], PATHINFO_EXTENSION) != 'dfn')
            return _t('_ch_profile_customize_err_format');

        $oUnZip = new SimpleUnzip($_FILES[$sFileImport]['tmp_name']);
        $aFiles = $this->_getZipFilesFromPath($oUnZip, '');

        // check exist 'conf.php'
        if (!isset($aFiles[CH_PROFILE_CUSTOMIZE_THEME_CONF]))
            return sprintf(_t('_ch_profile_customize_err_conf_php'), 'conf.php');

        eval($oUnZip->GetData($aFiles[CH_PROFILE_CUSTOMIZE_THEME_CONF]));

        // check parameters
        if (!isset($sThemeName) || !isset($sThemeStyle))
            return _t('_ch_profile_customize_err_theme_parameters');

        // check exist theme
        $aTheme = $this->_oDb->getThemeByName($sThemeName);
        if (!empty($aTheme))
            return sprintf(_t('_ch_profile_customize_err_already_exist'), $sThemeName);
        ...
        if ($iThemeId == -1)
            return _t('_ch_profile_customize_err_add_theme');

        // copy thumbnail
        if (isset($aFiles[CH_PROFILE_CUSTOMIZE_THEME_THUMB])) {
            $sThumbName = CH_PROFILE_CUSTOMIZE_THEME_PREFIX . $iThemeId . CH_PROFILE_CUSTOMIZE_THUMB_EXT;

            $oFile = fopen($sDestDir . $sThumbName, 'w', false);
            if ($oFile) {
                fwrite($oFile, $oUnZip->GetData($aFiles[CH_PROFILE_CUSTOMIZE_THEME_THUMB]));
                fclose($oFile);
            }
        }

        return _t('_ch_profile_customize_import_complete');
    }
