...
define('CH_SCTR_DIR_IMAGES', 'data/images/');
define('CH_SCTR_SMALL_PREFIX', 's_');
define('CH_SCTR_THEME_PREFIX', 't_');
define('CH_SCTR_THUMB_EXT', '.jpg');
define('CH_SCTR_THEME_CONF', 'conf.php');
define('CH_SCTR_THEME_THUMB', 'thumb.jpg');
define('CH_SCTR_IMAGES_DELETE', 0);
define('CH_SCTR_IMAGES_COPY', 1);

function ch_sctr_import($sClassPostfix, $aModuleOverwright = array())
{
    global $aModule;
 ...
 *
 */
class ChSctrModule extends ChWsbModule
{
	var $iUserId;
    var $_aCssMatch;
    ...
    function actionSave($isReset = '')
    {
        ...
        if (isset($_POST['page']) && isset($_POST['trg'])) {
            ...
            if (!$isReset) {
                ...
                $this->_parseImages($aTmpStyle[$sPage][$sTarget], CH_SCTR_IMAGES_DELETE);
    	...
    	if(isset($_POST['action']) && !empty($_POST['action'])) {
			$sAction = process_db_input($_POST['action'], CH_TAGS_STRIP);
            unset($_POST['action']);
                ...
                if ($iThemeId != -1) {
                    ...
                    if (isset($_FILES[$sThumb]) && is_uploaded_file($_FILES[$sThumb]['tmp_name'])) {
                        if (strpos($_FILES[$sThumb]['type'], 'image') !== false) {
                            ...
                            $sThumbName = CH_SCTR_THEME_PREFIX . $iThemeId . CH_SCTR_THUMB_EXT;
                    ...
                    $sComplete = _t('_ch_sctr_complete');
                } else
                    $sComplete = _t('_ch_sctr_err_add_theme');
            } else
                $sComplete = sprintf(_t('_ch_sctr_err_already_exist'), $sName);
        }
    ...
    function actionSaveTheme($iThemeId)
    {
        ...
        if (!empty($aTheme)) {
            ...
            if (!empty($aStyles)) {
                $this->_parseImages(unserialize($aStyles['css']), CH_SCTR_IMAGES_DELETE);
                $this->_parseImages(unserialize($aStyles['tmp']), CH_SCTR_IMAGES_DELETE);
            }

            $this->_parseImages($aTheme, CH_SCTR_IMAGES_COPY);
            $this->_parseImages($aTheme, CH_SCTR_IMAGES_COPY);
    ...
    function actionAdministration($sType = '', $iUnitId = '')
    {
        ...
        $this->_oTemplate->pageCodeAdmin (_t('_ch_sctr_administration'), $sType, $iUnitId, $this->_checkActions());
    }
    ...
    function serviceGetCustomizeButton()
    {
    	...
    	return array(
            'evalResult' => 'oChSctrMain.showBlock();',
            'evalResultCssClassWrapper' => 'ch-phone-hide'
        );
    }
    ...
    function serviceGetSiteStyle()
    {
        ...
        switch ($sAction) {
            case 'save':
                ...
                if (!empty($aStyles)) {
                    $this->_parseImages(unserialize($aStyles['css']), CH_SCTR_IMAGES_DELETE);
                    $this->_parseImages(unserialize($aStyles['tmp']), CH_SCTR_IMAGES_COPY);
                    $this->_oDb->saveSite();
    ...
    {
        return CH_WSB_URL_MODULES . $this->_aModule['path'] . CH_SCTR_DIR_IMAGES;
    }

    function _getImagesDir()
    {
        return CH_DIRECTORY_PATH_MODULES . $this->_aModule['path'] . CH_SCTR_DIR_IMAGES;
    }
            ...
            if (strpos($_FILES[$sName]['type'], 'image') !== false) {
                ...
                if ($sFileName) {
                    ...
                    if (move_uploaded_file($_FILES[$sName]['tmp_name'],  $sDestDir . $sFileName)) {
                        imageResize($sDestDir . $sFileName, $sDestDir . CH_SCTR_SMALL_PREFIX . $sFileName, 64, 64);
                        return $sFileName;
                    }
                }
            } else
    ...
    function _deleteImage($sFileName)
    {
        if ($this->_oDb->deleteImage($sFileName)) {
            ...
            if (file_exists($sDestDir . CH_SCTR_SMALL_PREFIX . $sFileName))
                unlink($sDestDir . CH_SCTR_SMALL_PREFIX . $sFileName);
        }
    }
    ...
    function _getThemeFromTmp()
    {
        ...
        $this->_parseImages($aTmpStyle, CH_SCTR_IMAGES_COPY);
    ...
    function _parseImages($aCss, $iOperation)
    {
        ...
        foreach ($aCss as $sKey => $mixedValue) {
            if (!is_array($mixedValue)) {
                if ($sKey == 'image') {
                    switch ($iOperation) {
                        case CH_SCTR_IMAGES_DELETE:
                            $this->_deleteImage($mixedValue);
                            break;

                        case CH_SCTR_IMAGES_COPY:
    ...
    {
        $sResult = _t('_ch_sctr_err_delete_theme');
        ...
        if (!empty($aTheme) && $this->_oDb->deleteTheme($iThemeId)) {
            $sFile = $this->_getImagesDir() . CH_SCTR_THEME_PREFIX . $iThemeId . CH_SCTR_THUMB_EXT;
            if (file_exists($sFile))
                unlink($sFile);
            $this->_parseImages(unserialize($aTheme['css']), CH_SCTR_IMAGES_DELETE);

            $sResult = _t('_ch_sctr_delete_complete');
        }
    ...
    function _exportTheme($iThemeId)
    {
        ...
        $oZipFile->addFile($sConf, CH_SCTR_THEME_CONF);
        $sFile = $sImagesPath . CH_SCTR_THEME_PREFIX . $iThemeId . CH_SCTR_THUMB_EXT;
        if (file_exists($sFile)) {
            $oData = implode("", file($sFile));
            $oZipFile->addFile($oData, CH_SCTR_THEME_THUMB);
        }
    ...
    function _importTheme($sFileImport)
    {
        ...
        if (pathinfo($_FILES[$sFileImport]['name'], PATHINFO_EXTENSION) != 'dfn')
            return _t('_ch_sctr_err_format');

        $oUnZip = new SimpleUnzip($_FILES[$sFileImport]['tmp_name']);
        $aFiles = $this->_getZipFilesFromPath($oUnZip, '');

        // check exist 'conf.php'
        if (!isset($aFiles[CH_SCTR_THEME_CONF]))
            return sprintf(_t('_ch_sctr_err_conf_php'), 'conf.php');

        eval($oUnZip->GetData($aFiles[CH_SCTR_THEME_CONF]));

        // check parameters
        if (!isset($sThemeName) || !isset($sThemeStyle))
            return _t('_ch_sctr_err_theme_parameters');

        // check exist theme
        $aTheme = $this->_oDb->getThemeByName($sThemeName);
        if (!empty($aTheme))
            return sprintf(_t('_ch_sctr_err_already_exist'), $sThemeName);
        ...
        if ($iThemeId == -1)
            return _t('_ch_sctr_err_add_theme');

        // copy thumbnail
        if (isset($aFiles[CH_SCTR_THEME_THUMB])) {
            $sThumbName = CH_SCTR_THEME_PREFIX . $iThemeId . CH_SCTR_THUMB_EXT;

            $oFile = fopen($sDestDir . $sThumbName, 'w', false);
            if ($oFile) {
                fwrite($oFile, $oUnZip->GetData($aFiles[CH_SCTR_THEME_THUMB]));
                fclose($oFile);
            }
        }

        return _t('_ch_sctr_import_complete');
    }
    ...
    {
        if (!$this->iUserId || !isAdmin($this->iUserId) || !getParam('ch_sctr_enable'))
            return '';
        ...
        foreach ($aMenuItems as $sItem) {
            $aTopMenu[_t('_ch_sctr_page_' . $sItem)] = array(
                'href' => $sUrl . $sItem,
                'dynamic' => true,
                'active' => $sItem == $sPage
            );
