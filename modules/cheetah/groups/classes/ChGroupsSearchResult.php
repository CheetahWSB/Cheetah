...
class ChGroupsSearchResult extends ChWsbTwigSearchResult
{
    var $aCurrent = array(
        'name' => 'ch_groups',
        'title' => '_ch_groups_page_title_browse',
    ...
    function __construct($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        switch ($sMode) {
            ...
            case 'pending':
                if (false !== ch_get('ch_groups_filter'))
                    $this->aCurrent['restriction']['keyword'] = array('value' => process_db_input(ch_get('ch_groups_filter'), CH_TAGS_STRIP), 'field' => '','operator' => 'against');
                $this->aCurrent['restriction']['activeStatus']['value'] = 'pending';
                $this->sBrowseUrl = "administration";
                $this->aCurrent['title'] = _t('_ch_groups_page_title_pending_approval');
            ...
            case 'search':
                ...
                $this->aCurrent['title'] = _t('_ch_groups_page_title_search_results') . ' ' . (is_array($sValue2) ? implode(', ',$sValue2) : $sValue2) . ' ' . $sValue;
                unset($this->aCurrent['rss']);
                break;

            case 'user':
                $iProfileId = $GLOBALS['oChGroupsModule']->_oDb->getProfileIdByNickName ($sValue, false);
                ...
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_by_author', $iProfileId ? getNickName($iProfileId) : $sValue);
            ...
            case 'joined':
                ...
                $iProfileId = getID($sValue);
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_by_author_joined_groups', $iProfileId ? getNickName($iProfileId) : $sValue);
            ...
            case 'admin':
                ...
                $this->aCurrent['title'] = _t('_ch_groups_page_title_admin_groups');
                break;
            ...
            case 'category':
                ...
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_by_category', $sValue);
                break;
            ...
            case 'tag':
                ...
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_by_tag', $sValue);
                break;

            case 'recent':
                $this->sBrowseUrl = 'browse/recent';
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_recent');
                break;

            case 'top':
                $this->sBrowseUrl = 'browse/top';
                $this->aCurrent['sorting'] = 'top';
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_top_rated');
                break;

            case 'popular':
                $this->sBrowseUrl = 'browse/popular';
                $this->aCurrent['sorting'] = 'popular';
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_popular');
                break;

            case 'featured':
            $this->aCurrent['restriction']['featured'] = array('value' => 1, 'field' => 'featured', 'operator' => '=');
                $this->sBrowseUrl = 'browse/featured';
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_featured');
                break;
            ...
            case 'calendar':
                ...
                $this->aCurrent['title'] = _t('_ch_groups_page_title_browse_by_day', getLocaleDate(strtotime("{$sValue}-{$sValue2}-{$sValue3}"), CH_WSB_LOCALE_DATE_SHORT));
                break;

            case '':
                $this->sBrowseUrl = 'browse/';
                $this->aCurrent['title'] = _t('_ch_groups');
                unset($this->aCurrent['rss']);
                break;
        ...
        $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('ch_groups_perpage_browse');
        ...
        if (ch_get('rss')) {
            ...
            $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('ch_groups_max_rss_num');
        }

        ch_import('Voting', $oMain->_aModule);
        $oVotingView = new ChGroupsVoting ('ch_groups', 0);
    ...
    function getAlterOrder()
    {
        if ($this->aCurrent['sorting'] == 'last') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `ch_groups_main`.`created` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'top') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `ch_groups_main`.`rate` DESC, `ch_groups_main`.`rate_count` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'popular') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `ch_groups_main`.`fans_count` DESC, `ch_groups_main`.`views` DESC";
            return $aSql;
        }
        return array();
    }
