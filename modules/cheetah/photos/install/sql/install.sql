...
INSERT INTO `sys_page_compose` (`Page`, `PageWidth`, `Desc`, `Caption`, `Column`, `Order`, `Func`, `Content`, `DesignBox`, `ColWidth`, `Visible`, `MinWidth`) VALUES
('index', '1140px', 'Public Photos', '_[db_prefix]_public', 1, 10, 'PHP', 'require_once(CH_DIRECTORY_PATH_MODULES . ''cheetah/photos/classes/ChPhotosSearch.php'');\r\n $oMedia = new ChPhotosSearch();\r\n $aVisible[] = CH_WSB_PG_ALL;\r\n if ($this->iMemberID > 0)\r\n $aVisible[] = CH_WSB_PG_MEMBERS;\r\n $aCode = $oMedia->getBrowseBlock(array(''allow_view''=>$aVisible), array(''menu_top''=>true, ''sorting'' => getParam(''[db_prefix]_mode_index''), ''per_page''=>(int)getParam(''[db_prefix]_number_index'')));\r\n return array($aCode[''code''], $aCode[''menu_top''], $aCode[''menu_bottom''], $aCode[''wrapper'']);', 1, 71.9, 'non,memb', 0),
('profile', '1140px', 'Profile Photo Block', '_[db_prefix]_photo_block', 0, 0, 'PHP', 'return ChWsbService::call(''photos'', ''profile_photo_block'', array(array(''PID'' => $this->oProfileGen->_iProfileID)), ''Search'');', 1, 28.1, 'non,memb', 263),
('profile', '1140px', 'Profile Photo Album Block', '_[db_prefix]_photo_album_block', 2, 1, 'PHP', 'return ChWsbService::call(''photos'', ''get_profile_album_block'', array($this->oProfileGen->_iProfileID), ''Search'');', 1, 28.1, 'non,memb', 0),
...
('[db_prefix]_albums', '[db_prefix]_cmts_albums', 'sys_cmts_track', 0, 1, 90, 20, 1, -3, 'none', 0, 1, 0, 'cmt', '', '', '', 'ChPhotosCmtsAlbums', 'modules/cheetah/photos/classes/ChPhotosCmtsAlbums.php');
...
VALUES ('[db_prefix]', '[db_prefix]_rating', '[db_prefix]_voting_track', 'gal_', 5, 'vote_send_result', 'CH_PERIOD_PER_VOTE', 1, 'ChPhotosRate', 'modules/cheetah/photos/classes/ChPhotosRate.php', '[db_prefix]_main', 'Rate', 'RateCount', 'ID');
...
(@iTMParentId, 'PhotosSearch', '_[db_prefix]_top_menu_search', 'searchKeyword.php?type=ch_photos', 45, 'non,memb', '', '', '', 1, 1, 1, 'custom', '', 0, '');
...
INSERT INTO `sys_menu_member` SET `Name` = 'ch_photos', `Eval` = 'return ChWsbService::call(''photos'', ''get_member_menu_item_add_content'');', `Position`='top_extra', `Type` = 'linked_item', `Parent` = IFNULL(@iMemberMenuParent, 0), `Order` = IFNULL(@iMemberMenuOrder, 1);
...
VALUES ('[db_prefix]', '_[db_prefix]', 'ChPhotosSearch', 'modules/cheetah/photos/classes/ChPhotosSearch.php');
...
('[db_prefix]_title', '{evalResult}', 'plus', '', 'showPopupAnyHtml(''{BaseUri}upload'');', 'return (getLoggedId() && ChWsbModule::getInstance(''ChPhotosModule'')->isAllowedAdd()) ? _t(''_sys_upload'') : '''';', 20),
...
('[db_prefix]_album', '{evalResult}', 'remove', '', 'getHtmlData(''ajaxy_popup_result_div_{ID}'', ''{moduleUrl}album_delete/{albumUri}'', false, ''post'', true); return false;', 'return (getLoggedId() && ChWsbModule::getInstance(''ChPhotosModule'')->isAllowedDeleteAlbum({ID})) ? _t(''_Delete'') : '''';', 1);


INSERT INTO `sys_email_templates` (`Name`, `Subject`, `Body`, `Desc`, `LangID`) VALUES
('t_ch_photos_share', 'Check This Out!', '<ch_include_auto:_email_header.html />\r\n\r\n<p><b>Hello</b>,</p>\r\n\r\n<p><SenderNickName> shared a <a href="<MediaUrl>"><MediaType></a> with you!</p>\r\n\r\n<hr>\r\n\r\n<UserExplanation>\r\n\r\n<hr>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Photo sharing', 0),
('t_ch_photos_report', '<SenderNickName> Reported A Photo', '<ch_include_auto:_email_header.html />\r\n\r\n<p><a href="<MediaUrl>">Reported <MediaType></a></p>\r\n\r\n<hr>\r\n\r\n<UserExplanation>\r\n\r\n<hr>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Photo report', 0),
('t_sbs_ch_photos_comments', 'New Comments To A Photo', '<ch_include_auto:_email_header.html />\r\n\r\n<p><b>Dear <RealName></b>,</p>\r\n\r\n<br /><p>The <a href="<ViewLink>">photo you subscribed to got new comments!</a></p>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Subscription: new comments to photo', 0);
...
(NULL, '_ch_photos', '__phs__ (<a href="modules/?r=photos/albums/my/main/">__l_add__</a>)');
...
(2, '[db_prefix]', '_[db_prefix]', '{siteUrl}modules/?r=photos/administration', 'Photo module by Cheetah', 'picture-o', @iMax+1);
...
('[db_prefix]', '', '', 'return ChWsbService::call(''photos'', ''get_subscription_params'', array($arg2, $arg3));'),
('[db_prefix]', 'commentPost', 't_sbs_[db_prefix]_comments', 'return ChWsbService::call(''photos'', ''get_subscription_params'', array($arg2, $arg3));');


INSERT INTO `sys_alerts_handlers` (`name`, `eval`) VALUES ('[db_prefix]_profile_delete', 'ChWsbService::call(''photos'', ''response_profile_delete'', array($this));');
...
('[db_prefix]', 'homepage', '_ch_photos', 'home_images.png', 7, '', '', '', @iMaxOrderHomepage, 1),
('[db_prefix]', 'profile', '_ch_photos', '', 7, '', 'return ChWsbXMLRPCMedia::_getMediaCount(''photo'', ''{profile_id}'', ''{member_id}'');', '', @iMaxOrderProfile, 1);
...
('[db_prefix]', '_ch_photos', '0.8', 'auto', 'ChPhotosSiteMapsPhotos', 'modules/cheetah/photos/classes/ChPhotosSiteMapsPhotos.php', @iMaxOrderSiteMaps, 1),
('[db_prefix]_albums', '_ch_photos_albums', '0.8', 'auto', 'ChPhotosSiteMapsAlbums', 'modules/cheetah/photos/classes/ChPhotosSiteMapsAlbums.php', @iMaxOrderSiteMaps + 1, 1);

-- chart

SET @iMaxOrderCharts = (SELECT MAX(`order`)+1 FROM `sys_objects_charts`);
INSERT INTO `sys_objects_charts` (`object`, `title`, `table`, `field_date_ts`, `field_date_dt`, `query`, `active`, `order`) VALUES
('[db_prefix]', '_ch_photos', 'ch_photos_main', 'Date', '', '', 1, @iMaxOrderCharts);

-- export

SET @iMaxOrderExports = (SELECT MAX(`order`)+1 FROM `sys_objects_exports`);
INSERT INTO `sys_objects_exports` (`object`, `title`, `class_name`, `class_file`, `order`, `active`) VALUES
('[db_prefix]', '_ch_photos', 'ChPhotosExport', 'modules/cheetah/photos/classes/ChPhotosExport.php', @iMaxOrderExports, 1);

-- member info

INSERT INTO `sys_objects_member_info` (`object`, `title`, `type`, `override_class_name`, `override_class_file`) VALUES
('ch_photos_thumb', '_ch_photos_member_info_profile_photo', 'thumb', 'ChPhotosMemberInfo', 'modules/cheetah/photos/classes/ChPhotosMemberInfo.php'),
('ch_photos_thumb_2x', '_ch_photos_member_info_profile_photo_2x', 'thumb_2x', 'ChPhotosMemberInfo', 'modules/cheetah/photos/classes/ChPhotosMemberInfo.php'),
('ch_photos_icon', '_ch_photos_member_info_profile_photo_icon', 'thumb_icon', 'ChPhotosMemberInfo', 'modules/cheetah/photos/classes/ChPhotosMemberInfo.php'),
('ch_photos_icon_2x', '_ch_photos_member_info_profile_photo_icon_2x', 'thumb_icon_2x', 'ChPhotosMemberInfo', 'modules/cheetah/photos/classes/ChPhotosMemberInfo.php');
