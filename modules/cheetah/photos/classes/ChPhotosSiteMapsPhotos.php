...
/**
 * Sitemaps generator for Photos
 */
class ChPhotosSiteMapsPhotos extends ChWsbSiteMaps
{
    protected $_oModule;
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array (
            ...
            'table' => "`ch_photos_main` AS `e`", // table name
            'join' => " INNER JOIN `sys_albums_objects` AS `o` ON (`o`.`id_object` = `e`.`ID`)
                        INNER JOIN `sys_albums` AS `a` ON (`a`.`Type` = 'ch_photos' AND `a`.`Status` = 'active' AND `a`.`AllowAlbumView` = '" . CH_WSB_PG_ALL . "' AND `a`.`ID` = `o`.`id_album`)", // join SQL part
            'where' => "AND `e`.`Status` = 'approved'", // SQL condition, without WHERE
            'order' => " `e`.`Date` ASC ", // SQL order, without ORDER BY
        );

        $this->_oModule = ChWsbModule::getInstance('ChPhotosModule');
    }
