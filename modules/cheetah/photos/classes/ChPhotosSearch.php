...
ch_import('ChTemplSearchResultSharedMedia');

class ChPhotosSearch extends ChTemplSearchResultSharedMedia
{
    function __construct ($sParamName = '', $sParamValue = '', $sParamValue1 = '', $sParamValue2 = '')
    {
        parent::__construct('ChPhotosModule');
        ...
        $this->aCurrent['rss']['title'] = _t('_ch_photos');
        $this->aCurrent['rss']['fields']['Image'] = 'Hash';

        // redeclaration some unique fav fields
        $this->aAddPartsConfig['favorite']['table'] = 'ch_photos_favorites';
        ...
        switch ($sParamName) {
            case 'calendar':
                ...
                $this->aCurrent['title'] = _t('_ch_photos_caption_browse_by_day') . sprintf("%04u-%02u-%02u", $sParamValue, $sParamValue1, $sParamValue2);
                break;
            case 'top':
            ...
            case 'featured':
                $this->aCurrent['restriction']['featured'] = array(
                    'value'=>'1', 'field'=>'Featured', 'operator'=>'=', 'paramName'=>'ch_photos_mode'
                );
                break;
            case 'favorited':
    ...
    function _getAlbumUnitItem($iIndex, $aPicture, $aParams = array())
    {
    	...
    	$aResult['ch_if:exist']['content']['url'] = '';

    	$sClass = '';
    	if(empty($aPicture)){
    		if($iIndex == 0) {
    			$aResult['ch_if:not-exist']['condition'] = true;
    			$aResult['ch_if:not-exist']['content']['class'] = ' sys-ai-empty';
    		}
    		else
    			$aResult['ch_if:not-exist']['condition'] = false;
    	}
    	else {
	    	switch($iIndex) {
	    		case 0:
    		...
    		$aResult['ch_if:exist']['content']['class'] = ' ' . $sClass;

    		if(!empty($aPicture['Uri']))
    			$aResult['ch_if:exist']['content']['url'] = $this->getCurrentUrl('file', $aPicture['id_object'], $aPicture['Uri']);
    		if(empty($aResult['ch_if:exist']['content']['url']) && !empty($aParams['album_url']))
    			$aResult['ch_if:exist']['content']['url'] = $aParams['album_url'];
    	}
	...
	function getProfilePhoto($aParams)
    {
		...
		if(!empty($sAlbum) && in_array($sAlbum, $aDefaultAlbums)) {
			...
			$sAlbum = ChWsbAlbums::getAbumUri($this->oModule->_oConfig->getGlParam($sAlbum), $iProfileId);
		}
        ...
        $aFile['view_url'] = CH_WSB_URL_ROOT . $this->oModule->_oConfig->getBaseUri() . 'view/' . $aFile['uri'];
    ...
    function getPhotoBlock ($aParams = array())
    {
        ...
        if($iCnt) {
            $aUnit = array();
            $aUnits = array();
            if (defined('CH_PROFILE_PAGE') || defined('CH_MEMBER_PAGE')) {
                $iPhotoWidth = 259;
            ...
            if((int)$aShowParams['showScroller'] == 1) {
                ...
                $aUnit['ch_if:show_scroller'] = array(
                    ...
                    'content' => array(
                        ...
                        'ch_if:scrollerBack' => array(
                            'condition' => $bScroller,
                            'content' => array(1)
                        ),
                        'ch_repeat:iconBlock' => $aUnits,
                        'ch_if:scrollerNext' => array(
                            'condition' => $bScroller,
                            'content' => array(1),
                        )
                    )
            ...
            ob_start();
            ?>
            <div class="paginate ch-def-padding-sec-left ch-def-padding-sec-right">
                <div class="view_all">
            ...
            ch_import('ChWsbAlbums');
            $sCaption = ChWsbAlbums::getAbumUri($this->oModule->_oConfig->getGlParam('profile_album_name'), $this->oModule->_iProfileId);

            $sLinkTitle = _t('_ch_photos_add');
