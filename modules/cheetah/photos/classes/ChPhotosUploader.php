...
//require_once(CH_DIRECTORY_PATH_MODULES . 'cheetah/photos/classes/ChPhotosModule.php' );

ch_import('ChWsbFilesUploader');
...
class ChPhotosUploader extends ChWsbFilesUploader
{
    // constructor
    function __construct()
    {
        ...
        $this->oModule = ChWsbModule::getInstance('ChPhotosModule');
    ...
    function getEmbedFormFile()
    {
        ...
        return ($sKey != '') ? $this->_getEmbedFormFile() : MsgBox(_t('_ch_photos_flickr_key_not_exist'));
    }
    ...
    function GenSendFileInfoForm($iFileID, $aDefaultValues = array())
    {
        ...
        if(isset($aDefaultValues['image']))
            ...
            $aPhotoInfo = ChWsbService::call('photos', 'get_photo_array', array($iFileID), 'Search');
    ...
    function serviceAcceptFileInfo()
    {
        ...
        if($iPhotoId && $iAuthorId) {
            $sTitle = $_POST['title'];
            $sTags = $_POST['tags'];
            if($sType == 'embed' && !empty($sTags) && strpos($sTags, CH_WSB_TAGS_DIVIDER) === false)
                $sTags = str_replace(' ', CH_WSB_TAGS_DIVIDER, $sTags);
    ...
    {
        $aCustom['Approved'] = getParam('ch_photos_activation') == 'on' ? 'approved' : 'pending';
            ...
            if(file_exists($sTempFileName) && filesize($sTempFileName)>0) {
                ...
                if ($iChangingPhotoID==0) {
                    if (is_array($aFileInfo) && count($aFileInfo)>0) {
                        ...
                        if (getParam('ch_photos_activation') == 'on') {
                            $bAutoActivate = true;
                            $sStatus = 'approved';
                        } else {
                            $bAutoActivate = false;
