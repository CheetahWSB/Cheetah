...
(NULL, 2, 'Quotes', '_ch_Quotes', '{siteUrl}modules/?r=quotes/administration/', 'Quotes administration', 'italic', '', '', @iExtOrd+1);
...
('index', '1140px', 'Daily Quotes', '_ch_quotes_bcaption_quotes', 2, IFNULL(@iPageColumnOrder, 0), 'PHP', 'return ChWsbService::call(''quotes'', ''get_quote_unit'');', 1, 28.1, 'non,memb', 0);

-- site stats
SET @iStatSiteOrder := (SELECT `StatOrder` + 1 FROM `sys_stat_site` WHERE 1 ORDER BY `StatOrder` DESC LIMIT 1);
INSERT INTO `sys_stat_site` VALUES(NULL, 'qts', 'ch_quotes_ss', '', 'SELECT COUNT(`ID`) FROM `[db_prefix]units` WHERE 1', '', '', 'italic', @iStatSiteOrder);
