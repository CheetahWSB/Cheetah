...
class ChQuotesModule extends ChWsbModule
{
    // constructor
    ...
    function actionAdministration($sSubaction = '', $sID = 0)
    {
        ...
        $iUnitID = (ch_get('action') == 'edit' && (int)ch_get('ID') > 0) ? (int)ch_get('ID') : $iUnitID;
    ...
    {
        $sAddNewC = _t('_ch_quotes_add_new');
        ...
        $aForm = array(
            'form_attrs' => array(
                ...
                'action' => CH_WSB_URL_ROOT . 'modules/?r=quotes/administration/',
                'method' => 'post',
            ),
            'params' => array (
                'db' => array(
                    'table' => 'ch_quotes_units',
                    'key' => 'ID',
            ...
            'inputs' => array(
                ...
                'Text' => array(
                    'type' => 'textarea',
                    'name' => 'Text',
                    'caption' => _t('_ch_quotes_text'),
                    'required' => true,
                    'value' => $sQText,
                    'checker' => array (
                        'func' => 'length',
                        'params' => array(3,1024),
                        'error' => _t('_ch_quotes_text_err', 1024),
                    ),
                    'db' => array (
                        'pass' => 'Xss',
                    ),
                ...
                'Author' => array(
                    'type' => 'text',
                    'name' => 'Author',
                    'caption' => _t('_ch_quotes_author'),
                    'required' => true,
                    'value' => $sQAuthor,
                    'checker' => array (
                        'func' => 'length',
                        'params' => array(3,128),
                        'error' => _t('_ch_quotes_author_err', 128),
                    ),
                    'db' => array (
                        'pass' => 'Xss',
                    ),
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            $sCode = MsgBox(_t('_ch_quotes_fail'), 1);
            ...
            if ($iUnitID>0) {
                ...
                $iLastId = $iUnitID;
                $sCode = MsgBox(_t('_ch_quotes_edited_success'), 1);
            } else {
                $iLastId = $oForm->insert($aValsAdd);
                $sCode = MsgBox(_t('_ch_quotes_success'), 1);
            }
        }
    ...
    {
        $sExistedC = _t('_ch_quotes_existed_list');
        ...
        $sAction = CH_WSB_URL_ROOT . 'modules/?r=quotes/administration/';
...
<form action="{$sAction}" method="post" name="quotes_moderation">
    <div class="adm-db-content-wrapper ch-def-bc-margin">
        <div id="quotes_box">
            {$sQuotes}
...
EOF;
        ...
        $sActions = /*ChWsbPageView::getBlockCaptionMenu(time(),*/ array(
            'add_unit' => array('href' => $sAction, 'title' => _t('_ch_quotes_add_new'), 'onclick' => '', 'active' => 0),
        )/*)*/;
