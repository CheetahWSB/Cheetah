...
require_once('ChSimpleMessengerPrivacy.php');
 ...
 *
 * Memberships/ACL:
 * use simple messenger - CH_USE_SIMPLE_MESSENGER
 *
 *
 *
 * Service methods:
 ...
 *
 * @see ChSimpleMessengerModule::serviceGetMessengerField
 *      ChWsbService::call('simple_messenger', 'get_messenger_field', array($iViewedMemberId));
 *
 * Generate messenger's core (javascript object);
 * @see ChSimpleMessengerModule::serviceGetMessengerCore
 * ChWsbService::call('simple_messenger', 'get_messenger_core', array());
 *
 * Generate privacy form for the currently logged in member;
 * @see ChSimpleMessengerModule::serviceGetSettings
 * ChWsbService::call('simple_messenger', 'get_settings', array());
 *
 *
 *
 * Alerts:
 * no alerts here.
 */
class ChSimpleMessengerModule extends ChWsbModule
{
    var $sHomeUrl;
    ...
    function __construct($aModule)
    {
        ...
        $this->aCoreSettings = array(
            ...
            'page_receiver'                => CH_WSB_URL_ROOT . 'modules/?r=' . $this->aModuleInfo['uri'] . '/get_operation',
        ...
        $this->oPrivacy = new ChSimpleMessengerPrivacy($this);
    }
    ...
    function actionGetOperation($sActionName, $iRecipientId = 0)
    {
        ...
        function _addMessage(&$oObject, $iRecipientId, $sMessage)
        {
            ...
            if (($iRecipientId && $sMessage)
                ...
                if (getProfileInfo($iRecipientId)) {
					...
					if($iMessage !== false) {
						$oAlert = new ChWsbAlerts($oObject->sModuleName, 'add', $iMessage, $oObject->iLoggedMemberId, array('RecipientId' => $iRecipientId, 'Message' => $sMessage));
						$oAlert->alert();
					}
        ...
        switch ($sActionName) {
            ...
            case 'new_messages' :
                ...
                $sRegisteredBoxes = false != ch_get('registered_chat_boxes')
                    ? ch_get('registered_chat_boxes')
                    : '';
    ...
    function getSimpleMessengerCore()
    {
        ...
        $sEmptyMessage = ch_js_string(_t('_simple_messenger_empty_message'));
        $sWaitMessage  = ch_js_string(_t('_simple_messenger_wait'));
    ...
    function isMessengerAlowed($iMemberId, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($iMemberId, CH_USE_SIMPLE_MESSENGER, $isPerformAction);
