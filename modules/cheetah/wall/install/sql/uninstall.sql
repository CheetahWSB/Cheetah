...
DELETE FROM `sys_menu_admin` WHERE `name`='ch_wall';

DELETE FROM `sys_objects_cmts` WHERE `ObjectName`='ch_wall' LIMIT 1;
...
SELECT @iHandlerId:=`id` FROM `sys_alerts_handlers` WHERE `name`='ch_wall' LIMIT 1;
DELETE FROM `sys_alerts_handlers` WHERE `name`='ch_wall' LIMIT 1;
DELETE FROM `sys_alerts` WHERE `handler_id`=@iHandlerId;

DELETE FROM `sys_sbs_entries` USING `sys_sbs_types`, `sys_sbs_entries` WHERE `sys_sbs_types`.`id`=`sys_sbs_entries`.`subscription_id` AND `sys_sbs_types`.`unit`='ch_wall';
DELETE FROM `sys_sbs_types` WHERE `unit`='ch_wall';

DELETE FROM `sys_email_templates` WHERE `Name` IN ('t_sbsWallUpdates');

-- chart
DELETE FROM `sys_objects_charts` WHERE `object` = 'ch_wall';

-- export
DELETE FROM `sys_objects_exports` WHERE `object` = 'ch_wall';
