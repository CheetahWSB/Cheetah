...
class ChWallCmts extends ChTemplCmtsView
{
    var $_oModule;
    ...
    function __construct($sSystem, $iId, $iInit = 1)
    {
        ...
        $this->_oModule = ChWsbModule::getInstance('ChWallModule');
    ...
    function actionCmtPost ()
    {
        ...
        if(isset($aEvent['owner_id']) && (int)$aEvent['owner_id'] > 0) {
            ...
            $oAlert = new ChWsbAlerts('ch_' . $this->_oModule->_oConfig->getUri(), 'update', $aEvent['owner_id']);
            $oAlert->alert();
            //--- Wall -> Update for Alerts Engine ---//
        }
    ...
    function getActionsExt($iCmtId, $sType = 'comment', $iEventId = 0)
    {
        ...
        return $this->_oModule->_oTemplate->parseHtmlByTemplateName('actions', array(
            ...
            'ch_if:show_vote' => array(
                'condition' => $bVote,
                'content' => $aVote
            ),
            'ch_if:show_votes' => array(
                'condition' => $bVotes,
                'content' => $aVotes
            ),
            'ch_if:show_repost' => array(
                'condition' => $bRepost,
                'content' => array(
                    'content' => $sRepost
                )
            ),
            'ch_if:show_reposts' => array(
                'condition' => $bRepost,
                'content' => array(
                    'content' => $sReposts
                )
