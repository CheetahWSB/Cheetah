...
class ChWallConfig extends ChWsbConfig
{
    var $_oDb;
    var $_bAllowDelete;
    ...
    function __construct($aModule)
    {
        ...
        $this->_sDividerDateFormat = getLocaleFormat(CH_WSB_LOCALE_DATE_SHORT, CH_WSB_LOCALE_DB);
    ...
    function getSystemData(&$aEvent, $sDisplayType = CH_WALL_VIEW_TIMELINE)
    {
		...
		return ChWsbService::call($aHandler['module_uri'], $aHandler['module_method'] . ($sDisplayType == CH_WALL_VIEW_OUTLINE ? '_' . CH_WALL_VIEW_OUTLINE : ''), array($aEvent), $aHandler['module_class']);
    }
    function getSystemDataByDescriptor($sType, $sAction, $iObjectId, $sDisplayType = CH_WALL_VIEW_TIMELINE)
    {
    	$aDescriptor = array(
    		'type' => $sType,
    		'action' => $sAction,
    ...
    {
        return strpos($mixedObjectId, CH_WALL_DIVIDER_OBJECT_ID) !== false;
    }

    function getCommonType($sName)
    {
    	return strtolower(str_replace('ch_', '', $sName));
    }
