...
class ChWallDb extends ChWsbModuleDb
{
    var $_oConfig;
    /*
     * Constructor.
    ...
    function insertEvent($aParams)
    {
        ...
        if($iId > 0 && isset($aParams['owner_id']) && (int)$aParams['owner_id'] > 0) {
            ...
            $oAlert = new ChWsbAlerts('ch_' . $this->_oConfig->getUri(), 'update', $aParams['owner_id']);
            $oAlert->alert();
            //--- Wall -> Update for Alerts Engine ---//
        }
    ...
    function getEvents($aParams)
    {
        ...
        if(isset($aParams['timeline']) && strpos($aParams['timeline'], CH_WALL_DIVIDER_TIMELINE) !== false) {
            list($iTLStart, $iTLEnd) = explode(CH_WALL_DIVIDER_TIMELINE, $aParams['timeline']);
        ...
        switch($aParams['browse']) {
            ...
            case 'owner':
		        if($sWhereModuleFilter == '') {
					$aHidden = $this->_oConfig->getHandlersHidden(CH_WALL_VIEW_TIMELINE);
    ...
    function getEventsCount($iOwnerId, $sFilter, $sTimeline, $aModules)
    {
    	...
    	if(!empty($sTimeline) && strpos($sTimeline, CH_WALL_DIVIDER_TIMELINE) !== false) {
            list($iTLStart, $iTLEnd) = explode(CH_WALL_DIVIDER_TIMELINE, $sTimeline);
    ...
    function updateSimilarObject($iId, &$oAlert, $sDuration = 'day')
    {
        ...
        $sSql = "UPDATE `" . $this->_sPrefix . "events`
            SET
                `object_id`=CONCAT(`object_id`, '" . CH_WALL_DIVIDER_OBJECT_ID . $oAlert->iObject . "'),
                `title`='',
                `description`='',
                `date`=UNIX_TIMESTAMP()
            WHERE
    ...
    {
        $aType2Db = array(
            'sharedPhoto' => array('table' =>'ch_shared_photo_files', 'id' => 'medID'),
    ...
    {
        switch($sFilter) {
            case CH_WALL_FILTER_OWNER:
                $sFilterAddon = " AND `te`.`action`='' AND `te`.`object_id`='" . $iOwnerId . "' ";
                break;
            case CH_WALL_FILTER_OTHER:
                $sFilterAddon = " AND `te`.`action`='' AND `te`.`object_id`<>'" . $iOwnerId . "' ";
                break;
            case CH_WALL_FILTER_ALL:
            default:
                $sFilterAddon = "";
        }
        return $sFilterAddon;
