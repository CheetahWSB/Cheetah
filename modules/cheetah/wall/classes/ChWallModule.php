...
require_once('ChWallCmts.php');
require_once('ChWallVoting.php');
require_once('ChWallPrivacy.php');
require_once('ChWallResponse.php');

define('CH_WALL_FILTER_ALL', 'all');
define('CH_WALL_FILTER_OWNER', 'owner');
define('CH_WALL_FILTER_OTHER', 'other');

define('CH_WALL_VIEW_TIMELINE', 'timeline');
define('CH_WALL_VIEW_OUTLINE', 'outline');

define('CH_WALL_PARSE_TYPE_TEXT', 'text');
define('CH_WALL_PARSE_TYPE_LINK', 'link');
define('CH_WALL_PARSE_TYPE_PHOTOS', 'photos');
define('CH_WALL_PARSE_TYPE_SOUNDS', 'sounds');
define('CH_WALL_PARSE_TYPE_VIDEOS', 'videos');
define('CH_WALL_PARSE_TYPE_REPOST', 'repost');

define('CH_WALL_MEDIA_CATEGORY_NAME', 'wall');

define('CH_WALL_DIVIDER_ID', ',');
define('CH_WALL_DIVIDER_OBJECT_ID', ',');
define('CH_WALL_DIVIDER_TIMELINE', '-');

class ChWallModule extends ChWsbModule
{
    var $_iOwnerId;
    var $_aPostElements;
    ...
    function actionPost()
    {
        ...
        if (!$this->_isCommentPostAllowed(true))
            return "<script>" . $sResult . "alert('" . ch_js_string(_t('_wall_msg_not_allowed_post')) . "');</script>";

        $sPostType = process_db_input($_POST['WallPostType'], CH_TAGS_STRIP);
        $sContentType = process_db_input($_POST['WallContentType'], CH_TAGS_STRIP);
        ...
        if(method_exists($this, $sMethod)) {
            ...
            if((int)$aResult['code'] == 0) {
                $iId = $this->_oDb->insertEvent(array(
                   ...
                   'action' => '',
                   'content' => process_db_input($aResult['content'], CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION),
                   'title' => process_db_input($aResult['title'], CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION),
                   'description' => process_db_input($aResult['description'], CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION)
                ));
                ...
                $sResult .= "alert('" . ch_js_string(_t($aResult['message'])) . "');";
        } else
           $sResult .= "alert('" . ch_js_string(_t('_wall_msg_failed_post')) . "');";
	...
	function actionRepost()
    {
        ...
        $iOwnerId = (int)ch_get('owner_id');
        $aContent = array(
            'type' => process_db_input(ch_get('type')),
            'action' => process_db_input(ch_get('action')),
            'object_id' => (int)ch_get('object_id'),
        );
    ...
    function actionGetPosts()
    {
        ...
        if(strpos($this->_iOwnerId, CH_WALL_DIVIDER_ID) !== false)
            $this->_iOwnerId = explode(CH_WALL_DIVIDER_ID, $this->_iOwnerId);
        ...
        $sFilter = isset($_POST['WallFilter']) && !empty($_POST['WallFilter']) ? process_db_input($_POST['WallFilter'], CH_TAGS_STRIP) : CH_WALL_FILTER_ALL;
        $sTimeline = isset($_POST['WallTimeline']) && !empty($_POST['WallTimeline']) ? process_db_input($_POST['WallTimeline'], CH_TAGS_STRIP) : '';
        $aModules = isset($_POST['WallModules']) && !empty($_POST['WallModules']) ? process_db_input($_POST['WallModules'], CH_TAGS_STRIP) : array();
	...
	function actionGetImage($iId, $sUrl)
    {
        ...
        echo ch_file_get_contents($sUrl);
    }
    /**
    ...
    function actionRss($sUsername)
    {
        ...
        foreach($aEvents as $aEvent) {
            ...
            $aRssData[$aEvent['id']] = array(
               ...
               'UnitLink' => CH_WSB_URL_ROOT . $sRssBaseUrl . '#wall-event-' . $aEvent['id'],
     ...
     * Process alert.
     *
     * @param ChWsbAlerts $oAlert an instance with accured alert.
     */
    function serviceResponse($oAlert)
    {
        $oResponse = new ChWallResponse($this);
    ...
    function servicePostBlockIndexTimeline($sType = 'id')
    {
        ...
        $aTopMenu = $this->_getPostTabs(CH_WALL_VIEW_TIMELINE);
        ...
        return array($sContent, $aTopMenu['tabs'], LoadingBox('ch-wall-post-loading'), true, 'getBlockCaptionMenu');
    }
	...
	function serviceViewBlockIndexTimeline($iStart = -1, $iPerPage = -1, $sFilter = '', $sTimeline = '', $sType = 'id', $aModules = array())
    {
        ...
        return array($this->_oTemplate->parseHtmlByName('view.html', $aVariables), array(), LoadingBox('ch-wall-view-loading'), false, 'getBlockCaptionMenu');
    }
    ...
    function _getPostTabs($sType)
    {
		...
		if(!empty($aTabs)) {
			...
			$aMask[ch_ltrim_str($sActive, 'wall-ptype-')] = 1;
		}
    ...
    function serviceViewBlockProfileTimeline($mixed, $iStart = -1, $iPerPage = -1, $sFilter = '', $sTimeline = '', $sType = 'id', $aModules = array())
    {
        ...
        $aButton = $oSubscription->getButton($this->_getAuthorId(), 'ch_wall', '', $this->_iOwnerId);
        ...
        $aTopMenu = array(
            ...
            'wall-get-rss' => array('href' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'rss/' . $aOwner['username'] . '/', 'target' => '_blank', 'title' => _t('_wall_get_rss')),
        ...
        $this->_oTemplate->addCss(array('view.css', 'view_phone.css'));
        $this->_oTemplate->addJs(array('modernizr.js', 'main.js', 'view.js'));
        return array($oSubscription->getData() . $this->_oTemplate->parseHtmlByName('view.html', $aVariables), $aTopMenu, LoadingBox('ch-wall-view-loading'), false, 'getBlockCaptionMenu');
    }
    ...
    function serviceViewBlockAccountTimeline($mixed, $iStart = -1, $iPerPage = -1, $sFilter = '', $sTimeline = '', $sType = 'id', $aModules = array())
    {
        ...
        $sOwnerId = implode(CH_WALL_DIVIDER_ID, $this->_iOwnerId);
    ...
    function serviceViewBlockIndexOutline($iStart = -1, $iPerPage = -1, $sFilter = '', $aModules = array())
    {
        ...
        return array($this->_oTemplate->parseHtmlByName('outline.html', $aTmplVars), array(), LoadingBox('ch-wall-view-loading'), true, 'getBlockCaptionMenu');
    }

    function serviceGetActionsChecklist($sType)
    {
        ch_instance('ChWsbModuleDb');
    ...
    function serviceUpdateHandlers($sModuleUri = 'all', $bInstall = true)
    {
        ...
        foreach($aModules as $aModule) {
           if(!ChWsbRequest::serviceExists($aModule, 'get_wall_data')) continue;

           $aData = ChWsbService::call($aModule['uri'], 'get_wall_data');
           if($bInstall)
    ...
    function serviceGetMemberMenuItem()
    {
        ...
        $aLinkInfo = array(
            ...
            'item_link'     => CH_WSB_URL_ROOT . $this -> _oConfig -> getBaseUri(),
            'item_onclick'  => null,
    ...
    function serviceGetSubscriptionParams($sUnit, $sAction, $iObjectId)
    {
        ...
        return array(
            'template' => array(
                ...
                'ViewLink' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri()  . 'index/' . $aProfileInfo['NickName']
            )
        );
    }
	...
	function getRepostedLanguageKey($sType, $sAction, $mixedObjectId, $bTitle = false)
    {
		...
		$sLanguageKey .= ch_ltrim_str($sType, $this->_oConfig->getPrefix('common_post'), '');
    ...
    function onPost($iId)
    {
		...
		$oAlert = new ChWsbAlerts($this->_oConfig->getAlertSystemName(), 'post', $iId, $iUserId);
		$oAlert->alert();
		//--- Event -> Post for Alerts Engine ---//
    }
    ...
    function onDelete($aEvent)
    {
        ...
        if($aEvent['type'] == $sCommonPostPrefix . CH_WALL_PARSE_TYPE_REPOST) {
		...
		ch_import('ChWsbAlerts');
		$oAlert = new ChWsbAlerts($this->_oConfig->getAlertSystemName(), 'delete', $iId, $iUserId);
		$oAlert->alert();
		//--- Event -> Delete for Alerts Engine ---//
    }
	...
	function onRepost($iId, $aReposted = array())
    {
        ...
        $oAlert = new ChWsbAlerts($this->_oConfig->getAlertSystemName(), 'repost', $aReposted['id'], $iUserId, array('repost_id' => $aEvent['id']));
        $oAlert->alert();
        //--- Wall -> Update for Alerts Engine ---//
    }
    ...
    function _processLinkUpload()
    {
        ...
        $sUrl = trim(process_db_input($_POST['url'], CH_TAGS_STRIP));
        if(empty($sUrl))
            return array(
                'code' => 1,
        ...
        return array(
           ...
           'content' => $this->_oTemplate->parseHtmlByName('common_link.html', array(
        		'ch_if:show_thumnail' => array(
        			'condition' => $bThumbnail,
        			'content' => array(
        				'thumbnail' => '{ch_wall_get_image_url}' . urlencode(base64_encode($sThumbnail))
        			)
        		),
				'title' => $sTitle,
    ...
    function _getPaginate($sFilter, $sTimeline, $aModules)
    {
        ...
        return new ChWsbPaginate(array(
    ...
    function _getPostsOutline($sOrder, $iStart, $iPerPage, $sFilter, $aModules)
    {
        ...
        foreach($aEvents as $aEvent) {
            ...
            $aEvent['content'] = $this->_oTemplate->getSystem($aEvent, CH_WALL_VIEW_OUTLINE);
            if(empty($aEvent['content']))
                continue;
	...
	function _getWriteFormCommon()
    {
        ...
        $aForm = array(
            'form_attrs' => array(
                'name' => 'WallPostText',
                'action' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'post/',
                'method' => 'post',
	...
	function _getObjectVoting($aEvent)
    {
		...
		$oVoting = new ChWallVoting($sType, $iObjectId);
    ...
    {
        return new ChWallVoting($this->_oConfig->getVotingSystemName(), $iEventId);
    }
