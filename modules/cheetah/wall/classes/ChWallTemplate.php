...
class ChWallTemplate extends ChWsbModuleTemplate
{
    var $_oModule;
     ...
     */
    function getSystem($aEvent, $sDisplayType = CH_WALL_VIEW_TIMELINE)
    {
        ...
        $sResult = "";
        switch($sDisplayType) {
			case CH_WALL_VIEW_TIMELINE:
	            if((empty($aEvent['title']) && !empty($aResult['title'])) || (empty($aEvent['description']) && !empty($aResult['description'])))
	                $this->_oDb->updateEvent(array(
	                    'title' => process_db_input($aResult['title'], CH_TAGS_STRIP),
	                    'description' => process_db_input($aResult['description'], CH_TAGS_STRIP)
	                ), $aEvent['id']);
    ...
    function getCommon($aEvent)
    {
		...
		$sEventType = ch_ltrim_str($aEvent['type'], $sPrefix, '');
		...
		switch($sEventType) {
			case CH_WALL_PARSE_TYPE_PHOTOS:
        	case CH_WALL_PARSE_TYPE_SOUNDS:
        	case CH_WALL_PARSE_TYPE_VIDEOS:
        		$aContent = unserialize($aEvent['content']);

        		$oComments = new ChWallCmts($this->_oConfig->getCommonName($aContent['type']), $aContent['id']);
        ...
        return $this->parseHtmlByTemplateName('balloon', array(
            'post_type' => ch_ltrim_str($aEvent['type'], $sPrefix, ''),
            'post_id' => $aEvent['id'],
    ...
    function _getSystemData(&$aEvent, $sDisplayType = CH_WALL_VIEW_TIMELINE)
    {
    	$sHandler = $aEvent['type'] . '_' . $aEvent['action'];
        if(!$this->_oConfig->isHandler($sHandler))
            return array();
        ...
        if(empty($aHandler['module_uri']) && empty($aHandler['module_class']) && empty($aHandler['module_method'])) {
            $sMethod = 'display' . ch_gen_method_name($aHandler['alert_unit'] . '_' . $aHandler['alert_action']);
	...
	function _getCommonData($aEvent)
    {
        ...
        switch($sEventType) {
        	case CH_WALL_PARSE_TYPE_TEXT:
        		$aResult['content'] = ch_linkify_html($aEvent['content'], 'class="' . CH_WSB_LINK_CLASS . '"');

        		$sTmplName = 'common';
        		$aTmplVars['cpt_added_new'] = _t('_wall_added_' . $sEventType);
        		break;

			case CH_WALL_PARSE_TYPE_LINK:
				$aResult['content'] = $this->parseHtmlByContent($aEvent['content'], array(
				    'ch_wall_get_image_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'get_image/' . $aEvent['id'] . '/'
				), array('{', '}'));
            ...
            case CH_WALL_PARSE_TYPE_REPOST:
    ...
    {
        $aConverter = array(
        	CH_WALL_PARSE_TYPE_PHOTOS => 'photo',
        	CH_WALL_PARSE_TYPE_SOUNDS => 'sound',
        	CH_WALL_PARSE_TYPE_VIDEOS => 'video'
        );

        $aMediaInfo = ChWsbService::call($sType, 'get_' . $aConverter[$sType] . '_array', array($iObject, 'browse'), 'Search');
		...
		return array(
			...
			'content' => $this->parseHtmlByTemplateName('common_media', array(
				...
				'title' => isset($aMediaInfo['title']) ? ch_html_attribute($aMediaInfo['title']) : '',
    ...
    function getTimeline($iStart, $iPerPage, $sFilter, $sTimeline, $aModules)
    {
        ...
        if(empty($sTimeline))
            $sTimeline = '0' . CH_WALL_DIVIDER_TIMELINE . $iMaxDuration;
    ...
    function getLoadMore($iStart, $iPerPage, $bEnabled = true, $bVisible = true)
    {
        $aTmplVars = array(
            'visible' => $bVisible ? 'block' : 'none',
            'ch_if:is_disabled' => array(
                'condition' => !$bEnabled,
                'content' => array()
            ),
            'ch_if:show_on_click' => array(
                'condition' => $bEnabled,
                'content' => array(
    ...
    function getUploader($iOwnerId, $sType, $sSubType = '')
    {
    	...
    	$aUploaders = ChWsbService::call($sModule, 'get_uploaders_list', array(), 'Uploader');
    	...
    	return $this->parseHtmlByName('uploader.html', array(
    		...
    		'uploader' => ChWsbService::call($sModule, 'get_uploader_form', array(array(
    			'mode' => $sSubType,
    			'category' => 'wall',
    ...
    function displayProfileEdit($aEvent)
    {
        ...
        return array(
            ...
            'content' => $this->parseHtmlByName('p_edit.html', array(
                ...
                'cpt_info_url' => CH_WSB_URL_ROOT . 'profile_info.php?ID=' . $aOwner['id'],
                'post_id' => $aEvent['id']
            ))
        );
    }
	...
	function displayProfileCommentAdd($aEvent)
    {
        ...
        $oCmts = new ChWsbCmtsProfile('profile', $iItem);
        if(!$oCmts->isEnabled())
            return array();
    ...
    function displayProfileCommentPost($aEvent)
    {
        ...
        ch_import('ChWsbCmtsProfile');
        $oCmts = new ChWsbCmtsProfile('profile', $iId);
        if(!$oCmts->isEnabled())
            return array();
    ...
    function getComments($aEvent, $aResult)
    {
        ...
        $oComments = new ChWallCmts($sType, $iObjectId);
        if($oComments->isEnabled())
    ...
    {
        $oComments = new ChWallCmts($this->_oConfig->getCommentSystemName(), $iEventId);
    ...
    function getJsCode($sType, $aParams = array(), $aRequestParams = array())
    {
    	...
    	$aParams = array_merge(array(
    		'sActionUri' => $sBaseUri,
    		'sActionUrl' => CH_WSB_URL_ROOT . $sBaseUri,
    		'sObjName' => $sJsObject,
    		'iOwnerId' => 0,
	...
	function getRepostElement($iOwnerId, $sType, $sAction, $iObjectId, $aParams = array())
    {
		...
		if($bShowDoRepostAsButton)
			...
			$sClass .= ' ch-btn ch-btn-small';

		$sOnClick = '';
		if(!$bDisabled) {
			$sCommonPrefix = $this->_oConfig->getPrefix('common_post');
			if($sType == $sCommonPrefix . CH_WALL_PARSE_TYPE_REPOST) {
			...
			$sClass .= $bShowDoRepostAsButton || $bShowDoRepostAsButtonSmall ? ' ch-btn-disabled' : ' ' . $sStylePrefixRepost . 'disabled';
        ...
        return $this->parseHtmlByName($sTmplMain, array(
            ...
            'ch_if:show_counter' => array(
                'condition' => $bShowCounter,
                'content' => array(
                    'style_prefix' => $sStylePrefix,
        			'ch_if:show_hidden' => array(
    ...
    function getRepostCounter($aEvent, $aParams = array())
    {
        ...
        return $this->parseHtmlByName($sTmplCounter, array(
            'href' => 'javascript:void(0)',
            'title' => ch_html_attribute(_t('_wall_txt_reposted_by')),
            'ch_repeat:attrs' => array(
