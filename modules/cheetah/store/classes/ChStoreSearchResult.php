...
class ChStoreSearchResult extends ChWsbTwigSearchResult
{
    var $aCurrent = array(
        'name' => 'ch_store',
        'title' => '_ch_store_page_title_browse',
    ...
    function __construct($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        switch ($sMode) {
            ...
            case 'pending':
                if (false !== ch_get('ch_store_filter'))
                    $this->aCurrent['restriction']['keyword'] = array('value' => process_db_input(ch_get('ch_store_filter'), CH_TAGS_STRIP), 'field' => '','operator' => 'against');
                $this->aCurrent['restriction']['activeStatus']['value'] = 'pending';
                $this->sBrowseUrl = "administration";
                $this->aCurrent['title'] = _t('_ch_store_page_title_pending_approval');
            ...
            case 'search':
                ...
                $this->aCurrent['title'] = _t('_ch_store_page_title_search_results') . ' ' . (is_array($sValue2) ? implode(', ',$sValue2) : $sValue2) . ' ' . $sValue;
                unset($this->aCurrent['rss']);
                break;

            case 'user':
                $iProfileId = $GLOBALS['oChStoreModule']->_oDb->getProfileIdByNickName ($sValue, false);
                ...
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_by_author', $iProfileId ? getNickName($iProfileId) : $sValue);
            ...
            case 'admin':
                ...
                $this->aCurrent['title'] = _t('_ch_store_page_title_admin_products');
                break;
            ...
            case 'category':
                ...
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_by_category', $sValue);
                break;
            ...
            case 'tag':
                ...
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_by_tag', $sValue);
                break;

            case 'free':
                $this->aCurrent['restriction']['price'] = array('value' => 'Free', 'field' => 'price_range', 'operator' => '=');
                $this->sBrowseUrl = "browse/free";
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_free_products') . ' ' . $sValue;
                break;

            case 'recent':
                $this->sBrowseUrl = 'browse/recent';
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_recent');
                break;

            case 'top':
                $this->sBrowseUrl = 'browse/top';
                $this->aCurrent['sorting'] = 'top';
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_top_rated');
                break;

            case 'popular':
                $this->sBrowseUrl = 'browse/popular';
                $this->aCurrent['sorting'] = 'popular';
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_popular');
                break;

            case 'featured':
                $this->aCurrent['restriction']['featured'] = array('value' => 1, 'field' => 'featured', 'operator' => '=');
                $this->sBrowseUrl = 'browse/featured';
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_featured');
                break;
            ...
            case 'calendar':
                ...
                $this->aCurrent['title'] = _t('_ch_store_page_title_browse_by_day', getLocaleDate(strtotime("{$sValue}-{$sValue2}-{$sValue3}"), CH_WSB_LOCALE_DATE_SHORT));
                break;

            case '':
                $this->sBrowseUrl = 'browse/';
                $this->aCurrent['title'] = _t('_ch_store');
                unset($this->aCurrent['rss']);
                break;
        ...
        $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('ch_store_perpage_browse');
        ...
        if (ch_get('rss')) {
            ...
            $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('ch_store_max_rss_num');
        }

        ch_store_import('Voting', $this->getModuleArray());
        $oVotingView = new ChStoreVoting ('ch_store', 0);
    ...
    function getAlterOrder()
    {
        if ($this->aCurrent['sorting'] == 'last') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `ch_store_products`.`created` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'top') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `ch_store_products`.`rate` DESC, `ch_store_products`.`rate_count` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'popular') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `ch_store_products`.`views` DESC";
            return $aSql;
        }
        return array();
    }
    ...
    function displayResultBlock ()
    {
        ...
        if($s) {
            $s = $oFunctions->centerContent ($s, '.ch_store_unit');
    ...
    {
        return db_arr ("SELECT * FROM `sys_modules` WHERE `title` = 'Store' AND `class_prefix` = 'ChStore' LIMIT 1");
    }

    function getMain()
    {
        if ($GLOBALS['oChStoreModule']) {
           return $GLOBALS['oChStoreModule'];
        } else {
            $aModule = ChStoreSearchResult::getModuleArray();
            ch_import ('Module', $aModule);
            ch_import ('Template', $aModule);
            ch_import ('Config', $aModule);
            $GLOBALS['oChStoreModule'] = new ChStoreModule($aModule);
            return $GLOBALS['oChStoreModule'];
        }
    }
    ...
    {
        $oMain = $this->getMain();
        return CH_WSB_URL_ROOT . $oMain->_oConfig->getBaseUri() . 'view/' . $a['uri'];
    }
