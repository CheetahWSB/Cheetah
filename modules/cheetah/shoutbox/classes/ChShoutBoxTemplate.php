 ...
 */
    ...
    class ChShoutBoxTemplate extends ChWsbModuleTemplate
    {
        /**
         * Class constructor
         */
        ...
        function getProcessedMessages($aMessages = array(), $bDeleteAllowed = false, $bBlockAllowed = false)
        {
            ...
            $sOutputCode = '';
            $aLanguageKeys  = array(
                'by'        => _t('_ch_shoutbox_by'),
                'visitor'   => _t('_Visitor'),
                'delete'	=> _t('_ch_shoutbox_delete_message'),
                'sure'		=> _t('_Are_you_sure'),
                'block'		=> _t('_ch_shoutbox_block_ip'),
            );
            ...
            foreach($aMessages as $iKey => $aItems) {
                ...
                (
                    ...
                    'ch_if:delete_allowed' => array (
                        'condition' =>  $bDeleteAllowed,
                        'content'   => array (
                            'delete_cpt' => ch_html_attribute($aLanguageKeys['delete']),
                            'sure_cpt' => ch_js_string($aLanguageKeys['sure']),
                            'message_id' => $aItems['ID'],
                        ),
                    ),
                    'ch_if:block_allowed' => array (
                        'condition' =>  $bBlockAllowed,
                        'content'   => array (
                            'block_cpt' => ch_html_attribute($aLanguageKeys['block']),
                            'sure_cpt' => ch_js_string($aLanguageKeys['sure']),
                            'message_id' => $aItems['ID'],
                        ),
                    ),
        ...
        function getShoutboxWindow($sObject, $iHandler, $sModulePath, $iLastMessageId = 0, $sMessagesList = '')
        {
            ...
            $sShoutboxWrapperClass = "ch_shoutbox_{$sObject}_{$iHandler}";
            $aForm = array (
