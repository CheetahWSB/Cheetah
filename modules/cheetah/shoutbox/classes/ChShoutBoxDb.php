...
class ChShoutBoxDb extends ChWsbModuleDb
{
    var $_oConfig;
    ...
    {
        if (!isset($GLOBALS['ch_dol_shoutbox_objects'])) {
            $GLOBALS['ch_dol_shoutbox_objects'] = $GLOBALS['MySQL']->fromCache('ch_shoutbox_objects', 'getAllWithKey',
                'SELECT * FROM `ch_shoutbox_objects`', 'name');
        }

        return $GLOBALS['ch_dol_shoutbox_objects'];
    }

    function clearShoutboxObjectsCache()
    {
        $this->cleanCache('ch_shoutbox_objects');
    }
    ...
    function writeMessage($sObject, $iHandler, $sMessage, $iOwnerId = 0, $iIP = 0)
    {
        ...
        $sMessage = process_db_input($sMessage, 0, CH_SLASHES_AUTO);
    ...
    {
        foreach ($aData as $a) {
            $this->query("INSERT INTO `ch_shoutbox_objects` (`name`, `title`, `table`, `code_allow_use`, `code_allow_delete`, `code_allow_block`) VALUES (?, ?, ?, ?, ?, ?)",
                [
                    $a['name'],
                    $a['title'],
                    $a['table'],
    ...
    {
        foreach ($aData as $a) {
            $this->query("DELETE FROM `ch_shoutbox_objects` WHERE `name` = ?", [$a['name']]);
        }
    }
