 ...
 *
 * @see ChShoutBoxModule::serviceGetShoutBox();
 *      ChWsbService::call('shoutbox', 'get_shoutbox');
 *
 * Alerts:
 *
 * no alerts here;
 *
 */
class ChShoutBoxModule extends ChWsbModule
{
	var $sModuleName;
    ...
    function __construct(&$aModule)
    {
		...
		$this->sModuleName   = 'ch_' . $aModule['uri'];
    ...
    function actionWriteMessage($sObject, $iHandler)
    {
        if ($this->_checkObjectAndHandler($sObject, $iHandler) && $this->isShoutBoxAllowed($sObject, $iHandler,
            ...
            if ($sMessage) {
				...
				if($iMessage !== false) {
					$oAlert = new ChWsbAlerts($this -> sModuleName, 'add', $iMessage, $this -> iMemberId, array('Object' => $sObject, 'Message' => $sMessage));
					$oAlert->alert();
				}
                ...
                echo _t('_ch_shoutbox_message_empty');
            }
        } else {
            echo _t('_ch_shoutbox_access_denied');
        }
    }
    ...
    function actionBlockMessage($sObject, $iHandler, $iMessageId = 0)
    {
        ...
        if ($this->_checkObjectAndHandler($sObject, $iHandler) && $this->isShoutBoxBlockIpAllowed($sObject, $iHandler,
            ...
            if (!$aMessageInfo) {
                ...
                //block user IP
                ch_block_ip((int)$aMessageInfo['IP'], $this->_oConfig->iBlockExpirationSec,
                    _t('_ch_shoutbox_ip_blocked'));

                $this->_oDb->deleteMessagesByIp($sObject, $iHandler, $aMessageInfo['IP']);
            }
        } else {
            $sCallBackMessage = _t('_ch_shoutbox_access_denied');
        }
    ...
    function actionAdministration()
    {
        ...
        $aLanguageKeys = array(
            'settings' => _t('_ch_shoutbox_settings'),
        );
        ...
        $this->_oTemplate->pageCodeAdmin(_t('_ch_shoutbox_module'));
    }
     ...
     * @param $iHandler handler id
     */
    function serviceGetShoutBox($sObject = 'ch_shoutbox', $iHandler = 0)
    {
        if (!$this->_checkObjectAndHandler($sObject, $iHandler)) {
            $sObject = 'ch_shoutbox';
        }
    ...
    function serviceUpdateObjects($sModuleUri = 'all', $bInstall = true)
    {
        ...
        foreach ($aModules as $aModule) {
            if (!ChWsbRequest::serviceExists($aModule, 'get_shoutbox_data')) {
                continue;
            }

            if (!($aData = ChWsbService::call($aModule['uri'], 'get_shoutbox_data'))) {
                continue;
            }
    ...
    function isShoutBoxAllowed($sObject, $iHandler, $iMemberId, $isPerformAction = false)
    {
        ...
        if (!defined('CH_SHOUTBOX_USE')) {
            $this->_defineActions();
        }

        $aCheck = checkAction($iMemberId, CH_SHOUTBOX_USE, $isPerformAction);
    ...
    function isShoutBoxDeleteAllowed($sObject, $iHandler, $iMemberId, $isPerformAction = false)
    {
        ...
        if (!defined('CH_SHOUTBOX_DELETE_MESSAGES')) {
            $this->_defineActions();
        }

        $aCheck = checkAction($iMemberId, CH_SHOUTBOX_DELETE_MESSAGES, $isPerformAction);
    ...
    function isShoutBoxBlockIpAllowed($sObject, $iHandler, $iMemberId, $isPerformAction = false)
    {
        ...
        if (!defined('CH_SHOUTBOX_BLOCK_BY_IP')) {
            $this->_defineActions();
        }

        $aCheck = checkAction($iMemberId, CH_SHOUTBOX_BLOCK_BY_IP, $isPerformAction);
