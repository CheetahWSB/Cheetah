...
define('CH_H5AV_FALLBACK', true);

define('CH_H5AV_VIDEO_EMBED_HEIGHT', 315);
define('CH_H5AV_VIDEO_EMBED_WIDTH', 560);

define('CH_H5AV_AUDIO_EMBED_HEIGHT', 54);
define('CH_H5AV_AUDIO_EMBED_WIDTH', 560);

class ChH5avModule extends ChWsbModule
{
    ...
    {
        list($sPlayer, $sMessage) = $this->getVideoPlayer ($iFileId, false, false, 'height:' . CH_H5AV_VIDEO_EMBED_HEIGHT . 'px;');
    ...
    {
        list($sPlayer, $sMessage) = $this->getAudioPlayer ($iFileId, false, 'height:' . CH_H5AV_AUDIO_EMBED_HEIGHT . 'px;');
        echo $this->_oTemplate->parseHtmlByName('embed.html', array(
            'body' => $sPlayer,
            'ch_if:message' => array(
                'condition' => !(bool)$sPlayer,
    ...
    function serviceResponseAudioEmbed ($oAlert)
    {
        ...
        $oAlert->aExtras['override'] = '<iframe width="' . CH_H5AV_AUDIO_EMBED_WIDTH . '" height="' . CH_H5AV_AUDIO_EMBED_HEIGHT . '" src="' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'audio_embed/' . $iFileId . '" frameborder="0" allowfullscreen></iframe>';
    ...
    function serviceResponseVideoEmbed ($oAlert)
    {
        ...
        if ("" == $aFile['Source']) {
            $oAlert->aExtras['override'] = '<iframe width="' . CH_H5AV_VIDEO_EMBED_WIDTH . '" height="' . CH_H5AV_VIDEO_EMBED_HEIGHT . '" src="' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'video_embed/' . $iFileId . '" frameborder="0" allowfullscreen></iframe>';
        }
    ...
    function getVideoPlayer ($iFileId, $bEnableAutoplay = true, $bSetMaxHeight = true, $sCustomStyles = '')
    {
        ...
        switch($aFile['Status']) {
            ...
            case STATUS_APPROVED:
                if (file_exists($sFilesPath . $iFileId . M4V_EXTENSION)) {
                    ...
                    if (file_exists($sFilesPath . $iFileId . '.webm'))
                        $sSourceWebm = '<source type=\'video/webm; codecs="vp8, vorbis"\' src="' . CH_WSB_URL_ROOT . "flash/modules/video/get_file.php?id=" . $iFileId . "&ext=webm&token=" . $sToken . '" />';
                    ...
                    $sId = 'ch-media-' . genRndPwd(8, false);
                    $sJs = $sSourceWebm ? // if no .webm video available - we need nice fallback in firefox and other browsers with no mp4 support
                            ...
                            if (eMedia.canPlayType && !eMedia.canPlayType("video/x-m4v")) {
                                var sReplace = "' . ch_js_string(CH_H5AV_FALLBACK ? $sFlash : '<b>Your browser doesn\'t support this media playback.</b>', CH_ESCAPE_STR_QUOTE) . '";
                                $("#' . $sId . '").replaceWith(sReplace);
                            }';
                    $sJs .= $aFile['Time'] ? // if length is not set
                            '' : '
                            eFile.on("canplay", function (e) {
                                $.post("' . CH_WSB_URL_ROOT . 'flash/XML.php", {
                                    module: "video",
                    ...
                    $sAutoPlay = $bEnableAutoplay && TRUE_VAL == getSettingValue('video', 'autoPlay') && class_exists('ChVideosPageView') ? 'autoplay' : '';

                    $sFilePoster = 'flash/modules/video/files/' . $iFileId . '.jpg';
                    $sPoster = file_exists(CH_DIRECTORY_PATH_ROOT . $sFilePoster) ? ' poster="' . CH_WSB_URL_ROOT . $sFilePoster . '" ' : '';
                    ...
                    $sPlayer = '
                        <video controls preload="metadata" autobuffer ' . $sAutoPlay . $sPoster . ' style="width:100%;' . $sStyleMaxHeight . $sCustomStyles . '" id="' . $sId . '">
                            ' . $sSourceWebm . '
                            <source src="' . CH_WSB_URL_ROOT . "flash/modules/video/get_file.php?id=" . $iFileId . "&ext=m4v&token=" . $sToken . '" />
                            ' . (CH_H5AV_FALLBACK ? $sFlash : '<b>Can not playback media - your browser doesn\'t support HTML5 audio/video tag.</b>') . '
                        </video>
                        <script>
                            var eFile = $("#' . $sId . '");
            ...
            case STATUS_FAILED:
            default:
                if (!CH_H5AV_FALLBACK || !file_exists($sFilesPath . $iFileId . FLV_EXTENSION))
    ...
    function serviceResponseVideoDelete ($oAlert)
    {
        ...
        $oMain = ChWsbModule::getInstance('ChVideosModule');
            ...
            case STATUS_APPROVED:
                if (file_exists($GLOBALS['sFilesPathMp3'] . $iFileId . MP3_EXTENSION)) {
                    ...
                    if (file_exists($GLOBALS['sFilesPathMp3'] . $iFileId . '.ogg'))
                        $sSourceOgg = '<source type=\'audio/ogg; codecs="vorbis"\' src="' . CH_WSB_URL_ROOT . "flash/modules/mp3/get_file.php?id=" . $iFileId . "&token=" . $sToken . '&ext=ogg" />';
                    ...
                    $sAutoPlay = $bEnableAutoplay && TRUE_VAL == getSettingValue('mp3', 'autoPlay') && class_exists('ChSoundsPageView') ? 'autoplay' : '';
                    $sPlayer = '
                        <audio controls ' . $sAutoPlay . ' preload="metadata" autobuffer style="width:100%; ' . $sCustomStyles . '" id="' . $sId . '">
                            <source type=\'audio/mpeg; codecs="mp3"\' src="' . CH_WSB_URL_ROOT . "flash/modules/mp3/get_file.php?id=" . $iFileId . "&token=" . $sToken . '" />
                            ' . $sSourceOgg . '
                            ' . (CH_H5AV_FALLBACK ? $sFlash : '<b>Can not playback media - your browser doesn\'t support HTML5 audio/video tag.</b>') . '
                        </audio>
                        <script>
    ...
    function serviceResponseAudioDelete ($oAlert)
    {
        ...
        $oMain = ChWsbModule::getInstance('ChSoundsModule');
        ...
        switch($aFile['Status']) {
            ...
            case VC_STATUS_APPROVED:
                if (file_exists($sFilesPath . $iFileId . VC_M4V_EXTENSION)) {
                    ...
                    if (file_exists($sFilesPath . $iFileId . '.webm'))
                        $sSourceWebm = '<source type=\'video/webm; codecs="vp8, vorbis"\' src="' . CH_WSB_URL_ROOT . "flash/modules/video_comments/get_file.php?id=" . $iFileId . "&ext=webm&token=" . $sToken . '" />';
                    ...
                    $sOverride = '
                        <video controls preload="metadata" autobuffer id="' . $sId . '">
                            ' . $sSourceWebm . '
                            <source src="' . CH_WSB_URL_ROOT . "flash/modules/video_comments/get_file.php?id=" . $iFileId . "&ext=m4v&token=" . $sToken . '" />
                            ' . (CH_H5AV_FALLBACK ? $sFlash : '<b>Can not playback media - your browser doesn\'t support HTML5 audio/video tag.</b>') . '
                        </video>' .
