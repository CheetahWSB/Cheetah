...
class ChFilesUploader extends ChWsbFilesUploader
{
    // constructor
    ...
    function GenSendFileInfoForm($iFileID, $aDefaultValues = array())
    {
        ...
        if(isset($aDefaultValues['image']))
            ...
            $aFileInfo = ChWsbService::call('files', 'get_file_array', array($iFileID), 'Search');
    ...
    {
        $aCustom['Approved'] = getParam('ch_files_activation') == 'on' ? 'approved' : 'pending';
    ...
    function performFileUpload($sTmpFile, $aFileInfo, $isMoveUploadedFile = true, $sOriginalFilename = '', $sFileType = '')
    {
        ...
        if (($isMoveUploadedFile && is_uploaded_file($sTmpFile)) || !$isMoveUploadedFile) {
            ...
            if (file_exists($sTempFileName)) {
                $sOriginalFilenameSafe = process_db_input($sOriginalFilename, CH_TAGS_STRIP);
                ...
                if (is_array($aFileInfo) && count($aFileInfo) > 0) {
                    ...
                    if (getParam('ch_files_activation') == 'on') {
                        $bAutoActivate = true;
                    ...
                    if ($sFileType)
                        $aPassArray['Type'] = process_db_input($sFileType, CH_TAGS_STRIP);
