...
class ChFilesPageAlbumsMy extends ChWsbPageView
{
    var $oTemplate;
    var $oConfig;
    var $oDb;
    ...
    {
        parent::__construct('ch_files_albums_my');
        ...
        $this->oSearch = new ChFilesSearch('album', $this->aAddParams[1], 'owner', getUsername($this->iOwnerId));
        $this->oAlbum = new ChWsbAlbums('ch_files', $this->iOwnerId);
    ...
    function getBlockCode_add ()
    {
        ...
        $aPrivFieldView = $this->oAlbumPrivacy->getGroupChooser($this->iOwnerId, $this->oConfig->getUri(), 'album_view', array(), _t('_' . $this->oConfig->getMainPrefix() . '_album_view'));
    ...
    function getBlockCode_edit ()
    {
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            if ($this->oAlbum->updateAlbumById((int)$_POST['ID'], $aData))
                $sCode = MsgBox(_t('_ch_' . $this->oConfig->getUri() . '_album_save_success'));
        } else
            $sCode = $oForm->getCode();
