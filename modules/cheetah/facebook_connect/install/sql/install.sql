    ...
    --
    -- Table structure for table `ch_facebook_accounts`
    --
    ...
    ('t_fb_connect_password_generated', 'New Password Generated', '<ch_include_auto:_email_header.html />\r\n\r\n<p><b>Hello <NickName></b>, \r\n\r\n<p>\r\nYour new password - <b><NewPassword></b></p>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Facebook connect password generated', 0);
    ...
    (`Name`, `Title`, `Link`, `Icon`)
        VALUES
    ('facebook', '_ch_facebook_auth_title', 'modules/?r=facebook_connect/login_form', 'facebook-square');
    ...
    INSERT INTO
        `sys_alerts_handlers`
    SET
        `name`  = 'ch_facebook_connect',
        `class` = 'ChFaceBookConnectAlerts',
        `file`  = 'modules/cheetah/facebook_connect/classes/ChFaceBookConnectAlerts.php';
    ...
    INSERT INTO
        `sys_options`
    SET
        `Name` = 'ch_facebook_connect_api_key',
        `kateg` = @iKategId,
    ...
    INSERT INTO
        `sys_options`
    SET
        `Name` = 'ch_facebook_connect_secret',
        `kateg` = @iKategId,
    ...
    INSERT INTO
        `sys_options`
    SET
        `Name` = 'ch_facebook_connect_redirect_page',
        `kateg` = @iKategId,
    ...
    INSERT INTO
        `sys_options`
    SET
        `Name` = 'ch_facebook_connect_auto_friends',
        `kateg` = @iKategId,
    ...
    INSERT INTO
        `sys_options`
    SET
        `Name` = 'ch_facebook_connect_extended_info',
        `kateg` = @iKategId,
    ...
    SET
        ...
        `check`     = 'ch_facebook_connect_permalinks';
    ...
    (`Name`, `VALUE`, `kateg`, `desc`, `Type`, `check`, `err_text`, `order_in_kateg`, `AvailableValues`)
        VALUES
    ('ch_facebook_connect_permalinks', 'on', 26, 'Enable friendly permalinks in facebook connect', 'checkbox', '', '', '0', '');
    ...
    ('ch_facebook', '_ch_facebook_chart', '', '', 'DateReg', 0, 1, '', '', 'SELECT {field_date_formatted} AS `period`, COUNT(*) AS {object} FROM `Profiles` INNER JOIN `ch_facebook_accounts` ON (`id_profile` = `ID`) WHERE {field_date} >= ''{from}'' AND {field_date} <= ''{to}'' GROUP BY `period` ORDER BY {field_date} ASC', 1, @iMaxOrderCharts);
    ...
    ('ch_facebook', '_sys_module_facebook_connect', 'ChFaceBookConnectExport', 'modules/cheetah/facebook_connect/classes/ChFaceBookConnectExport.php', @iMaxOrderExports, 1);
