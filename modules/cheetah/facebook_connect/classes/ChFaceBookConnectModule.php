...
class ChFaceBookConnectModule extends ChWsbConnectModule
{
    var $oFacebook;
    ...
    function __construct(&$aModule)
    {
        ...
        require_once(CH_DIRECTORY_PATH_PLUGINS . 'facebook-php-sdk/src/Facebook/autoload.php');
    ...
    {
        parent::_actionAdministration('ch_facebook_connect_api_key', '_ch_facebook_settings',
            '_ch_facebook_information', '_ch_facebook_information_block', $this->_oConfig->sPageReciver);
    }
    ...
    function actionLoginCallback()
    {
        ...
        if (!$this->_oConfig->mApiID || !$this->_oConfig->mApiSecret) {
            $sCode = MsgBox(_t('_ch_facebook_profile_error_api_keys'));
        }
        ...
        if (!$sCode) {
            ...
            if ($aFacebookProfileInfo) {
                ...
                if ($iProfileId) {
                    // get profile info
                    $aCheetahProfileInfo = getProfileInfo($iProfileId);
                    $this->setLogged($iProfileId, $aCheetahProfileInfo['Password']);
                } else {
                ...
                $sCode = MsgBox(_t('_ch_facebook_profile_error_info'));
            }
        }


        $this->_oTemplate->getPage(_t('_ch_facebook'), $sCode);
    }
    ...
    {
        if (getParam('enable_cheetah_footer') == 'on') {
            return array('error' => _t('_ch_facebook_error_unlicensed_site'));
        }
        ...
        if (!isset($aFacebookProfileInfoCheck['id']) || $aFacebookProfileInfoCheck['id'] != $aFacebookProfileInfo['id']) {
            return array('error' => _t('_ch_facebook_profile_error_info'));
        }
        ...
        if ($iProfileId) {
            ...
            return array(
                ...
                'member_pwd_hash' => $aCheetahProfileInfo['Password'],
                'member_username' => getUsername($iProfileId),
                'protocol_ver'    => CH_XMLRPC_PROTOCOL_VER,
            );
        } else {
        ...
        do {
            foreach ($oPagesEdge as $oPage) {
                ...
                if ($iFriendId && !is_friends($iProfileId, $iFriendId)) {
                    ...
                    //create system alert
                    $oZ = new ChWsbAlerts('friend', 'accept', $iProfileId, $iFriendId);
                    $oZ->alert();
                }
            }
