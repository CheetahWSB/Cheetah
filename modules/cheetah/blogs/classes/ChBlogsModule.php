...
require_once('ChBlogsPrivacy.php');

if (!defined('CH_BLOGS_IMAGES_PATH')) {
    define('CH_BLOGS_IMAGES_PATH', CH_DIRECTORY_PATH_ROOT . "media/images/blog/");
}

if (!defined('CH_BLOGS_IMAGES_URL')) {
    define('CH_BLOGS_IMAGES_URL', CH_WSB_URL_ROOT . "media/images/blog/");
}

class ChWsbBlogsPageView extends ChWsbPageView
{
    var $oBlogs;
 ...
 *
 * Memberships/ACL:
 * View blog - CH_BLOG_VIEW
 * View posts - CH_BLOG_POST_VIEW
 * Browse blogs - CH_BLOGS_BROWSE
 * Browse posts - CH_BLOGS_POSTS_BROWSE
 * Use search and view search results - CH_BLOG_POST_SEARCH
 * Add posts - CH_BLOG_POST_ADD
 * Edit any post (as admin) - CH_BLOG_POSTS_EDIT_ANY_POST
 * Delete any post (as admin) - CH_BLOG_POSTS_DELETE_ANY_POST
 * Approve any post (as admin) - CH_BLOG_POSTS_APPROVING
 *
 *
 *
 * Service methods:
 ...
 * @see ChBlogsModule::serviceActionDeleteBlog
 * ChWsbService::call('blogs', 'action_delete_blog', array());
 *
 * Blogs block for index page (as PHP function)
 * @see ChBlogsModule::serviceBlogsIndexPage
 * ChWsbService::call('blogs', 'blogs_index_page', array());
 *
 * Blogs block for profile page (as PHP function)
 * @see ChBlogsModule::serviceBlogsProfilePage
 * ChWsbService::call('blogs', 'blogs_profile_page', array($_iProfileID));
 *
 * Generation of member RSS feeds
 * @see ChBlogsModule::serviceBlogsRss
 * ChWsbService::call('blogs', 'blogs_rss', array());
 *
 * Get common css
 * @see ChBlogsModule::serviceGetCommonCss
 * ChWsbService::call('blogs', 'get_common_css', array());
 *
 * Get member menu item - my content
 * @see ChBlogsModule::serviceGetMemberMenuItem
 * ChWsbService::call('blogs', 'get_member_menu_item');
 *
 * Get member menu item - add content
 * @see ChBlogsModule::serviceGetMemberMenuItemAddContent
 * ChWsbService::call('blogs', 'get_member_menu_item_add_content');
 *
 * Get number of posts for particular member
 * @see ChBlogsModule::serviceGetPostsCountForMember
 * ChWsbService::call('blogs', 'get_posts_count_for_member', array($iMemberId));
 *
 * Get Spy data
 * @see ChBlogsModule::serviceGetSpyData
 * ChWsbService::call('blogs', 'get_spy_data', array());
 *
 * Get Spy blog post
 * @see ChBlogsModule::serviceGetSpyPost
 * ChWsbService::call('blogs', 'get_spy_post', array($sAction, $iObjectId, $iSenderId));
 *
 *
 *
 * Alerts:
 * Alerts type/unit - 'ch_blogs'
 ...
 *
 */
class ChBlogsModule extends ChWsbModule
{
    //variables
    ...
    function __construct($aModule)
    {
        ...
        $this->iIconSize = (int)getParam('ch_blogs_iconsize');
        $this->iThumbSize = (int)getParam('ch_blogs_thumbsize');
        $this->iBigThumbSize = (int)getParam('ch_blogs_bigthumbsize');
        $this->iImgSize = (int)getParam('ch_blogs_imagesize');
        ...
        $this->oPrivacy = new ChBlogsPrivacy($this);
    }
    ...
    function GetHeaderString()
    {
        switch (ch_get('action')) {
            case 'home':
                $sCaption = _t('_ch_blog_Blogs_Home');
                break;
            case 'all':
                $sCaption = _t('_ch_blog_All_Blogs');
                break;
            case 'tags':
                $sCaption = _t('_Tags');
                break;
            case 'all_posts':
                $sCaption = _t('_ch_blog_All_Posts');
                break;
            case 'top_posts':
                $sCaption = _t('_ch_blog_Top_Posts');
                break;
            case 'popular_posts':
                $sCaption = _t('_ch_blog_Popular_Posts');
                break;
            case 'featured_posts':
                $sCaption = _t('_ch_blog_Featured_Posts');
                break;
            /*case 'show_categories':
                $sCaption = _t('_ch_blog_Categories');
                break;*/
            case 'show_calendar':
                $sCaption = _t('_ch_blog_Calendar');
                break;
            case 'show_calendar_day' :
                ...
                $sCaption = _t('_ch_blog_caption_browse_by_day')
            ...
            case 'show_member_blog':
                ...
                if (!$iMemberID) {
                    ...
                    $sAsBlog = _t('_ch_blog_Members_blog', $sUser);
                }
            ...
            case 'show_member_post':
                $sCaption = _t('_Post');
                if (false !== ch_get('postUri')) {
                    $sPostUri = process_db_input(ch_get('postUri'), CH_TAGS_STRIP);
                    $sPostCapt = $this->_oDb->getPostCaptionByUri($sPostUri);
                } elseif (false !== ch_get('post_id')) {
                    $iPostID = (int)ch_get('post_id');
            ...
            case 'my_page':
                switch (ch_get('mode')) {
                    case 'add':
                        $sCaption = _t('_ch_blog_Add') . ' ' . _t('_Post');
                        break;
                    case 'manage':
                        $sCaption = _t('_ch_blog_Manage');
                        break;
                    case 'pending':
                        $sCaption = _t('_ch_blog_pending_approval');
                        break;
                    default:
                        $sCaption = _t('_ch_blog_My_blog');
                        break;
    ...
    function isAllowedCreatorCommentsDeleteAndEdit(&$aBlogPost, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POSTS_COMMENTS_DELETE_AND_EDIT, $isPerformAction);
    ...
    function isAllowedBlogView($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_VIEW, $isPerformAction);
    ...
    function isAllowedBlogPostView($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POST_VIEW, $isPerformAction);
    ...
    function isAllowedBlogsBrowse($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOGS_BROWSE, $isPerformAction);
    ...
    function isAllowedBlogsPostsBrowse($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOGS_POSTS_BROWSE, $isPerformAction);
    ...
    function isAllowedBlogPostSearch($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POST_SEARCH, $isPerformAction);
    ...
    function isAllowedPostAdd($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POST_ADD, $isPerformAction);
    ...
    function isAllowedPostEdit($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POSTS_EDIT_ANY_POST, $isPerformAction);
    ...
    function isAllowedPostDelete($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POSTS_DELETE_ANY_POST, $isPerformAction);
    ...
    function isAllowedApprove($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_BLOG_POSTS_APPROVING, $isPerformAction);
    ...
    function GenBlogAdminIndex()
    {
        if ($this->bAdminMode) {
            //actions
            if (ch_get('action_approve') && is_array(ch_get('bposts'))) {
                foreach (ch_get('bposts') as $iBPostID) {
                    if ($this->_oDb->setPostStatus((int)$iBPostID, 'approval')) {
                        $this->onPostApproveDisapprove($iBPostID, true);
                    }
                }
            } elseif (ch_get('action_disapprove') && is_array(ch_get('bposts'))) {
                foreach (ch_get('bposts') as $iBPostID) {
                    if ($this->_oDb->setPostStatus((int)$iBPostID)) {
                        $this->onPostApproveDisapprove($iBPostID, false);
                    }
                }
            } elseif (ch_get('action_delete') && is_array(ch_get('bposts'))) {
                foreach (ch_get('bposts') as $iBPostID) {
            ...
            require_once($this->_oConfig->getClassPath() . 'ChBlogsSearchUnit.php');
            $oBlogSearch = new ChBlogsSearchUnit();
            ...
            $sPosts = ($oBlogSearch->aCurrent['paginate']['totalNum'] == 0) ? MsgBox(_t('_ch_blog_No_blogs_available')) : $sPosts;
    ...
    function GenBlogHome()
    {
        ...
        $oHomePageView = new ChBlogsPageHome($this);
    ...
    function GenBlogLists($sType = '', $bBlock = true)
    {
        ...
        // lang keys
        $sPostsC = _t('_ch_blog_Posts');
        $sNoBlogsC = _t('_Empty');
        $sAllBlogsC = _t('_ch_blog_All_Blogs');
        $sTopBlogsC = _t('_ch_blog_Top_Blogs');

        $iCheckedMemberID = $this->_iVisitorID;

        //////////////////pagination addition//////////////////////////
        //number elements for per page
        $iPerPage = (false !== ch_get('per_page')) ? (int)ch_get('per_page') : 10;

        if ($iPerPage > 100) {
            $iPerPage = 100;
        }

        $iCurPage = (false !== ch_get('page')) ? (int)ch_get('page') : 1;
        if ($iCurPage < 1) {
            $iCurPage = 1;
        }
        ...
        while ($aBlogsRes = $vBlogsRes->fetch()) {
            ...
            if ($sOwnerNickname) {
                ...
                $aBlogUnitVariables = array(
                    ...
                    'owner_nickname'   => $sOwnerNickname . ' ' . _t('_ch_blog_Blog'),
        ...
        if ($this->isPermalinkEnabled() == false) {
            $sRequest = ch_html_attribute($_SERVER['PHP_SELF']) . '?action=top_blogs&page={page}&per_page={per_page}';
        } else {
            $sRequest = (ch_get('action') == 'top_blogs')
                ? CH_WSB_URL_ROOT . 'blogs/top/'
                : CH_WSB_URL_ROOT . 'blogs/all/';
...
EOF;
    ...
    function GenPostListMobile($iAuthor = 0, $sMode = false)
    {
        ...
        {
            ch_login($this->_iVisitorID);
        }

        ch_import('ChWsbMobileTemplate');
        $oMobileTemplate = new ChWsbMobileTemplate($this->_oConfig, $this->_oDb);
        $oMobileTemplate->pageStart();
        echo $oMobileTemplate->addCss('blogs_common.css', 1);

        $iPerPage = 10;
        $iPage = (int)ch_get('page');
        if ($iPage < 1) {
            $iPage = 1;
        }
        ...
        switch ($sMode) {
            case 'post':
                $aViewingPostInfo = $this->_oDb->getPostInfo((int)ch_get('id'));
                if (!$this->oPrivacy->check('view', (int)ch_get('id'),
                ...
                $aParams = array('id' => (int)ch_get('id'));
                $sCaption = _t('_ch_blog_post_view');
                $sMobileWrapper = 'mobile_box.html';
                echo $oMobileTemplate->addCss('blogs.css', 1);
                break;
            case 'user':
                $aParams = array('id' => (int)ch_get('id'));
                $sCaption = _t('_ch_blog_Members_blog', getNickName((int)ch_get('id')));
                break;
            case 'featured':
                $sCaption = _t('_ch_blog_Featured_Posts');
                break;
            ...
            default:
                $sMode = 'last';
                $sCaption = _t('_ch_blog_Latest_posts');
        }
    ...
    function GenPostLists($sType = '', $bBlock = true)
    {
        ...
        $sCaption = ($this->_sPageHeader != '') ? $this->_sPageHeader : _t('_ch_blog_Top_Posts');
...
EOF;
    ...
    function _GenPosts($iPostViewType, $iPerPage, $sMode, $aParams, $sOrder, &$oSearchResult, $sMobileWrapper = false)
    {
        ...
        $oTmpBlogSearch = new ChBlogsSearchUnit($this);
    ...
    {
        $sCatUri = process_db_input(ch_get('uri'), CH_TAGS_STRIP);
        if ($sCatUri) {
            $this->iPostViewType = 4;
            ...
            $sRetHtmlVal = '<div class="ch-def-bc-padding">' . $sCode . '</div>';
    ...
    function GenMemberDescrAndCat($aBlogsRes, $sCategoryName = '')
    {
        ...
        if ($this->iViewingPostID == -1) {
            ...
            $aBlogActionKeys = array(
                ...
                'site_url'            => CH_WSB_URL_ROOT,
                'blog_id'             => $aBlogID,
                'blog_description_js' => $sDescrAct,
            );

            $sBlogActionsVal = $GLOBALS['oFunctions']->genObjectsActions($aBlogActionKeys, 'ch_blogs_m', false);
...
<div id="edited_blog_div" style="display: none; position:relative;">
    <div class="ch-def-bc-padding" style="padding-top:0;">
        <form action="{$sWorkLink}" method="post" name="EditBlogForm">
            ...
            <div class="ch-def-margin-sec-top">
                <input type="submit" value="{$sApplyChangesC}" class="form_input_submit ch-btn ch-btn-small" />
            </div>
...
EOF;
        ...
        $sFeaturedSect = ($sFeaturedSectCont) ? DesignBoxContent(_t('_ch_blog_Featured_Posts'),
    ...
    function GenMemberBlog($iUserID = 0)
    {
        ...
        $sBreadCrumb = ($sCurCategory != '') ? $sCurCategory : _t('_ch_blog_Latest_posts');
        ...
        if (!$this->isAllowedBlogsPostsBrowse()) {
            ...
            if ($this->bUseFriendlyLinks == false || $this->bAdminMode == true) { //old variant
                ...
                $sRequest = ch_append_url_params($sOwnerBlogLink, $sCategUrlAdd . '&page={page}&per_page={per_page}');
            } else {
                $sCategUrlAdd = $sCategoryName ? "/category/{$sCategoryName}" : '';
                $sRequest = ch_append_url_params($sOwnerBlogLink . $sCategUrlAdd, 'page={page}&per_page={per_page}');
            }
...
EOF;
    ...
    function ActionDelImg()
    {
        ...
        $sSuccUpdPost = _t('_ch_blog_Post_succ_updated');
        $sFailUpdPost = _t('_ch_blog_Post_fail_updated');
        ...
        if ((($this->_iVisitorID == $iPostOwnerID && $iPostOwnerID > 0) || $this->bAdminMode) && $iPostID > 0) {
            ...
            $sDFilePath = CH_BLOGS_IMAGES_PATH . "small_{$sFileName}";
            @unlink($sDFilePath);
            $sDFilePath = CH_BLOGS_IMAGES_PATH . "big_{$sFileName}";
            @unlink($sDFilePath);
            $sDFilePath = CH_BLOGS_IMAGES_PATH . "orig_{$sFileName}";
            @unlink($sDFilePath);
    ...
    function ActionDeletePost($iPostID = 0)
    {
        ...
        if ($iPostID > 0) {
            $oCmts = new ChWsbCmts($this->_oConfig->getCommentSystemName(), (int)$iPostID);
            $oCmts->onObjectDelete();

            // delete votings
            ch_import('ChTemplVotingView');
            $oVoting = new ChTemplVotingView ($this->_oConfig->getRateSystemName(), $iPostID);
            $oVoting->deleteVotings($iPostID);

            $sFileName = $this->_oDb->getPostPhotoByID($iPostID);
            $sFilePathPost = 'big_' . $sFileName;
            if ($sFilePathPost != '' && file_exists(CH_BLOGS_IMAGES_PATH . $sFilePathPost) && is_file(CH_BLOGS_IMAGES_PATH . $sFilePathPost)) {
                @unlink(CH_BLOGS_IMAGES_PATH . $sFilePathPost);
            }
            ...
            $oCategories->reparseObjTags('ch_blogs', $iPostID);

            // delete views
            ch_import('ChWsbViews');
            $oViews = new ChWsbViews($this->_oConfig->getViewSystemName(), $iPostID, false);
            $oViews->onObjectDelete();

            //delete all subscriptions
            $oSubscription = ChWsbSubscription::getInstance();
            $oSubscription->unsubscribe(array('type' => 'object_id', 'unit' => 'ch_blogs', 'object_id' => $iPostID));

            ch_import('ChWsbAlerts');
            $oZ = new ChWsbAlerts('ch_blogs', 'delete_post', $iPostID, $iPostOwnerID);
            $oZ->alert();
    ...
    function getViewingPostInfo()
    {
        if ($this->iViewingPostID < 0) {
            if (false !== ch_get('postUri')) {
                ...
                $this->iViewingPostID = (int)ch_get('post_id');
            }
        }
        if ($this->iViewingPostID) {
            ...
            if (is_array($this->aViewingPostInfo) && $this->aViewingPostInfo) {
                ...
                ch_import('ChWsbViews');
                new ChWsbViews($this->_oConfig->getViewSystemName(), $this->iViewingPostID);

                ch_import('ChWsbAlerts');
                $oZ = new ChWsbAlerts('ch_blogs', 'view_post', $this->iViewingPostID, $this->_iVisitorID);
                $oZ->alert();

                if ($this->aViewingPostInfo['PostPhoto'] != '' && file_exists(CH_BLOGS_IMAGES_PATH . 'small_' . $this->aViewingPostInfo['PostPhoto'])) {
                    $GLOBALS['oTopMenu']->setCustomSubIconUrl(CH_BLOGS_IMAGES_URL . 'small_' . $this->aViewingPostInfo['PostPhoto']);
                } else {
                ...
                $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
                    _t('_ch_blog_Blogs') => $this->genBlogLink('home'),
                    $sPostCaption        => '',
                ));
...
<a title="{$sPrevPostCaption}" href="{$sUnitUrl}" class="backMembers ch-def-margin-thd-left">
    {$sPrevPostCaption}
</a>
EOF;
        }
...
<a title="{$sNextPostCaption}" href="{$sUnitUrl}" class="moreMembers ch-def-margin-thd-right">
    {$sNextPostCaption}
</a>
EOF;
        }
...
EOF;
        ...
        return DesignBoxContent(_t('_ch_blog_post_view'), $sPostStringVal, 11, '', $sSPaginate);
    }
    ...
    function getPostSocialSharingBlock()
    {
        ...
        if ($this->aViewingPostInfo['PostPhoto']) {
            $sImgUrl = CH_BLOGS_IMAGES_URL . 'orig_' . $this->aViewingPostInfo['PostPhoto'];
            $aCustomParams = array(
                'img_url'         => $sImgUrl,
    ...
    {
        require_once($this->_oConfig->getClassPath() . 'ChBlogsCmts.php');
        $_oCmtsView = new ChBlogsCmts($this->_oConfig->getCommentSystemName(), $this->iViewingPostID);
        $sPostComm = $_oCmtsView->getExtraCss();
        $sPostComm .= $_oCmtsView->getExtraJs();
        $sPostComm .= (!$_oCmtsView->isEnabled()) ? MsgBox(_t('_ch_blog_Comments_is_disabled')) : $_oCmtsView->getCommentsFirst();
    ...
    function getActionsBlock()
    {
        if ($this->iPostViewType == 3 && $this->iViewingPostID > 0) {
            ...
            $oSubscription = ChWsbSubscription::getInstance();
            $aButton = $oSubscription->getButton($this->_iVisitorID, 'ch_' . $this->_oConfig->getUri(), '',
                $this->iViewingPostID);
	        ...
	        if(ChWsbRequest::serviceExists('wall', 'get_repost_js_click')) {
				...
				$aActionKeys['repostScript'] = ChWsbService::call('wall', 'get_repost_js_click', array($this->_iVisitorID, 'ch_blogs', 'create', $this->iViewingPostID));
	        }

            $sActionsVal = $GLOBALS['oFunctions']->genObjectsActions($aActionKeys, 'ch_blogs', false);
    ...
    function serviceGetSubscriptionParams($sAction, $iEntryId)
    {
        ...
        $aActionList = array(
            'commentPost' => '_ch_blog_sbs_comments'
        );
    ...
    function getRateBlock()
    {
        ...
        $oVotingView = new ChTemplVotingView ($this->_oConfig->getRateSystemName(), $this->iViewingPostID);
    ...
    function getPostOverviewBlock()
    {
        $iMemberID = (int)$this->aViewingPostInfo['OwnerID'];
        $aBlogInfo = $this->_oDb->getBlogInfo($iMemberID);
        $sBlogDescription = '<div class="blog_desc ch-def-margin-sec-top">' . process_html_output($aBlogInfo['Description']) . '</div>';
        ...
        if ($this->iPostViewType == 3 && $this->iViewingPostID > 0) {
            ...
            $aVars['date'] = getLocaleDate($this->aViewingPostInfo['PostDate'], CH_WSB_LOCALE_DATE_SHORT);
...
EOF;
    ...
    function getPostCategoriesBlock()
    {
        ...
        $sCategoriesC = _t('_ch_blog_Categories');
        $sPostsCL = mb_strtolower(_t('_ch_blog_Posts'));

        $sCategories = '';

        $oCategories = new ChWsbCategories();
        $aAllCategories = $oCategories->getCategoriesList('ch_blogs', $iMemberID);
        ...
        if (is_array($aAllCategories) && count($aAllCategories) > 0) {
            foreach ($aAllCategories as $iCatID => $sCategoryName) {
                ...
                $sCategories .= <<<EOF
<div class="cls_result_row ch-def-margin-sec-top-auto">
    <div class="cls_categ_name">
        <i class="sys-icon folder"></i>
        <a href="{$sCatLink}">{$sCatName}</a>&nbsp;<span class="blog_author ch-def-font-grayed ch-def-font-small">({$iCountCatPost} {$sPostsCL})</span>
    </div>
</div>
EOF;
            }
...
EOF;
    ...
    function GenPostPage($iParamPostID = 0)
    {
        ...
        $sRetHtml .= $sCode;
        if ($bShowBlocks) {
            $oBPV = new ChWsbBlogsPageView($this);
    ...
    function GenMyPageAdmin($sMode = '')
    {
        ...
        $sMainC = _t('_ch_blog_Manage_main');
        $sAddC = _t('_ch_blog_Add');
        $sManageC = _t('_ch_blog_Manage');
        $sPendingC = _t('_ch_blog_pending_approval');
        $sAdministrationC = _t('_ch_blog_Administration');
        $sMyBlogC = _t('_ch_blog_My_blog');
        $sPendApprC = _t('_ch_blog_pending_approval');
        $sMyPostsC = _t('_ch_blog_My_posts');
        ...
        if (ch_get('action_delete') && is_array(ch_get('bposts'))) {
            foreach (ch_get('bposts') as $iBPostID) {
                $this->ActionDeletePost((int)$iBPostID);
            }
        }
        ...
        require_once($this->_oConfig->getClassPath() . 'ChBlogsSearchUnit.php');
        $oTmpBlogSearch = new ChBlogsSearchUnit();
        $oTmpBlogSearch->PerformObligatoryInit($this, 4);
        ...
        switch ($sMode) {
            ...
            case 'manage':
                ...
                $sManagePostsPagination = $oTmpBlogSearch->showPaginationAjax('ch_blogs_user_form', $sBlogManageLink);

                $sAdmPanel = $oTmpBlogSearch->showAdminActionsPanel('bposts_box', array('action_delete' => '_Delete'),
                    'bposts');
                $sManagePostsUnits = <<<EOF
<div id="bposts_box" class="ch-def-bc-padding">
    {$sActivePosts}
    <div class="clear_both"></div>
</div>
...
EOF;
                ...
                $sAdmContent = '<form id="ch_blogs_user_form" method="post">' . $sManagePostsUnits . '</form>';
                break;
            case 'pending':
                $sPendingTabClass = 1;
                ...
                $sPendingPostsPagination = $oTmpBlogSearch->showPaginationAjax('ch_blogs_pending', $sBlogPendingLink);
...
EOF;
                ...
                $sAdmContent = '<div id="ch_blogs_pending">' . $sPendingPostsUnits . '</div>';
                break;
            case 'main':
            default:
                $sMainTabClass = 1;
                ...
                $sAdmContent = '<div class="ch-def-font-large" style="text-align: center;">' . _t('_ch_blog_admin_box_desc',
    ...
    function AddNewPostForm($iPostID = 0, $bBox = true)
    {
        ...
        $sEditPostC = _t('_ch_blog_Edit_post');
        $sDelImgC = _t('_Delete image');
        $sErrorC = _t('_Error Occured');
        $sCaptionErrorC = _t('_ch_blog_Caption_error');
        $sTextErrorC = _t('_ch_blog_Text_error');
        ...
        $aForm = array(
            ...
            'inputs'     => array(
                ...
                'Categories'      => $oCategories->getGroupChooser('ch_blogs', $this->_iVisitorID, true),
                'File'            => array(
                    'type'    => 'file',
                    'name'    => 'BlogPic[]',
        ...
        if ($iPostID > 0) {
            ...
            if ($sPostPicture != '') {
                $sBlogsImagesUrl = CH_BLOGS_IMAGES_URL;
                $sPostPictureTag = <<<EOF
<div class="blog_edit_image" id="edit_post_image_{$iPostID}">
    <img class="ch-def-shadow ch-def-round-corners ch-def-margin-sec-right" style="max-width:{$this->iThumbSize}px; max-height:{$this->iThumbSize}px;" src="{$sBlogsImagesUrl}big_{$sPostPicture}" />
...
EOF;
            ...
            if ($aBlogPost['PostPhoto'] != '' && file_exists(CH_BLOGS_IMAGES_PATH . 'small_' . $aBlogPost['PostPhoto'])) {
                $GLOBALS['oTopMenu']->setCustomSubIconUrl(CH_BLOGS_IMAGES_URL . 'small_' . $aBlogPost['PostPhoto']);
            } else {
                $GLOBALS['oTopMenu']->setCustomSubIconUrl('book');
            }
            $GLOBALS['oTopMenu']->setCustomSubHeader($sPostCaption);
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            $sCurTime = time();
            $sPostUri = uriGenerate(ch_get('PostCaption'), $this->_oConfig->sSQLPostsTable, 'PostUri');
            ...
            if ($iBlogPostID) {
                        ...
                        if (0 < $_FILES['BlogPic']['size'][$i] && 0 < strlen($_FILES['BlogPic']['name'][$i]) && 0 < $iBlogPostID) {
                            ...
                            switch ($aSize[2]) {
                                ...
                                case IMAGETYPE_PNG:
                                    ...
                                    move_uploaded_file($sTmpFile, CH_BLOGS_IMAGES_PATH . $sFileName . $sExt);
                                    @unlink($sTmpFile);

                                    if (strlen($sExt)) {
                                        $sPathSrc = CH_BLOGS_IMAGES_PATH . $sFileName . $sExt;
                                        $sPathDst = CH_BLOGS_IMAGES_PATH . '%s_' . $sFileName . $sExt;
                ...
                //reparse categories
                $oCategories = new ChWsbCategories();
                $oCategories->reparseObjTags('ch_blogs', $iBlogPostID);

                $sAlertAction = ($iPostID == 0) ? 'create' : 'edit_post';
                ch_import('ChWsbAlerts');
                $oZ = new ChWsbAlerts('ch_blogs', $sAlertAction, $iBlogPostID, $this->_iVisitorID);
                $oZ->alert();
        ...
        $sAddingFormVal = '<div class="blogs-view ch-def-bc-padding">' . $sAddingForm . '</div>';

        return ($bBox) ? DesignBoxContent($sCaption,
            '<div class="blogs-view ch-def-bc-padding">' . $sAddingForm . '</div>', 1) : $sAddingFormVal;
    }
    ...
    function GenAddCategoryForm()
    {
        ...
        return DesignBoxContent($sAddCategoryC, '<div class="blogs-view ch-def-bc-padding">' . $sRetHtml . '</div>', 1);
    }

    function ActionChangeFeatureStatus()
    {
        if (false == ch_get('do') || ch_get('do') != 'cfs') {
            return;
        }

        $this->CheckLogged();
        $iPostID = (int)ch_get('id');
    ...
    function GenSearchResult()
    {
        ...
        $bNoProfileMode = (false !== ch_get('ownerID') || false !== ch_get('blogOwnerName')) ? false : true;

        $sRetHtml = '';
        $sSearchedTag = uri2title(process_db_input(ch_get('tagKey'), CH_TAGS_STRIP));
        $iMemberID = $this->defineUserId();

        $sTagsC = _t('_Tags');
        $sNoBlogC = _t('_ch_blog_No_blogs_available');
        ...
        $sBlogPostsVal = $oTmpBlogSearch->displayResultBlock();
        $sBlogPostsVal = '<div class="blogs-view ch-def-bc-padding">' . $sBlogPostsVal . '</div>';
...
EOF;
    ...
    function GenBlogCalendar($isMiniMode = false, $iBlockID = 0, $sDynamicUrl = '')
    {
        ...
        require_once($this->_oConfig->getClassPath() . 'ChBlogsCalendar.php');
        $oCalendar = new ChBlogsCalendar ((int)$aDateParams[0], (int)$aDateParams[1], $this);
        $oCalendar->setBlockId($iBlockID);
        $oCalendar->setDynamicUrl($sDynamicUrl);
        $sBlogPostsCalendar = $oCalendar->display($isMiniMode);

        return DesignBoxContent(_t('_ch_blog_Calendar'), $sBlogPostsCalendar, 1);
    }
    ...
    { //  date=2009/3/18
        ...
        if ($iValue1 > 0 && $iValue2 > 0 && $iValue3 > 0) {
            ...
            $sRequest = CH_WSB_URL_ROOT . 'modules/cheetah/blogs/blogs.php?action=show_calendar_day&date=' . "{$iValue1}/{$iValue2}/{$iValue3}" . '&page={page}&per_page={per_page}';
            $oTmpBlogSearch->aCurrent['paginate']['page_url'] = $sRequest;
...
EOF;
    ...
    function GenCreateBlogForm($bBox = true)
    {
        ...
        $sPleaseCreateBlogC = _t('_ch_blog_Please_create_blog');
        $sNoBlogC = _t('_ch_blog_No_blogs_available');
        $sMyBlogC = _t('_ch_blog_My_blog');
        $sNewBlogDescC = _t('_ch_blog_description');
        $sErrorC = _t('_ch_blog_create_blog_form_error');
        $sSubmitC = _t('_Submit');
            ...
            if ($oForm->isSubmittedAndValid()) {
                ...
                if (!$aBlogsRes) {
                    ...
                    $sBlogAddLink = ($bUseFriendlyLinks)
                        ? CH_WSB_URL_ROOT . 'blogs/my_page/add/'
    ...
    {
        $sTitleC = _t('_ch_blog_Administration');
        $sPendingC = _t('_ch_blog_pending_approval');
    ...
    function ActionEditBlog()
    {
        ...
        if ((($this->_iVisitorID == $iBlogOwner && $iBlogOwner > 0) || $this->bAdminMode) && $iBlogID > 0) {
            $sDescription = process_db_input(ch_get('Description'), CH_TAGS_VALIDATE);
    ...
    {
        $this->CheckLogged();
        $iBlogID = (int)ch_get('DeleteBlogID');
    ...
    function defineUserId()
    {
        ...
        if (false !== ch_get('blogOwnerName')) {
            $sNickName = process_db_input(ch_get('blogOwnerName'), CH_TAGS_STRIP);
            $iMemberId = $this->_oDb->getMemberIDByNickname($sNickName);
        } elseif (ch_get('ownerID')) {
            $iMemberId = (int)ch_get('ownerID');
        }

        if ($this->isPermalinkEnabled() && $iMemberId == 0 && ch_get('action') == 'show_member_post') {
            $sPostUri = process_db_input(ch_get('postUri'), CH_TAGS_STRIP);
    ...
    function defineCategoryName()
    {
        ...
        if (false !== ch_get('categoryUri')) {
            $sCat = uri2title(process_db_input(ch_get('categoryUri'), CH_TAGS_STRIP));
        } elseif (false !== ch_get('category')) {
            $sCat = uri2title(process_db_input(ch_get('category'), CH_TAGS_STRIP));
        }
    ...
    function genBlogFormUrl()
    {
        ...
        $sLink = $this->isPermalinkEnabled() ? CH_WSB_URL_ROOT . $this->_oConfig->sUserExPermalink : $sMainLink;
    ...
    function genBlogSubUrl()
    {
        ...
        if ($this->isPermalinkEnabled()) {
            return CH_WSB_URL_ROOT . $this->_oConfig->sUserExPermalink;
        }
    ...
    {
        if ($this->isPermalinkEnabled()) {
            $sUrl = CH_WSB_URL_ROOT . $this->_oConfig->sUserExPermalink . "{$sType}/{$sEntryUri}";
        } else {
    ...
    function GenTagsPage()
    {
        ...
        $sLink = $this->isPermalinkEnabled() ? CH_WSB_URL_ROOT . 'blogs/' . 'tags' : CH_WSB_URL_ROOT . 'modules/cheetah/blogs/blogs.php?action=tags';
        $oTags = new ChTemplTagsModule($aParam, _t('_all'), $sLink);
    ...
    function serviceBlogsCalendarIndexPage($iBlockID)
    {
        ...
        return $this->GenBlogCalendar(true, $iBlockID, CH_WSB_URL_ROOT);
    }
    ...
    function serviceBlogsIndexPage($bShortPgn = true, $iPerPage = 0)
    {
        ...
        $aVis = array(CH_WSB_PG_ALL);
        if ($this->getUserId()) {
            $aVis[] = CH_WSB_PG_MEMBERS;
        }
...
EOF;
    ...
    function serviceBlogsRss()
    {
        ...
        if (is_array($aRssUnits) && count($aRssUnits) > 0) {
            ...
            foreach ($aRssUnits as $iUnitID => $aUnitInfo) {
                ...
                $sFileName = $this->_oDb->getPostPhotoByID($iPostID);
                $sPostPhoto = ($sFileName != '') ? CH_BLOGS_IMAGES_URL . 'orig_' . $sFileName : '';
                $aRssUnits[$iUnitID]['UnitIcon'] = $sPostPhoto;
            }

            $sUnitTitleC = _t('_ch_blog_Blogs');
    ...
    function serviceGetMemberMenuItemAddContent()
    {
        ...
        $aLinkInfo = array(
            'item_img_src' => 'book',
            'item_img_alt' => _t('_ch_blog_post'),
            'item_link'    => CH_WSB_URL_ROOT . (getParam('permalinks_blogs') == 'on' ? 'blogs/my_page/add/' : 'modules/cheetah/blogs/blogs.php?action=my_page&mode=add'),
            'item_title'   => _t('_ch_blog_post'),
        );
    ...
    */
    ...
    function serviceGetWallPost($aEvent)
    {
        ...
        if ($iItems > 1) {
            ...
            return array(
            	'owner_id' => $iOwner,
                'title' => _t('_ch_blog_wall_added_new_title_items', $sOwner, $iItems),
                'description' => '',
                'content' => $sCss . $this->_oTemplate->parseHtmlByName('wall_post_grouped.html', array(
	                'cpt_user_name' => $sOwner,
	                'cpt_added_new' => _t('_ch_blog_wall_added_new_items', $iItems),
        ...
        $sTextWallObject = _t('_ch_blog_wall_object');

        return array(
        	'owner_id' => $iOwner,
            'title' => _t('_ch_blog_wall_added_new_title', $sOwner, $sTextWallObject),
            'description' => $aItem['PostText'],
            'content' => $sCss . $this->_oTemplate->parseHtmlByName('wall_post.html', array(
	            'cpt_user_name' => $sOwner,
	            'cpt_added_new' => _t('_ch_blog_wall_added_new'),
    ...
    function serviceGetWallAddComment($aEvent)
    {
        ...
        $oCmts = new ChBlogsCmts($this->_oConfig->getCommentSystemName(), $iItem);
        if (!$oCmts->isEnabled()) {
            return '';
        }
        ...
        return array(
            'title'       => _t('_ch_blog_wall_added_new_title_comment', $sOwner, $sTextWallObject),
            'description' => $aComment['cmt_text'],
            'content'     => $sCss . $this->_oTemplate->parseHtmlByName('wall_post_comment.html', array(
                    'cpt_user_name'    => $sOwner,
                    'cpt_added_new'    => _t('_ch_blog_wall_added_new_comment'),
                    'cpt_object'       => $sTextWallObject,
                    'cpt_item_url'     => $aItem['url'],
    ...
    function serviceGetWallPostComment($aEvent)
    {
        ...
        ch_import('Cmts', $this->_aModule);
        $oCmts = new ChBlogsCmts($this->_oConfig->getCommentSystemName(), $iId);
        if (!$oCmts->isEnabled()) {
            return '';
        }
    ...
    function serviceGetWallPostOutline($aEvent)
    {
            ...
            if (empty($aItem)) {
                ...
                if ($aItem['PostStatus'] == 'approval' && $this->oPrivacy->check('view', $aItem['PostID'],
                    ...
                    if (!empty($aItem['PostPhoto'])) {
                        $aItem['thumb_file'] = CH_BLOGS_IMAGES_URL . 'browse_' . $aItem['PostPhoto'];
                        $aItem['thumb_file_path'] = CH_BLOGS_IMAGES_PATH . 'browse_' . $aItem['PostPhoto'];
                        if (!file_exists($aItem['thumb_file_path'])) {
                            $aItem['thumb_file'] = CH_BLOGS_IMAGES_URL . 'big_' . $aItem['PostPhoto'];
                            $aItem['thumb_file_path'] = CH_BLOGS_IMAGES_PATH . 'big_' . $aItem['PostPhoto'];
                        }
                        ...
                        $aItem['thumb_file_2x'] = CH_BLOGS_IMAGES_URL . 'orig_' . $aItem['PostPhoto'];
                        $aItem['thumb_file_2x_path'] = CH_BLOGS_IMAGES_PATH . 'orig_' . $aItem['PostPhoto'];
                    }
        ...
        $sTmplName = empty($aItem['thumb_file']) ? 'modules/cheetah/wall/|outline_item_text.html' : 'modules/cheetah/wall/|outline_item_image.html';
    ...
    function serviceGetSpyData()
    {
        return array(
            ...
            'alerts'   => array(
                array('unit' => 'ch_blogs', 'action' => 'create'),
                array('unit' => 'ch_blogs', 'action' => 'rate'),
                array('unit' => 'ch_blogs', 'action' => 'delete_post'),
                array('unit' => 'ch_blogs', 'action' => 'commentPost'),
                array('unit' => 'ch_blogs', 'action' => 'commentRemoved')
            )
        );
    }
    ...
    function serviceGetSpyPost($sAction, $iObjectId = 0, $iSenderId = 0, $aExtraParams = array())
    {
        ...
        switch ($sAction) {
            case 'create' :
                $sLangKey = '_ch_blog_added_spy';
                $iRecipientId = 0;
                break;

            case 'rate' :
                $sLangKey = '_ch_blog_rated_spy';
                $iRecipientId = $aPostInfo['OwnerID'];
                break;

            case 'commentPost' :
                $sLangKey = '_ch_blog_commented_spy';
                $iRecipientId = $aPostInfo['OwnerID'];
                break;
        }
    ...
    function onPostApproveDisapprove($iBPostID, $isApprove)
    {
        ...
        //reparse categories
        ch_import('ChWsbCategories');
        $oCategories = new ChWsbCategories($aPostInfo['OwnerID']);
        $oCategories->reparseObjTags('ch_blogs', $iBPostID);

        $oZ = new ChWsbAlerts('ch_blogs', $isApprove ? 'approve' : 'disapprove', $iBPostID, $this->_iVisitorID);
        $oZ->alert();
    }
}
