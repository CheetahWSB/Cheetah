...
class ChBlogsSearchUnit extends ChTemplSearchResultText
{
    var $sHomePath;
    var $sHomeUrl;
    ...
    function __construct($oBlogObject = null)
    {
        ...
        if(!$oBlogObject) {
            $oBlogObject =  ChWsbModule::getInstance('ChBlogsModule');
        }
    ...
    {
        return ChWsbModule::getInstance('ChBlogsModule');
    }
    ...
    {
        $GLOBALS['oChBlogsModule'] = $oBlogsModule;
        $oMain = $this->getBlogsMain();
    ...
    function getCurrentUrl($sType, $iId, $sUri, $aOwner = '')
    {
        ...
        $oMain = $this->getBlogsMain();
        return ($oMain->isPermalinkEnabled()) ? CH_WSB_URL_ROOT . $sLink : $this->sHomeUrl . $sLink;
    }

    function displaySearchBox ($sCode, $sPaginate = '', $bAdminBox = false)
    {
        $sCode = DesignBoxContent(_t($this->aCurrent['title']), '<div class="ch-def-bc-padding">'.$sCode .'<div class="clear_both"></div></div>'. $sPaginate, 1);
        if (true !== ch_get('searchMode'))
...
<a class="unit_title ch-def-font-h2" href="{$sPostLink}">{$sPostCaption}</a>{$sAdminStatus}
EOF;

        if ($this->iPostViewType==3 || $this->sMobileWrapper) {
            $sFriendStyle="2";
            $sPostMode = '_post';
            $sPostCaptionHref = '<div class="unit_title ch-def-font-h2">'.$sPostCaption.'</div>';
        }

        $sDateTime = defineTimeInterval($aResSQL['date']);

        //$oCmtsView = new ChTemplCmtsView ('blogposts', (int)$iPostID);
...
EOF;
    ...
    function setSorting ()
    {
        $this->aCurrent['sorting'] = (false !== ch_get('blogs_mode')) ? ch_get('blogs_mode') : $this->aCurrent['sorting'];
    ...
    function showPagination($aParams = array())
    {
        ...
        $oPaginate = new ChWsbPaginate(array(
            ...
            'on_change_page' => 'return !loadDynamicBlock('.$this->id.', \'searchKeywordContent.php?searchMode=ajax&blogs_mode='.$this->aCurrent['sorting'].'&section[]=blog&keyword='.ch_get('keyword').$aLinkAddon['params'].'&page={page}&per_page={per_page}\');',
            'on_change_per_page' => 'return !loadDynamicBlock('.$this->id.', \'searchKeywordContent.php?searchMode=ajax&blogs_mode='.$this->aCurrent['sorting'].'&section[]=blog&keyword='.ch_get('keyword').$aLinkAddon['params'].'&page=1&per_page=\' + this.value);'
        ));
    ...
    function showPagination3($bAdmin = false)
    {
            ...
            $oPaginate = new ChWsbPaginate(array(
                'page_url' => ch_append_url_params($this->aCurrent['paginate']['page_url'], $sPgnAdd),
                'count' => $this->aCurrent['paginate']['totalNum'],
                'per_page' => $this->aCurrent['paginate']['perPage'],
                'page' => $this->aCurrent['paginate']['page'],
            ));
    ...
    function showPagination2($bAdmin = false, $sOverrideViewAllUrl = false, $bShort = true)
    {
        ...
        $sLink = ch_html_attribute($_SERVER['PHP_SELF']) . '?blogs_mode=' . $this->aCurrent['sorting'] . $aLinkAddon['params'];
        $oPaginate = new ChWsbPaginate(array(
            'page_url' => $sLink,
            'count' => $this->aCurrent['paginate']['totalNum'],
            'per_page' => $this->aCurrent['paginate']['perPage'],
    ...
    function showPaginationAjax($sContainerId, $sBaseUrl = false)
    {
        ...
        $sLink = CH_WSB_URL_ROOT . ($sBaseUrl ? $sBaseUrl : $this->aCurrent['paginate']['page_url']);
        $sLink = ch_append_url_params($sLink, 'ajax=1');
        ...
        $oPaginate = new ChWsbPaginate(array(
            ...
            'per_page' => $this->aCurrent['paginate']['perPage'],
            'page' => $this->aCurrent['paginate']['page'],
            'on_change_page' => 'getHtmlData(\'' . ch_html_attribute($sContainerId) . '\', \'' . ch_html_attribute($sLink) . '&page={page}&per_page={per_page}\'); return false;',
            'on_change_per_page' => 'getHtmlData(\'' . ch_html_attribute($sContainerId) . '\', \'' . ch_html_attribute($sLink) . '&page=1&per_page=\' + this.value); return false;',
        ));
    ...
    function _wrapMobileUnit ($sContent, $iPostID, $oMain)
    {
        $aVars = array (
            'content' => $sContent,
            'url' => ch_js_string($oMain->genBlogSubUrl() . '?action=mobile&mode=post&id=' . $iPostID),
        );
        ch_import('ChWsbMobileTemplate');
        $oMobileTemplate = new ChWsbMobileTemplate($oMain->_oConfig, $oMain->_oDb);
