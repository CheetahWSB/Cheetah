...
class ChMbpTemplate extends ChWsbModuleTemplate
{
    /**
     * Constructor
     */
    ...
    function displaySelectLevelBlock($aLevels, $bDynamic = false)
    {
		...
		$aTmplParams = array(
			...
			'submit_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'join_submit',
			'ch_repeat:levels' => $aTmplVarsLevels,
			'ch_if:show_providers_selector' => array(
				'condition' => !$bSelectedProvider,
				'content' => array(
					'ch_if:show_providers_selector_hidden' => array(
						'condition' => !$bSelectedLevelPaid,
						'content' => array()
					),
					'ch_repeat:providers' => $aTmplVarsProviders
				)
			),
			'ch_if:show_captcha' => array(
				'condition' => $bCaptcha,
				'content' => array(
					'captcha' => $sCaptcha
				)
			),
			'ch_if:show_selected_provider' => array(
				'condition' => $bSelectedProvider,
				'content' => array(
	...
	function getJsCode($sType, $bWrap = false)
    {
        ...
        $aOptions = array(
        	...
        	'sTxtPay' => ch_js_string(_t('_membership_btn_pay')),
        	'sTxtSelect' => ch_js_string(_t('_membership_btn_select')),
        	'sErrSelectLevel' => ch_js_string(_t('_membership_err_need_select_level')),
        	'sErrSelectProvider' => ch_js_string(_t('_membership_err_need_select_provider'))
        );
