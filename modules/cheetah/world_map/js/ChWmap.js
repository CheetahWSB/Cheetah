...
function ChWmap (map, sGetDataUrl, sInstanceName, sElementId) {
    this._map = map;
...
ChWmap.prototype.updateLocations = function () {
...
ChWmap.prototype.loading = function (b) {

    if (!this._sElementId)
        return;

    ch_map_loading (this._sElementId, b);
}

ChWmap.prototype.createMarker = function (point, html, image, options) {
    var marker;
...
ChWmap.prototype.magnify = function (fLat, fLng, iZoom) {
    this._map.setCenter(new google.maps.LatLng(fLat, fLng), iZoom);
}

ChWmap.prototype.setShadowUrl = function (sUrl) {
    this._sShadowUrl = sUrl;
}

ChWmap.prototype.setSaveLocationUrl = function (sUrl) {
    this._sSaveLocationUrl = sUrl;
}

ChWmap.prototype.setSaveDataUrl = function (sUrl) {
...
ChWmap.prototype.saveData = function (fLat, fLng, iZoom, sMapType) {
...
ChWmap.prototype.saveLocation = function () {
...
ChWmap.prototype.onStartLoadMap = function () {
    this._isMapLoaded = 0;
    if (window.glChWmapLocationsMapOnStartLoadCallback === undefined)
        return;
    glChWmapLocationsMapOnStartLoadCallback(this);
}

ChWmap.prototype.onLoadMap = function () {
    this._isMapLoaded = 1;
    if (window.glChWmapLocationsMapOnLoadCallback === undefined)
        return;
    glChWmapLocationsMapOnLoadCallback(this);
}

ChWmap.prototype.onUpdateAddress = function (sAddress, sCountry) {
    $('#ch_map_curr_loc').html (sAddress);
}

ChWmap.prototype.setParts = function (s) {
    this._sParts = s;
}

ChWmap.prototype.setCustom = function (s) {
    this._sCustom = s;
}

ChWmap.prototype.addOverlay = function (o) {
    this._aOverlays[this._aOverlays.length] = o;
}

ChWmap.prototype.clearOverlays = function () {
    while(this._aOverlays[0])
        this._aOverlays.pop().setMap(null);
}

function ch_map_loading (sId, b) {
    ch_loading(sId, b);
}
