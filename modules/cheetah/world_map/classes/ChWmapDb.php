...
define('CH_WMAP_CAT_HOME', 'home');
define('CH_WMAP_CAT_ENTRY', 'entry');
define('CH_WMAP_CAT_EDIT', 'edit');

class ChWmapDb extends ChWsbModuleDb
{
    var $_aCategs = array (
    ...
    function updateLocationPrivacy ($iId, $mixedPrivacy = CH_WMAP_PRIVACY_DEFAULT)
    {
        return $this->query ("UPDATE `" . $this->_sPrefix . "locations` SET `privacy` = '$mixedPrivacy' WHERE `id` = '$iId'");
    }

    function insertLocation ($iId, $sPart, $sTitle, $sUri, $fLat, $fLng, $iMapZoom, $sMapType, $sAddress, $sCity, $sCountry, $mixedPrivacy = CH_WMAP_PRIVACY_DEFAULT, $isFailed = 0)
    {
        $sFields = '';
        if ($sAddress)
    ...
    function _removePartSettings($sPart)
    {
        ...
        $sHiddenSettingsSql = "DELETE FROM `sys_options` WHERE `Name` = 'ch_wmap_home_{part}_lat' OR `Name` = 'ch_wmap_home_{part}_lng' OR `Name` = 'ch_wmap_home_{part}_zoom' OR `Name` = 'ch_wmap_home_{part}_map_type'";
    ...
    {
        $aSettings = array (
            'ch_wmap_{type}_{part}_control_type' => array (
                'title' => 'Map control type',
                'type' => 'select',
                'values' => 'none,small,large',
                'defaults' => array (CH_WMAP_CAT_HOME => 'large', CH_WMAP_CAT_ENTRY => 'small', CH_WMAP_CAT_EDIT => 'small'),
            ),
            'ch_wmap_{type}_{part}_is_type_control' => array (
                'title' => 'Display map type controls',
                'type' => 'checkbox',
                'values' => '',
                'defaults' => array (CH_WMAP_CAT_HOME => 'on', CH_WMAP_CAT_ENTRY => 'on', CH_WMAP_CAT_EDIT => 'on'),
            ),
            'ch_wmap_{type}_{part}_is_scale_control' => array (
                'title' => 'Display map scale control',
                'type' => 'checkbox',
                'values' => '',
                'defaults' => array (CH_WMAP_CAT_HOME => 'on', CH_WMAP_CAT_ENTRY => '', CH_WMAP_CAT_EDIT => ''),
            ),
            'ch_wmap_{type}_{part}_is_overview_control' => array (
                'title' => 'Display map overview control',
                'type' => 'checkbox',
                'values' => '',
                'defaults' => array (CH_WMAP_CAT_HOME => 'on', CH_WMAP_CAT_ENTRY => '', CH_WMAP_CAT_EDIT => ''),
            ),
            'ch_wmap_{type}_{part}_is_map_dragable' => array (
                'title' => 'Is map dragable?',
                'type' => 'checkbox',
                'values' => '',
                'defaults' => array (CH_WMAP_CAT_HOME => 'on', CH_WMAP_CAT_ENTRY => 'on', CH_WMAP_CAT_EDIT => 'on'),
            ),
            'ch_wmap_{type}_{part}_zoom' => array (
                'title' => 'Default zoom',
                'type' => 'digit',
                'values' => '',
                'defaults' => array (CH_WMAP_CAT_HOME => false, CH_WMAP_CAT_ENTRY => '10', CH_WMAP_CAT_EDIT => 10),
            ),
            'ch_wmap_{type}_{part}_map_type' => array (
                'title' => 'Default map type',
                'type' => 'select',
                'values' => 'normal,satellite,hybrid,terrain',
                'defaults' => array (CH_WMAP_CAT_HOME => false, CH_WMAP_CAT_ENTRY => 'normal', CH_WMAP_CAT_EDIT => 'normal'),
            ),
        );
        ...
        $sHiddenSettingsSql = "INSERT INTO `sys_options` (`Name`, `VALUE`, `kateg`, `desc`, `Type`, `check`, `err_text`, `order_in_kateg`, `AvailableValues`) VALUES
            ('ch_wmap_home_{part}_lat', '20', {categ_id}, 'Home map latitude: {Part}', 'digit', '', '', '0', ''),
            ('ch_wmap_home_{part}_lng', '35', {categ_id}, 'Home map longitude: {Part}', 'digit', '', '', '0', ''),
            ('ch_wmap_home_{part}_zoom', '2', {categ_id}, 'Home map zoom: {Part}', 'digit', '', '', '0', ''),
            ('ch_wmap_home_{part}_map_type', 'normal', {categ_id}, 'Home map type: {Part}', 'digit', '', '', '0', '')";
