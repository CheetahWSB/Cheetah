...
ch_import('ChWsbCacheFile');

class ChPageACModule extends ChWsbModule
{
    /**
     * Constructor
     */
    ...
    function actionGetPageRules()
    {
        ...
        $sRet = DesignBoxAdmin(_t('_ch_pageac_current_rules'), $this->actionGetRulesList(true), '', '', 11);
        $sRet .= DesignBoxAdmin(_t('_ch_pageac_new_rule'), $this->_oTemplate->displayNewRuleForm(), '', '', 11);
        $sRet .= DesignBoxAdmin(_t('_ch_pageac_note'), $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array('content' => _t('_ch_pageac_note_text'))));
        return $sRet;
    }
    ...
    function actionNewRule()
    {
        ...
        if (!empty($sRule)) {
        	...
        	header('Content-Type: text/html; charset=utf-8');
            return _t('_ch_pageac_page_url_empty');
        }
    ...
    function actionSaveRule()
    {
        ...
        if (!strlen($sRule)) {
            ...
            return MsgBox(_t('_ch_pageac_saved'), 1).$this->actionGetRulesList();
        }
    }
    ...
    function actionDeleteRule()
    {
        ...
        return MsgBox(_t('_ch_pageac_deleted'), 1).$this->actionGetRulesList();
    }
    ...
    function actionTopMenu($sAction, $iMenuItemID)
    {
        ...
        if ($sAction == 'edit') {
            ...
            header('Content-Type: text/html; charset=utf-8');
            return PopupBox('pageac_popup_edit_form', _t('_ch_pageac_visible_for'), $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array('content' => $this->_oTemplate->getMenuItemEditForm('top', $iMenuItemID, $aMenuItemVisibility).LoadingBox('formItemEditLoading'))));
        }elseif ($sAction == 'save') {
    ...
    function actionMemberMenu($sAction, $iMenuItemID)
    {
        ...
        if ($sAction == 'edit') {
            ...
            header('Content-Type: text/html; charset=utf-8');
            return PopupBox('pageac_popup_edit_form', _t('_ch_pageac_visible_for'), $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array('content' => $this->_oTemplate->getMenuItemEditForm('member', $iMenuItemID, $aMenuItemVisibility).LoadingBox('formItemEditLoading'))));
        }elseif ($sAction == 'save') {
    ...
    function actionPageBlock($sAction, $iID)
    {
        ...
        if ($sAction == 'edit') {
            ...
            header('Content-Type: text/html; charset=utf-8');
            return PopupBox('pageac_popup_edit_form', _t('_ch_pageac_visible_for'), $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array('content' => $this->_oTemplate->getPageBlockEditForm($iID, $aVisibility).LoadingBox('formItemEditLoading'))));
        }elseif ($sAction == 'save') {
            $aVisibleTo = array();
            if (is_array($_POST['mlv_visible_to'])) {
                $aData = array_flip($_POST['mlv_visible_to']);
    ...
    function _validateRule($sRule, $bIsAdvanced)
    {
        ...
        $sBaseURL = basename(CH_WSB_URL_ROOT);

        if (strpos($sRule, CH_WSB_URL_ROOT) === 0) $sRule = substr($sRule, strlen(CH_WSB_URL_ROOT)); //if URL starts from http://www.site.com
    ...
    function serviceMenuItemsFilter($sType, &$aItems)
    {
        ...
        if(isset($_REQUEST['modules-uninstall']) && $_REQUEST['modules-uninstall'] && in_array('cheetah/pageac/', $_REQUEST['pathes']))
            return ;

        if(!defined('CH_WSB_ROLE_MEMBER'))
            define('CH_WSB_ROLE_MEMBER', 1);	//this code is required here because at the time of ChWsbMenu::load function call profiles.inc.php isn't fully included yet,
        if(!defined('CH_WSB_ROLE_ADMIN'))
            define('CH_WSB_ROLE_ADMIN', 2);	//thus all defines and function calls located in profiles.inc.php aren't executed at this moment

        check_logged(); //so a call to isLogged or check_logged always would fail here because CH_WSB_ROLE_MEMBER/CH_WSB_ROLE_ADMIN aren't defined yet.

        if(!isLogged())
            return; //non-members visibility controlled by default in builders
        else if (isRole(CH_WSB_ROLE_ADMIN, getLoggedId()))
    ...
    function servicePageBlocksFilter(&$oChWsbPageView)
    {
        if (!isLogged()) return; //non-members visibility controlled by default in builders
        elseif (isRole(CH_WSB_ROLE_ADMIN, getLoggedId())) return; //admin isn't affected by this module
        ...
        foreach ($oChWsbPageView->aPage['Columns'] as $iColumn => $aColumn) {
            foreach ($aColumn['Blocks'] as $iBlockID => $aBlock) {
                if (!empty($aPageBlocksCache[$iBlockID]) && !$aPageBlocksCache[$iBlockID]['MemLevels'][$iMemLevel]) unset($oChWsbPageView->aPage['Columns'][$iColumn]['Blocks'][$iBlockID]);
            }
        }
    }
    function serviceIsUrlAccessable($sURL, $iUserId = 0)
    {
        if ($iUserId && isRole(CH_WSB_ROLE_ADMIN, $iUserId) || strpos($sURL, '/'.$GLOBALS['admin_dir']) === 0) return true; //admin isn't affected by this module also access to admin panel shouldn't ever be protected
    ...
    function serviceResponceProtectURL($sURL)
    {
        ...
        if (!$this->serviceIsUrlAccessable($sURL, getLoggedId())) {
            ...
            global $_page_cont;
            $_page['name_index'] = -1;
            $_page['header'] = _t("_ch_pageac_access_denied");
            $_page_cont[$_page['name_index']]['page_main_code'] = MsgBox(_t("_ch_pageac_deny_text"));
            PageCode();
            exit;
        }
    }
