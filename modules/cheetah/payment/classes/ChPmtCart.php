...
class ChPmtCart
{
    ...
    {
        if($iVendorId == CH_PMT_EMPTY_ID || empty($iModuleId) || empty($iItemId) || empty($iItemCount))
    ...
    function getInfo($iUserId, $iVendorId = CH_PMT_EMPTY_ID, $aItems = array())
    {
        if($iVendorId != CH_PMT_EMPTY_ID && !empty($aItems))
    ...
    function updateInfo($mixedPending)
    {
        ...
        foreach($aItems as $aItem) {
            $aItemInfo = ChWsbService::call((int)$aItem['module_id'], 'register_cart_item', array($aPending['client_id'], $aPending['seller_id'], $aItem['item_id'], $aItem['item_count'], $sOrderId));
    ...
    function _getInfo($iClientId, $iVendorId, $aItems)
    {
        ...
        $aItemsInfo = array();
        foreach($aItems as $aItem) {
            $aItemInfo = ChWsbService::call((int)$aItem['module_id'], 'get_cart_item', array($iClientId, $aItem['item_id']));
