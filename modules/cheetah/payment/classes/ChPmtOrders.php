...
<?php
    ...
    function getOrdersBlock($sType, $iUserId = CH_PMT_EMPTY_ID)
    {
        ...
        $sJsObject = $this->_oConfig->getJsObject('orders');
        $aTopMenu = array(
            'pmt-orders-processed-lnk' => array('href' => $sBaseUrl . 'orders/processed/', 'title' => _t($sLangsPrefix . 'btn_orders_processed'), 'active' => $sType == CH_PMT_ORDERS_TYPE_PROCESSED ? 1 : 0),
            'pmt-orders-pending-lnk' => array('href' => $sBaseUrl . 'orders/pending/', 'title' => _t($sLangsPrefix . 'btn_orders_pending'), 'active' => $sType == CH_PMT_ORDERS_TYPE_PENDING ? 1 : 0),
        ...
        $sContent = $this->_oTemplate->displayOrdersBlock($sType, $iUserId != CH_PMT_EMPTY_ID ? $iUserId : $this->_iUserId);
    ...
    function getManualOrderWindow()
    {
        ...
        $aForm = array(
            'form_attrs' => array(
                ...
                'name' => 'text_data',
                'action' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'act_manual_order_submit/',
                'method' => 'post',
    ...
    function addManualOrder($aData)
    {
        ...
        $iClientId = 0;
        if(($iClientId = $this->_oDb->userExists(process_db_input($aData['client'], CH_TAGS_STRIP))) === false)
            return array('code' => 2, 'message' => $this->_sLangsPrefix . 'err_wrong_client');
        ...
        $sOrder = trim(process_db_input($aData['order'], CH_TAGS_STRIP));
        if(empty($sOrder))
            return array('code' => 4, 'message' => $this->_sLangsPrefix . 'err_wrong_order');
