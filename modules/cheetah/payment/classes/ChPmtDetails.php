...
class ChPmtDetailsFormCheckerHelper extends ChWsbFormCheckerHelper
{
	function checkHttps ($s)
    {
        return empty($s) || substr(CH_WSB_URL_ROOT, 0, 5) == 'https';
    }
}
...
class ChPmtDetails
{
    ...
    function __construct(&$oDb, &$oConfig)
    {
        ...
        $this->_aForm = array(
            'form_attrs' => array(
                'id' => 'pmt_details',
                'name' => 'pmt_details',
                'action' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'details/',
                'method' => 'post',
            ...
            'params' => array(
                ...
                'checker_helper' => 'ChPmtDetailsFormCheckerHelper'
            ),
            'inputs' => array (
            )
        );
    ...
    function getForm($iUserId)
    {
        ...
        if($iUserId == CH_PMT_ADMINISTRATOR_ID)
            $this->_aForm['form_attrs']['action'] = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'admin/';
        ...
        if($oForm->isSubmittedAndValid()) {
            ...
            foreach($aOptions as $aOption)
                $this->_oDb->updateOption($iUserId, $aOption['id'], process_db_input(isset($_POST[$aOption['name']]) ? $_POST[$aOption['name']] : "", CH_TAGS_STRIP));
        	...
        	foreach($oForm->aInputs as $aInput)
        		if(!empty($aInput['error'])) {
        			$iProviderId = (int)$aInput['attrs']['ch-data-provider'];
