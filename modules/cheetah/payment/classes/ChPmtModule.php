...
define('CH_PMT_ORDERS_TYPE_PENDING', 'pending');
define('CH_PMT_ORDERS_TYPE_PROCESSED', 'processed');
define('CH_PMT_ORDERS_TYPE_SUBSCRIPTION', 'subscription');
define('CH_PMT_ORDERS_TYPE_HISTORY', 'history');

define('CH_PMT_EMPTY_ID', -1);
define('CH_PMT_ADMINISTRATOR_ID', 0);
define('CH_PMT_ADMINISTRATOR_USERNAME', 'administrator');
 ...
 * @see ChPmtModule::serviceGetAddToCartLink
 ...
 * @see ChPmtModule::serviceGetTransactionsInfo
 * ChWsbService::call('payment', 'get_transactions_info', array($aConditions));
 *
 * Get total count of items in Shopping Cart.
 * @see ChPmtModule::serviceGetCartItemCount
 * ChWsbService::call('payment', 'get_cart_item_count', array($iUserId, $iOldCount));
 * @note is needed for internal usage(integration with member tool bar).
 *
 * Get Shopping cart content.
 * @see ChPmtModule::serviceGetCartItems
 * ChWsbService::call('payment', 'get_cart_items');
 * @note is needed for internal usage(integration with member tool bar).
 *
 *
 * Alerts:
 ...
 *
 */
class ChPmtModule extends ChWsbModule
{
    var $_iUserId;
    var $_oDetails;
    var $_oCart;
    ...
    function getOrdersBlock($sType, $iUserId = CH_PMT_EMPTY_ID)
    {
	...
	function serviceGetOrdersUrl()
    {
    	...
    	return CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'orders/';
    }
    function actionGetItems($iModuleId)
    {
        $aItems = ChWsbService::call((int)$iModuleId, 'get_items', array($this->_iUserId));
    ...
    function actionGetOrders()
    {
        ...
        if(isset($aData['filter']))
            $sFilter = process_db_input($aData['filter'], CH_TAGS_STRIP);
        ...
        if($aData['type'] == CH_PMT_ORDERS_TYPE_HISTORY)
    ...
    function actionOrdersSubmit($sType)
    {
        ...
        else if(isset($aData['pmt-process']) && !empty($aData['pmt-process']) && $sType == CH_PMT_ORDERS_TYPE_PENDING)
     ...
     *
     */
    function getDetailsForm($iUserId = CH_PMT_EMPTY_ID)
    {
	...
	function serviceUpdateDependentModules($sModuleUri = 'all', $bInstall = true)
    {
        ...
        foreach($aModules as $aModule) {
			if(!ChWsbRequest::serviceExists($aModule, 'get_payment_data'))
				continue;

			$aData = ChWsbService::call($aModule['uri'], 'get_payment_data');
			if($bInstall)
    ...
    {
        $aTopMenu = array(
            'pmt-cart' => array('href' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'cart/', 'title' => _t($this->_sLangsPrefix . 'btn_cart')),
            'pmt-cart-history' => array('href' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'history/' . ($iVendorId == CH_PMT_ADMINISTRATOR_ID ? 'site/' : ''), 'title' => _t($this->_sLangsPrefix . 'btn_history'), 'active' => 1)
        );
    ...
    function getCartContent($mixedVendor = null)
    {
        ...
        $aTopMenu = array(
            'pmt-cart' => array('href' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'cart/', 'title' => _t($this->_sLangsPrefix . 'btn_cart'), 'active' => 1),
            'pmt-cart-history' => array('href' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'history/' . ($iVendorId == CH_PMT_ADMINISTRATOR_ID ? 'site/' : ''), 'title' => _t($this->_sLangsPrefix . 'btn_history'))
        );
    ...
    function serviceGetCartUrl()
    {
    	...
    	return CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'cart/';
    }
    ...
    function actionCartSubmit()
    {
        ...
        header('Location: ' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'cart/');
        exit;
    }
    ...
    function initializeCheckout($iVendorId, $sProvider, $aItems = array())
    {
        ...
        if(empty($aInfo) || $aInfo['vendor_id'] == CH_PMT_EMPTY_ID || empty($aInfo['items']))
		...
		foreach($aInfo['items'] as $iIndex => $aItem)
			if((int)$aInfo['client_id'] != 0 && (float)$aItem['price'] == 0) {
				$aItemInfo = ChWsbService::call((int)$aItem['module_id'], 'register_cart_item', array($aInfo['client_id'], $aInfo['vendor_id'], $aItem['id'], $aItem['quantity'], $this->_oConfig->generateLicense()));
	            ...
	            $sCartItems = trim(preg_replace("'" . implode(ChPmtCart::$DESCRIPTOR_DIVIDER, array($aInfo['vendor_id'], $aItem['module_id'], $aItem['id'], $aItem['quantity'])) . ":?'", "", $sCartItems), ":");
    ...
    function actionFinalizeCheckout($sProvider, $mixedVendorId = "")
    {
        ...
        $aProvider = is_numeric($mixedVendorId) && (int)$mixedVendorId != CH_PMT_EMPTY_ID ? $this->_oDb->getVendorInfoProviders((int)$mixedVendorId, $sProvider) : $this->_oDb->getProviders($sProvider);
        $sClassPath = !empty($aProvider['class_file']) ? CH_DIRECTORY_PATH_ROOT . $aProvider['class_file'] : $this->_oConfig->getClassPath() . $aProvider['class_name'] . '.php';
        if(empty($aProvider) || !file_exists($sClassPath)) {
    ...
    function performJoin($iPendingId, $aPayment = array())
    {
		...
		if(!empty($aPayment['payer_name']) && !empty($aPayment['payer_email'])) {
			...
			$aTemplate = $oEmailTemplates->parseTemplate($this->_sEmailTemplatesPrefix . 'paid_need_join', array(
				...
				'JoinLink' => ch_append_url_params($this->_oConfig->getJoinUrl(), array($this->_sRequestKeyPending => (int)$iPendingId))
			));
	...
	function actionJoin()
    {
    	...
    	if(empty($iPendingId) && ch_get($this->_sRequestKeyPending) !== false) {
    		$iPendingId = (int)ch_get($this->_sRequestKeyPending);
        ...
        $oAlert = new ChWsbAlerts('system', 'join_after_payment', 0, 0, array('override' => &$bOverride, 'override_error' => &$sOverrideError));
        $oAlert->alert();
    	...
    	$oProfileFields = new ChPmtProfileFields(1, $this);
