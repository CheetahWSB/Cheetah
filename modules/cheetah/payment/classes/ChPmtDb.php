...
class ChPmtDb extends ChWsbModuleDb
{
    var $_oConfig;
    /*
     * Constructor.
    ...
    function getVendorInfoProfile($iId)
    {
        ...
        if($iId == CH_PMT_ADMINISTRATOR_ID)
            return array(
                'id' => CH_PMT_ADMINISTRATOR_ID,
                'username' => CH_PMT_ADMINISTRATOR_USERNAME,
                'profile_name' => getParam('site_title'),
                'profile_icon' => $GLOBALS['oFunctions']->getMemberThumbnail(0, 'none', false, 'site', true, 'small'),
                'profile_url' => CH_WSB_URL_ROOT,
                'status' => 'Active',
    ...
    {
        return (int)$this->getOne("SELECT `ID` FROM `Profiles` WHERE `Role`&" . CH_WSB_ROLE_ADMIN . " AND `Status`='Active' ORDER BY `ID` ASC LIMIT 1");
    }
    function getAdminsIds()
    {
        return $this->getColumn("SELECT `ID` FROM `Profiles` WHERE `Role`&" . CH_WSB_ROLE_ADMIN . " AND `Status`='Active' ORDER BY `ID` ASC");
    }
    ...
    function getOptions($iUserId = CH_PMT_EMPTY_ID, $iProviderId = 0)
    {
        if($iUserId == CH_PMT_EMPTY_ID && empty($iProviderId))
    ...
    function getPendingOrders($aParams)
    {
        ...
        foreach($aOrders as $iKey => $aOrder) {
            $aProducts = ChPmtCart::items2array($aOrder['items']);
    ...
    function getHistoryOrders($aParams)
    {
        ...
        $sFilterAddon = $aParams['seller_id'] != CH_PMT_EMPTY_ID ? " AND `tt`.`seller_id`='" . $aParams['seller_id'] . "'" : " AND `tt`.`seller_id`<>'" . CH_PMT_ADMINISTRATOR_ID . "'";
        if(!empty($aParams['filter']))
            $sFilterAddon = " AND (DATE_FORMAT(FROM_UNIXTIME(`tt`.`date`), '" . $sDateFormat . "') LIKE '%" . $aParams['filter'] . "%' OR `tt`.`order_id` LIKE '%" . $aParams['filter'] . "%' OR `tp`.`NickName` LIKE '%" . $aParams['filter'] . "%' OR `ttp`.`order` LIKE '%" . $aParams['filter'] . "%')";
    ...
    {
    	$this->query("DELETE FROM `ch_pmt_cart` WHERE `client_id`='" . $iId . "'");
    	$this->query("DELETE FROM `ch_pmt_user_values` WHERE `user_id`='" . $iId . "'");
    }
}
