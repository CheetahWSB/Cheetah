		...
		if (bAdminMan==false) {
			if (sCustomElementID && sUnitValue != '' && typeof(sUnitValue) != 'undefined') {
				...
				if (custFieldName1 != '' && typeof(custFieldName1) != 'undefined') {
					sCustomValues += '<div class="ordered_block_select ch-def-margin-sec-right"><span id="CustomFieldCaption1">' + custFieldName1 + ' ' + sUnitValue + '</span><div class="input_wrapper input_wrapper_text ch-def-margin-sec-left clearfix"><input class="form_input_text ch-def-font-inputs" type="text" value="" name="CustomFieldValue1" /></div></div>';
				}
				if (custFieldName2 != '' && typeof(custFieldName2) != 'undefined') {
					sCustomValues += '<div class="ordered_block_select ch-def-margin-sec-right"><span id="CustomFieldCaption2">' + custFieldName2 + ' ' + sUnit2Value + '</span><div class="input_wrapper input_wrapper_text ch-def-margin-sec-left clearfix"><input class="form_input_text ch-def-font-inputs" type="text" value="" name="CustomFieldValue2" /></div></div>';
				}
...
function AdmCreateSubcategory(oElement, sSubmitUrl) {
    ...
    if (iCateg > 0) {
    	...
    	$.post(sSubmitUrl, oRequest, function(oData) {
            oForm.parents('.boxContent:first').find('.ch-def-bc-margin:first').html(oData);
    	}, 'json');
    }
}
...
function AdmAction2Category(sSubmitUrl, sSA, sID) {
    ...
    if (iID > 0) {
        ...
        var sReceiver = $('#ads_category_manager .boxContent .ch-def-bc-margin');
    	$.post(sSubmitUrl, oRequest, function(oData) {
