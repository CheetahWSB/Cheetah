...
class ChAdsSearchUnit extends ChTemplSearchResultText
{
    var $sHomePath;
    var $sHomeUrl;
    ...
    var $aCurrent = array(
        ...
        'join' => array(
            ...
            'category' => array(
                'type' => 'inner',
                'table' => 'ch_ads_category',
                'mainTable' => 'ch_ads_category_subs',
        ...
        'restriction' => array(
            ...
            'owner' => array('value'=>'', 'field'=>'IDProfile', 'operator'=>'=', 'table' => 'ch_ads_main'),
            'tag' => array('value'=>'', 'field'=>'Tags', 'operator'=>'like'),
            'categoryID'=> array('value'=>'', 'field'=>'ID', 'operator'=>'=', 'table' => 'ch_ads_category'),
            'subcategoryID'=> array('value'=>'', 'field'=>'ID', 'operator'=>'=', 'table' => 'ch_ads_category_subs'),
            'id'=> array('value'=>'', 'field'=>'ID', 'operator'=>'='),
            'country'=> array('value'=>'', 'field'=>'Country', 'operator'=>'=', 'table' => 'ch_ads_main'),
            'message_filter'=> array('value'=>'', 'field'=>'Subject', 'operator'=>'like'),
            'today'=> array('value'=>'', 'field'=>'ID', 'operator'=>'=', 'table' => 'ch_ads_category'),
            'allow_view' => array('value'=>'', 'field'=>'AllowView', 'operator'=>'in', 'table'=> 'ch_ads_main'),
        ),
    ...
    function getAdsMain()
    {
        return ChWsbModule::getInstance('ChAdsModule');
    }
       ...
       function getCurrentUrl($sType, $iId, $sUri, $aOwner = '')
       {
        ...
        $sPath = ($bUseFriendlyLinks) ? CH_WSB_URL_ROOT : $this->sHomeUrl;
    ...
    function displayResultBlock ($bWrap = true)
    {
        ...
        if ($sCode) {
            ...
            $sCode .= ChWsbService::call('ads', 'get_common_css', array(true));
        }
    ...
    function displaySearchUnit($aResSQL)
    {
        ...
        $oMain = $this->getAdsMain();
        $sCategUrl = ($oMain->_oConfig->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/cat/'.$sCEntryUri : "{$oMain->_oConfig->sCurrBrowsedFile}?bClassifiedID={$iCatID}";
        $sSubCategUrl = ($oMain->_oConfig->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/subcat/'.$sSEntryUri : "{$oMain->_oConfig->sCurrBrowsedFile}?bSubClassifiedID={$iSubCatID}";
...
EOF;
    ...
    function getTopMenu ($aExclude = array())
    {
        ...
        foreach( array('last') as $sMyMode ) {
            ...
            $sLink  = ch_html_attribute($_SERVER['PHP_SELF']) . "?";
         ...
         if ($this->aCurrent['paginate']['totalPages'] > 1) {
            ...
            $oMain = $this->getAdsMain();
             if ($this->aCurrent['paginate']['page'] > 1) {
                ...
                $aDBBottomMenu[ _t('_Back') ] = array( 'href' => ch_html_attribute($_SERVER['PHP_SELF']) . "?ads_mode={$this->aCurrent['sorting']}&amp;page=$iPrevPage{$sUserAddon}{$sFileAddon}", 'dynamic' => true, 'class' => 'backMembers', 'icon_class' => 'left' );
            } else
            ...
            if( $this->aCurrent['paginate']['page'] < $this->aCurrent['paginate']['totalPages'] ) {
                ...
                $aDBBottomMenu[ _t('_Next') ] = array( 'href' => ch_html_attribute($_SERVER['PHP_SELF']) . "?ads_mode={$this->aCurrent['sorting']}&amp;page=$iNextPage{$sUserAddon}{$sFileAddon}", 'dynamic' => true, 'class' => 'moreMembers' );
            } else
    ...
    function setSorting ()
    {
        $this->aCurrent['sorting'] = (false !== ch_get('ads_mode')) ? ch_get('ads_mode') : $this->aCurrent['sorting'];
    ...
    function showPagination($aParams = array())
    {
        ...
        if (mb_strlen($aPgnParams['page_url']) == 0) {
            ...
            if(!$this->id)
                $this->id = 0;
            $sLoadDynamicUrl = $this->id . ', \'' . CH_WSB_URL_ROOT . 'searchKeywordContent.php?searchMode=ajax&section[]=ch_ads' . $aLinkAddon['params'];
        ...
        $oPaginate = new ChWsbPaginate($aPgnParams);
    ...
    function showPagination2($bAdmin = false)
    {
        ...
        $oPaginate = new ChWsbPaginate(array(
            ...
            'per_page' => $this->aCurrent['paginate']['perPage'],
            'page' => $this->aCurrent['paginate']['page'],
            'on_change_page' => 'return !loadDynamicBlock({id}, \''.ch_html_attribute($_SERVER['PHP_SELF']).'?ads_mode='.$this->aCurrent['sorting'].$aLinkAddon['params'].'&page={page}&per_page={per_page}\');',
        ));
