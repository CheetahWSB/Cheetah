...
require_once(CH_DIRECTORY_PATH_INC . 'header.inc.php');
 ...
 *
 * Memberships/ACL:
 * View ads - CH_ADS_VIEW
 * Browse ads - CH_ADS_BROWSE
 * Use search and view search results - CH_ADS_SEARCH
 * Add ads - CH_ADS_ADD
 * Edit any ad (as admin) - CH_ADS_EDIT_ANY_AD
 * Delete any ad (as admin) - CH_ADS_DELETE_ANY_AD
 * Approve any ad (as admin) - CH_ADS_APPROVING
 *
 *
 *
 * Service methods:
 ...
 *
 * @see ChAdsModule::serviceAdsIndexPage
 *      ChWsbService::call('ch_ads', 'ads_index_page', array());
 *
 * Ads block for profile page (as PHP function)
 * @see ChAdsModule::serviceAdsProfilePage
 * ChWsbService::call('ch_ads', 'ads_profile_page', array($_iProfileID));
 *
 * Generation of member RSS feeds
 * @see ChAdsModule::serviceAdsRss
 * ChWsbService::call('ch_ads', 'ads_rss', array());
 *
 * Get common css
 * @see ChAdsModule::serviceGetCommonCss
 * ChWsbService::call('ch_ads', 'get_common_css', array());
 *
 * Get Spy data
 * @see ChAdsModule::serviceGetSpyData
 * ChWsbService::call('ch_ads', 'get_spy_data', array());
 *
 * Get Spy ad units
 * @see ChAdsModule::serviceGetSpyPost
 * ChWsbService::call('ch_ads', 'get_spy_post', array($sAction, $iObjectId, $iSenderId));
 *
 *
 *
 * Alerts:
 ...
 *
 */
class ChAdsModule extends ChWsbModule
{
    ...
    function __construct($aModule)
    {
        ...
        ch_import('Privacy', $this->_aModule);
        $this->oPrivacy = new ChAdsPrivacy($this);
    ...
    function actionGetList($sMode = '', $sOwnerId = '', $sAdd = '', $sAdd1 = '', $sAdd2 = '')
    {
        ...
        ch_import('SearchUnit', $this->_aModule);
        ...
        if ($oTmpAdsSearch->aCurrent['paginate']['totalNum'] == 0) {
            ...
            $sLink  = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'get_list/' . $sMode . '/' . $iOwnerId;
            $oPgn   = new ChWsbPaginate(array(
    ...
    function actionSearch()
    {
        ...
        $sCategory    = (int)ch_get('FilterCat');
        $sSubCategory = (int)ch_get('FilterSubCat');
        $sCountry     = process_db_input(ch_get('FilterCountry'), CH_TAGS_STRIP);
        $sCountry     = (isset($aPreValues['Country'][$sCountry]) == true) ? $sCountry : '';
        $sKeywords    = process_db_input(ch_get('FilterKeywords'), CH_TAGS_STRIP);
        ...
        $sCustomFieldCaption1 = process_db_input(ch_get('CustomFieldCaption1'), CH_TAGS_STRIP);
        $sCustomFieldCaption2 = process_db_input(ch_get('CustomFieldCaption2'), CH_TAGS_STRIP);

        require_once($this->_oConfig->getClassPath() . 'ChAdsSearchUnit.php');
        $oTmpAdsSearch                                  = new ChAdsSearchUnit();
        $oTmpAdsSearch->aCurrent['paginate']['perPage'] = 10;
        ...
        if ($oTmpAdsSearch->aCurrent['paginate']['totalNum'] == 0) {
            ...
            $sRequest = ch_html_attribute($_SERVER['PHP_SELF']) . '?';
...
EOF;
        ...
        $this->aPageTmpl['header']   = _t('_ch_ads_Filter');
    ...
    function actionSharePopup($iEntryId)
    {
        ...
        require_once(CH_DIRECTORY_PATH_INC . "shared_sites.inc.php");
...
EOF;
    ...
    function GetHeaderString()
    {
        ...
        switch (ch_get('action')) {
            case 'show_featured':
                $sMsgMain = '_ch_ads_Featured';
                break;
            case 'show_top_rated':
                $sMsgMain = '_ch_ads_Top_Rated';
                break;
            case 'show_all_ads':
                $sMsgMain = '_ch_ads_All_ads';
                break;
            case 'show_popular':
                $sMsgMain = '_ch_ads_Popular';
                break;
            case 'tags':
                $sMsgMain = '_Tags';
                break;
            case 'show_categories':
                $sMsgMain = '_ch_ads_Categories';
                break;
            case 'show_calendar':
                $sMsgMain = '_ch_ads_Calendar';
                break;
            case 'show_calendar_ads':
                ...
                $sMsgMain = '_ch_ads_caption_browse_by_day';
                $sMsgAdd  = getLocaleDate(strtotime("{$iValue1}-{$iValue2}-{$iValue3}"), CH_WSB_LOCALE_DATE_SHORT);
                break;
            case 'my_page':
                switch (ch_get('mode')) {
                    case 'add':
                        $sMsgMain = '_ch_ads_Add';
                        break;
                    case 'manage':
                        $sMsgMain = '_ch_ads_Manage_ads';
                        break;
                    case 'pending':
                        $sMsgMain = '_ch_ads_pending_approval';
                        break;
                    case 'expired':
                        $sMsgMain = '_ch_ads_expired';
                        break;
                    case 'disapproved':
                        $sMsgMain = '_ch_ads_Disapproved';
                        break;
                    default:
                        $sMsgMain = '_ch_ads_My_Ads';
                }
                break;
            case '3':
                $sMsgMain = '_ch_ads_Filtered_ads';
                break;
            default:
                if (false !== ch_get('ShowAdvertisementID')) {
                    $sMsgAdd = $this->_oDb->getAdSubjectByID((int)ch_get('ShowAdvertisementID'));
                } elseif (false !== ch_get('entryUri')) {
                    $sMsgAdd = $this->_oDb->getAdSubjectByUri(process_db_input(ch_get('entryUri'), CH_TAGS_STRIP));
                } elseif (false !== ch_get('UsersOtherListing') && (int)ch_get('IDProfile') > 0) {
                    $sMsgMain = '_ch_ads_Users_other_listing';
                } elseif (false !== ch_get('bClassifiedID') || false !== ch_get('catUri')) {
                    if (false !== ch_get('bClassifiedID') && (int)ch_get('bClassifiedID') > 0) {
                        $sMsgAdd = $this->_oDb->getCategoryNameByID((int)ch_get('bClassifiedID'));
                    } elseif (false !== ch_get('catUri') && ch_get('catUri') != '') {
                        $sMsgAdd = $this->_oDb->getCategoryNameByUri(process_db_input(ch_get('catUri'), CH_TAGS_STRIP));
                    }
                } elseif (false !== ch_get('bSubClassifiedID') || false !== ch_get('scatUri')) {
                    $aSubcatRes = null;
                    if (false !== ch_get('bSubClassifiedID') && (int)ch_get('bSubClassifiedID') > 0) {
                        $aSubcatRes = $this->_oDb->getCatSubCatNameBySubCatID((int)ch_get('bSubClassifiedID'));
                    } elseif (false !== ch_get('scatUri') && ch_get('scatUri') != '') {
                        $aSubcatRes = $this->_oDb->getCatSubCatNameBySubCatUri(process_db_input(ch_get('scatUri'),
                            CH_TAGS_STRIP));
                    }
    ...
    function isAllowedView($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_VIEW, $isPerformAction);
    ...
    function isAllowedBrowse($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_BROWSE, $isPerformAction);
    ...
    function isAllowedSearch($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_SEARCH, $isPerformAction);
    ...
    function isAllowedAdd($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_ADD, $isPerformAction);
    ...
    function isAllowedEdit($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_EDIT_ANY_AD, $isPerformAction);
    ...
    function isAllowedDelete($iOwnerID, $isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_DELETE_ANY_AD, $isPerformAction);
    ...
    function isAllowedApprove($isPerformAction = false)
    {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iVisitorID, CH_ADS_APPROVING, $isPerformAction);
    ...
    {
        if ($aDataEntry['AllowView'] != CH_WSB_PG_ALL) {
            return false;
        }
    ...
    function GenAdminTabbedPage()
    {
        ...
        $sContent .= DesignBoxAdmin(_t('_ch_ads_pending_approval'), $sPendingTab);
    ...
    function GenMyPageAdmin($sForceMode = '')
    {
        ...
        $sAdministrationC = _t('_ch_ads_Administration');
        $sMyAdsC          = _t('_ch_ads_My_Ads');
        $sPendApprC       = _t('_ch_ads_pending_approval');
        $sDisApprC        = _t('_ch_ads_Disapproved');
        $sAddC            = _t('_ch_ads_Add');

        if (ch_get('action_delete') && is_array(ch_get('ads'))) {
            foreach (ch_get('ads') as $iAdID) {
        ...
        if ($this->bUseFriendlyLinks) {
            $sAdsMainLink = $sAdsAddLink = $sAdsManageLink = $sAdsPendingLink = $sAdsDisapprovedLink = $sAdsExpiredLink = CH_WSB_URL_ROOT;
        ...
        $sBoxId    = '';
        $sAjLink   = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'get_list/';
        ...
        $sMode         = ($sForceMode != '') ? $sForceMode : ch_get('mode');
        ...
        switch ($sMode) {
            case 'add':
                $sAddTabClass = 1;
                $sNewPostForm = $this->AddNewPostForm((int)ch_get('EditPostID'), false);
            ...
            default:
                ...
                $sAdministrationDescC = _t('_ch_ads_admin_box_desc', $iMyAdsCnt, $sAdsManageLink, $sAdsAddLink);
        ...
        $sAdmPost = ChWsbPageView::getBlockCaptionMenu(time(), array(
            'ads_main'        => array(
                'href'   => $sAdsMainLink,
                'title'  => _t('_ch_ads_Manage_main'),
                'active' => $sMainTabClass
            ),
            'ads_add'         => array('href' => $sAdsAddLink, 'title' => _t('_ch_ads_Add'), 'active' => $sAddTabClass),
            'ads_manage'      => array(
                'href'   => $sAdsManageLink,
                'title'  => _t('_ch_ads_Manage_ads'),
                'active' => $sManageTabClass
            ),
            'ads_pending'     => array(
                'href'   => $sAdsPendingLink,
                'title'  => _t('_ch_ads_pending_approval'),
                'active' => $sPendingTabClass
            ),
            'ads_expired'     => array(
                'href'   => $sAdsExpiredLink,
                'title'  => _t('_ch_ads_expired'),
                'active' => $sExpiredTabClass
            ),
            'ads_disapproved' => array(
                'href'   => $sAdsDisapprovedLink,
                'title'  => _t('_ch_ads_Disapproved'),
    ...
    function AddNewPostForm($iPostID = 0, $bBox = true)
    {
        ...
        $sDaysC          = _t('_days');
        $iMaxLt          = (int)getParam('ch_ads_max_live_days');
        $sMaxedString    = _t('_ch_ads_Warn_max_live_days');
        ...
        $iCategoryID    = (int)ch_get('Classified');
        $iSubCategoryID = (int)ch_get('IDClassifiedsSubs');
        $sCustomValues  = $sScriptHandle = '';
        if (false !== ch_get('IDClassifiedsSubs')) {
            $sScriptHandle = <<<EOF
...
EOF;
        ...
        $iSubCatID        = (int)ch_get('IDClassifiedsSubs');
        ...
        $aVars             = array(
            ...
            'ch_if:cat_warning' => array(
                'condition' => (false !== ch_get('add_button') && !$iSubCatID),
                'content'   => array()
            ),
        );
            ...
            'inputs'     => array(
                ...
                'Subject'          => array(
                    ...
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(3, 100),
                        'error'  => _t('_ch_ads_Caption_error'),
                    ),
                    'db'       => array(
                        'pass' => 'Xss',
                    ),
                ...
                'LifeTime'         => array(
                    'type'     => 'select',
                    'name'     => 'LifeTime',
                    'caption'  => _t('_ch_ads_Life_Time') . " ({$sDaysC})",
                    'info'     => $sMaxedString,
                    'value'    => $iMaxLt,
                ...
                'Message'          => array(
                    'type'     => 'textarea',
                    'name'     => 'Message',
                    'caption'  => _t('_ch_ads_post_text'),
                    'required' => true,
                    'html'     => 2,
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(3, 65535),
                        'error'  => _t('_ch_ads_Text_error'),
                    ),
                    'db'       => array(
                        'pass' => 'XssHtml',
                    ),
                ...
                'Country'          => array(
                    'type'     => 'select',
                    'name'     => 'Country',
                    'caption'  => _t('_ch_ads_caption_country'),
                    'values'   => $aCountries,
                    'value'    => $sCountry,
                    'required' => true,
                    'checker'  => array(
                ...
                'City'             => array(
                    'type'     => 'text',
                    'name'     => 'City',
                    'caption'  => _t('_ch_ads_caption_city'),
                    'required' => true,
                    'value'    => $sCity,
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(3, 50),
                        'error'  => _t('_ch_ads_err_city'),
                    ),
                    'db'       => array(
                        'pass' => 'Xss',
                    ),
        ...
        if ($iPostID > 0) {
            ...
            if ($sPostPictureElements != '') {
                ...
                $aForm['inputs']['ExistedImages']['caption'] = _t('_ch_ads_Existed_images');
            }
        ...
        if ($oForm->isSubmittedAndValid() && $iSubCatID) {
            ...
            $sCurTime         = time();
            $sPostUri         = uriGenerate(ch_get('Subject'), $this->_oConfig->sSQLPostsTable, 'EntryUri');
            $sAutoApprovalVal = (getParam('ch_ads_auto_approving') == 'on') ? 'active' : 'new';

            $sCustomFieldValue1 = floatval(ch_get('CustomFieldValue1'));
            $sCustomFieldValue2 = floatval(ch_get('CustomFieldValue2'));
            ...
            if ($iLastId > 0) {
                ...
                if (ChWsbModule::getInstance('ChWmapModule')) {
                    ChWsbService::call('wmap', $iPostID ? 'response_entry_change' : 'response_entry_add',
                        array($this->_oConfig->getUri(), $iLastId ? $iLastId : $iPostID));
                }

                ch_import('ChWsbAlerts');
                $sAlertAction = ($iPostID) ? 'edit' : 'create';
                $oZ           = new ChWsbAlerts('ads', $sAlertAction, $iLastId, $this->_iVisitorID);
                $oZ->alert();

                $sResult = ($iPostID > 0) ? _t('_ch_ads_Ad_succ_updated') : _t('_ch_ads_Ad_succ_added');
            ...
            $sNewAdFormVal = '<div class="blogs-view ch-def-bc-margin">' . $oForm->getCode() . '</div>';

            return ($bBox) ? DesignBoxContent(_t('_ch_ads_Add_ad'), $sNewAdFormVal, 1) : $sNewAdFormVal;
        }
    }
        ...
        if ($_FILES) {
            ...
            for ($i = 0; $i < count($_FILES['userfile']['tmp_name']); $i++) {
                ...
                if ($_FILES['userfile']['size'][$i] > $this->iMaxUplFileSize) {
                    echo _t_err('_ch_ads_Warn_max_file_size', $_FILES['userfile']['name'][$i]);
                    continue;
                }
                ...
                $sImg               = CH_DIRECTORY_PATH_ROOT . "{$this->sUploadDir}img_{$sBaseName}{$sExt}";
                $sImgThumb          = CH_DIRECTORY_PATH_ROOT . "{$this->sUploadDir}thumb_{$sBaseName}{$sExt}";
                $sImgThumbBig       = CH_DIRECTORY_PATH_ROOT . "{$this->sUploadDir}big_thumb_{$sBaseName}{$sExt}";
                $sImgIcon           = CH_DIRECTORY_PATH_ROOT . "{$this->sUploadDir}icon_{$sBaseName}{$sExt}";
    ...
    function getManageArea($oSearch, $sBoxIdSpec, $aButtons = array(), $sPgnUrl = '', $aPgn = array())
    {
        ...
        if ($oSearch->aCurrent['paginate']['totalNum'] == 0) {
            ...
            if ($oSearch->aCurrent['paginate']['perPage'] < $oSearch->aCurrent['paginate']['totalNum']) {
                ...
                $oPgnSpec   = new ChWsbPaginate($aPgnParams);
        ...
        if (!empty($aButtons) && $oSearch->bShowCheckboxes) {
            ...
            $sCode         = <<<EOF
<form id="ch_ads_user_form" method="post">
    <div id="ads_box">
        {$sCode}
        {$sPgn}
    </div>
...
EOF;
    ...
    {
        $sBrowseAllAds = _t('_ch_ads_Browse_All_Ads');
        $sUserListC    = _t('_ch_ads_Users_other_listing');

        $sHomeLink = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/' : "{$this->sCurrBrowsedFile}?Browse=1";

        $sSiteUrl = CH_WSB_URL_ROOT;

        $sBreadCrumbs = <<<EOF
<div class="paginate ch-def-padding-left ch-def-padding-right">
    <div class="view_all">
...
EOF;
    ...
    function ActionDeleteAdvertisement($iID)
    {
        ...
        if ($iDeleteAdvertisementID > 0) {
            $sSuccDel = _t("_ch_ads_Ad_succ_deleted");
            $sFailDel = _t("_ch_ads_Ad_fail_delete");
            ...
            if ($sMediaIDs != '') {
                ...
                foreach ($aChunks as $sMedId) {
                    ...
                    if ($iMedId) {
                        ...
                        if ($sMediaFileName != '') {
                            @unlink(CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . 'img_' . $sMediaFileName);
                            @unlink(CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . 'thumb_' . $sMediaFileName);
                            @unlink(CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . 'big_thumb_' . $sMediaFileName);
                            @unlink(CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . 'icon_' . $sMediaFileName);
                        }
            ...
            if ($this->_oDb->deleteAd($iDeleteAdvertisementID)) {
                ...
                $oCmts = new ChWsbCmts('ads', $iDeleteAdvertisementID);
                ...
                $oViews = new ChWsbViews('ads', $iDeleteAdvertisementID, false);
                $oViews->onObjectDelete();

                // delete associated locations
                if (ChWsbModule::getInstance('ChWmapModule')) {
                    ChWsbService::call('wmap', 'response_entry_delete',
                ...
                ch_import('ChWsbAlerts');
                $oZ = new ChWsbAlerts('ads', 'delete', $iDeleteAdvertisementID, $iDeleteAdvertisementID);
                $oZ->alert();
        ...
        if ($sMediaIDs != '') {
            ...
            foreach ($aChunks as $sMedId) {
                ...
                if (is_numeric($iMedId) && $iMedId) {
                    ...
                    if ($aSqlRes) {
                        $sFileName    = CH_WSB_URL_ROOT . $this->sUploadDir . 'thumb_' . $aSqlRes['MediaFile'];
                        $sAdsEditLink = ($this->bUseFriendlyLinks ? CH_WSB_URL_ROOT . "ads/my_page/edit/{$iPostID}/dimg/{$iMedId}" : "{$this->sCurrBrowsedFile}?action=my_page&mode=add&EditPostID={$iPostID}&dimg={$iMedId}");
                        $sImgTag .= <<<EOF
<div style="float:left;">
    <img class="photo1 ch-def-round-corners ch-def-shadow ch-def-margin-sec-right" src="{$sFileName}" style="width:{$this->iThumbSize}px;" />
    <br />
    ...
    function getAdCover($sMediaIDs, $sType = 'thumb', $isSubstituteNoImage = true)
    {
        ...
        if ($sMediaIDs != '') {
            ...
            if (is_numeric($iMedId) && $iMedId) {
                $aSqlRes = $this->_oDb->getMediaInfo($iMedId);
                if ($aSqlRes) {
                    if (file_exists(CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . $sType . '_' . $aSqlRes['MediaFile'])) {
                        $sFileName = CH_WSB_URL_ROOT . $this->sUploadDir . $sType . '_' . $aSqlRes['MediaFile'];
                    }
                }
            }
        }
    ...
    function getAdCoverPath($sMediaIDs, $sType = 'thumb', $isSubstituteNoImage = true)
    {
        ...
        if ($sMediaIDs != '') {
            ...
            if (is_numeric($iMedId) && $iMedId) {
                ...
                if ($aSqlRes) {
                    if (file_exists(CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . $sType . '_' . $aSqlRes['MediaFile'])) {
                        $sFileName = CH_DIRECTORY_PATH_ROOT . $this->sUploadDir . $sType . '_' . $aSqlRes['MediaFile'];
                    }
                }
            }
        }
    ...
    function ActionBuyAdvertisement($iAdvertisementID)
    {
        ...
        if ($this->_iVisitorID > 0) {
            ...
            if ($aSqlResStr) {
                ...
                $sBuyMsg1     = _t('_ch_ads_BuyMsg1');
                $sBuyDet1     = _t('_ch_ads_BuyDetails1');
...
<div class="dbContent">
    ...
    <div>
        <input class="button ch-btn" type="submit" onclick="javascript:this.value='Wait...';this.disabled=true;document.bid_form.submit();" value="{$sContinue}" />
        <div class="clear_both"></div>
    </div>
</div>
EOF;
...
EOF;
    ...
    function ActionBuySendMailAdvertisement($iAdvertisementID)
    {
        ...
        if ($this->_iVisitorID > 0) {
            ...
            if ($aSqlResStr) {
                ...
                $sPowDol      = _t('_powered_by_Cheetah');
                $sBuyMsg2     = _t('_ch_ads_BuyMsg2');
                $sBuyDet1     = _t('_ch_ads_BuyDetails1');
                $sReturnBackC = _t('_ch_ads_Back');
                ...
                if (sendMail($aSqlSellerRes['Email'], $sSubjectS, $sMessageS, $aSqlSellerRes['ID'], $aPlus, 'html')) {
                    ...
                    ch_import('ChWsbAlerts');
                    $oZ = new ChWsbAlerts('ads', 'buy', $iAdvertisementID, $this->_iVisitorID);
                    $oZ->alert();
                }
    ...
    <div>
        <a class="ch-btn" href="{$sGenUrl}">{$sReturnBackC}</a>
        <div class="clear_both"></div>
    </div>
EOF;
    ...
    function ActionPrintAdvertisement($iID)
    {
        ...
        if ($aSqlResStr) {
            ...
            ch_import('ChWsbViews');
            new ChWsbViews('ads', $iAdvertisementID);
            ...
            $sPostedByC      = _t('_ch_ads_Posted_by');
            $sPhoneC         = _t('_Phone');
            $sDetailsC       = _t('_ch_ads_Details');
            $sUserOtherListC = _t('_ch_ads_Users_other_listing');
            ...
            if ($this->_iVisitorID > 0 && $this->_iVisitorID != $aNameRet['ID']) {//print Send PM button and other actions
                if (getParam('ch_ads_enable_paid') == 'on') {
...
EOF;
            ...
            $sPictureSect           = ($sPictureSectContent != '') ? DesignBoxContent(_t('_ch_ads_Ad_photos'),
	        ...
	        if(ChWsbRequest::serviceExists('wall', 'get_repost_js_click')) {
	        	$sSubsAddon .= ChWsbService::call('wall', 'get_repost_js_script');

                $aActionKeys['repostCpt']    = _t('_Repost');
                $aActionKeys['repostScript'] = ChWsbService::call('wall', 'get_repost_js_click',
                    array($this->_iVisitorID, 'ads', 'create', $iAdvertisementID));
            }
            $sActionsTable = $GLOBALS['oFunctions']->genObjectsActions($aActionKeys, 'ch_ads', false);
            ...
            ch_import('Cmts', $this->_aModule);
            $this->oCmtsView      = new ChAdsCmts ($this->_oConfig->getCommentSystemName(), $iAdvertisementID);
            ...
            $sCategLink  = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/cat/' . $aSqlResStr['CEntryUri'] : "{$this->sCurrBrowsedFile}?bClassifiedID={$aSqlResStr['CatID']}";
            $sSCategLink = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/subcat/' . $aSqlResStr['SEntryUri'] : "{$this->sCurrBrowsedFile}?bSubClassifiedID={$aSqlResStr['SubID']}";
            ...
            $sDescriptionContent  = '<div class="dbContent ch-def-bc-margin ch-def-font-large">' . $sAdsMessage . '</div>';
            ...
            $sOtherInfoSect     = DesignBoxContent(_t('_ch_ads_Custom_Values'), $sOtherInfoContent, 1);
            $this->sTAOtherInfo = $sOtherInfoContent;

            $bPossibleToRate = $this->oPrivacy->check('rate', $iAdvertisementID, $this->_iVisitorID);
            $oVotingView     = new ChTemplVotingView ('ads', $iAdvertisementID);
            ...
            $this->sTARateContent = '<div class="ch-def-bc-margin">' . $sVotePostRating . '</div>';
...
EOF;
            ...
            ch_import('ChWsbAlerts');
            $oZ = new ChWsbAlerts('ads', 'view', $iAdvertisementID, $this->_iVisitorID);
            $oZ->alert();
    ...
    function serviceGetSubscriptionParams($sAction, $iEntryId)
    {
        ...
        $aActionList = array(
            'commentPost' => '_ch_ads_sbs_comments'
        );
    ...
    function serviceGetMemberMenuItem()
    {
        ...
        $aLinkInfo   = array(
            'item_img_src' => 'money',
            'item_img_alt' => _t('_ch_ads_Ads'),
            'item_link'    => CH_WSB_URL_ROOT . (getParam('permalinks_module_ads') == 'on' ? 'ads/my_page/' : 'modules/cheetah/ads/classifieds.php?action=my_page'),
            'item_title'   => _t('_ch_ads_Ads'),
    ...
    function serviceGetMemberMenuItemAddContent()
    {
        ...
        $aLinkInfo   = array(
            'item_img_src' => 'money',
            'item_img_alt' => _t('_ch_ads_Ad'),
            'item_link'    => CH_WSB_URL_ROOT . (getParam('permalinks_module_ads') == 'on' ? 'ads/my_page/add/' : 'modules/cheetah/ads/classifieds.php?action=my_page&mode=add'),
            'item_title'   => _t('_ch_ads_Ad'),
        );
    ...
    function serviceMapInstall()
    {
        ...
        return ChWsbService::call('wmap', 'part_install', array(
            ...
            array(
                'part'               => 'ads',
                'title'              => '_ch_ads_Ads',
                'title_singular'     => '_ch_ads_Ad',
                'icon'               => 'modules/cheetah/ads/|map_marker.png',
                'icon_site'          => 'money',
                'join_table'         => 'ch_ads_main',
                ...
                'permalink'          => 'modules/cheetah/ads/classifieds.php?entryUri=',
            )
        ));
    }
    ...
    function PrintAllSubRecords($iClassifiedID)
    {
        ...
        if ($oTmpAdsSearch->aCurrent['paginate']['totalNum'] == 0) {
            ...
            if ($this->bUseFriendlyLinks == false) { //old variant
                $sRequest = ch_html_attribute($_SERVER['PHP_SELF']) . '?bClassifiedID=' . $iSafeCatID . '&page={page}&per_page={per_page}';
            } else {
                $sRequest    = CH_WSB_URL_ROOT . 'ads/all/cat/';
                $sPaginAddon = '/' . process_db_input(ch_get('catUri'), CH_TAGS_STRIP);
...
EOF;
    ...
    function PrintSubRecords($iIDClassifiedsSubs)
    {
        ...
        if ($oTmpAdsSearch->aCurrent['paginate']['totalNum'] == 0) {
            ...
            // Prepare link to pagination
            if ($this->bUseFriendlyLinks == false) { //old variant
                $sRequest = ch_html_attribute($_SERVER['PHP_SELF']) . '?bSubClassifiedID=' . $iIDClassifiedsSubs . '&page={page}&per_page={per_page}';
            } else {
                $sRequest    = CH_WSB_URL_ROOT . 'ads/all/subcat/';
                $sPaginAddon = '/' . process_db_input(ch_get('scatUri'), CH_TAGS_STRIP);
                $sRequest .= '{per_page}/{page}' . $sPaginAddon;
            }
            // End of prepare link to pagination
        ...
        $sHomeLink  = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/' : "{$this->sCurrBrowsedFile}?Browse=1";
        $sCategLink = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/cat/' . $aSubcatRes['CEntryUri'] : "{$this->sCurrBrowsedFile}?bClassifiedID={$aSubcatRes['ClassifiedsID']}";
...
EOF;
    ...
    {
        $sHomeLink = ($this->bUseFriendlyLinks && $this->bAdminMode == false) ? CH_WSB_URL_ROOT . 'ads/' : "{$this->sCurrBrowsedFile}?Browse=1";
...
EOF;
    ...
    {
        $sCaption     = _t('_ch_ads_last_ads');
        $sDisplayMode = '';
        $sTypeMode    = '';
        switch ($sType) {
            case 'last':
                $sCaption     = _t('_ch_ads_All_ads');
                $sDisplayMode = 'last';
                break;
            case 'featured':
                $sCaption     = _t('_ch_ads_Featured');
                $sTypeMode    = 'featured';
                $sDisplayMode = 'last';
                break;
            case 'popular':
                $sCaption     = _t('_ch_ads_Popular');
                $sDisplayMode = 'popular';
                break;
            case 'top':
            default:
                $sCaption     = _t('_ch_ads_Top_Rated');
                $sDisplayMode = 'top';
                break;
        }
        ...
        $sRequest  = ch_html_attribute($_SERVER['PHP_SELF']) . '?' . $sParams;
    ...
    function genCategoriesBlock()
    {
        ...
        while ($aSqlResStr = $vSqlRes->fetch()) {
            ...
            $sCategLink = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/cat/' . $sCatUri : "{$this->sCurrBrowsedFile}?bClassifiedID={$iID}";
            ...
            while ($aSqlResSubsStr = $sqlResSubs->fetch()) {
                ...
                $sSCategLink = ($this->bUseFriendlyLinks) ? CH_WSB_URL_ROOT . 'ads/subcat/' . $aSqlResSubsStr['SEntryUri'] : "{$this->sCurrBrowsedFile}?bSubClassifiedID={$iSubID}";
...
EOF;
            ...
            if ($iResidueOfDiv == 0) {
                $sOpenColDiv = <<<EOF
<div class="ch_ads_categories_col" style="width:{$iColumnWidth}%;">
EOF;
            }
...
<div class="ch_ads_categories_cols ch-def-bc-padding-thd">
    {$sAddJS}
    {$sCategoriesHtml}
    <div class="clear_both"></div>
</div>
EOF;
        ...
        return DesignBoxContent(_t('_ch_ads_Categories'), $sCategoriesBlocks, 1);
    }
    ...
    function getAdsMainPage()
    {
        ...
        ch_import('PageHome', $this->_aModule);
        $oAdsPageHome = new ChAdsPageHome($this);
    ...
    function PrintFilterForm($iClassifiedID = 0, $iSubClassifiedID = 0)
    {
        ...
        $sCategoriesC = _t('_ch_ads_Categories');
        $sViewAllC    = _t('_View All');

        $iClassifiedID    = (false !== ch_get('FilterCat') && (int)ch_get('FilterCat') > 0) ? (int)ch_get('FilterCat') : (int)$iClassifiedID;
        $iSubClassifiedID = (false !== ch_get('FilterSubCat') && (int)ch_get('FilterSubCat') > 0) ? (int)ch_get('FilterSubCat') : (int)$iSubClassifiedID;
        $sCountry         = process_db_input(ch_get('FilterCountry'), CH_TAGS_STRIP);
...
EOF;
            ...
            $sCateg = <<<EOF
<br />
<div class="ordered_block_select ch-def-margin-sec-top ch-def-margin-sec-right">
<span>{$sCategoriesC}:</span>
<div class="input_wrapper input_wrapper_select ch-def-margin-sec-left clearfix">
<select class="form_input_select ch-def-font-inputs" name="FilterCat" id="FilterCat" {$sOnChange} style="width:250px;">
...
<select class="form_input_select ch-def-font-inputs" name="FilterSubCat" id="FilterSubCat" style="display:{$sSubDspStyle};"></select>
</div>
</div>
...
EOF;
        ...
        $aVariables = array(
            ...
            'sCateg'           => $sCateg,
            'sSubCateg'        => $sSubCateg,
            'sKeywordsStr'     => ch_html_attribute($sKeywordsStr),
        );
    ...
    function getManageClassifiedsForm($iCategoryID = 0, $bOnlyForm = false)
    {
            ...
            'inputs'     => array(
                ...
                'Name'             => array(
                    ...
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(3, 64),
                        'error'  => _t('_ch_ads_title_error_desc', 64),
                    ),
                    'db'       => array(
                        'pass' => 'Xss',
                    ),
                ...
                'Description'      => array(
                    ...
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(3, 128),
                        'error'  => _t('_ch_ads_description_error_desc', 128),
                    ),
                    'db'       => array(
                        'pass' => 'Xss',
                    ),
                ...
                'CustomFieldName1' => array(
                    ...
                    'caption'  => _t('_ch_ads_customFieldName1'),
                    'required' => false,
                    'value'    => $sCustomName1,
                    'db'       => array(
                        'pass' => 'Xss',
                ...
                'CustomFieldName2' => array(
                    ...
                    'caption'  => _t('_ch_ads_customFieldName2'),
                    'required' => false,
                    'value'    => $sCustomName2,
                    'db'       => array(
                        'pass' => 'Xss',
                ...
                'Picture'          => array(
                    ...
                    'name'    => 'Picture',
                    'caption' => _t('_Picture'),
                    'info'    => _t('_In') . ' \modules\cheetah\ads\templates\base\images\icons\'',
                    'value'   => $sPicture,
                    'db'      => array(
                        'pass' => 'Xss',
                    ),
                ...
                'add_button'       => array(
                    'type'  => 'submit',
                    'name'  => 'add_button',
                    'value' => ($iCategoryID) ? _t('_Edit') : _t('_ch_ads_add_main_category'),
                ),
            ),
        );
        ...
        if ($oForm->isSubmittedAndValid()) {
            $aValsAdd = array();
            if ($iCategoryID == 0) {
                $sCategUri             = uriGenerate(ch_get('Name'), $this->_oConfig->sSQLCatTable, 'CEntryUri');
            ...
            if ($iLastId > 0) {
                $sCode = MsgBox(_t('_ch_ads_Main_category_successfully_added'), 3);
            } else {
                $sCode = MsgBox(_t('_ch_ads_Main_category_failed_add'), 3);
            }
        }
        ...
        $sActions = array(
            'add_subcat' => array(
                ...
                'title'   => _t('_ch_ads_add_subcategory'),
                'onclick' => 'loadHtmlInPopup(\'ads_add_sub_category\', \'modules/cheetah/ads/post_mod_ads.php?action=add_sub_category\');',
                'active'  => 0
            ),
            'manager'    => array(
                'href'    => 'javascript: void(0);',
                'title'   => _t('_ch_ads_category_manager'),
                'onclick' => 'loadHtmlInPopup(\'ads_category_manager\', \'modules/cheetah/ads/post_mod_ads.php?action=category_manager\');',
                'active'  => 0
            )
        );

        return DesignBoxAdmin(_t('_ch_ads_Manage_categories_form'), $sCode . $oForm->getCode(), $sActions, '', 11);
    }

    function getAddSubcatForm($iSubCategoryID = 0, $bOnlyForm = false)
    { //admin side only
        $sAction  = 'add_sub_category';
        $sSubmitC = !empty($iSubCategoryID) ? _t('_Edit') : _t('_ch_ads_add_subcategory');
        ...
        $aForm = array(
            ...
            'inputs'     => array(
                ...
                'IDClassified' => array(
                    'type'    => 'select',
                    'name'    => 'IDClassified',
                    'caption' => _t('_ch_ads_parent_category'),
                ...
                'NameSub'      => array(
                    ...
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(3, 128),
                        'error'  => _t('_ch_ads_title_error_desc', 128),
                    ),
                    'db'       => array(
                        'pass' => 'Xss',
                    ),
        ...
        if ($oForm->isSubmittedAndValid()) {
            $aValsAdd = array();
            if ($iSubCategoryID == 0) {
                $sCategUri             = uriGenerate(ch_get('NameSub'), $this->_oConfig->sSQLSubcatTable, 'SEntryUri');
            ...
            if ($iSubCategoryID > 0) {
                ...
                $sMessage = '_ch_ads_Sub_category_successfully_updated';
            } else {
                $iLastId  = $oForm->insert($aValsAdd);
                $sMessage = '_ch_ads_Sub_category_successfully_added';
            }

            $sCode = MsgBox(_t($iLastId > 0 ? $sMessage : '_ch_ads_Sub_category_failed_add'), 3);
        }
        ...
        return $GLOBALS['oFunctions']->popupBox('ads_add_sub_category', _t('_ch_ads_add_subcategory'), $sResult);
    }

    function getCategoryManager()
    {
        $sCatID = (int)ch_get('id');
        if ($sCatID && false !== ch_get('sa')) {
            switch (ch_get('sa')) {
                case 'editcat':
                    $sResult = $this->getManageClassifiedsForm($sCatID, true);
                    $sResult = '<div class="ch-def-margin-sec-bottom">' . $sResult . '</div>';
                    break;
                case 'editscat':
                    ...
                    break;
                case 'delcat':
                    $sResult = MsgBox(_t('_ch_ads_Main_category_failed_delete'), 1);
                    if ($this->_oDb->deleteCat($sCatID)) {
                        $sResult = MsgBox(_t('_ch_ads_Main_category_successfully_deleted'), 1);
                    }
                    ...
                    break;
                case 'delscat':
                    $sResult = MsgBox(_t('_ch_ads_Sub_category_failed_deleted'), 1);
                    if ($this->_oDb->deleteSubCat($sCatID)) {
                        $sResult = MsgBox(_t('_ch_ads_Sub_category_successfully_delete'), 1);
                    }
                    break;
            }
        }
...
EOF;
        ...
        return $GLOBALS['oFunctions']->popupBox('ads_category_manager', _t('_ch_ads_category_manager'), $sResult);
    }
    ...
    function GenAdsAdminIndex()
    {
        if ($this->bAdminMode) {
            //actions
            if (ch_get('action_approve') && is_array(ch_get('ads'))) {
                foreach (ch_get('ads') as $iAdID) {
                    if ($this->_oDb->setPostStatus((int)$iAdID, 'active')) {
                    }
                }
            } elseif (ch_get('action_disapprove') && is_array(ch_get('ads'))) {
                foreach (ch_get('ads') as $iAdID) {
                    $this->_oDb->setPostStatus((int)$iAdID);
                }
            } elseif (ch_get('action_delete') && is_array(ch_get('ads'))) {
                foreach (ch_get('ads') as $iAdID) {
                    $this->ActionDeleteAdvertisement((int)$iAdID);
                }
            }

            $sCap2C      = _t('_ch_ads_Moderating');
            $sAct        = _t("_ch_ads_Activate");
            $sWholesaleC = _t("_ch_ads_wholesale");
...
EOF;
    ...
    function PrintAdvertisementsByTag($sTag)
    {
        ...
        $sTagResultC   = _t('_ch_ads_search_results_by_tag');
        $sBrowseAllAds = _t('_ch_ads_Browse_All_Ads');
...
EOF;
        ...
        if ($oTmpAdsSearch->aCurrent['paginate']['totalNum'] == 0) {
            ...
            if ($this->bUseFriendlyLinks == false) {
                ...
                $sRequest = CH_WSB_URL_ROOT . "ads/tag/{$sSafeTagS}/{per_page}/{page}";
            }
            // End of prepare link to pagination
            $oTmpAdsSearch->aCurrent['paginate']['page_url'] = $sRequest;
    ...
    function GenAdsCalendar()
    {
        ...
        require_once($this->_oConfig->getClassPath() . 'ChAdsCalendar.php');
        $oCalendar    = new ChAdsCalendar((int)$aDateParams[0], (int)$aDateParams[1], $this);
        $sAdsCalendar = $oCalendar->display();

        return DesignBoxContent(_t('_ch_ads_Calendar'), $sAdsCalendar, 1);
    }
    ...
    function GenAdsByDate()
    {
        ...
        if ($iValue1 > 0 && $iValue2 > 0 && $iValue3 > 0) {
            ...
            $sCaption = _t('_ch_ads_caption_browse_by_day')
                . getLocaleDate(strtotime("{$iValue1}-{$iValue2}-{$iValue3}"), CH_WSB_LOCALE_DATE_SHORT);
...
EOF;
    ...
    {
        $iMediaID = (int)ch_get('dimg');
        if (!$iMediaID) {
            return '';
        }

        if (!($iEditAdvertisementID = (int)ch_get('EditPostID'))) {
            return '';
        }
        ...
        if ($this->_oDb->updatePostMedia($iEditAdvertisementID, $sNewMedia)) {
            ...
            if ($this->_oDb->deleteMedia($iMediaID)) {
                return MsgBox(_t('_ch_ads_img_succ_deleted'));
            }
    ...
    function _blockPhoto(&$aReadyMedia, $iAuthorId, $sPrefix = false)
    {
        ...
        foreach ($aReadyMedia as $sMedId) {
            ...
            $aImages[] = array(
                'icon_url'  => CH_WSB_URL_ROOT . $this->sUploadDir . 'icon_' . $aSqlRes['MediaFile'],
                'image_url' => CH_WSB_URL_ROOT . $this->sUploadDir . 'img_' . $aSqlRes['MediaFile'],
                'title'     => '',
            );
        }
    ...
    function GenTagsPage()
    {
        ...
        $oTags  = new ChTemplTagsModule($aParam, _t('_all'), CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'tags');
    ...
    function genUrl($iEntryId, $sEntryUri, $sType = 'entry', $bForce = false)
    {
        ...
        if ($this->bUseFriendlyLinks) {
            $sUrl = CH_WSB_URL_ROOT . "ads/{$sType}/{$sEntryUri}";
        } else {
            $sUrl = '';
            switch ($sType) {
                case 'entry':
...
EOF;
    ...
    function serviceAdsRss()
    {
        ...
        if (is_array($aRssUnits) && count($aRssUnits) > 0) {
            ...
            $sUnitTitleC = _t('_ch_ads_Ads');
            $sMainLink   = 'modules/cheetah/ads/classifieds.php';

            ch_import('ChWsbRssFactory');
            $oRssFactory = new ChWsbRssFactory();
    ...
    */
    ...
    function serviceGetSpyPost($sAction, $iObjectId = 0, $iSenderId = 0, $aExtraParams = array())
    {
        ...
        switch ($sAction) {
            case 'create':
                $sLangKey     = '_ch_ads_added_spy';
                $iRecipientId = 0;
                break;

            case 'rate' :
                $sLangKey     = '_ch_ads_rated_spy';
                $iRecipientId = $aPostInfo['OwnerID'];
                break;

            case 'commentPost' :
                $sLangKey     = '_ch_ads_commented_spy';
                $iRecipientId = $aPostInfo['OwnerID'];
                break;
        }
    ...
    function serviceGetWallPost($aEvent)
    {
        ...
        if ($iItems > 1) {
            ...
            return array(
            	'owner_id' => $iOwner,
                'title' => _t('_ch_ads_wall_added_new_title_items', $sOwner, $iItems),
                'description' => '',
                'content' => $sCss . $this->_oTemplate->parseHtmlByName('wall_post_grouped.html', array(
	                'cpt_user_name' => $sOwner,
	                'cpt_added_new' => _t('_ch_ads_wall_added_new_items', $iItems),
        ...
        $sPostTxt = _t('_ch_ads_wall_object');

        return array(
        	'owner_id' => $iOwner,
            'title' => _t('_ch_ads_wall_added_new_title', $sOwner, $sPostTxt),
            'description' => _t('_ch_ads_wall_added_new_title', $sOwner, $sPostTxt),
            'content' => $sCss . $this->_oTemplate->parseHtmlByName('wall_post.html', array(
                'cpt_user_name' => $sOwner,
                'cpt_added_new' => _t('_ch_ads_wall_added_new'),
                'cpt_object' => $sPostTxt,
    ...
    function serviceGetWallAddComment($aEvent)
    {
        ...
        ch_import('Cmts', $this->_aModule);
        $oCmts = new ChAdsCmts($this->_oConfig->getCommentSystemName(), $iItem);
        if (!$oCmts->isEnabled()) {
            return '';
        }
        ...
        $sTextWallObject = _t('_ch_ads_wall_object');

        return array(
            'title'       => _t('_ch_ads_wall_added_new_title_comment', $sOwner, $sTextWallObject),
            'description' => $aComment['cmt_text'],
            'content'     => $sCss . $this->_oTemplate->parseHtmlByName('wall_post_comment.html', array(
                    'cpt_user_name'    => $sOwner,
                    'cpt_added_new'    => _t('_ch_ads_wall_added_new_comment'),
    ...
    function serviceGetWallPostComment($aEvent)
    {
        ...
        ch_import('Cmts', $this->_aModule);
        $oCmts = new ChAdsCmts($this->_oConfig->getCommentSystemName(), $iId);
        if (!$oCmts->isEnabled()) {
            return '';
        }
    ...
    {
        $sPrefix  = 'ch_' . $this->_oConfig->getUri();
                ...
                if ($aItem['Status'] == 'active' && $this->oPrivacy->check('view', $aItem['ID'], $this->_iVisitorID)) {
                    ...
                    if (!empty($aItem['Media']) && !empty($sPath)) {
                        if (!isset($aContent['idims'][$iId])) {
                            $aContent['idims'][$iId] = ChWsbImageResize::instance()->getImageSize($sPath);
                            $bSave                   = true;
                        }
        ...
        $sTmplName          = 'modules/cheetah/wall/|outline_item_image.html';
