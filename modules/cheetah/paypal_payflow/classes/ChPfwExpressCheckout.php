...
class ChPfwExpressCheckout extends ChPfwPayPal
{
	...
	function initializeCheckout($iPendingId, $aCartInfo)
	{
		...
		$sToken = process_db_input($aResponse['TOKEN'], CH_TAGS_STRIP);

		$sRedirectUrl = $this->_oConfig->getPpEndpoint(CH_PFW_ENDPOINT_TYPE_HOSTED);
		$sRedirectUrl = ch_append_url_params($sRedirectUrl, array('cmd' => '_express-checkout', 'token' => $sToken));
	...
	function finalizeCheckout(&$aData)
	{
		...
		$sResponseMessage = process_db_input($aResponse['RESPMSG'], CH_TAGS_STRIP);
        ...
        $this->_oDb->updatePending($iPending, array(
            'order' => process_db_input($aResponse['PPREF'], CH_TAGS_STRIP),
        	'order_ref' => process_db_input($aResponse['PNREF'], CH_TAGS_STRIP),
