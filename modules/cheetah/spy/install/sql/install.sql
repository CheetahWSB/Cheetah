    ...
    --
    -- Table structure for table `ch_spy_data`
    --
	...
	('ch_spy_content_activity', '', '', 'ChWsbService::call(\'spy\', \'response_content\', array($this));'),
	('ch_spy_profiles_activity', '', '', 'ChWsbService::call(\'spy\', \'response_profiles\', array($this));');

    SET @iLastHandler = (SELECT `id` FROM `sys_alerts_handlers` WHERE `name`='ch_spy_profiles_activity' LIMIT 1);
    ...
    (118, 'Spy Personal', '_ch_spy_notifications', 'modules/?r=spy/', @iTMOrder+1, 'memb', '', '', '', 1, 1, 1, 'custom', '', 0, '');
    ...
    INSERT INTO
        `sys_menu_member`
    SET
        `Caption`   = '_ch_spy_notifications',
        `Name`      = 'Spy',
        `Icon`      = 'bell',
        `Link`      = 'modules/?r=spy/',
        `Position`  = 'top_extra',
        `Order`     = 3,
        `PopupMenu` = 'ChWsbService::call(''spy'', ''get_member_menu_spy_data''); ',
        `Description` = '_ch_spy_notifications',
        `Bubble`    = '$aRetEval = ChWsbService::call(''spy'', ''get_member_menu_bubbles_data'', array({iOldCount}));';
    ...
    ('index', '1140px', 'Spy General', '_ch_spy', 0, 0, 'PHP', 'return ChWsbService::call(''spy'', ''get_spy_block'');', 1, 28.1, 'non,memb', 0),
    ('member', '1140px', 'Spy Personal', '_ch_spy_notifications', 0, 0, 'PHP', 'return ChWsbService::call(''spy'', ''get_spy_block'', array(''member.php'', $this->iMember));', 1, 28.1, 'memb', 0),
    ('friends', '1140px', 'Spy Friends', '_ch_spy_friends', 2, 2, 'PHP', 'return ChWsbService::call(''spy'', ''get_spy_block_friends'', array($this->iProfileID));', 1, 28.1, 'non,memb', 0);
    ...
    (`name`, `time`, `class`, `file`)
        VALUES
    ('ch_spy', '1 */12 * * *', 'ChSpyCron', 'modules/cheetah/spy/classes/ChSpyCron.php');
    ...
    (`Name`, `VALUE`, `kateg`, `desc`, `Type`, `check`, `err_text`, `order_in_kateg`, `AvailableValues`)
        VALUES
    ('ch_spy_permalinks', 'on', 26, 'Enable friendly permalinks in spy', 'checkbox', '', '', '0', '');
    ...
    ('ch_spy', '_ch_spy_chart', 'ch_spy_data', '', 'date', '', 1, @iMaxOrderCharts);
    ...
    ('ch_spy', '_sys_module_spy', 'ChSpyExport', 'modules/cheetah/spy/classes/ChSpyExport.php', @iMaxOrderExports, 1);
