 ...
 */
    ...
    require_once('ChSpyResponseContent.php');
    require_once('ChSpyResponseProfiles.php');
    require_once('ChSpySearch.php');
     ...
     *
     * Process all alerts.
     * @see ChSpyModule::serviceResponse
     * ChWsbService::call('spy', 'response', array($oAlert));
     *
     * This module will check for spy module alerts across all other Cheetah modules.
     * @see ChSpyModule::serviceUpdateHandlers
     * ChWsbService::call('spy', 'update_handlers', array(sModuleUri, $bInstall));
     *
     * Function will get spy block;
     * @see ChSpyModule::serviceGetSpyBlock
     * ChWsbService::call('spy', 'get_spy_block', array($sPage, $iProfileId));
     *
     * Function will generate content for member menu (latest activity);
     * @see ChSpyModule::serviceGetMemberMenuSpyData
     * ChWsbService::call('spy', 'get_member_menu_spy_data', array());
     *
     * Function will generate content for member menu's bubbles;
     * @see ChSpyModule::serviceGetMemberMenuBubblesData
     * ChWsbService::call('spy', 'get_member_menu_spy_data', array(iOldCount));
     *
     *
     * Alerts:
     * no alerts;
     *
     */
    class ChSpyModule extends ChWsbModule
    {
        ...
        function __construct(&$aModule)
        {
            ...
            $this -> oSearch        = new ChSpySearch($this);
        ...
        function actionAdministration()
        {
            ...
            $aLanguageKeys = array(
                'premoderation' => _t('_ch_spy_admin'),
            );
            ...
            switch ($sAction) {
                default:
                    $aMenu['ch_spy_main']['active'] = 1;

                    $iCategoryId = $this-> _oDb -> getSettingsCategory('ch_spy_keep_rows_days');
                    if(!$iCategoryId) {
                        $sContent = MsgBox( _t('_Empty') );
                    } else {
                            $mixedResult = '';
                            if(isset($_POST['save']) && isset($_POST['cat'])) {
                                $oSettings = new ChWsbAdminSettings($iCategoryId);
            ...
            $this -> _oTemplate->pageCodeAdmin( _t('_ch_spy_module') );
        }
        ...
        function actionCheckUpdates($sMode = 'global', $iLastActivityId = 0, $sType = '', $iProfileId = 0)
        {
            ...
            // set filter;
            if($sType && $sType != 'all') {
                $this -> oSearch -> aCurrent['restriction']['type']['value'] = process_db_input($sType, CH_TAGS_STRIP);
            }
        ...
        function getInitPart($sType = '', $iProfileId = 0, $bActive = true)
        {
            ...
            $iProfileId = (int)$iProfileId;
            if($sType)
                $sType = ch_js_string( strip_tags($sType));
...
JS;
        ...
        function getActivityTypesToggles($sPageUrl = '')
        {
            ...
            $aToggleItems = array (
                'all'                =>  _t( '_ch_spy_all_activity' ),
                'content_activity'   =>  _t( '_ch_spy_content_updates' ),
                'profiles_activity'  =>  _t( '_ch_spy_profiles_updates' ),
            );
            ...
            return ChWsbPageView::getBlockCaptionItemCode(time(), $aTopToggleEllements );
        }
        ...
        {
            $oResponse = new ChSpyResponseContent($this);
            $oResponse -> response($oAlert);
        }

        function serviceResponseProfiles($oAlert)
        {
            $oResponse = new ChSpyResponseProfiles($this);
            $oResponse -> response($oAlert);
        }
        ...
        function serviceUpdateHandlers($sModuleUri = 'all', $bInstall = true)
        {
            ...
            foreach($aModules as $aModule) {
                if(!ChWsbRequest::serviceExists($aModule, 'get_spy_data'))
                    continue;

                $aData = ChWsbService::call($aModule['uri'], 'get_spy_data');
                if($bInstall)
        ...
        function _getSpyBlock($aVars)
        {
            ...
            if(!isset($aVars['dynamic']))
                $aVars['dynamic'] = ch_get('dynamic') !== false;
            if(!isset($aVars['type']))
                $aVars['type'] = ch_get('type') !== false ? ch_get('type') : 'all';
            if(!isset($aVars['page_ajax']))
                $aVars['page_ajax'] = true;
            if(!isset($aVars['page']))
                $aVars['page'] = ch_get('page') !== false ? (int) ch_get('page') : 1;
            $aVars['page']= $aVars['page'] > 0 ? $aVars['page'] : 1;

            //-- set search filter --//
            $this -> oSearch -> aCurrent['restriction']['viewed']['value']  = '';
            if($aVars['type'] != 'all')
                $this -> oSearch -> aCurrent['restriction']['type']['value'] = process_db_input($aVars['type'], CH_TAGS_STRIP);
                ...
                foreach($aActivites as $iKey => $aItems) {
                    ...
                    if($aItems['sender_id']) {
                        $aTemplateKeys = array(
                            ...
                            'date_add'        => getLocaleDate( strtotime($aItems['date']), CH_WSB_LOCALE_DATE),
                        );
