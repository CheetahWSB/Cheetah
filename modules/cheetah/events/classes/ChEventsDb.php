 ...
 * Events module Data
 */
class ChEventsDb extends ChWsbTwigModuleDb
{
    /*
     * Constructor.
     */
    ...
    function getUpcomingEvent ($isFeatured)
    {
        ...
        WHERE `EventEnd` > ? AND `Status` = ? AND `{$this->_sFieldAllowViewTo}` = ? $sWhere ORDER BY `Featured` DESC, `EventStart` ASC LIMIT 1", [time(), 'approved', CH_WSB_PG_ALL]);
    }
