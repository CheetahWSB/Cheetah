...
* echo $oChCRSSModule->GenCustomRssBlock($iID); //member ID
*
*
*
* Profile's Wall:
...
* @see ChAvaModule::serviceGenCustomRssBlock
* ChWsbService::call('custom_rss', 'gen_custom_rss_block', array($iID));
*
*
*
* Alerts:
...
*
*/
class ChCRSSModule extends ChWsbModule
{
    // Variables
    var $_iProfileID;
    ...
    */
    ...
    function GenCustomRssBlock($iID)
    {
        ...
        $bAjaxMode = (false !== ch_get('mode') && ch_get('mode') == 'ajax');
        $bAjaxMode2 = (false !== ch_get('mode') && ch_get('mode') == 'ajax2');
        ...
        if ($iVisitorID>0 && $iVisitorID == $this->_iProfileID) {
            ...
            if (0 === strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') && isset($sAction) && $sAction != '') {
                $sNewUrl = process_db_input(ch_get('rss_url'), CH_TAGS_STRIP);
                $sNewDesc = process_db_input(ch_get('rss_desc'), CH_TAGS_STRIP);
                $iOldID = (int)ch_get('rss_id');
                $iQuantity = (int)ch_get('rss_quantity');
            ...
            if(count($aMemberRSS)==0) {
                ...
                foreach($aMemberRSS as $sKey => $aRSSInfo) {
                    ...
                    $aFormVariables = array (
                        ...
                        'rss_url_js' => $sRssUrl,
                        'php_self' => ch_html_attribute($_SERVER['PHP_SELF']),
                        'rss_id' => $iRssID,
...
EOF;
        ...
        $sActions = ChWsbPageView::getBlockCaptionMenu(time(), array(
            'crss_t1' => array('href' => ch_html_attribute($_SERVER['PHP_SELF']), 'title' => $sEditC, 'onclick' => "ShowHideEditCRSSForm('{$sPreferSpeed}'); return false;")
        ));
