...
INSERT INTO `sys_menu_admin` (`id`, `parent_id`, `name`, `title`, `url`, `description`, `icon`, `icon_large`, `check`, `order`) VALUES (NULL, 2, 'Custom RSS', '_crss_ami_custom_rss', '{siteUrl}modules/cheetah/custom_rss/post_mod_crss.php', 'Custom RSS Moderation', 'rss', '', '', @iExtOrd+1);
...
(NULL, 'profile', '1140px', 'Custom RSS block', '_crss_Custom_Feeds', 3, IFNULL(@iMaxOrder, 0), 'PHP', 'return ChWsbService::call(''custom_rss'', ''gen_custom_rss_block'', array($this->oProfileGen->_iProfileID));', 0, 71.9, 'non,memb', 0);

-- site stats
SET @iStatSiteOrder := (SELECT `StatOrder` + 1 FROM `sys_stat_site` WHERE 1 ORDER BY `StatOrder` DESC LIMIT 1);
INSERT INTO `sys_stat_site` VALUES(NULL, 'crss', 'crss_ss', '', 'SELECT COUNT(`ID`) FROM `[db_prefix]_main` WHERE `Status`=''active''', 'modules/cheetah/custom_rss/post_mod_crss.php', 'SELECT COUNT(`ID`) FROM `[db_prefix]_main` WHERE `Status`=''passive''', 'rss', @iStatSiteOrder);

-- export
SET @iMaxOrderExports = (SELECT MAX(`order`)+1 FROM `sys_objects_exports`);
INSERT INTO `sys_objects_exports` (`object`, `title`, `class_name`, `class_file`, `order`, `active`) VALUES
('ch_crss', '_sys_module_custom_rss', 'ChCRSSExport', 'modules/cheetah/custom_rss/classes/ChCRSSExport.php', @iMaxOrderExports, 1);
