 ...
 */
    ...
    require_once('ChPollCalendar.php');
    require_once('ChPollPrivacy.php');
    require_once('ChPollSearch.php');

    define( 'CH_POLL_VOTE_UNIQ', 60*60*24*30); // within 1 month vote poll is uniq
    define( 'CH_POLL_ANS_DIVIDER', '<delim>');
    define( 'CH_POLL_RES_DIVIDER', ';');

    define( 'POLL_ERROR_OCCURED', _t( '_Error occured') );
    define( 'POLL_EMPTY_FIELDS',  _t( '_please_fill_next_fields_first') );
    define( 'POLL_CREATED',       _t( '_ch_poll_created') );
    define( 'POLL_EDITED',        _t( '_ch_poll_was_edited') );
    define( 'POLL_MAX_REACHED',   _t( '_ch_poll_max_reached') );
    define( 'POLL_NOT_ALLOW',     _t( '_ch_poll_not_available') );

    class ChCheckerPoll extends ChWsbFormCheckerHelper
    {
     ...
     *
     * Generate polls list.
     * @see ChPollModule::serviceGetPolls
     * ChWsbService::call('poll', 'get_polls', array($sAction, $iProfileId));
     *
     * Generate poll's link into member menu in 'profile' section (this link will show the number of profile's polls).
     * @see ChPollModule::serviceGetMemberMenuLink
     * ChWsbService::call('poll', 'get_member_menu_link', array($iMemberId));
     *
     * Will draw edit poll's button ;
     * @see ChPollModule::serviceEditActionButton
     * ChWsbService::call('poll', 'edit_action_button', array($iMemberId, $iPollId));
     *
     * Will draw delete poll's button ;
     * @see ChPollModule::serviceDeleteActionButton
     * ChWsbService::call('poll', 'delete_action_button', array($iMemberId, $iPollId));
     *
     * will return all needed systems alerts for 'spy' module;
     * @see ChPollModule::serviceGetSpyData
     * ChWsbService::call('poll', 'get_spy_data', array());
     *
     * will return sys alerts' answer on some action for 'spy' module;
     * @see ChPollModule::serviceGetSpyPost
     * ChWsbService::call('poll', 'get_spy_post', array($sAction, $iObjectId, $iSenderId));
     *
     *
     *
     * Alerts:
     * Alerts type/unit - 'ch_poll'
     ...
     *      $iPollId - poll's Id
     */
    class ChPollModule extends ChWsbModule
    {
        var $sHomeUrl;
        ...
        function __construct($aModule, $aPollSettings = array() )
        {
            ...
            $this -> sPathToModule = CH_WSB_URL_ROOT . $this -> _oConfig -> getBaseUri();
                if ($this -> aPollSettings['action']) {
                $this -> sPathToModule .= '&action=' . rawurlencode($this -> aPollSettings['action']);
            }

            $this -> oPrivacy = new ChPollPrivacy($this);
            $this -> oSearch  = new ChPollSearch($this);
        }
        ...
        function getOwnerBlock($iPollOwner, $aPollInfo)
        {
            ...
            $aTemplateKeys = array(
                ...
                'date'            => getLocaleDate($aPollInfo['poll_date'], CH_WSB_LOCALE_DATE_SHORT),
                'date_ago'        => defineTimeInterval($aPollInfo['poll_date'], false),
                'tags'            => getLinkSet($aPollInfo['poll_tags'], $this -> getModulePath() . '&action=tag&tag=', CH_WSB_TAGS_DIVIDER),
                'fields'          => null,
        ...
        function genMemberAdministration()
        {
            ...
            switch($this -> aPollSettings['mode']) {
                ...
                case 'edit_poll' :
                    $sPageCaption = _t('_ch_poll_edit_poll');
                ...
                case 'pending' :
                    ...
                    $sPageCaption = _t($this->aPollSettings['mode'] == 'manage' ? '_ch_poll_manage_poll' : '_ch_poll_pending_poll');
                    $_page['header']        = $sPageCaption ;
                    $_page['header_text']   = $sPageCaption ;
                ...
                case 'main' :
                default :
                    $sOutputCode = '<center class="ch-def-font-large">' . _t('_ch_poll_have_not_approval',
            ...
            return DesignBoxContent(_t('_ch_poll_administration'), $sOutputCode, 1, $this->genToggleElements());
        }
        ...
        function genToggleElements()
        {
            ...
            $aToggleItems = array (
                'main' => _t('_ch_poll_main'),
                'add' => _t('_ch_poll_add'),
                'manage' => _t('_ch_poll_manage'),
                'pending' => _t('_ch_poll_pending'),
            );
            ...
            if($this->aPollSettings['mode'] == 'edit_poll')
                $aToggleItems['edit_poll'] = _t('_ch_poll_edit');
            ...
            return ChWsbPageView::getBlockCaptionItemCode(0, $aItems);
        }
        ...
        function getManagePollsPage($bApproval = true)
        {
            ...
            $aAdminSection = $this -> showAdminActionsPanel('manage_form', array( _t('_ch_poll_delete') ), 'poll_id');
        ...
        function showAdminActionsPanel($sWrapperId, $aButtons, $sCheckboxName = 'entry', $bSelectAll = true, $bSelectAllChecked = false)
        {
            ...
            $aUnit['ch_if:actionButtons'] = array(
                'condition' => !empty($aBtns),
                'content' => array(
                    'ch_repeat:buttons' => $aBtns
                )
            );
            $aUnit['ch_if:selectAll'] = array(
        ...
        function getPollBlock(&$aPollInfo, $bAjaxQuery = false, $bViewMode = false, $bAdminMode = false)
        {
            ...
            // language keys ;
            $aLanguageKeys  = array (
                'results'   => _t('_ch_poll_result'),
                'delete'    => _t('_ch_poll_delete'),
                'edit'      => _t('_ch_poll_edit'),
                'active'    => _t('_ch_poll_active'),
                'sure'      => _t('_Are_you_sure'),
                'by'        => _t('_ch_poll_by' ),
                'poll'      => _t('_ch_poll'),

                'featured'        => _t('_ch_poll_featured'),
                'unfeatured'      => _t('_ch_poll_unfeatured'),
                'poll_approved'   => _t('_ch_poll_approved'),
                'poll_unapproved' => _t('_ch_poll_unapproved'),
            );
            ...
            $aTemplateKeys = array (
               ...
               'ch_if:manage_page' => array (
                ...
                'poll_url'       => CH_WSB_URL_ROOT . $this -> _oConfig -> getBaseUri() . '&action=show_poll_info&id=' . $aPollInfo['id_poll'],
        ...
        function getInitPollPage($bDynamic = true)
        {
            ...
            (
                'delete'        => _t('_ch_poll_delete'),
                'loading'       => _t('_ch_poll_loading') . '...',
                'poll_deleted'  => _t('_ch_poll_was_deleted'),
                'make_it'       => _t('_ch_poll_make_it'),
                'you_should'    => _t('_ch_poll_specify_least'),
            );
            ...
            if (isset($_POST['question'])
                ...
                if( $_POST['answers'] and is_array($_POST['answers']) ) {
                    ...
                    foreach( $_POST['answers'] as $iKey => $sValue ) {
                        if ($sValue) {
                            ...
                            $sPollAnsers  .= $sValue . CH_POLL_ANS_DIVIDER;
                            $sPollResults .= '0;';
                        }
                    }
                } else {
                ...
                if( isset($_POST['Categories']) && is_array($_POST['Categories']) ) {
                    foreach($_POST['Categories'] as $iKey => $sValue) {
                        if($sValue) {
                            $sCategory .= strip_tags( trim($sValue, CH_TAGS_STRIP) ) . CATEGORIES_DIVIDER;
                        }
                    }
                }
        ...
        function getCreationForm()
        {
            ...
            if(!$this -> isPollCreateAlowed($this -> aPollSettings['member_id'], false) ) {
                return MsgBox( _t('_ch_poll_access_denied') );
            }
            ...
            $aLanguageKeys = array
            (
                'create'     => _t('_ch_poll_create'),
                'tags'       => _t('_ch_poll_tags'),
                'tags_sep'   => _t('_sys_tags_note'),
                'generate'   => _t('_ch_poll_generate'),
                'question'   => _t('_ch_poll_question'),
                'answer'     => _t('_ch_poll_answer'),
                'add_answer' => _t('_ch_poll_add'),
                'max_pool'   => _t('_ch_poll_max_reached'),

                'question_length_req' => _t('_ch_poll_question_length_required',
                            $this -> aPollSettings['question_min_length'], $this -> aPollSettings['question_max_length']),

                'answer_length_req' => _t('_ch_poll_answer_length_required',
            ...
            $aForm = array (
                ...
                'inputs' => array(
                    ...
                    'allow_view_to' => $this -> oPrivacy -> getGroupChooser($this -> aPollSettings['member_id']
                                            , $this -> aModuleInfo['uri'], 'view', array(), _t('_ch_poll_allow_view') ),

                    'allow_comments_to' => $this -> oPrivacy -> getGroupChooser($this -> aPollSettings['member_id']
                                            , $this -> aModuleInfo['uri'], 'comment', array(), _t('_ch_poll_allow_comment') ),

                    'allow_vote_to' => $this -> oPrivacy -> getGroupChooser($this -> aPollSettings['member_id']
                                            , $this -> aModuleInfo['uri'], 'vote', array(), _t('_ch_poll_allow_vote') ),
                ),
            );
            ...
            $aForm['inputs']['category'] = $oCategories -> getGroupChooser ('ch_poll', $this -> aPollSettings['member_id'], true);
        ...
        function getCustomActionButton()
        {
            ...
            if( $iProfileId && isMember() ) {
                ...
                $GLOBALS['oTopMenu'] -> setCustomSubActions($aOpt, 'ch_poll_title');
            }
        }
               ...
               $aPollInfo = array_shift($aPollInfo);
                ...
                if ($this -> aPollSettings['admin_mode']
                    ...
                    (
                        'question'            => _t('_ch_poll_question'),
                        'answer'              => _t('_ch_poll_answer'),
                        'save'                => _t('_ch_poll_save'),
                        'close'               => _t('_ch_poll_close'),
                        'active'              => _t('_ch_poll_active'),
                        'approve'             => _t('_ch_poll_approve'),
                        'tags'                => _t('_ch_poll_tags'),
                        'tags_sep'            => _t('_ch_poll_tags_separeted'),
                    ...
                    $aAnswers = explode(CH_POLL_ANS_DIVIDER, $aPollInfo['poll_answers']);
                    $iIndex = 0;
                    ...
                    $aForm['inputs']['category'] = $oCategories -> getGroupChooser ('ch_poll', $this -> aPollSettings['member_id'], true, $aPollInfo['poll_categories']);
                    ...
                    $aForm['inputs'] = array_merge($aForm['inputs'], array(
                        'allow_view_to' => $this -> oPrivacy -> getGroupChooser($this -> aPollSettings['member_id'], $this -> aModuleInfo['uri'], 'view', array(), _t('_ch_poll_allow_view') ),
                        'allow_comments_to' => $this -> oPrivacy -> getGroupChooser($this -> aPollSettings['member_id'], $this -> aModuleInfo['uri'], 'comment', array(), _t('_ch_poll_allow_comment') ),
                        'allow_vote_to' => $this -> oPrivacy -> getGroupChooser($this -> aPollSettings['member_id'], $this -> aModuleInfo['uri'], 'vote', array(), _t('_ch_poll_allow_vote') ),
                    ));
                    ...
                    if ( $oForm -> isSubmittedAndValid() ) {
                            $this -> proccesData();
                            $sOutputCode = MsgBox( _t('_ch_poll_was_edited') );
                    } else {
                        $sOutputCode = $oForm -> getCode();
                    }
                }
        ...
        function actionResultsList($iPollID)
        {
            ...
            $aAnswersResult = explode(CH_POLL_RES_DIVIDER, $aPoll[0]['poll_results'] );
            $aAnswersNames  = explode(CH_POLL_ANS_DIVIDER, $aPoll[0]['poll_answers'] );
            ...
            $aTemplateKeys = array
            (
                'ch_repeat:points' => $aPoints,
                'ch_repeat:names'  => $aNames,
            );
        ...
        function actionGetQuestions($iPollID)
        {
            ...
            if ($aPoll) {
                $aQuestions = explode(CH_POLL_ANS_DIVIDER, $aPoll[0]['poll_answers']);
            ...
            (
                ...
                'ch_repeat:answer_list' => $aQuestionsList,
            );
        ...
        function actionSetAnswer($iPollID, $iVoteNumber)
        {
            if ( (int) $iVoteNumber >= 0 ) {
                ...
                $aVotes = explode(CH_POLL_RES_DIVIDER, $aPoll[0]['poll_results'] );
                $aVotes[$iVoteNumber]++;
                ...
                if ( !isset($_COOKIE['profile_polls_question_' . $iPollID]) ) {
                    if ( $this -> _oDb -> setVotes( (int) $iPollID, $sVotes, $iPoll_total_votes) ) {
                        ...
                        setcookie( 'profile_polls_question_' . $iPollID, 1 , time() + CH_POLL_VOTE_UNIQ, $sPath );

                          // create system event
                        $oZ = new ChWsbAlerts('ch_poll', 'answered',  $aPoll[0]['id_profile'], $this -> aPollSettings['member_id'], array('poll_id' => $iPollID, 'vote' => $iVoteNumber));
                        $oZ->alert();
                    }
                }
            }
        ...
        function actionSharePopup ($iEntryId = 0)
        {
            ...
            $sEntryUrl = CH_WSB_URL_ROOT . $this -> _oConfig -> getBaseUri()
        ...
        function actionViewCalendar($iYear = 0, $iMonth = 0, $iDay = 0)
        {
            ...
            $sCaption = _t('_ch_poll_browse_by_day')
                . ': ' . getLocaleDate( strtotime("{$iYear}-{$iMonth}-{$iDay}"), CH_WSB_LOCALE_DATE_SHORT);
        ...
        function actionCalendar($iYear = null, $iMonth = null )
        {
            ...
            $sPageCaption = _t('_ch_poll_calendar');

            $_page['header']        = $sPageCaption ;
            $_page['header_text']   = $sPageCaption ;

            $this -> getCustomActionButton();
            $oCalendar = new ChPollCalendar($iYear, $iMonth, $this->_oDb, $this->_oTemplate, $this->_oConfig);
        ...
        function actionCategories()
        {
            ...
            $oCateg = new ChTemplCategoriesModule($aParam, '', CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories');
            $sCode = $oCateg->getCode();
            $this -> _oTemplate -> defaultPage( _t('_ch_poll_categories'), $sCode);
       }
        ...
        function actionTag($sTag = '')
        {
            ...
            echo $this -> _oTemplate -> defaultPage( _t('_ch_poll_view_tag')
        ...
        function actionTags()
        {
            ...
            $oTags = new ChTemplTagsModule($aParam, '', CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'tags');
            $sCode = $oTags->getCode();
            $this -> _oTemplate -> defaultPage( _t('_ch_poll_tags'), $sCode);
        }
        ...
        function getAdminForm()
        {
            ...
            $oNewSearchresult = new ChTemplSearchResult();
            ...
            if ( isset($_POST['poll_id']) and is_array($_POST['poll_id']) ) {
                foreach($_POST['poll_id'] as $iKey => $iValue ) {
                    ...
                    $oTag = new ChWsbTags();
                    $oTag -> reparseObjTags('ch_poll', $iValue);

                    $oCateg = new ChWsbCategories();
                    $oCateg->reparseObjTags('ch_poll', $iValue);
                }
            }

            $aLanguageKeys = array
            (
                'premoderation'   => _t('_ch_poll_moderation'),
                'select_all'      => _t('_ch_poll_select_all'),
                'approve'         => _t('_ch_poll_approve'),
                'disapprove'      => _t('_ch_poll_disapprove'),
        ...
        function actionAdministration()
        {
            ...
            if( !isAdmin() ) {
                header('location: ' . CH_WSB_URL_ROOT);
            }

            $aLanguageKeys = array(
                'premoderation' => _t('_ch_poll_tags_premoderation'),
            );

            $aMenu = array(
                'ch_poll_main'      => array('title' => _t('_ch_poll_main'), 'href' => $this -> sPathToModule . 'administration&action=main'),
                'ch_poll_settings'  => array('title' => _t('_ch_poll_settings'), 'href' => $this -> sPathToModule . 'administration&action=settings'),
            );
            ...
            switch ($sAction) {
                case 'main':
                    $aMenu['ch_poll_main']['active'] = 1;
                    $sContent = $this -> getAdminForm();
                    break;
                case 'settings':
                    $aMenu['ch_poll_settings']['active'] = 1;
            ...
            $this -> _oTemplate->pageCodeAdmin( _t('_ch_poll_module') );
        }
        ...
        function searchTags($sTag, $sExtraParam = '', $bUseInitPart = true)
        {
            ...
            $sOutputCode .= $this
                -> showSearchResult( _t('_ch_poll_browse_tag') . ': ' . htmlspecialchars_adv($sTag), $sExtraParam );
        ...
        function searchCategories($sCategory, $sExtraParam = null, $bUseInitPart = true)
        {
            ...
            $sOutputCode .= $this -> showSearchResult( _t('_ch_poll_browse_category')
        ...
        function searchFeatured($bUseInitPart = true, $iLimit = 0)
        {
            ...
            $sOutputCode .= $this -> showSearchResult( _t('_ch_poll_featured') );
        ...
        function searchFeaturedHome($bUseInitPart = true)
        {
            ...
            $sOutputCode .= $this -> showSearchResult( _t('_ch_poll_featured'), null, 0, false, 1);
        ...
        function searchPopular($bUseInitPart = true)
        {
            ...
            $sOutputCode .= $this -> showSearchResult( _t('_ch_poll_popular') );
        ...
        function searchMy($bUseInitPart = true)
        {
            ...
            if($this->aPollSettings['mode'] != 'manage' && $this->aPollSettings['mode'] != 'pending')
                $sOutputCode .= $this -> showSearchResult( _t('_ch_poll_my') );
        ...
        function searchAll($iLimit = 0, $iPerLine = 0, $bUseInitPart = true, $sCaption = null, $bShowEmptyMsg = true)
        {
            ...
            $sPageCaption = $sCaption ? _t($sCaption) : _t('_ch_poll_latest');
        ...
        function searchProfilePolls($iProfileId, $iLimit = 6, $bUseInitPart = true, $bShowEmptyMsg = true)
        {
            ...
            $sOutputCode .= $this -> showSearchResult( _t('_ch_polls_profile'), null, $iLimit, false, 1, true, $bShowEmptyMsg );
        ...
        function searchAllProfilePolls($iProfileId, $iLimit = 0, $iPerLine = 0, $bUseInitPart = true)
        {
            ...
            $this -> oSearch -> aCurrent['sorting'] = 'popular';
            $sOutputCode .= $this -> showSearchResult( _t('_ch_poll_popular'), $sExtraParam );
            ...
            if ( !$sPollQuestion or !$sPollAnswers ) {
                ...
                switch($iResponse) {
                    ...
                    case '1' :
                        ...
                        // create system event
                        $oZ = new ChWsbAlerts('ch_poll', 'add', $iLastPoll);
                        $oZ->alert();

                        $oTag = new ChWsbTags();
                        $oTag -> reparseObjTags('ch_poll', $iLastPoll);

                        $oCateg = new ChWsbCategories();
                        $oCateg->reparseObjTags('ch_poll', $iLastPoll);
                        break;
        ...
        function editPoll($iPollId, $sPollQuestion, $sPollAnswers, $sCategory, $bActive = true, $bApprove = false, $sTags = null, $iCommentGroupValue = 3, $iVoteGroupValue = 3, $iViewGroupValue = 3)
        {
            if ( !$sPollQuestion or !$sPollAnswers ) {
                ...
                if ( $this -> aPollSettings['admin_mode']
                    ...
                    // create system event
                    $oZ = new ChWsbAlerts('ch_poll', 'edit',  $iPollId);
                    $oZ->alert();

                    // reparse poll's tags;
                    $oTag = new ChWsbTags();
                    $oTag -> reparseObjTags('ch_poll', $iPollId);

                    $oCateg = new ChWsbCategories();
                    $oCateg->reparseObjTags('ch_poll', $iPollId);
                } else {
                    $this -> sActionAnswer  = MsgBox(POLL_NOT_ALLOW);
                }
            }
        ...
        function deletePoll($iId)
        {
            ...
            $oTag = new ChWsbTags();
            $oTag -> reparseObjTags('ch_poll', $iId);

            $oCateg = new ChWsbCategories();
            $oCateg->reparseObjTags('ch_poll', $iId);

            //delete all subscriptions
			$oSubscription = ChWsbSubscription::getInstance();
			$oSubscription->unsubscribe(array('type' => 'object_id', 'unit' => 'ch_poll', 'object_id' => $iId));

            // create system event
            $oZ = new ChWsbAlerts('ch_poll', 'delete_poll',  $iId);
            $oZ->alert();
        ...
        function serviceGetPolls($sAction, $iProfileId = 0)
        {
            ...
            switch($sAction) {
                ...
                case 'get_polls' :
                    ...
                    $sOutputCode = $this -> searchAll($iRowsLimit, 1, true, '_ch_polls_public', false);
                break;
            }
        ...
        function serviceGetMemberMenuLink($iMemberId = false)
        {
            ...
            $aLinkInfo = array(
                'item_img_src'  => 'tasks',
                'item_img_alt'  => _t( '_ch_polls' ),
                'item_link'     => $this -> sPathToModule . '&action=my',
                'item_title'    => _t( '_ch_polls' ),
        ...
        function serviceGetSubscriptionParams ($sAction, $iEntryId)
        {
            ...
            $aActionList = array(
                'commentPost' => '_ch_poll_sbs_comment'
            );
            ...
            return array (
                ...
                'template' => array (
                    ...
                    'ViewLink' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . '&action=show_poll_info&id=' . $aDataEntry['id_poll'],
                ),
            );
        }
        ...
        function serviceGetWallPost($aEvent)
        {
            ...
            $sJs = $sCss = $sInit = '';
            $sBaseUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/';
            if($aEvent['js_mode']) {
            ...
            //--- Single public event
            $aItem['url'] = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . '&action=show_poll_info&id=' . $aItem['id_poll'];

            $sTextWallObject = _t('_ch_poll_wall_object');
            return array(
            	'owner_id' => $iOwner,
                'title' => _t('_ch_poll_wall_added_new_title', $sOwner, $sTextWallObject),
                ...
                'content' => $sJs . $sCss . $sInit . $this->_oTemplate->parseHtmlByName('wall_post.html', array(
	                ...
	                'cpt_added_new' => _t('_ch_poll_wall_added_new'),
        ...
        function serviceGetWallPostOutline($aEvent)
        {
            ...
            //--- Single public event
            $aItem['poll_url'] = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . '&action=show_poll_info&id=' . $aItem['id_poll'];

            return array(
                'content' => $sCss . $this->_oTemplate->parseHtmlByName('modules/cheetah/wall/|outline_item_text.html', array(
	                'mod_prefix' => $sPrefix,
	                'mod_icon' => 'tasks',
	                'user_name' => $sOwner,
    	...
    	function serviceGetWallAddComment($aEvent)
        {
            ...
            $oCmts = new ChTemplCmtsView('ch_poll', $iItem);
            if(!$oCmts->isEnabled())
                return '';
            ...
            $sTextWallObject = _t('_ch_poll_wall_object');
            return array(
                'title' => _t('_ch_poll_wall_added_new_title_comment', $sOwner, $sTextWallObject),
                'description' => $aComment['cmt_text'],
                'content' => $sCss . $this->_oTemplate->parseHtmlByName('wall_post_comment.html', array(
	                'cpt_user_name' => $sOwner,
	                'cpt_added_new' => _t('_ch_poll_wall_added_new_comment'),
	                'cpt_object' => $sTextWallObject,
	                'cpt_item_url' => $aItem['url'],
        ...
        function serviceGetWallPostComment($aEvent)
        {
            ...
            ch_import('ChTemplCmtsView');
            $oCmts = new ChTemplCmtsView('ch_poll', $iId);
            if(!$oCmts->isEnabled())
                return '';
        ...
        {
            $aItemAns  = explode(CH_POLL_ANS_DIVIDER, trim($aItem['poll_answers'], CH_POLL_ANS_DIVIDER));
            $iItemAns = count($aItemAns);
            $aItemRes = explode(CH_POLL_RES_DIVIDER, trim($aItem['poll_results'], CH_POLL_RES_DIVIDER));
            ...
            return $bAsArray ? $aResult : $this->_oTemplate->parseHtmlByName('poll_answers.html', array('ch_repeat:items' => $aResult));
        }
        ...
        function serviceGetSpyData ()
        {
            return array(
                'handlers' => array(
                    array('alert_unit' => 'ch_poll', 'alert_action' => 'add', 'module_uri' => 'poll', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                    array('alert_unit' => 'ch_poll', 'alert_action' => 'answered', 'module_uri' => 'poll', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                    array('alert_unit' => 'ch_poll', 'alert_action' => 'rate', 'module_uri' => 'poll', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                    array('alert_unit' => 'ch_poll', 'alert_action' => 'commentPost', 'module_uri' => 'poll', 'module_class' => 'Module', 'module_method' => 'get_spy_post'),
                ),
                'alerts' => array(
                    array('unit' => 'ch_poll', 'action' => 'add'),
                    array('unit' => 'ch_poll', 'action' => 'answered'),
                    array('unit' => 'ch_poll', 'action' => 'rate'),
                    array('unit' => 'ch_poll', 'action' => 'delete_poll'),
                    array('unit' => 'ch_poll', 'action' => 'commentPost'),
                    array('unit' => 'ch_poll', 'action' => 'commentRemoved')
                )
            );
        }
        ...
        function serviceGetSpyPost($sAction, $iObjectId = 0, $iSenderId = 0, $aExtraParams = array())
        {
            ...
            switch($sAction) {
                ...
                case 'answered' :
                    $aRet = $this -> _getSpyArray($aExtraParams['poll_id'], $iSenderId, '_ch_poll_answered', 'content_activity', $iObjectId);
                    break;

                case 'rate' :
                    $aRet = $this -> _getSpyArray($iObjectId, $iSenderId, '_ch_poll_rated');
                    break;

                case 'commentPost' :
                    $aRet = $this -> _getSpyArray($iObjectId, $iSenderId, '_ch_poll_commented');
                    break;
            }
            return $aRet;
        }
        ...
        function isPollCreateAlowed($iMemberId = false, $isPerformAction = false)
        {
            if (false === $iMemberId)
                $iMemberId = getLoggedId();
            $this -> _defineActions();
            $aCheck = checkAction($iMemberId, CH_CREATE_POLLS, $isPerformAction);
