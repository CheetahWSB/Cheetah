...
class ChPollDb extends ChWsbModuleDb
{
    var $_oConfig;
    var $_sTable;
    var $sTablePrefix;
    ...
    function getFeaturedCount($iStatus = 1, $bOnlyPublic = false)
    {
        ...
        $sExtraSql = $bOnlyPublic ? ' AND `allow_view_to` = ' . CH_WSB_PG_ALL : null;
...
//        $bOnlyPublic = (bool) $bOnlyPublic;
    ...
    {
        $iPollId = (int) $iPollId;
        $sVotes = process_db_input($sVotes, CH_TAGS_STRIP);
     ...
     * @param : $aPoolInfo (array) - contain some poll's information;
                    ...
                    [ answers ]  - (string)  poll's answers (string separated by tag CH_POLL_ANS_DIVIDER);
    ...
    function createPoll($aPoolInfo, $isAdmin = false)
    {
        ...
        foreach($aPoolInfo as $sKey => $sValue) {
            $aPoolInfo[$sKey] = process_db_input($sValue, CH_TAGS_NO_ACTION
                , CH_SLASHES_AUTO);
        }
    ...
    function editPoll($aPoolInfo)
    {
        ...
        foreach($aPoolInfo as $sKey => $sValue) {
            if($sKey != 'answers' ) {
                $aPoolInfo[$sKey] = process_db_input($sValue, CH_TAGS_STRIP);
            } else {
    ...
    {
        $sCatName = process_db_input($sCatName, CH_TAGS_STRIP);
