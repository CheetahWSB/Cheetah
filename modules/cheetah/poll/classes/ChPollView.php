...
require_once('ChPollCmts.php');

class ChPollView extends ChWsbPageView
{
    ...
    function __construct($sPageName, &$aModule, &$oPollModule, $iPollId)
    {
        ...
        if($this -> iPollId) {
            ...
            if(!$this -> oModule -> oPrivacy -> check('view', $this -> aPollInfo[0]['id_poll'], $this -> iMemberId) ) {
                echo $this -> oModule -> _oTemplate -> defaultPage( _t('_ch_poll'), MsgBox(_t('_ch_poll_access_denied')), 2 );
                exit;
            }
        }
        ...
        $this -> oVotingView = new ChTemplVotingView('ch_poll', $this -> iPollId);
        $this -> oCmtsView   = new ChPollCmts('ch_poll', $this -> iPollId);

        $this -> aModule = $aModule;

        if($sPageName == 'show_poll_info')
        	$GLOBALS['oTopMenu']->setCustomSubHeaderUrl(CH_WSB_URL_ROOT . $this -> oModule -> _oConfig -> getBaseUri() . '&action=show_poll_info&id=' . $this -> iPollId);

        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_ch_poll') => CH_WSB_URL_ROOT . $this -> oModule -> _oConfig -> getBaseUri() . '&action=poll_home',
    ...
    function getBlockCode_ActionsBlock()
    {
        ...
        $aUnitInfo['base_url']     =  CH_WSB_URL_ROOT . $aUnitInfo['BaseUri'];
        ...
        if(isLogged() && ($this -> aPollInfo['id_profile'] == $this->iMemberId || isAdmin())) {
            ...
            $aUnitInfo['del_poll_title'] = _t('_ch_poll_delete');
            $aUnitInfo['del_poll_url'] = $sDeleteLink;
            $aUnitInfo['del_poll_script'] = "if(confirm('" . ch_js_string(_t('_Are_you_sure')) . "')) window.open ('" . $sDeleteLink . "','_self'); return false;";
        }
    	...
    	if(ChWsbRequest::serviceExists('wall', 'get_repost_js_click')) {
			...
			$aUnitInfo['repostScript'] = ChWsbService::call('wall', 'get_repost_js_click', array($this->iMemberId, $sMainPrefix, 'add', $this->aPollInfo['id_poll']));
        }
    ...
    function getBlockCode_CommentsBlock()
    {
        ...
        if(  $this -> oModule -> oPrivacy -> check('comment', $this -> aPollInfo['id_poll'], $oPoll -> aPollSettings['member_id']) ) {
            ...
            $sOutputCode = MsgBox( _t( '_ch_poll_privacy_comment_error' ) );
    ...
    function getBlockCode_VotingsBlock()
    {
        ...
        if(  $this -> oModule -> oPrivacy -> check('vote', $this -> aPollInfo['id_poll'], $oPoll -> aPollSettings['member_id']) ) {
            ...
            $sOutputCode = MsgBox( _t( '_ch_poll_privacy_vote_error' ) );
    ...
    function getBlockCode_SocialSharing()
    {
        ...
        $sUrl = CH_WSB_URL_ROOT . $this -> oModule -> _oConfig -> getBaseUri() . '&action=show_poll_info&id=' . $this -> aPollInfo['id_poll'];
