 ...
 */
    ...
    require_once( CH_DIRECTORY_PATH_MODULES . $aModule['path'] . 'classes/' . $aModule['class_prefix'] . 'View.php');
    ...
    $oPoll -> _oTemplate -> setPageDescription( _t('_ch_poll_PH') );
    $oPoll -> _oTemplate -> addPageKeywords( _t('_ch_poll_keyword') );
    ...
    if(!$aPollSettings['action']) {
        ...
        switch($aPollSettings['action']) {
            ...
            case 'category' :
                $sPageCaption = _t('_ch_poll_view_category');
            ...
            case 'featured' :
                $sPageCaption = _t('_ch_poll_featured_polls');
            ...
            case 'popular' :
                $sPageCaption = _t('_ch_poll_popular_polls');
            ...
            case 'poll_home' :
                ...
                if ($aPollSettings['action'] == 'show_poll_info') {
                    ...
                    if ($isAllowView)
                    {
                        $oViewPoll = ch_instance($aModule['class_prefix'] . 'View', array($aPollSettings['action'], $aModule, $oPoll, $iPollId), $aModule);
                        $sPageTitle = $aPollInfo['poll_question'];

                        $sPageCaption = _t('_ch_poll_view', $aPollInfo['poll_question']);
                        $_page['header']        = $sPageCaption ;
                        $_page['header_text']   = $sPageCaption ;

                        $oPoll -> _oTemplate -> addJsTranslation(array('_Are_you_sure'));
                        $oPoll -> _oTemplate -> setPageDescription($aPollInfo['poll_question']);
                        $oPoll -> _oTemplate -> addPageKeywords($aPollInfo['poll_answers'], CH_POLL_ANS_DIVIDER);
