...
function ChVideoUpload(oOptions) {
...
ChVideoUpload.prototype.genSendFileInfoForm = function(iMID, sForm) {
    if (iMID > 0 && sForm != '') {
...
ChVideoUpload.prototype.getType = function() {
	return $('#video_upload_form').attr("name");
}

ChVideoUpload.prototype.changeContinueButtonStatus = function () {
	switch(this.getType()) {
		case 'upload':
...
ChVideoUpload.prototype.doValidateFileInfo = function(oButtonDom, iFileID) {
	var bRes = true;
	if ($('#send_file_info_' + iFileID + ' input[name=title]').val()=='') {
		$('#send_file_info_' + iFileID + ' input[name=title]').parent().parent().children('.warn').show().attr('float_info', _t('_ch_videos_val_title_err'));
		bRes = false;
	}
	else
...
ChVideoUpload.prototype.cancelSendFileInfo = function(iMID, sWorkingFile) {
	if(iMID == "")
...
ChVideoUpload.prototype.cancelSendFileInfoResult = function(iMID) {
...
ChVideoUpload.prototype.onSuccessSendingFileInfo = function(iMID) {
	$('#send_file_info_'+iMID).remove();
...
ChVideoUpload.prototype.showErrorMsg = function(sErrorCode) {
...
ChVideoUpload.prototype.onFileChangedEvent = function (oElement) {
	this.changeContinueButtonStatus();
};

ChVideoUpload.prototype._loading = function (bShow) {
	$('.upload-loading-container').ch_loading(bShow);
}

ChVideoUpload.prototype.resetUpload = function () {
...
ChVideoUpload.prototype.resetEmbed = function () {
	var tText = $('#video_upload_form [name="embed"]');
	tText.attr("value", "");
	shVideoEnableSubmit(false);
}

ChVideoUpload.prototype.checkEmbed = function (bAlert) {
	var tText = $('#video_upload_form [name="embed"]');
