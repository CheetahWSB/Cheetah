 ...
 */
class ChVideosSiteMapsVideos extends ChWsbSiteMaps
{
    protected $_oModule;
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array (
            ...
            'join' => " INNER JOIN `sys_albums_objects` AS `o` ON (`o`.`id_object` = `e`.`ID`)
                        INNER JOIN `sys_albums` AS `a` ON (`a`.`Type` = 'ch_videos' AND `a`.`Status` = 'active' AND `a`.`AllowAlbumView` = '" . CH_WSB_PG_ALL . "' AND `a`.`ID` = `o`.`id_album`)", // join SQL part
