...
/**
 * Sitemaps generator for Videos Albums
 */
class ChVideosSiteMapsAlbums extends ChWsbSiteMaps
{
    protected $_oModule;
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array (
            ...
            'join' => "", // join SQL part
            'where' => "AND `Type` = 'ch_videos' AND `Status` = 'active' AND `ObjCount` > 0 AND `AllowAlbumView` = '" . CH_WSB_PG_ALL . "'", // SQL condition, without WHERE
            'order' => " `Date` ASC ", // SQL order, without ORDER BY
        );

        $this->_oModule = ChWsbModule::getInstance('ChVideosModule');
    }
