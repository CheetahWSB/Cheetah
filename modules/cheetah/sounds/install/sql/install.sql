...
INSERT INTO `sys_page_compose` (`Page`, `PageWidth`, `Desc`, `Caption`, `Column`, `Order`, `Func`, `Content`, `DesignBox`, `ColWidth`, `Visible`, `MinWidth`) VALUES
('index', '1140px', 'Public Sounds', '_[db_prefix]_public', 0, 0, 'PHP', 'require_once(CH_DIRECTORY_PATH_MODULES . ''cheetah/sounds/classes/ChSoundsSearch.php'');\r\n $oMedia = new ChSoundsSearch();\r\n $aVisible[] = CH_WSB_PG_ALL;\r\n if ($this->iMemberID > 0)\r\n $aVisible[] = CH_WSB_PG_MEMBERS;\r\n $aCode = $oMedia->getBrowseBlock(array(''allow_view''=>$aVisible), array(''menu_top''=>true, ''sorting'' => getParam(''[db_prefix]_mode_index''), ''per_page''=>(int)getParam(''[db_prefix]_number_index'')));\r\n return array($aCode[''code''], $aCode[''menu_top''], $aCode[''menu_bottom''], $aCode[''wrapper'']);', 1, 71.9, 'non,memb', 0),
('member', '1140px', 'Sound Albums', '_[db_prefix]_albums', 0, 0, 'PHP', 'return ChWsbService::call(''sounds'', ''get_profile_albums_block'', array($this->iMember, CH_WSB_URL_ROOT . ''member.php''), ''Search'');', 1, 71.9, 'non,memb', 0),
('profile', '1140px', 'Sound Albums', '_[db_prefix]_albums', 0, 0, 'PHP', 'return ChWsbService::call(''sounds'', ''get_profile_albums_block'', array($this->oProfileGen->_iProfileID), ''Search'');', 1, 28.1, 'non,memb', 0),
('profile', '1140px', 'Profile Sounds', '_[db_prefix]_profile_sounds', 0, 0, 'PHP', 'return ChWsbService::call(''sounds'', ''profile_sound_block'', array($this->oProfileGen->_iProfileID), ''Search'');', 1, 71.9, 'non,memb', 0);
...
INSERT INTO `sys_menu_member` SET `Name` = 'ch_sounds', `Eval` = 'return ChWsbService::call(''sounds'', ''get_member_menu_item_add_content'');', `Position`='top_extra', `Type` = 'linked_item', `Parent` = IFNULL(@iMemberMenuParent, 0), `Order` = IFNULL(@iMemberMenuOrder, 1);
...
('[db_prefix]', '[db_prefix]_cmts', 'sys_cmts_track', 0, 1, 90, 20, 1, -3, 'none', 0, 1, 0, 'cmt', 'RayMp3Files', 'ID', 'CommentsCount', 'ChSoundsCmts', 'modules/cheetah/sounds/classes/ChSoundsCmts.php'),
('[db_prefix]_albums', '[db_prefix]_cmts_albums', 'sys_cmts_track', 0, 1, 90, 20, 1, -3, 'none', 0, 1, 0, 'cmt', '', '', '', 'ChSoundsCmtsAlbums', 'modules/cheetah/sounds/classes/ChSoundsCmtsAlbums.php');
...
VALUES ('[db_prefix]', '[db_prefix]_rating', '[db_prefix]_voting_track', 'gal_', 5, 'vote_send_result', 'CH_PERIOD_PER_VOTE', 1, 'ChSoundsRate', 'modules/cheetah/sounds/classes/ChSoundsRate.php', 'RayMp3Files', 'Rate', 'RateCount', 'ID');
...
(@iTMParentId, 'SoundsSearch', '_[db_prefix]_top_menu_search', 'searchKeyword.php?type=ch_sounds', 45, 'non,memb', '', '', '', 1, 1, 1, 'custom', '', 0, '');
...
VALUES ('[db_prefix]', '_[db_prefix]', 'ChSoundsSearch', 'modules/cheetah/sounds/classes/ChSoundsSearch.php');

INSERT INTO `sys_categories` (`Category`, `ID`, `Type`, `Owner`, `Status`) VALUES
('Profile music', 0, 'ch_sounds', 0, 'active'),
('Classical', 0, 'ch_sounds', 0, 'active'),
('POP', 0, 'ch_sounds', 0, 'active'),
('I am fond of this...', 0, 'ch_sounds', 0, 'active'),
('Good meaning', 0, 'ch_sounds', 0, 'active'),
('Christmas songs', 0, 'ch_sounds', 0, 'active');
...
('[db_prefix]_title', '{evalResult}', 'plus', '', 'showPopupAnyHtml(''{BaseUri}upload'');', 'return (getLoggedId() && ChWsbModule::getInstance(''ChSoundsModule'')->isAllowedAdd()) ? _t(''_sys_upload'') : '''';', 1),
...
('[db_prefix]_album', '{evalResult}', 'remove', '', 'getHtmlData(''ajaxy_popup_result_div_{ID}'', ''{moduleUrl}album_delete/{albumUri}'', false, ''post'', true); return false;', 'return (getLoggedId() && ChWsbModule::getInstance(''ChSoundsModule'')->isAllowedDeleteAlbum({ID})) ? _t(''_Delete'') : '''';', 1);


INSERT INTO `sys_email_templates` (`Name`, `Subject`, `Body`, `Desc`, `LangID`) VALUES
('t_ch_sounds_share', 'Check This Out!', '<ch_include_auto:_email_header.html />\r\n\r\n<p><b>Hello</b>,</p>\r\n\r\n<p><SenderNickName> shared a <a href="<MediaUrl>"><MediaType></a> with you!</p>\r\n\r\n<hr>\r\n\r\n<UserExplanation>\r\n\r\n<hr>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Sounds sharing', 0),
('t_ch_sounds_report', '<SenderNickName> Reported A Sound', '<ch_include_auto:_email_header.html />\r\n\r\n<p><a href="<MediaUrl>">Reported <MediaType></a></p>\r\n\r\n<hr>\r\n\r\n<UserExplanation>\r\n\r\n<hr>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Sound report', 0),
('t_sbs_ch_sounds_comments', 'New Comments To A Sound', '<ch_include_auto:_email_header.html />\r\n\r\n<p><b>Dear <RealName></b>,</p>\r\n\r\n<br /><p>The <a href="<ViewLink>">sound you subscribed to got new comments!</a></p>\r\n\r\n<ch_include_auto:_email_footer.html />', 'Subscription: new comments to sound', 0);
...
(NULL, '_ch_sounds', '__pmu__ (<a href="modules/?r=sounds/albums/my/main/">__l_add__</a>)');
...
(2, '[db_prefix]', '_[db_prefix]', '{siteUrl}modules/?r=sounds/administration', 'Sounds module by Cheetah', 'music', @iMax+1);

INSERT INTO `sys_cron_jobs` (`name`, `time`, `class`, `file`, `eval`) VALUES
('[db_prefix]', '*/5', 'ChWsbCronMp3', 'flash/modules/mp3/inc/classes/ChWsbCronMp3.php', '');
...
('[db_prefix]', '', '', 'return ChWsbService::call(''sounds'', ''get_subscription_params'', array($arg2, $arg3));'),
('[db_prefix]', 'commentPost', 't_sbs_[db_prefix]_comments', 'return ChWsbService::call(''sounds'', ''get_subscription_params'', array($arg2, $arg3));');

INSERT INTO `sys_alerts_handlers` (`name`, `eval`) VALUES ('[db_prefix]_profile_delete', 'ChWsbService::call(''sounds'', ''response_profile_delete'', array($this));');
...
('[db_prefix]', 'homepage', '_ch_sounds', 'home_sounds.png', 9, '', '', '', @iMaxOrderHomepage, 1),
('[db_prefix]', 'profile', '_ch_sounds', '', 9, '', 'return ChWsbXMLRPCMedia::_getMediaCount(''music'', ''{profile_id}'', ''{member_id}'');', '', @iMaxOrderProfile, 1);
...
('[db_prefix]', '_ch_sounds', '0.8', 'auto', 'ChSoundsSiteMapsSounds', 'modules/cheetah/sounds/classes/ChSoundsSiteMapsSounds.php', @iMaxOrderSiteMaps, 1),
('[db_prefix]_albums', '_ch_sounds_albums', '0.8', 'auto', 'ChSoundsSiteMapsAlbums', 'modules/cheetah/sounds/classes/ChSoundsSiteMapsAlbums.php', @iMaxOrderSiteMaps + 1, 1);

-- chart

SET @iMaxOrderCharts = (SELECT MAX(`order`)+1 FROM `sys_objects_charts`);
INSERT INTO `sys_objects_charts` (`object`, `title`, `table`, `field_date_ts`, `field_date_dt`, `query`, `active`, `order`) VALUES
('[db_prefix]', '_ch_sounds', 'RayMp3Files', 'Date', '', '', 1, @iMaxOrderCharts);

-- export

SET @iMaxOrderExports = (SELECT MAX(`order`)+1 FROM `sys_objects_exports`);
INSERT INTO `sys_objects_exports` (`object`, `title`, `class_name`, `class_file`, `order`, `active`) VALUES
('[db_prefix]', '_ch_sounds', 'ChSoundsExport', 'modules/cheetah/sounds/classes/ChSoundsExport.php', @iMaxOrderExports, 1);
