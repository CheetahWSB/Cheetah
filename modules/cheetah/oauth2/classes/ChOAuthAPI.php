...
class ChOAuthAPI
{
    ...
    function service($aToken)
    {
        ...
        ch_login($aToken['user_id'], false, false);

        $sUri = ch_get('uri');
        $sMethod = ch_get('method');

        if (!($aParams = ch_get('params')))
            $aParams = array();
        ...
        if (!($sClass = ch_get('class')))
            $sClass = 'Module';

        if (!ChWsbRequest::serviceExists($sUri, $sMethod, $sClass)) {
            $this->errorOutput(404, 'not_found', 'Service was not found');
            return false;
        }

        $mixedRet = ChWsbService::call($sUri, $sMethod, $aParams, $sClass);
    ...
    protected function _getProfileInfoWithAccessChecking ($iProfileId)
    {
        ...
        if (!ch_check_profile_visibility($iProfileId, $aToken['user_id'], true)) {
    ...
    protected function _prepareProfileArray ($aProfileInfo, $bPublicFieldsOnly = true)
    {
        ...
        if (ChWsbRequest::serviceExists('photos', 'profile_photo', 'Search'))
            $aProfileInfo['picture'] = ChWsbService::call('photos', 'profile_photo', array($aProfileInfo['id'], 'file'), 'Search');
        else
