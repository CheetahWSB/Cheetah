...
DELETE FROM `sys_page_compose_pages` WHERE `Name` IN('ch_gsearch');
DELETE FROM `sys_page_compose` WHERE `Page` IN('ch_gsearch');
...
DELETE FROM `sys_options` WHERE `Name` = 'ch_gsearch_permalinks';

DELETE FROM `sys_menu_admin` WHERE `name` = 'ch_gsearch';
