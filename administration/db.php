...
$oZ = new ChWsbAlerts('system', 'admin_db_backup_actions', 0, 0, array(
...
<script type="text/javascript">
    ...
    function switchAdmPage(oLink)
    {
        ...
        jQuery(sName).siblings('div:visible').ch_anim('hide', 'fade', 'slow', function(){
            jQuery(sName).ch_anim('show', 'fade', 'slow');
        });
...
$oZ = new ChWsbAlerts('system', 'admin_db_backup_page', 0, 0, array(
	'page_vars' => &$_page,
...
function getActionResultBlock()
{
    ...
    if (isset($_POST['TablesBackup'])) { ##Block of table backup create
        ...
        $oNewBackup = new ChWsbDatabaseBackup();
        ...
        if ($_POST['savetype'] == 'server') {
            $sqlfile = CH_DIRECTORY_PATH_ROOT . 'backup/'.date("Y-m-d_H-i-s").'_'.$_POST['tbl'].'.sql';
    ...
    if (isset($_POST['DatabasesBackup'])) {
        ...
        if ($_POST['savetype'] == 'server') {
            $sqlfile = CH_DIRECTORY_PATH_ROOT . 'backup/'.date("Y-m-d_H-i-s").'_all.sql';
            $file = fopen($sqlfile, 'w');
            fputs($file, $oNewBackup -> sInputs);
            $status_text .= "<hr size=1 /><font color='green'><center>{$sSuccDumpedIntoFileC} <b>{$sqlfile}</b></center></font>\n";
            fclose($file);
    ...
    if (isset($_POST['DatabasesRestore'])) {
        if ($_POST['savetype'] == 'delete') {
            if(is_file(CH_DIRECTORY_PATH_ROOT.'backup/'.$_POST['dump_file'])) {
                @unlink(CH_DIRECTORY_PATH_ROOT.'backup/'.$_POST['dump_file']);
        ...
        if ($_POST['savetype'] == 'restore') {
            if(is_file(CH_DIRECTORY_PATH_ROOT.'backup/'.$_POST['dump_file'])) {
                $oNewBackup = new ChWsbDatabaseBackup();
                $oNewBackup ->	_restoreFromDumpFile(CH_DIRECTORY_PATH_ROOT.'backup/'.$_POST['dump_file']);
...
function getDatabaseRestoreBlock($status_text)
{
    ...
    if ( $handle = @opendir(CH_DIRECTORY_PATH_ROOT.'backup/') ) {
    ...
    $oForm2 = new ChTemplFormView($aForm2);
