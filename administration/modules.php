...
define('CH_WSB_ADM_MM_JS_NAME', 'oMM');
...
$oZ = new ChWsbAlerts('system', 'admin_modules_actions', 0, 0, array(
...
if(isset($_POST['action'])) {
	...
	switch($_POST['action']) {
		case 'check_for_updates':
			$sPath = process_db_input($_POST['path'], CH_TAGS_STRIP);
			$aResult = $oInstallerUi->checkForUpdatesByPath($sPath);
			break;

		case 'download_updates':
			$sLink = process_db_input($_POST['link'], CH_TAGS_STRIP);
...
$oZ = new ChWsbAlerts('system', 'admin_modules_page', 0, 0, array(
	'page_vars' => &$_page,
