...
switch(ch_get('action')) {
    case 'get':
        ...
        echo ch_charts_get_json(ch_get('o'), ch_get('from'), ch_get('to'));
        break;
}

function ch_charts_get_json($sObject, $sFrom, $sTo)
{
    ...
    $iFrom = ch_charts_get_ts($sFrom);
    $iTo = ch_charts_get_ts($sTo, true);
    if (!$iFrom || !$iTo)
        return json_encode(array('error' => _t('_Error Occured')));

    $aData = ch_charts_get_data($aObject, $iFrom, $iTo);
    if (!$aData)
...
function ch_charts_get_ts($s, $isNowIfError = false)
{
...
function ch_charts_get_dt_from_ts($iTs)
{
    return date('Y-m-d', $iTs);
}

function ch_charts_get_data($aObject, $iFrom, $iTo)
{
    // build query
    ...
    $a = array (
        ...
        'from' => $aObject['field_date_dt'] ? ch_charts_get_dt_from_ts($iFrom) . ' 00:00:00' : $iFrom,
        'to' => $aObject['field_date_dt'] ? ch_charts_get_dt_from_ts($iTo) . ' 23:59:59' : $iTo + 24*3600 - 1,
    );
    foreach ($a as $k => $v)
