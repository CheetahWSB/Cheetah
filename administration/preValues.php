...
if(ch_get('popup') !== false && (int)ch_get('popup') == 1) {
    $iAmInPopup = true;
...
function PageCompPageMainCode()
{
    ...
    <script type="text/javascript">
        ...
        {
            var sNewList = prompt( '<?php echo ch_js_string(_t('_adm_pvalues_msg_enter_list_name')); ?>' );
            ...
            if( !sNewList.length ) {
                alert( '<?php echo ch_js_string(_t('_adm_pvalues_msg_enter_correct_name')); ?>' );
                return false;
            }
            ...
            $( eImg ).parent().parent().before(
                ...
                ?>
                    '<th class="row_controls">' +
                    	'<a class="row_control ch-def-margin-thd-left-auto" href="javascript:void(0)" onclick="javascript:delRow(this);" title="<?php echo ch_html_attribute(_t('_Delete')); ?>"><i class="sys-icon times"></i></a>' +
                		'<a class="row_control ch-def-margin-thd-left-auto" href="javascript:void(0)" onclick="javascript:moveUpRow(this);" title="<?php echo ch_html_attribute(_t('_adm_pvalues_txt_move_up')); ?>"><i class="sys-icon arrow-up"></i></a>' +
                		'<a class="row_control ch-def-margin-thd-left-auto" href="javascript:void(0)" onclick="javascript:moveDownRow(this);" title="<?php echo ch_html_attribute(_t('_adm_pvalues_txt_move_down')); ?>"><i class="sys-icon arrow-down"></i></a>' +
                    '</th>' +
                '</tr>'
            );
    ...
    <form action="<?=$GLOBALS['site']['url_admin'] . 'preValues.php'; ?>" method="post" enctype="multipart/form-data">
    	<div class="adm-pv-cp-selector ch-def-margin-bottom">
			<div class="adm-pv-cp-item">
				<span><?php echo _t('_adm_pvalues_txt_select_list'); ?>:</span>
				<div class="input_wrapper input_wrapper_select ch-def-margin-sec-leftright clearfix">
					<select class="form_input_select ch-def-font-inputs" name="list" onchange="if( this.value != '' ) window.location = '<?=$GLOBALS['site']['url_admin'] . 'preValues.php'; ?>' + '?list=' + encodeURIComponent( this.value ) + '<?= $sPopupAdd ?>';"><?= genListOptions( $aLists, $sList ) ?></select>
				</div>
			</div>
			<div class="input_wrapper input_wrapper_submit clearfix">
				<input  class="form_input_submit ch-btn" type="button" value="<?php echo ch_html_attribute(_t('_adm_pvalues_txt_create_new')); ?>" onclick="createNewList();" />
			</div>
		</div>
        <table class="ch-def-table" id="listEdit" cellpadding="0" cellspacing="1"><?php $iNextInd = $sList !== '' ? genListRows( $sList_db ) : 0; ?></table>
		<div class="adm-pv-submit ch-def-margin-top">
			<input type="hidden" name="popup" value="<?= $iAmInPopup ?>" />
			<input type="hidden" name="action" value="Save" />
			<div class="input_wrapper input_wrapper_submit clearfix">
				<input class="form_input_submit ch-btn" type="submit" name="submit" value="<?= ch_html_attribute(_t('_Save')) ?>" />
			</div>
		</div>
        <script type="text/javascript">
            iNextInd = <?= $iNextInd ?>;
...
function genListRows( $sList )
{
    ...
    foreach( $aFields as $sField => $sHelp ) {
        ?>
            <th>
                <span class="tableLabel" onmouseover="showFloatDesc( '<?= ch_js_string($sHelp) ?>' );" onmousemove="moveFloatDesc( event );" onmouseout="hideFloatDesc();"><?= $sField ?></span>
            </th>
        <?php
    }
    ?>
    ...
    ?>
        <tr class="headers">
            ...
            <th class="row_controls">
				<a class="row_control" href="javascript:void(0)" onclick="javascript:addRow(this);" title="<?php echo ch_html_attribute(_t('_adm_pvalues_txt_add_record')); ?>"><i class="sys-icon plus"></i></a>
            </th>
        </tr>
    <?php
...
function saveList( $sList, $aData )
{
    ...
    $sQuery = "DELETE FROM `" . CH_SYS_PRE_VALUES_TABLE . "` WHERE `Key` = '$sList_db'";
    ...
    foreach( $aData as $iInd => $aRow ) {
        ...
        $sQuery = "INSERT INTO `" . CH_SYS_PRE_VALUES_TABLE . "` ( `Key`, $sInsFields, `Order` ) VALUES ( '$sList_db', $sInsValues, $iInd )";
