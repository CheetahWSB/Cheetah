...
$iLevelId = ch_get('level') !== false ? (int)ch_get('level') : 0;
...
$_page_cont[$iNameIndex] = array(
    ...
    'page_code_actions' => ch_get('action') !== false && ch_get('action') == 'actions' && $iLevelId > 0 ? PageCodeActions($iLevelId, $mixedResultActions) : "",
    'page_code_prices' => ch_get('action') !== false && ch_get('action') == 'prices' && $iLevelId > 0 ? PageCodePrices($iLevelId, $mixedResultPrices) : "",
);
...
function _getLevelsList($mixedResult, $bActive = false)
{
    ...
    $sControls = ChTemplSearchResult::showAdminActionsPanel('adm-mlevels-list-form', $aButtons, 'levels');

    return $GLOBALS['oAdmTemplate']->parseHtmlByName('mlevels_list.html', array(
        'display' => $bActive ? 'block' : 'none',
        'action' => $sSubmitUrl,
        'result' => $sResult,
        'ch_repeat:items_system' => $aItemsSystem,
        'ch_repeat:items_custom' => $aItemsCustom,
        'controls' => $sControls
    ));
}
...
function PageCodePrices($iId, $mixedResult)
{
    ...
    $sControls = ChTemplSearchResult::showAdminActionsPanel('adm-mlevels-prices-form', $aButtons, 'prices');
