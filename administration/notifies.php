...
$aPages = array (
    'massmailer' => array (
        ...
        'url' => CH_WSB_URL_ADMIN . 'notifies.php?mode=massmailer',
    ...
    'manage_subscribers' => array (
        ...
        'url' => CH_WSB_URL_ADMIN . 'notifies.php?mode=manage_subscribers',
    ...
    'settings' => array (
        'index' => 9,
        'title' => _t('_Settings'),
        'url' => CH_WSB_URL_ADMIN . 'notifies.php?mode=settings',
...
{
    $iStart = ch_get('start') !== false ? (int)ch_get('start') : 0;
    $iPerPage = 20;
    $oPaginate = new ChWsbPaginate(array(
        'start' => $iStart,
        'per_page' => $iPerPage,
        'count' => $oSubscription->getSubscribersCount(),
        'page_url' => CH_WSB_URL_ADMIN . 'notifies.php?mode=manage_subscribers&start={start}',

    ));

    $sControls = ChTemplSearchResult::showAdminActionsPanel('adm-ms-form', array(
        'adm-ms-delete' => _t('_adm_btn_ms_delete')
    ), 'members');

    $aSubscribers = $oSubscription->getSubscribers(CH_WSB_SBS_TYPE_VISITOR, $iStart, $iPerPage);

    $s = $GLOBALS['oAdmTemplate']->parseHtmlByName('manage_subscribers.html', array(
        'ch_repeat:items' => is_array($aSubscribers) && !empty($aSubscribers) ? $aSubscribers : MsgBox(_t('_Empty')),
...
function PrintStatus($sActionResult)
{
    ...
    if($iCount > 0) {
        $sEmptyQueueTable = "
        <div class=\"ch-def-hr\"></div>
        <table class=\"text\" width=\"50%\" style=\"height: 30px;\">
            <tr class=\"table\">
                <td align=\"center\" colspan=\"3\">
                    <a href=\"" . CH_WSB_URL_ADMIN . "notifies.php?action=empty\">{$sEmptyQueueC}</a>
                </td>
            </tr>
        </table>
        <div class=\"ch-def-hr\"></div>";
...
function QueueMessage()
{
    ...
    if($_POST['send_to_subscribers'] == 'non') {
        $sSql = "SELECT
                ...
                INNER JOIN `sys_sbs_entries` AS `tse` ON `tst`.`id`=`tse`.`subscription_id` AND `tse`.`subscriber_type`='" . CH_WSB_SBS_TYPE_VISITOR . "'
        ...
        foreach($aSubscribers as $aSubscriber) {
            ...
            $mixedResult = $MySQL->query("INSERT INTO `sys_sbs_queue`(`email`, `subject`, `body`) VALUES('" . $aSubscriber['user_email'] . "', '" . process_db_input($aMessage['subject'], CH_TAGS_STRIP) . "', '" . process_db_input($aMessage['body'], CH_TAGS_VALIDATE) . "')");
    ...
    if($_POST['send_to_members'] == 'memb') {
        ...
        foreach($aMembers as $aMember) {
            ...
            $mixedResult = $MySQL->query("INSERT INTO `sys_sbs_queue`(`email`, `subject`, `body`) VALUES('" . $aMember['email'] . "', '" . process_db_input($aMessage['subject'], CH_TAGS_STRIP) . "', '" . process_db_input($aMessage['body'], CH_TAGS_VALIDATE) . "')");
            if($mixedResult === false) {
