...
ChWsbPFM.prototype.getAreaByID = function( iAreaID ) {
...
ChWsbPFM.prototype.updateAreas = function( sText, iItemID, sNewName, iAreaID ) {
	switch( sText ) {
		case 'deleteItem':
			location.reload();
		break;
...
function ChWsbPFMArea(parent, id) {
	this.id     = id;
	this.parent = parent;
...
ChWsbPFMArea.prototype.getElement = function() {
	this.element = this.parent.config.getAreaElem(this.id);
}

ChWsbPFMArea.prototype.requestData = function() {
	var oThisArea = this;
	$.getJSON(
...
ChWsbPFMArea.prototype.getData = function(oAreaData) {
	if( this.id != oAreaData.id )
		return false;

	for( var iBlockInd = 0; iBlockInd < oAreaData.active_blocks.length;   iBlockInd++ )    this.active_blocks[   this.active_blocks.length   ] = new ChWsbPFMBlock( this, oAreaData.active_blocks[   iBlockInd ] );
	for( var iBlockInd = 0; iBlockInd < oAreaData.inactive_blocks.length; iBlockInd++ )    this.inactive_blocks[ this.inactive_blocks.length ] = new ChWsbPFMBlock( this, oAreaData.inactive_blocks[ iBlockInd ] );
	for( var iItemInd  = 0; iItemInd  < oAreaData.active_items.length;    iItemInd ++ )    this.active_items[    this.active_items.length    ] = new ChWsbPFMItem(  this, oAreaData.active_items[    iItemInd  ] );
	for( var iItemInd  = 0; iItemInd  < oAreaData.inactive_items.length;  iItemInd ++ )    this.inactive_items[  this.inactive_items.length  ] = new ChWsbPFMItem(  this, oAreaData.inactive_items[  iItemInd  ] );

	this.draw();
}

ChWsbPFMArea.prototype.draw = function() {
...
ChWsbPFMArea.prototype.fixZonesWidths = function() {
...
ChWsbPFMArea.prototype.activateSortable = function() {
	var oThisArea = this;
...
ChWsbPFMArea.prototype.stopItemsSort = function( item ) {
...
ChWsbPFMArea.prototype.resortAndSubmit = function(){
	var oThisArea = this;
...
ChWsbPFMArea.prototype.createNewBlock = function() {
	var oThisArea = this;
...
ChWsbPFMArea.prototype.createNewItem = function() {
	var oThisArea = this;
...
ChWsbPFMArea.prototype.insertNewBlock = function( newBlockID ) {
	...
	//create object
	this.active_blocks[iNewInd] = new ChWsbPFMBlock( this, oNewBlockData );
...
ChWsbPFMArea.prototype.insertNewItem = function( newItemID ) {
	var iNewInd = this.active_items.length;
	var oNewItemData = {id:newItemID,name:'NEW_ITEM'};
	var oThisArea = this;

	//create object
	this.active_items[iNewInd] = new ChWsbPFMItem( this, oNewItemData );
...
ChWsbPFMArea.prototype.moveFakeBlockBack = function() {
	$( '#build_block_new_' + this.id ).prependTo( this.inactiveBlocksZoneElement );
}

ChWsbPFMArea.prototype.moveFakeItemBack = function() {
	$( '#build_item_new_' + this.id ).prependTo( '#build_inac_items_area_' + this.id + '_col_1' );
}

ChWsbPFMArea.prototype.resortArrays = function() {
	var oThisArea = this;
...
ChWsbPFMArea.prototype.submitPositions = function(){
	var oRequest = {};
...
ChWsbPFMArea.prototype.processSaveResult = function( sResult ) {
	if( $.trim( sResult ) != 'OK' )
		alert( sResult );
}

ChWsbPFMArea.prototype.getBlockByElementID = function( getID ) {
	for( var iBlockInd = 0; iBlockInd < this.active_blocks.length; iBlockInd ++ )
...
ChWsbPFMArea.prototype.getItemByElementID = function( getID ) {
	for( var iItemInd = 0; iItemInd < this.active_items.length; iItemInd ++ )
...
ChWsbPFMArea.prototype.getItemOrBlockByID = function( getID ) {
...
ChWsbPFMArea.prototype.openFieldDialog = function( iItemID, iAreaID ) {
	var oDate = new Date();
...
function ChWsbPFMBlock( parent, oBlockData ) {
...
ChWsbPFMBlock.prototype.draw = function( oParentElement ) {
	var oThisBlock = this;
...
ChWsbPFMBlock.prototype.getCode = function() {
...
ChWsbPFMBlock.prototype.getElement = function() {
	this.element = $( '#' + this.elementID ).get(0);
}

ChWsbPFMBlock.prototype.updateName = function( sNewName ) {
	$( this.element ).children( 'div.build_block_header' ).children( 'a' ).html( sNewName );
}

/* * * * Item Class * * * */

function ChWsbPFMItem( parent, oItemData ) {
...
ChWsbPFMItem.prototype.draw = function( oParentElement ) {
	var oThisItem = this;
...
ChWsbPFMItem.prototype.getCode = function( oParentElement ) {
...
ChWsbPFMItem.prototype.getElement = function() {
	this.element = $( '#' + this.elementID ).get(0);
}

ChWsbPFMItem.prototype.updateName = function( sNewName ) {
