...
for ($i=1; $i<255 ; ++$i) {
    $aChSecurityExceptions[] = 'POST.string_for_'.$i;
    $aChSecurityExceptions[] = 'REQUEST.string_for_'.$i;
}
...
$oSettingsLanguage = new ChWsbAdminSettings(21);
...
function PageCodeMain($aResults)
{
    $aTopItems = array(
        ...
        'adm-langs-btn-help' => array('href' => 'https://github.com/cheetah/cheetah.pro/wiki/Using-Language-Keys-to-Change-Site-Text', 'target' => '_blank', 'title' => _t('_help'))
    );
...
function _getKeysList($mixedResult, $bActive = false)
{
    ...
    if(isset($_GET[$sFilterName])) {
        $sFilter = process_db_input($_GET[$sFilterName], CH_TAGS_STRIP);
    ...
    $sControls = ChTemplSearchResult::showAdminActionsPanel('adm-keys-form', $aButtons, 'keys');

    $sFilter = ChTemplSearchResult::showAdminFilterPanel(false !== ch_get($sFilterName) ? ch_get($sFilterName) : '', 'adm-langs-look-for', 'adm-langs-apply', $sFilterName);
...
function _getLanguagesList($mixedResult, $bActive = false)
{
    ...
    $sControls = ChTemplSearchResult::showAdminActionsPanel('adm-langs-form', $aButtons, 'langs');
...
function _getLanguageCreateForm($mixedResult, $bActive = false)
{
    ...
    $oForm = new ChTemplFormView($aFormCreate);
...
function _getLanguageImportForm($mixedResult, $bActive = false)
{
    ...
    $oForm = new ChTemplFormView($aFormImport);
