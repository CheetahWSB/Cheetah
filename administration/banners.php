...
if ( $iBannerID > 0 && ch_get('action') == "delete" ) {
...
function getManageBannersBlock()
{
    ...
    $start_date = ch_get('start_date') !== false ? ch_get('start_date') : $start_date_default;
    $end_date = ch_get('end_date') !== false ? ch_get('end_date') : $end_date_default;

    $Title = $Url = $Active = $Text = $Position = $lhshift = $lvshift = $rhshift = $rvshift = '';
    $iBannerID = (int)getGetFieldIfSet('banner_id');
    $action	= "new";

    if ($iBannerID > 0 && ! strlen(ch_get('action'))) { //banner edit
