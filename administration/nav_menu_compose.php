...
if(ch_get('action') !== false) {
    switch(ch_get('action')) {
        ...
        case 'deactivate_item':
            $res = db_res( "UPDATE `sys_menu_top` SET `Active`=0 WHERE `ID`=" . (int)ch_get('id'));
...
while(($aTopItem = $rTopItems->fetch()) !== false) {
    ...
    while(($aCustomItem = $rCustomItems->fetch()) !== false) {
        $sComposerInit .= "
        aCustomItems[$iIndex][" . ($iSubIndex++) . "] = [{$aCustomItem['ID']}, '" . ch_js_string( $aCustomItem['Name'], CH_ESCAPE_STR_APOS ) . "', " . $aCustomItem['Movable'] . "];";
    }
...
while(($aSystemItem = $rSysItems->fetch()) !== false) {
    $sComposerInit .= "
        ...
        aSystemItems[$iIndex] = [{$aSystemItem['ID']}, '" . ch_js_string( $aSystemItem['Name'], CH_ESCAPE_STR_APOS ) . "', " . $aSystemItem['Movable'] . "];
        aCustomItems[$iIndex] = {};";
