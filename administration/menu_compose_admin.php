...
while(($aTopItem = $rTopItems->fetch()) !== false) {
    $sTopestTitle = ch_js_string(_t($aTopItem['title']), CH_ESCAPE_STR_APOS);
    $sComposerInit .= "
    ...
    while(($aCustomItem = $rCustomItems->fetch()) !== false) {
        $sCustomTitle = ch_js_string(_t($aCustomItem['title']), CH_ESCAPE_STR_APOS);
        $sComposerInit .= "
...
foreach ($aAllTopItems as $iId => $sTitle) {
    $sTopTitle = ch_js_string($sTitle, CH_ESCAPE_STR_APOS);
    $sComposerInit .= "
