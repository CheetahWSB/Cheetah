...
define('CH_RETINA_PREFIX', 'retina_');
...
$oSettings = new ChWsbAdminSettings(7);
...
if(isset($_POST['save_splash'])) {
    ...
    setParam('splash_editor', ($_POST['editor'] == 'on' ? 'on' : ''));
    ...
    setParam('splash_visibility', $_POST['visibility']);
    //setParam('splash_logged', (process_db_input($_POST['logged']) == 'on' ? 'on' : ''));
    setParam('splash_logged', ($_POST['logged'] == 'on' ? 'on' : ''));
    ...
    $aForm = array(
        ...
        'inputs' => array(
            ...
            'visibility' => array(
                ...
                'values' => array(
                    CH_WSB_SPLASH_VIS_DISABLE => _t('_adm_txt_settings_splash_visibility_disable'),
                    CH_WSB_SPLASH_VIS_INDEX => _t('_adm_txt_settings_splash_visibility_index'),
                    CH_WSB_SPLASH_VIS_ALL => _t('_adm_txt_settings_splash_visibility_all')
                ),
                'dv' => '<br />'
            ),
            'logged' => array(
...
function setLogo(&$aData, &$aFile)
{
    ...
    $sFileName = time() . '.' . $sExt;
    $sFileName2x = CH_RETINA_PREFIX . time() . '.' . $sExt;
    ...
    @unlink($dir['mediaImages'] . CH_RETINA_PREFIX . getParam('sys_main_logo'));
    setParam('sys_main_logo', $sFileName);

    ch_import('ChWsbImageResize');
    $aFileNewSize = ChWsbImageResize::getImageSize($sFilePath);
