...
function login_form($text = "", $member = 0, $bAjaxMode = false, $sLoginFormParams = '')
{
    ...
    if($bAjaxMode) {
        ...
        $sCaptionItems = '<div class="dbTopMenu"><i class="ch-popup-element-close sys-icon times"></i></div>';
...
function activation_mail( $ID, $text = 1 )
{
    ...
    $sConfirmationLink	= CH_WSB_URL_ROOT . "profile_activate.php?ConfID={$ID}&ConfCode=" . urlencode( $sConfirmationCode );
    ...
    if ( $mail_ret ) {
        if ( $text ) {
            ...
            $page_text .= "<center><form method=get action=\"" . CH_WSB_URL_ROOT . "profile_activate.php\">";
...
function getID( $str, $with_email = 1 )
{
    if ( $with_email ) {
        ch_import('ChWsbForm');
        if (ChWsbFormCheckerHelper::checkEmail($str)) {
...
function check_login($ID, $passwd, $iRole = CH_WSB_ROLE_MEMBER, $error_handle = true)
{
    $ID = (int)$ID;
    ...
    switch ($iRole) {
        case CH_WSB_ROLE_MEMBER: $member = 0; break;
        case CH_WSB_ROLE_ADMIN:  $member = 1; break;
    }
    ...
    if(((int)$aProfile['Role'] & CH_WSB_ROLE_ADMIN) || ((int)$aProfile['Role'] & CH_WSB_ROLE_MODERATOR)) {
    ...
    // if IP is banned
    if ((2 == getParam('ipBlacklistMode') && ch_is_ip_blocked()) || ('on' == getParam('sys_dnsbl_enable') && 'block' == getParam('sys_dnsbl_behaviour') && ch_is_ip_dns_blacklisted('', 'login'))) {
        if ($error_handle) {
...
function check_logged()
{
	...
	if($bLogged)
		$GLOBALS['oSysTemplate']->addCssStyle('.ch-hide-when-logged-in', array(
			'display' => 'none'
		));
}
...
function member_auth($member = 0, $error_handle = true, $bAjx = false)
{
       ...
       switch ($member) {
        case 0:
               $mem	    = 'member';
               $login_page = CH_WSB_URL_ROOT . "member.php";
            $iRole      = CH_WSB_ROLE_MEMBER;
        break;
        case 1:
               $mem	    = 'admin';
               $login_page = CH_WSB_URL_ADMIN . "index.php";
    ...
    if (empty($_COOKIE['memberID']) || !isset($_COOKIE['memberPassword'])) {
        if ($error_handle) {
            ...
            if ($member == 0)
               $text .= "<br />"._t("_LOGIN_REQUIRED_AE2", $site['images'], CH_WSB_URL_ROOT, $site['title']);
...
function check_password($sUsername, $sPassword, $iRole = CH_WSB_ROLE_MEMBER, $error_handle = true)
{
    $iId = getID($sUsername);
    if (!$iId) return false;
...
function profile_delete($ID, $isDeleteSpammer = false)
{
    ...
    require_once( CH_DIRECTORY_PATH_CLASSES . 'ChWsbVoting.php' );
    $oVotingProfile = new ChWsbVoting ('profile', 0, 0);
    $oVotingProfile->deleteVotings ($ID);

    // delete profile comments
    require_once (CH_DIRECTORY_PATH_CLASSES . 'ChWsbCmts.php');
    $oCmts = new ChWsbCmts('profile', $ID);
    ...
    if (ChWsbModule::getInstance('ChWmapModule'))
        ChWsbService::call('wmap', 'response_entry_delete', array('profiles', $ID));
    ...
    if ($isDeleteSpammer) {
        ...
        $oChWsbStopForumSpam->submitSpammer(array('username' => $aProfileInfo['NickName'], 'email' => $aProfileInfo['Email'], 'ip' => ch_member_ip_get_last($ID)));
    }

    // delete moxiemanager files
    $sMoxieFilesPath  = CH_DIRECTORY_PATH_ROOT . 'media/moxie/files/' . substr($aProfileInfo['NickName'], 0, 1) . '/' . substr($aProfileInfo['NickName'], 0, 2) . '/' . substr($aProfileInfo['NickName'], 0, 3) . '/' . $aProfileInfo['NickName'];
    ch_rrmdir($sMoxieFilesPath);

    // create system event
    $oZ = new ChWsbAlerts('profile', 'delete',  $ID, 0, array('profile_info' => $aProfileInfo, 'logged_in' => $iLoggedInId, 'delete_spammer' => $isDeleteSpammer));
    $oZ->alert();
 ...
 */
function ch_admin_profile_ban_control($iProfileId, $bBan = true, $iDuration = 0)
{
 ...
 */
function ch_admin_profile_change_status($mixedIds, $sStatus, $bSendActMail = FALSE)
{
    ...
    foreach ($mixedIds as $iId) {
        ...
        if ($sStatus == 'Active' && $bSendActMail) {
            if (ChWsbModule::getInstance('ChWmapModule'))
                ChWsbService::call('wmap', 'response_entry_add', array('profiles', $iId));
        ...
        $oAlert = new ChWsbAlerts('profile', 'change_status', $iId, 0, array('status' => $sStatus));
        $oAlert->alert();
    }
 ...
 * @return boolean               - TRUE on success / FALSE on failure
 */
function ch_admin_profile_featured_control($iProfileId, $bFeature = TRUE)
{
    $iProfileId = (int)$iProfileId;
