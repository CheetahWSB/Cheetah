...
$.fn.chwsbcmtanim = function(action, effect, speed, h)
{
...
ChWsbCmts.prototype.showReplacement = function(iCmtId) {
    $('#cmt' + iCmtId + '-hidden').chwsbcmtanim('hide', this._sAnimationEffect, this._iAnimationSpeed, function(){
        $(this).next('#cmt' + iCmtId).chwsbcmtanim('show', this._sAnimationEffect, this._iAnimationSpeed);
    });
};
...
ChWsbCmts.prototype.reloadComment = function(iCmtId) {
    ...
    jQuery.post (
        ...
        function (s) {
        	...
        	$('#cmts-box-' + $this._sSystem + '-' + $this._iObjId + ' li#cmt' + iCmtId).chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                $(this).replaceWith(s);
        	});
...
ChWsbCmts.prototype.toggleReply = function(oElement, iCmtParentId) {
    ...
    if(iCmtParentId == 0) {
        ...
        if(oParent.children().length) {
        	oParent.chwsbcmtanim('hide', this._sAnimationEffect, this._iAnimationSpeed, function() {
            ...
            jQuery.post (
                ...
                function (s) {
                	...
                	oParent.append($(s).addClass('cmt-post-reply-expanded').css('display', 'none')).children('.cmt-post-reply').chwsbcmtanim('toggle', $this._sAnimationEffect, $this._iAnimationSpeed);
    ...
    //--- Get form for posting a reply ---//
		...
		if (oParent.children('.cmt-post-reply').length)
			oParent.children('.cmt-post-reply').chwsbcmtanim('toggle', this._sAnimationEffect, this._iAnimationSpeed);
		else {
		    var $this = this;
		    var oData = this._getDefaultActions();
            oData['action'] = 'FormGet';
            ...
            jQuery.post (
                ...
                function (s) {
                	...
                	oParent.children('.cmt-cont').after($(s).addClass('cmt-post-reply-expanded').css('display', 'none')).next('.cmt-post-reply').chwsbcmtanim('toggle', $this._sAnimationEffect, $this._iAnimationSpeed);
                }
            );
		}
	}
...
ChWsbCmts.prototype.toggleType = function(oLink) {
    ...
    if($(oLink).parents('.cmt-balloon').find('div.' + sKey).is(':hidden')) {
        $(oLink).addClass('inactive').parents('.cmt-balloon').find('div.' + aSwitcher[sKey]).chwsbcmtanim('hide', this._sAnimationEffect, this._iAnimationSpeed, function(){
            ...
            $(this).siblings('div.' + sKey).chwsbcmtanim('show', this._sAnimationEffect, this._iAnimationSpeed);
            if(sType == 'video')
...
ChWsbCmts.prototype.toggleCmts = function(e, iCmtParentId) {
    ...
    if(iCmtParentId == 0) {
        ...
        if(!$(e).parents('#' + sCmtsBox).find('div.cmts > ul > li').length)
        	...
        	$(e).parents('#' + sCmtsBox).find('div.cmts').chwsbcmtanim('toggle', this._sAnimationEffect, this._iAnimationSpeed);
    }
    //--- Load Replies ---//
    else {
        ...
        if ($(sId + ' > ul').length) {
            if ($(sId + ' > ul').is(':visible'))
            	$( sId + ' > ul').chwsbcmtanim('hide', this._sAnimationEffect, this._iAnimationSpeed, function() {
            		$(sId + '  > .cmt-cont .cmt-replies-hide').hide().siblings('.cmt-replies-show').show();
            	});
            else
                $(sId + ' > ul').chwsbcmtanim('show', this._sAnimationEffect, this._iAnimationSpeed, function() {
		    ...
		    jQuery.post (
		        ...
		        function (s) {
		        	...
		        	if (jQuery.trim(s).length)
		        		...
		        		$('#cmt' + iCmtId).chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function () { $(this).remove(); } );
		        }
		    );
		}
	});
...
ChWsbCmts.prototype.cmtEdit = function(e, iCmtId) {
    ...
    if (oBody.find('textarea.cmt-text-edit').length) {
    	oBody.chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
    		if($this._iGlobAllowHtml == 1)
    			$this.editorDestroy($(this).find("textarea.cmt-text-edit[name='CmtText']"));

    		$(this).html($this._oSavedTexts[iCmtId]).chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed);
    	});
    ...
    jQuery.post (
        ...
        function(s) {
        	...
        	oBody.chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
				...
				$(this).chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed);
    		});
        }
    );
};
...
ChWsbCmts.prototype._getPaginate = function(e, iStart, iPerPage) {
    ...
    jQuery.post (
        ...
        oData,
        function(s) {
            $('#cmts-box-' + $this._sSystem + '-' + $this._iObjId + ' .cmt-show-more').chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                if(s.length > 0) {
...
ChWsbCmts.prototype._getCmts = function (e, iCmtParentId, onLoad, iStart, iPerPage)
{
    ...
    jQuery.post (
        ...
        function(s) {
            if(iCmtParentId == 0) {
                $(e).parents('#cmts-box-' + $this._sSystem + '-' + $this._iObjId + ':first').find('.cmts > ul').chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                    $(this).replaceWith(s).chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                    	$(this).find('a.ch-link').dolEmbedly();
                    });
                });
            }
            else
                $(e).parents('#cmt' + iCmtParentId + ':first').append($(s).filter('.cmts').addClass('cmts-margin').hide()).children('.cmts').chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                	$(this).find('a.ch-link').dolEmbedly();
                });
            onLoad();
            $this._loading (e, false);
        ...
        function (s) {
        	...
        	if (iCmtParentId == 0) {
        		var oProcessResults = function () {
	        		...
	        		if(oParent.children('li.cmt:last').length) {
	        			...
	        			if(oParent.is(':visible'))
	        				oParent.append($(s).hide()).find('li.cmt:hidden').chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
		                    	$(this).find('a.ch-link').dolEmbedly();
		                    });
                        else
                        	oParent.append(s).parents('div.cmts:first').chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
		                    	$(this).find('a.ch-link').dolEmbedly();
		                    });
	        		}
        		...
        		if($this._bAutoHideRootPostForm)
        			oBox.find('.cmt-reply').chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
        				$this.removeReply($(this));
        		...
        		oBox.find('#cmt' + iCmtParentId + ' > .cmt-post-reply').chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                    ...
                    if(!oBox.find('#cmt' + iCmtParentId + ' > ul').length && !iRepliesCount)
                        $(s).wrap($('<ul />').addClass('cmts').addClass('cmts-margin')).parent().hide().appendTo(oBox.find('#cmt' + iCmtParentId)).chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                        	$(this).find('a.ch-link').dolEmbedly();
                        });
                    ...
                    //--- there is some number of comments and they are loaded.
                        ...
                        if(oBox.find('#cmt' + iCmtParentId + ' > ul').is(':visible')) {
                        	...
                        	oBox.find('#cmt' + iCmtParentId + ' > ul > :last').chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                            	$(this).find('a.ch-link').dolEmbedly();
                            });
                        }
                        else
                        	oBox.find('#cmt' + iCmtParentId + ' > ul').append(s).chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
                            	$(this).find('a.ch-link').dolEmbedly();
                            });
                    }
                });
    ...
    jQuery.post (
        ...
        function (oResponse) {
            ...
            oBody.chwsbcmtanim('hide', $this._sAnimationEffect, $this._iAnimationSpeed, function() {
    			...
    			$(this).html(oResponse.text).chwsbcmtanim('show', $this._sAnimationEffect, $this._iAnimationSpeed);
    		});
        },
        'json'
    );
...
ChWsbCmts.prototype._toggleHidden = function(e, iCmtId) {
	$('#cmt'+iCmtId+' > .cmt-cont').chwsbcmtanim('toggle', this._sAnimationEffect, this._iAnimationSpeed);
};
