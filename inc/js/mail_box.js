...
function MailBox() {
    ...
    this.hideDeletedMessages = function(sContainer, sCallbackFunction) {
        ...
        $(document).dolPopupConfirm({
            ...
            onClickYes: function() {
                $('#' + $this.sResponceBlock).load($this.sPageReceiver + '&messages=' + sMessagesId + '&callback_function=' + sCallbackFunction + $this.ExtendedParameters, {
                    'action': 'hide_deleted'
                });
            }
        });
    ...
    this.spamMessages = function(sContainer, iMemberID) {
        ...
        var oCheckBoxes = $("." + sContainer + " input:checkbox:checked").each(function() {
            if ($(this).attr('owner')) {
                ...
                if (iValue) sMembersId += iValue + ',';
            }
        });
        if (typeof iMemberID != 'undefined') sMembersId = iMemberID + ',';
        if (!sMembersId) {
            alert(this.sErrorMessage);
            return;
        }
    ...
    this.sendMessage = function(vRecipientId) {
        var sErrorMessage = '';
        var enable_tiny = ch_get_param('enable_tiny_in_mail');
        if ('on' == enable_tiny) {
            ...
            // No editor in mail, so get message from textarea box instead.
            var sComposeMessage = $.trim($('#' + htmlSelectors[0]).attr('value'));
        }
        if (!sComposeMessage) {
        ...
        if (typeof vRecipientId == 'undefined') {
            ...
            if (oId.length) vRecipientId = $.trim(oId.attr('value'));
        }
        if (!vRecipientId) {
            sErrorMessage = _t('_Mailbox recipient empty');
        }
        // collect the all needed parameters ;
        var sPageUrl = this.sPageReceiver + '&action=compose_mail&recipient_id=' + vRecipientId;
        // if data are correct ;
        if (!sErrorMessage) {
            ...
            // send data ;
            $.post(sPageUrl, {
                'subject': sComposeSubject,
                'message': sComposeMessage
            }, function(sReceivedData) {
                if ('on' == enable_tiny) {
