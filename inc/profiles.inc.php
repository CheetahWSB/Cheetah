...
// user roles
define('CH_WSB_ROLE_GUEST',     0);
define('CH_WSB_ROLE_MEMBER',    1);
define('CH_WSB_ROLE_ADMIN',     2);
define('CH_WSB_ROLE_AFFILIATE', 4);
define('CH_WSB_ROLE_MODERATOR', 8);
...
{
    return isRole(CH_WSB_ROLE_MEMBER, $iId);
}
if(!function_exists("isAdmin")) {
    function isAdmin($iId = 0)
    {
        return isRole(CH_WSB_ROLE_ADMIN, $iId);
    }
}
function isAffiliate($iId = 0)
{
    return isRole(CH_WSB_ROLE_AFFILIATE, $iId);
}
function isModerator($iId = 0)
{
    return isRole(CH_WSB_ROLE_MODERATOR, $iId);
}
...
function GetMembershipStatus($memberID, $bOfferUpgrade = true, $bViewActions = true)
{
    ...
    $sViewActions = $bViewActions ? "<a onclick=\"javascript:loadHtmlInPopup('explanation_popup', '" . CH_WSB_URL_ROOT . "explanation.php?explain=membership&amp;type=" . $aMembership['ID'] . "');\" href=\"javascript:void(0);\">" . _t("_VIEW_MEMBERSHIP_ACTIONS") . "</a>" : "";

    // Show colored membership name
    if($aMembership['ID'] == MEMBERSHIP_ID_STANDARD) {
        $ret .= $aMembership['Name'];

        if($bOfferUpgrade && ChWsbRequest::serviceExists('membership', 'get_upgrade_url'))
            $sViewActions = _t('_MEMBERSHIP_UPGRADE_FROM_STANDARD', ChWsbService::call('membership', 'get_upgrade_url')) . '<span class="sys-bullet"></span>' . $sViewActions;
...
function deleteUserDataFile( $userID )
{
    ...
    $userID = (int)$userID;
    $fileName = CH_DIRECTORY_PATH_CACHE . 'user' . $userID . '.php';
...
function getProfileInfo($iProfileID = 0, $checkActiveStatus = false, $forceCache = false)
{
    ...
    if(!isset( $aUser[$iProfileID]) || !is_array($aUser[$iProfileID]) || $forceCache) {
        $sCacheFile = CH_DIRECTORY_PATH_CACHE . 'user' . $iProfileID . '.php';
...
function ch_login($iId, $bRememberMe = false, $bAlert = true)
{
    ...
    if($bAlert) {
	    ...
	    $oZ = new ChWsbAlerts('profile', 'login',  $iId);
	    $oZ->alert();
    }

    return getProfileInfo($iId);
}
function ch_logout($bNotify = true)
{
    if($bNotify && isMember()) {
        require_once(CH_DIRECTORY_PATH_CLASSES . 'ChWsbAlerts.php');
        $oZ = new ChWsbAlerts('profile', 'logout', (int)$_COOKIE['memberID']);
        $oZ->alert();
    }
 ...
 * @return nothing
 */
function ch_check_profile_visibility ($iViewedId, $iViewerId = 0, $bReturn = false)
{
    ...
    $oProfile = new ChTemplProfileGenerator( $iViewedId );
    ...
    if (!$logged['admin'] && !$logged['moderator'] && $iViewerId != $iViewedId) {
        ...
        if (!$oPrivacy->check('view', $iViewedId, $iViewerId)) {
            if ($bReturn)
                return false;
            ch_import('ChWsbProfilePrivatePageView');
            $oProfilePrivateView = new ChWsbProfilePrivatePageView($oProfile, $site, $dir);
            $_page['name_index'] = 7;
