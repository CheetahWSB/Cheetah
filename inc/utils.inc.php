...
define('CH_WSB_LINK_CLASS', 'ch-link');

define('CH_WSB_LOCALE_TIME', 2);
define('CH_WSB_LOCALE_DATE_SHORT', 4);
define('CH_WSB_LOCALE_DATE', 5);

define('CH_WSB_LOCALE_PHP', 1);
define('CH_WSB_LOCALE_DB', 2);

define('CH_TAGS_NO_ACTION', 0); // default
define('CH_TAGS_STRIP', 1);
define('CH_TAGS_SPECIAL_CHARS', 8);
define('CH_TAGS_VALIDATE', 16);
define('CH_TAGS_STRIP_AND_NL2BR', 32);

define('CH_SLASHES_AUTO', 0); // default
define('CH_SLASHES_ADD', 1);
define('CH_SLASHES_STRIP', 2);
define('CH_SLASHES_NO_ACTION', 3);

define('CH_ESCAPE_STR_AUTO', 0); ///< turn apostropes and quote signs into html special chars, for use in @see ch_js_string and @see ch_html_attribute
define('CH_ESCAPE_STR_APOS', 1); ///< escape apostrophes only, for js strings enclosed in apostrophes, for use in @see ch_js_string and @see ch_html_attribute
define('CH_ESCAPE_STR_QUOTE', 2); ///< escape quotes only, for js strings enclosed in quotes, for use in @see ch_js_string and @see ch_html_attribute

define('CH_URL_RE', "@\b((https?://)|(www\.))(([0-9a-zA-Z_!~*'().&=+$%-]+:)?[0-9a-zA-Z_!~*'().&=+$%-]+\@)?(([0-9]{1,3}\.){3}[0-9]{1,3}|([0-9a-zA-Z_!~*'()-]+\.)*([0-9a-zA-Z][0-9a-zA-Z-]{0,61})?[0-9a-zA-Z]\.[a-zA-Z]{2,6})(:[0-9]{1,4})?((/[0-9a-zA-Z_!~*'().;?:\@&=+$,%#-]+)*/?)@"); ///< regular expression to match URL
 ...
 */
function getLocaleDate($sTimestamp = '', $iCode = CH_WSB_LOCALE_DATE_SHORT)
{
 ...
 *                       2 - for database.
 */
function getLocaleFormat($iCode = CH_WSB_LOCALE_DATE_SHORT, $iType = CH_WSB_LOCALE_PHP)
{
    $sPostfix = (int)$iType == CH_WSB_LOCALE_PHP ? '_php' : '';
 ...
 */
function ch_time_utc($iUnixTimestamp)
{
 ...
 *                                 CH_TAGS_STRIP - strip tags
 *                                 CH_TAGS_SPECIAL_CHARS - translate to special html chars (not good to use this, it is better to do such thing during output to browser)
 *                                 CH_TAGS_VALIDATE - validate HTML
 *                                 CH_TAGS_NO_ACTION - do not perform any action with tags
 * @return string
 */
...
function templates_select_txt()
{
    ...
    foreach ($templ_choices as $tmpl_key => $tmpl_value) {
        if ($current_template == $tmpl_key) {
            ...
            $ReturnResult .= '<a href="' . ch_html_attribute($_SERVER['PHP_SELF']) . '?' . $sGetTransfer . 'skin=' . $tmpl_key . '">' . $tmpl_value . '</a> | ';
        }
    }
...
function clear_xss($val)
{
    ...
    if (!isset($oHtmlPurifier) && !$GLOBALS['logged']['admin']) {
        ...
        $oConfig->set('Cache.SerializerPath', rtrim(CH_DIRECTORY_PATH_CACHE, '/'));
        ...
        if (getParam('sys_antispam_add_nofollow')) {
            $sHost = parse_url(CH_WSB_URL_ROOT, PHP_URL_HOST);
        ...
        $oConfig->set('Filter.Custom', array(
            ...
            new HTMLPurifier_Filter_AddChLinksClass()
        ));
 ...
 * @return void
 */
function ch_block_ip($mixedIP, $iExpirationInSec = 86400, $sComment = '')
{
...
function ch_is_ip_dns_blacklisted($sCurIP = '', $sType = '')
{
...
function ch_is_ip_blocked($sCurIP = '')
{
    if (defined('CH_WSB_CRON_EXECUTE')) {
        return false;
    }
...
function ch_is_spam($val)
{
    ...
    if ('on' == getParam('sys_uridnsbl_enable')) {
        ...
        if ($oChWsbDNSURIBlacklists->isSpam($val)) {
            $oChWsbDNSURIBlacklists->onPositiveDetection($val);
            $bRet = true;
        }
    }
    ...
    if ('on' == getParam('sys_akismet_enable')) {
        ...
        if ($oChWsbAkismet->isSpam($val)) {
            $oChWsbAkismet->onPositiveDetection($val);
            $bRet = true;
        }
    }
 ...
 */
function ch_import($sClassName, $aModule = array())
{
    ...
    if ($aModule) {
        ...
        require_once(CH_DIRECTORY_PATH_MODULES . $a['path'] . 'classes/' . $a['class_prefix'] . $sClassName . '.php');
    }

    if (0 === strncmp($sClassName, 'ChWsb', 5)) {
        require_once(CH_DIRECTORY_PATH_CLASSES . $sClassName . '.php');

        return;
    }
    if (0 === strncmp($sClassName, 'ChBase', 6)) {
        require_once(CH_DIRECTORY_PATH_BASE . 'scripts/' . $sClassName . '.php');

        return;
    }
    if (0 === strncmp($sClassName, 'ChTempl', 7) && !class_exists($sClassName)) {
        if (isset($GLOBALS['iAdminPage']) && (int)$GLOBALS['iAdminPage'] == 1) {
            if (!defined('CH_WSB_TEMPLATE_DEFAULT_CODE')) {
                require_once(CH_DIRECTORY_PATH_CLASSES . 'ChWsbTemplate.php');
            }
            require_once(CH_DIRECTORY_PATH_ROOT . "templates/tmpl_" . CH_WSB_TEMPLATE_DEFAULT_CODE . "/scripts/" . $sClassName . '.php');
        } else {
            require_once(CH_DIRECTORY_PATH_ROOT . "templates/tmpl_{$GLOBALS['tmpl']}/scripts/" . $sClassName . '.php');
        }
 ...
 * @return unknown
 */
function ch_instance($sClassName, $aParams = array(), $aModule = array())
{
    if (isset($GLOBALS['chWsbClasses'][$sClassName])) {
        return $GLOBALS['chWsbClasses'][$sClassName];
    } else {
        ch_import((empty($aModule) ? $sClassName : str_replace($aModule['class_prefix'], '', $sClassName)), $aModule);

        if (empty($aParams)) {
            $GLOBALS['chWsbClasses'][$sClassName] = new $sClassName();
        } else {
            $sParams = "";
            ...
            $GLOBALS['chWsbClasses'][$sClassName] = eval("return new " . $sClassName . "(" . $sParams . ");");
        }
 ...
 * @param $iQuoteType - string escaping method: CH_ESCAPE_STR_AUTO(default), CH_ESCAPE_STR_APOS or CH_ESCAPE_STR_QUOTE
 * @return converted string / array
 */
function ch_js_string($mixedInput, $iQuoteType = CH_ESCAPE_STR_AUTO)
{
    $aUnits = array(
        "\n" => "\\n",
        "\r" => "",
 ...
 * @return converted string / array
 */
function ch_html_attribute($mixedInput)
{
    $aUnits = array(
        "\"" => "&quot;",
        "'"  => "&apos;",
 ...
 * @return converted string / array
 */
function ch_php_string_apos($mixedInput)
{
    return str_replace("'", "\\'", $mixedInput);
}

function ch_php_string_quot($mixedInput)
{
    return str_replace('"', '\\"', $mixedInput);
}
 ...
 */
function ch_file_get_contents($sFileUrl, $aParams = array(), $sMethod = 'get', $aHeaders = array(), &$sHttpCode = null)
{
    if ('post' != $sMethod) {
        $sFileUrl = ch_append_url_params($sFileUrl, $aParams);
    }
...
{
    $sFileName = CH_DIRECTORY_PATH_ROOT . 'tmp/log.txt';
 ...
 */
function ch_parse_time_duration($sContent)
{
...
function ch_encode_url_params($a, $aExcludeKeys = array(), $aOnlyKeys = false)
{
    $s = '';
...
function ch_clear_folder($sPath, $aExts = array())
{
    if (substr($$sPath, -1) == "/") {
...
function ch_ltrim_str($sString, $sPrefix, $sReplace = '')
{
...
function ch_member_ip_store($iMemberId, $sIP = false)
{
...
function ch_member_ip_get_last($iMemberId)
{
 ...
 */
function ch_show_service_unavailable_error_and_exit($sMsg = false, $iRetryAfter = 86400)
{
...
function ch_mkdir_r($sDirName, $rights = 0777)
{
    ...
    foreach ($aDirs as $sPart) {
        ...
        if (!is_dir(CH_DIRECTORY_PATH_ROOT . $sDir) && strlen(CH_DIRECTORY_PATH_ROOT . $sDir) > 0 && !file_exists(CH_DIRECTORY_PATH_ROOT . $sDir)) {
            if (!mkdir(CH_DIRECTORY_PATH_ROOT . $sDir, $rights)) {
                return false;
            }
        }
    }
 ...
 */
function ch_proto($sUrl = CH_WSB_URL_ROOT)
{
 ...
 */
function ch_linkify($text, $sAttrs = '', $bHtmlSpecialChars = false)
{
    if ($bHtmlSpecialChars) {
        $text = htmlspecialchars($text, ENT_NOQUOTES, 'UTF-8');
    }

    preg_match_all(CH_URL_RE, $text, $matches, PREG_OFFSET_CAPTURE);
 ...
 */
function ch_linkify_html($sHtmlOrig, $sAttrs = '')
{
    if (!trim($sHtmlOrig)) {
        return $sHtmlOrig;
    }

    $sId = 'ch-linkify-' . md5(microtime());
    $dom = new DOMDocument();
    ...
    foreach ($xpath->query('//text()') as $text) {
        ...
        $frag->appendXML(ch_linkify($text->nodeValue, $sAttrs, true));
 ...
 */
function ch_gen_method_name($s, $sWordsDelimiter = '_')
{
