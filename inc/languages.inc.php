...
define('CH_WSB_LANGUAGE_DEFAULT', 'en');
define('CH_WSB_LANGUAGE_CATEGORY_SYSTEM', 1);
...
function getCurrentLangName($isSetCookie = true)
{
    ...
    if( !$sLang ) $sLang = tryToGetLang( CH_WSB_LANGUAGE_DEFAULT );
...
function checkLangExists( $sLang )
{
    if (!preg_match('/^[A-Za-z0-9_]+$/', $sLang))
        return false;
    if( file_exists( CH_DIRECTORY_PATH_ROOT . "langs/lang-{$sLang}.php" ) )
        return true;
...
{
    return (int)db_value( "SELECT `ID` FROM `sys_localization_languages` WHERE `Name` = '" . process_db_input($sLangName, CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION) . "'" );
}
...
function deleteLanguage($langID = 0)
{
    ...
    @unlink( CH_DIRECTORY_PATH_ROOT . 'langs/lang-'.$arrLang['Name'].'.php');
...
function compileLanguage($langID = 0)
{
    ...
    while($arrLanguage = $resLangs->fetch()) {
    	...
    	if($arrLanguage['Name'] != CH_WSB_LANGUAGE_DEFAULT && getParam('lang_subst_from_en') == 'on') {
			$aKeysAll = getLocalizationKeysBy(array('type' => 'by_language_name_key_key', 'value' => CH_WSB_LANGUAGE_DEFAULT));
			$aKeys = array_merge($aKeysAll, $aKeys);
    	}

    	$handle = fopen( CH_DIRECTORY_PATH_ROOT . "langs/lang-{$arrLanguage['Name']}.php", 'w');
        if($handle === false)
        	return false;
...
/*                                                                           */
/*  This file is automatically generated each time the system and/or module  */
...
/*****************************************************************************/
        ...
        @chmod( CH_DIRECTORY_PATH_ROOT . "langs/lang-{$arrLanguage['Name']}.php", 0666);
    }

    return true;
}

function addStringToLanguage($langKey, $langString, $langID = -1, $categoryID = CH_WSB_LANGUAGE_CATEGORY_SYSTEM)
{
    // input validation
    $langID = (int)$langID;
    ...
    $langKey = process_db_input($langKey, CH_TAGS_STRIP);
    $langString = process_db_input($langString, CH_TAGS_VALIDATE);
...
function deleteStringFromLanguage($langKey, $langID = -1)
{
    ...
    $langKey = process_db_input($langKey, CH_TAGS_STRIP);
    $langString = empty($langString) ? '' : process_db_input($langString, CH_TAGS_VALIDATE);
...
function ch_lang_name()
{
    return $GLOBALS['sCurrentLanguage'];
}

function ch_lang_info()
{
