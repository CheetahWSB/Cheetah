...
ch_import("ChTempFormView");

class ChWsbFilesModule extends ChWsbModule
{
    var $_iProfileId;
    var $aPageTmpl;
    var $oPrivacy;
    ...
    function __construct(&$aModule)
    {
        ...
        $this->oAlbums       = new ChWsbAlbums($this->_oConfig->getMainPrefix(), $this->_iProfileId);
    ...
    function _deleteFile($iFileId)
    {
        ...
        if ($this->_oDb->deleteData($iFileId)) {
            ...
            $oVoting = new ChWsbVoting($this->_oConfig->getMainPrefix(), 0, 0);
            $oVoting->deleteVotings($iFileId);
            ch_import('ChWsbCmts');
            $oCmts = new ChWsbCmts($this->_oConfig->getMainPrefix(), $iFileId);
            ...
            $oAlert = new ChWsbAlerts($this->_oConfig->getMainPrefix(), 'delete', $iFileId, $this->_iProfileId, $aInfo);
            $oAlert->alert();
    ...
    function actionCategories()
    {
        ...
        $oCateg                    = new ChTemplCategoriesModule($aParam, _t('_categ_users'),
            CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories');
    ...
    function actionTags()
    {
        ...
        $oTags                     = new ChTemplTagsModule($aParam,
            _t('_' . $this->_oConfig->getMainPrefix() . '_bcaption_all'),
            CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'tags');
        $sCode                     = $oTags->getCode();
    ...
    function actionView($sUri)
    {
        ...
        if (!empty($aInfo)) {
            if ($aInfo['AllowAlbumView'] == CH_WSB_PG_HIDDEN || !$this->isAllowedView($aInfo)) {
                ...
                $this->aPageTmpl['js_name']  = 'ChWsbFiles.js';
        ...
        $GLOBALS['oTopMenu']->setCustomSubHeaderUrl(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'browse/album/' . $aInfo['albumUri'] . '/owner/' . $aInfo['NickName']);
    ...
    function actionBrowse(
        ...
        if ($sParamName == 'album' && $sParamValue1 == 'owner') {
            ...
            if (empty($aAlbumInfo)) {
                ...
                $GLOBALS['oTopMenu']->setCustomSubHeaderUrl(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'browse/album/' . $aAlbumInfo['Uri'] . '/owner/' . $sParamValue2);
                $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
                    _t('_' . $this->_oConfig->getMainPrefix()) => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
                    $aAlbumInfo['Caption']                     => '',
                ));
        ...
        if ($bAlbumView) {
            ...
            $oSearch->aCurrent['restriction']['not_allow_view']['value'] = array(CH_WSB_PG_HIDDEN);
        }
        ...
        $sCode = DesignBoxContent($sCaption, $sCode . $sPaginate, 1,
            $this->_oTemplate->getExtraTopMenu($aMenu, CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri()));
        if ($bAlbumView) {
    ...
    function actionEdit($iFileId)
    {
        ...
        $iFileId = (int)$iFileId > 0 ? (int)$iFileId : (int)ch_get('fileId');
        if (!$iFileId || !$bAjax) {
            return;
        }
        ...
        if (!$this->isAllowedEdit($aInfo)) {
            ...
            $sUrlPref = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri();
            $aForm    = array(
                'form_attrs' => array(
                    'id'       => $sLangPref . '_upload_form',
                    'method'   => 'post',
                    'action'   => $sUrlPref . 'edit/' . $iFileId,
                    'onsubmit' => "return ch_ajax_form_check(this)",
                ),
                'params'     => array(
                    'db'             => array(
                        'submit_name' => 'do_submit',
                    ),
                    'checker_helper' => 'ChSupportCheckerHelper',
                ),
                'inputs'     => array(
                    'header'      => array(
                        'type'    => 'block_header',
            ...
            if(getParam($sPrefix . '_cat_required') == '') {
            ...
            if ($oForm->isSubmittedAndValid()) {
				...
				if ($this->_oDb->updateData($iFileId, $aValues)) {
					...
					$oAlert = new ChWsbAlerts($sType, 'change', $iFileId, $this->_iProfileId, array('Info' => $this->_oDb->getFileInfo(array('fileId' => $iFileId), false, $aManageArray)));
					$oAlert->alert();
    ...
    function actionReport($sFileUri)
    {
        ...
        $sFileUrl  = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $sFileUri;
    ...
    function actionDelete($iFileId, $sAlbumUri = '', $sOwnerNick = '')
    {
        ...
        if ($this->_deleteFile($iFileId)) {
            ...
            $sRedirect = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . $sRedirectMain;
    ...
    function actionAlbums(
        ...
        ch_import('PageAlbumsOwner', $this->_aModule);
    ...
    function actionAlbumsViewMy($sParamValue = '', $sParamValue1 = '', $sParamValue2 = '', $sParamValue3 = '')
    {
        ...
        if ($sAction !== false) {
            ...
            if (!isLogged() && ch_get('oid') && ch_get('pwd')) { // in case of request from flash, cookies are not passed, and we have to set it explicitly
                $_COOKIE['memberID']       = ch_get('oid');
                $_COOKIE['memberPassword'] = ch_get('pwd');
                check_logged();
            }
        ...
        if (!$bNotAllowView && !empty($sParamValue1)) {
            ...
            if (!empty($aAlbumInfo)) {
                $bNotAllowView = $aAlbumInfo['AllowAlbumView'] == CH_WSB_PG_HIDDEN;
            }
        }
        if ($bNotAllowView) {
            $sKey  = _t('_' . $this->_oConfig->getMainPrefix() . '_access_denied');
        ...
        ch_import('PageAlbumsMy', $this->_aModule);
        ...
        ch_import($sClassPostfix, $this->_aModule);
    ...
    function actionAlbumDelete($sAlbumUri)
    {
        ...
        if (!$this->isAllowedDeleteAlbum($aAlbumInfo['ID'], $aAlbumInfo)) {
            ...
            if ($iCount > 0) {
                ...
                $sRedirect = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'albums/';
    ...
    function actionUploadSubmit()
    {
        ...
        $mixedAlbum = ch_get('album');
        if (is_numeric($mixedAlbum)) {
            $iAlbumId = (int)$mixedAlbum;
            if ($iAlbumId == 0) {
                $sTitle = clear_xss(ch_get('title'));
                if (empty($sTitle)) {
                ...
                $aNew     = array(
                    'caption'        => $sTitle,
                    'AllowAlbumView' => (int)ch_get('AllowAlbumView'),
                    'owner'          => $this->_iProfileId,
                );
    ...
    function checkDefaultAlbums($iProfileId)
    {
        ...
        foreach ($aAlbums as $sAlbum) {
            ...
            $this->oAlbums->addAlbum(array(
                'caption'        => $sAlbum,
                'owner'          => $iProfileId,
                'AllowAlbumView' => ($sAlbum == getParam('sys_album_default_name')) ? CH_WSB_PG_HIDDEN : $this->oAlbumPrivacy->_oDb->getDefaultValueModule($sUri,
                    'album_view'),
            ), false);
        }
    }
    ...
    function getAdminMainPage(&$aMenu, $sParam = '')
    {
        ...
        $oSearch->aCurrent['restriction']['not_allow_view']['value'] = CH_WSB_PG_HIDDEN;
        ...
        foreach ($aSections as $sKey => $aValue) {
            $aSections[$sKey]['href']  = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'administration/home/' . $sKey;
        ...
        if ($bAjaxMode) {
            $oSearch->aCurrent['restriction']['activeStatus']['value'] = process_db_input($sParam, CH_TAGS_STRIP);
            $sPostOwner                                                = ch_get('owner');
    ...
    function getAlbumPageView($aInfo, $sBrowseCode = '')
    {
        ...
        ch_import('PageAlbumView', $this->_aModule);
    ...
    function getAlbumPageBrowse($aParams)
    {
        ...
        if ($oSearch->aCurrent['paginate']['totalAlbumNum'] > 0) {
            ...
            $sLink     = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'albums' . $sArg;
            $oPaginate = new ChWsbPaginate(array(
                'page_url'           => $sLink . '&page={page}&per_page={per_page}',
    ...
    function getBlockActionsAlbum($aAlbumInfo)
    {
        ...
        $aReplacement = array(
            ...
            'moduleUrl' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri(),
        );
    ...
    function getInstanceUploadFormArray(
        ...
        $aAlbums['select'] = _t('_sys_album_select');
        ...
        $aForm = array(
            'form_attrs' => array(
                ...
                'method'   => 'post',
                'onSubmit' => 'return submit_quick_upload_form("' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . '", $(this).serialize());'
            ),
            'params'     => array(
                'db'             => array(
                    'submit_name' => 'submit',
    ...
    function getResultCodeArray(&$oSearch, $sCode)
    {
        ...
        if ($iCount > 0) {
            ...
            $sLink         = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'administration/home/' . $oSearch->aCurrent['restriction']['activeStatus']['value'];
            $sKeyWord      = ch_get('keyword');
            if ($sKeyWord !== false) {
    ...
    {
        $aEmails   = array();
        $sFileLink = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $sFileUri;
        switch ($sAction) {
            case 'share':
                $aEmails     = array(
                    'type'     => 'text',
        ...
        $aForm = array(
            'form_attrs' => array(
                'name'     => 'submitAction',
                'action'   => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . $sAction . '/' . $sFileUri,
                'method'   => 'post',
                'onsubmit' => "return ch_ajax_form_check(this)",
            ),
            'params'     => array(
    ...
    function serviceGetMemberMenuItem($sIcon = 'square-o')
    {
        ...
        $aLinkInfo   = array(
            ...
            'item_link'    => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'albums/my/',
            'item_onclick' => null,
    ...
    function serviceGetMemberMenuItemAddContent($sIcon = 'square-o')
    {
        ...
        $aLinkInfo   = array(
            ...
            'item_onclick' => "showPopupAnyHtml('" . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . "upload');",
            'item_title'   => _t('_' . $this->_oConfig->getMainPrefix()),
        );
    ...
    function serviceGetWallAddComment($aEvent, $aParams = array())
    {
        ...
        $oCmts = new ChTemplCmtsView($this->_oConfig->getMainPrefix(), $iItem);
        if (!$oCmts->isEnabled()) {
            return '';
        }
        ...
        $sTmplNameSnippet = isset($aParams['templates']['snippet']) ? $aParams['templates']['snippet'] : 'modules/cheetah/wall/|timeline_comment_files.html';

        return array(
            'title'       => _t('_ch_' . $sUri . '_wall_added_new_comment_title', $sOwner, $sTextWallObject),
            ...
            'content'     => $sCss . $this->_oTemplate->parseHtmlByName($sTmplName, array(
                    ...
                    'snippet'          => $this->_oTemplate->parseHtmlByName($sTmplNameSnippet, array(
                        ...
                        'cnt_item_title_attr'  => ch_html_attribute($aItem['title']),
    ...
    function getWallPost($aEvent, $sIcon = 'save', $aParams = array())
    {
        ...
        if ($iItems > 1) {
            ...
            $oAlbum     = new ChWsbAlbums($sPrefix);
            $aAlbumInfo = $oAlbum->getAlbumInfo(array('fileUri' => $sAlbumUri, 'owner' => $iOwner));

            $sTemplateName = isset($aParams['templates']['grouped']) ? $aParams['templates']['grouped'] : 'modules/cheetah/wall/|timeline_post_files_grouped.html';
        ...
        $sTemplateName = isset($aParams['templates']['single']) ? $aParams['templates']['single'] : 'modules/cheetah/wall/|timeline_post_files.html';
    ...
    function getWallPostOutline($aEvent, $sIcon = 'save', $aParams = array())
    {
        ...
        if ($iItems > 1) {
            ...
            $oAlbumCmts                   = new ChTemplCmtsView($sPrefixAlbum, $aAlbumInfo['ID']);
            ...
            $sTmplName          = isset($aParams['templates']['grouped']) ? $aParams['templates']['grouped'] : 'modules/cheetah/wall/|outline_item_image_grouped.html';
        ...
        $sTmplName          = isset($aParams['templates']['single']) ? $aParams['templates']['single'] : 'modules/cheetah/wall/|outline_item_image.html';
    ...
    function serviceGetSpyPost($sAction, $iObjectId = 0, $iSenderId = 0, $aExtraParams = array())
    {
        ...
        $aRet  = array(
            'params'       => array(
                ...
                'entry_url'        => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aInfo['medUri'],
    ...
    function serviceGetSubscriptionParams($sAction, $iEntryId)
    {
        ...
        return array(
            ...
            'template' => array(
                ...
                'ViewLink'     => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry['medUri'],
            ),
        );
    }
...
// support classes
class ChSupportCheckerHelper extends ChWsbFormCheckerHelper
{
