...
require_once( CH_DIRECTORY_PATH_CLASSES . 'ChWsbVotingQuery.php' );

define( 'CH_PERIOD_PER_VOTE', 7 * 86400 );
define( 'CH_OLD_VOTES', 365*86400 ); // votes older than this number of seconds will be deleted automatically

/**
 * Votings for any content
 *
 * Related classes:
 *  ChWsbVotingQuery - votings database queries
 *  ChBaseVotingView - votings base representation
 *  ChTemplVotingView - custom template representation
 *
 ...
 * ClassName - custom class name for HotOrNot @see ChWsbRate
 ...
 * $o = new ChTemplVotingView ('value of ObjectName field', $iYourEntryId);
 ...
 *
 * $o = new ChTemplVotingView ('value of ObjectName field', 0);
 * foreach ($aRecords as $aData)
 *     echo $o->getJustVotingElement(0, $aData['ID'], $aData['voting_rate']);
 *
 * Please note that you never need to use ChWsbVoting class directly, use ChTemplVotingView instead.
 * Also if you override votings class with your own then make it child of ChTemplVotingView class.
 *
 *
 *
 * Memberships/ACL:
...
class ChWsbVoting
{
    ...
    function __construct( $sSystem, $iId, $iInit = 1)
    {
        ...
        $this->_oQuery = new ChWsbVotingQuery($this->_aSystem);
    ...
    static function getObjectInstance($sSys, $iId, $iInit = true)
    {
        ...
        ch_import('ChTemplVotingView');
        $sClassName = 'ChTemplVotingView';
        if ($aSystems[$sSys]['override_class_name']) {
            require_once (CH_DIRECTORY_PATH_ROOT . $aSystems[$sSys]['override_class_file']);
    ...
    {
        if (isset($GLOBALS['ch_dol_voting_systems'])) {
            return $GLOBALS['ch_dol_voting_systems'];
        }

        $oCache = $GLOBALS['MySQL']->getDbCacheObject();

        $GLOBALS['ch_dol_voting_systems'] = $oCache->getData($GLOBALS['MySQL']->genDbCacheKey('sys_objects_vote'));

        if (null === $GLOBALS['ch_dol_voting_systems']) {
            ...
            $GLOBALS['ch_dol_voting_systems'] = array();
            while( $aRow = $rResult ->fetch() ) {
                $GLOBALS['ch_dol_voting_systems'][$aRow['ObjectName']] = array
                (
            ...
            $oCache = $GLOBALS['MySQL']->getDbCacheObject();
            $oCache->setData ($GLOBALS['MySQL']->genDbCacheKey('sys_objects_vote'), $GLOBALS['ch_dol_voting_systems']);
        }
    ...
    function makeVote ($iVote)
    {
		...
		$oZ = new ChWsbAlerts($this->_sSystem, 'rate', $this->getId(), getLoggedId(), array ('rate' => $iVote));
		$oZ->alert();
    ...
    function maintenance ()
    {
        ...
        foreach ($this->_aSystems as $aSystem) {
            ...
            $iDeletedRecords += $GLOBALS['MySQL']->query("DELETE FROM `{$aSystem['table_track']}` WHERE `{$sPre}date` < DATE_SUB(NOW(), INTERVAL " . CH_OLD_VOTES . " SECOND)");
    ...
    {
        if(strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') !== 0 || ch_get($this->_aSystem['post_name']) === false)
			return false;

        $iVote = (int)ch_get($this->_aSystem['post_name']);
        if(!$iVote)
			return false;
