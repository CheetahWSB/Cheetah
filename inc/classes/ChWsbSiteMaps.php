...
ch_import('ChWsbSiteMapsQuery');

define('CH_SITE_MAPS_URLS_PER_FILE', 20000);
define('CH_SITE_MAPS_FILES_PREFIX', 'sitemap_');
define('CH_SITE_MAPS_FILE_INDEX', 'sitemap.xml');
...
class ChWsbSiteMaps
{
    static protected $BASE_PATH = CH_DIRECTORY_PATH_CACHE_PUBLIC; // path to generated sitemaps
    static protected $BASE_URL = CH_WSB_URL_CACHE_PUBLIC; // url to generated sitemaps
    static protected $BASE_PATH_INDEX = CH_DIRECTORY_PATH_ROOT; // path to generated index sitemap
    static protected $BASE_URL_INDEX = CH_WSB_URL_ROOT; // url to generated index sitemap
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_oQuery = new ChWsbSiteMapsQuery($this->_aSystem);
    }
    ...
    {
        if (isset($GLOBALS['chWsbClasses']['ChWsbSiteMaps!'.$sObject]))
            return $GLOBALS['chWsbClasses']['ChWsbSiteMaps!'.$sObject];
        ...
        if (!empty($aObject['class_file']))
            require_once(CH_DIRECTORY_PATH_ROOT . $aObject['class_file']);
        else
            ch_import($sClass);

        $o = new $sClass($aObject);

        return ($GLOBALS['chWsbClasses']['ChWsbSiteMaps!'.$sObject] = $o);
    }
    ...
    static public function & getSystems () {
        if (!isset($GLOBALS['ch_dol_site_maps_systems']))
            $GLOBALS['ch_dol_site_maps_systems'] = ChWsbSiteMapsQuery::getAllActiveSystemsFromCache ();
        return $GLOBALS['ch_dol_site_maps_systems'];
    }
    ...
    static public function generateAllSiteMaps ()
    {
        ...
        if (!file_put_contents (self::$BASE_PATH_INDEX . CH_SITE_MAPS_FILE_INDEX, $sFileContents))
            return false;
    ...
    {
        if (file_exists(self::$BASE_PATH_INDEX . CH_SITE_MAPS_FILE_INDEX) && false === file_put_contents(self::$BASE_PATH_INDEX . CH_SITE_MAPS_FILE_INDEX, ''))
            return false;

        if (!($rHandler = opendir(self::$BASE_PATH)))
            return false;

        $l = strlen(CH_SITE_MAPS_FILES_PREFIX);
        while (($sFile = readdir($rHandler)) !== false)
            if (0 === strncmp($sFile, CH_SITE_MAPS_FILES_PREFIX, $l) && file_exists(self::$BASE_PATH . $sFile))
    ...
    {
        return self::$BASE_URL_INDEX . CH_SITE_MAPS_FILE_INDEX;
    }
    ...
    {
        return self::$BASE_PATH_INDEX . CH_SITE_MAPS_FILE_INDEX;
    }
    ...
    public function generate ()
    {
        ...
        do {
            ...
            $iFileIndex = round($iStart / CH_SITE_MAPS_URLS_PER_FILE);
            $sFileName = CH_SITE_MAPS_FILES_PREFIX . $this->_aSystem['object'] . '_' . $iFileIndex . '.xml';
            ...
            $iStart += CH_SITE_MAPS_URLS_PER_FILE;
    ...
    protected function _genXmlUrl ($a)
    {
        ...
        if (ChWsbRequest::serviceExists('pageac', 'is_url_accessable')) {
            if (!ChWsbService::call('pageac', 'is_url_accessable', array($sUrl)))
                return '';
        }
    ...
    {
        return $this->_oQuery->getRecords($this->_aQueryParts, $iStart, CH_SITE_MAPS_URLS_PER_FILE);
    }
