...
define('CH_WSB_TABLE_PROFILES', '`Profiles`');

require_once(CH_DIRECTORY_PATH_CLASSES . 'ChWsbParams.php');
...
class ChWsbDb
{
    ...
    */
    protected function __construct()
    {
        ...
        if (empty($GLOBALS['ch_db_param'])) {
            $GLOBALS['ch_db_param'] = new ChWsbParams($this);
        }

        $this->oParams = $GLOBALS['ch_db_param'];
    }
    ...
    public function res($sQuery, $aBindings = [], $bReplaying = false)
    {
        ...
        if (isset($GLOBALS['ch_profiler'])) {
            $GLOBALS['ch_profiler']->beginQuery($sQuery);
        }
        ...
        if (isset($GLOBALS['ch_profiler'])) {
            $GLOBALS['ch_profiler']->endQuery($oStmt);
        }
    ...
    public function getDbCacheObject()
    {
        if ($this->oDbCacheObject != null) {
            ...
            $this->oDbCacheObject = ch_instance('ChWsbCache' . $sEngine);
            if (!$this->oDbCacheObject->isAvailable()) {
                $this->oDbCacheObject = ch_instance('ChWsbCacheFile');
            }
