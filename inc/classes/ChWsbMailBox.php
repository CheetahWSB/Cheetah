 ...
 */
    ...
    define ('CH_MAILBOX_SEND_SUCCESS', 0);
    define ('CH_MAILBOX_SEND_FAILED', 1);
    define ('CH_MAILBOX_SEND_WAIT', 3);
    define ('CH_MAILBOX_SEND_BLOCKED', 5);
    define ('CH_MAILBOX_SEND_RECIPIENT_NOT_ACTIVE', 10);
    define ('CH_MAILBOX_SEND_UNKNOWN_RECIPIENT', 1000);
    define ('CH_MAILBOX_SEND_FAILED_MEMBERSHIP_DISALLOW', 1001);

    class ChWsbMailBox extends ChWsbPageView
    {
         ...
         */
         function __construct( $sPageName, &$aMailBoxSettings )
         {
            ...
            $aMailBoxSettings['mailbox_mode'] 	= process_db_input($aMailBoxSettings['mailbox_mode'], CH_TAGS_STRIP);
            $aMailBoxSettings['contacts_mode'] 	= process_db_input($aMailBoxSettings['contacts_mode'], CH_TAGS_STRIP);
            $aMailBoxSettings['sort_mode'] 		= process_db_input($aMailBoxSettings['sort_mode'], CH_TAGS_STRIP);

            if($aMailBoxSettings['messages_types']) {
                $aMailBoxSettings['messages_types'] = process_db_input($aMailBoxSettings['messages_types'],CH_TAGS_STRIP);
            }
          ...
          */
          function getArchivesList($sSqlLimit)
          {
               ...
               if ( $this -> aMailBoxSettings['messageID'] ) {
                    ...
                    if ( $iSecondPersonID  ) {
                         ...
                         "
                              SELECT
                                   DATE_FORMAT(`Date`, '" . getLocaleFormat(CH_WSB_LOCALE_DATE_SHORT, CH_WSB_LOCALE_DB) . "') AS `When`,
          ...
          function getFriendsList( $sSqlLimit )
          {
               ...
               "
                    SELECT p.*,
                    DATE_FORMAT(f.`When`, '" . getLocaleFormat(CH_WSB_LOCALE_DATE_SHORT, CH_WSB_LOCALE_DB) . "' ) AS 'When'
                    FROM (
          ...
          function getFavesList( $sSqlLimit )
          {
               ...
               "
                    SELECT
                         `Profiles`.*,
                         DATE_FORMAT(`sys_fave_list`.`When`, '" . getLocaleFormat(CH_WSB_LOCALE_DATE_SHORT, CH_WSB_LOCALE_DB) . "') AS `When`
                    FROM
                         `sys_fave_list`
                    INNER JOIN
                         `Profiles`
          ...
          function getContactedList( $sSqlLimit )
          {
               ...
               "
                    SELECT
                         DISTINCT `Profiles`.*,
                         DATE_FORMAT(`sys_messages`.`Date`, '" . getLocaleFormat(CH_WSB_LOCALE_DATE_SHORT, CH_WSB_LOCALE_DB) . "') AS `When`
                    FROM
                         `sys_messages`
                    INNER JOIN
                         `Profiles`
          ...
          function getMessages()
          {
               ...
               if (  $sWhereParameter ) {
                    ...
                    if ( $this -> iTotalMessageCount ) {
                         ...
                         "
                              SELECT
                                   `sys_messages`.`ID`, DATE_FORMAT(`sys_messages`.`Date`, '" . getLocaleFormat(CH_WSB_LOCALE_DATE, CH_WSB_LOCALE_DB) . "') AS `Date`,
          ...
          {
                  $iMessageID = (int) $iMessageID;
                $sField = process_db_input($sField, CH_TAGS_STRIP, CH_SLASHES_NO_ACTION);
          ...
          {
               $sMessageSubject = process_db_input($sMessageSubject, CH_TAGS_STRIP);
               $sMessageSubjectCopy = $GLOBALS['MySQL']->unescape ($sMessageSubject);

               $sMessageBody = process_db_input($sMessageBody, CH_TAGS_VALIDATE);
               $sCopyMessage = $GLOBALS['MySQL']->unescape ($sMessageBody);

               if(!$isSimulateSending && (!$sMessageSubject || !$sMessageBody)) {
                        $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_FAILED;
                        return MsgBox( _t('_please_fill_next_fields_first') );
               }

               // init some needed variables ;
               $sReturnMessage   = null;
               $sComposeUrl      =  CH_WSB_URL_ROOT . 'mail.php?mode=compose';

               // try to define member's ID ;
               $iRecipientID        = (int) getId($vRecipientID);
               if(!$iRecipientID) {
                    $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_UNKNOWN_RECIPIENT;
               ...
               $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_FAILED;
                ...
                if (!isAdmin($this -> aMailBoxSettings['member_id']) && db_arr($sQuery)) {
                    ...
                    $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_BLOCKED;
                    $bAllowToSend = false;
                }
                ...
                if (ch_is_spam($sCopyMessage)) {
                    $sReturnMessage = MsgBox( sprintf(_t("_sys_spam_detected"), CH_WSB_URL_ROOT . 'contact.php') );
                    $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_FAILED;
                    $bAllowToSend = false;
                }
                ...
                if ( $aRecipientInfo['Status'] != 'Active' ) {
                    ...
                    $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_RECIPIENT_NOT_ACTIVE;
                    $bAllowToSend   = false;
                }
                ...
                if(!$this -> isSendMessageAlowed($this -> aMailBoxSettings['member_id'], $isSimulateSending ? false : true) ) {
                    ...
                    $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_FAILED_MEMBERSHIP_DISALLOW;
                    $bAllowToSend   = false;
                }
                ...
                if ( !$isSimulateSending && $bAllowToSend ) {
                    ...
                    if ( db_res($sQuery ) ) {
                        ...
                        $this -> iSendMessageStatusCode = CH_MAILBOX_SEND_SUCCESS;
                        ...
                        $oZ = new ChWsbAlerts('profile', 'send_mail_internal'
        ...
        function isSendMessageAlowed($iMemberId, $isPerformAction = false)
        {
          ...
          $this -> _defineActions();
          $aCheck = checkAction($iMemberId, CH_SEND_MESSAGES, $isPerformAction);
