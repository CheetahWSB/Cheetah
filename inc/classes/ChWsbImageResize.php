 ...
 */
function ch_intervention_image_autoload($sClass)
{
    $sClass = trim($sClass, '\\');
    if (0 === strncmp('Intervention', $sClass, 12)) {
        $sFile = CH_DIRECTORY_PATH_PLUGINS . 'intervention-image/' . str_replace('\\', '/', $sClass) . '.php';
        if (file_exists($sFile))
            require_once($sFile);
    }
}
spl_autoload_register('ch_intervention_image_autoload');
...
if (!defined('CH_WSB_FILE_RIGHTS'))
    define('CH_WSB_FILE_RIGHTS', 0666);
...
class ChWsbImageResize
{
    ...
    {
        if (isset($GLOBALS['chWsbClasses'][get_class($this)]))
    ...
    {
        if(!isset($GLOBALS['chWsbClasses'][__CLASS__]))
            $GLOBALS['chWsbClasses'][__CLASS__] = new ChWsbImageResize();

        return $GLOBALS['chWsbClasses'][__CLASS__];
    }
    ...
    function getExifInfo($sSrcImage, $bCreateLocalFileIfUrl = true)
    {
        ...
        if ($bCreateLocalFileIfUrl && preg_match('/^https?:\/\//', $sSrcImage)) {
            $sTmpFileName = tempnam(CH_DIRECTORY_PATH_ROOT . 'tmp', '');
    ...
    function _grayscale ($sSrcImage, $sDstImage = '')
    {
        ...
        try {
            ...
            chmod($sDstImage ? $sDstImage : $sSrcImage, CH_WSB_FILE_RIGHTS);
        }
        catch (Exception $e) {
            $this->_sError = $e->getMessage();
            return IMAGE_ERROR_WRONG_TYPE;
