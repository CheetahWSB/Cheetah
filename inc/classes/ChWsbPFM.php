...
class ChWsbPFM
{
    ...
    {
        $this -> fillMyArrays();
        $this -> oJSONObject = new ChWsbPFMAreaJSONObj( $this );
    ...
    function genFieldEditForm( $iItemID )
    {
        ...
        ?>
        <table class="field_edit_tab ch-def-margin-sec-topbottom"> <!--Controls-->
            <tr>
                <td class="buttons" colspan="2">
                    <input type="submit" name="action-save" value="<?= ch_html_attribute(_t('_Save')) ?>" class="ch-btn ch-btn-small" />
        <?php

        if( $aField['Type'] != 'system' and $aField['Deletable'] ) {
            ?>
                    <input type="submit" name="action-delete" value="<?= ch_html_attribute(_t('_Delete')) ?>" onclick="return confirm('<?= $sSureDeleteItemC ?>');" class="ch-btn ch-btn-small" />
            <?php
        }
        ?>
                </td>
...
class ChWsbPFMAreaJSONObj
{
    function __construct( $oArea )
    {
        ...
        foreach( $oArea -> aBlocks as $iID => $sName )
            $this -> active_blocks[] = new ChWsbPFMItem( $iID, $sName );

        foreach( $oArea -> aItems as $iID => $aItem )
            $this -> active_items[] = new ChWsbPFMItem( $iID, $aItem[0], $aItem[1] );

        foreach( $oArea -> aBlocksInac as $iID => $sName )
            $this -> inactive_blocks[] = new ChWsbPFMItem( $iID, $sName );

        foreach( $oArea -> aItemsInac as $iID => $sName )
            $this -> inactive_items[] = new ChWsbPFMItem( $iID, $sName );
    }
}
...
class ChWsbPFMCacher
{
    ...
    function createCache()
    {
        ...
        ch_import('ChWsbPageViewAdmin'); // for clear caching abilities
        ...
        foreach ($this -> aAreasProps as $iAreaID => $aArea) {
            $oArea = new ChWsbProfileFieldsArea( $iAreaID, $this );
