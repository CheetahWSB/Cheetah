...
ch_import('ChWsbParams');
ch_import('ChWsbPageViewAdmin');
ch_import('ChWsbPFM');
ch_import('ChWsbModuleDb');

define("CH_WSB_INSTALLER_SUCCESS", 0);
define("CH_WSB_INSTALLER_FAILED", 1);
 ...
 * An object of the class is created automatically with Cheetah's modules installer.
 ...
 *
 */
class ChWsbInstaller extends ChWsbInstallerUtils
{
    var $_aConfig;
    var $_sBasePath;
    var $_sHomePath;
    ...
    function __construct($aConfig)
    {
        ...
        $this->_sBasePath = CH_DIRECTORY_PATH_MODULES;
    ...
    {
        if(!defined('CH_SKIP_INSTALL_CHECK') && !$GLOBALS['logged']['admin'])
        ...
        foreach($this->_aConfig[$sOperationName] as $sAction => $iEnabled) {
            ...
            //--- On Success ---//
            if((is_int($mixedResult) && (int)$mixedResult == CH_WSB_INSTALLER_SUCCESS) || (isset($mixedResult['code']) && (int)$mixedResult['code'] == CH_WSB_INSTALLER_SUCCESS)) {
    ...
    function actionCheckDependencies($bInstall = true)
    {
        ...
        return empty($sContent) ? CH_WSB_INSTALLER_SUCCESS : array('code' => CH_WSB_INSTALLER_FAILED, 'content' => $sContent);
    }
    ...
    function actionCheckPermissions($bInstall = true)
    {
        ...
        foreach($aPermissions as $sPermissions => $aFiles) {
            ...
            foreach($aFiles as $sFile)
                if(!ChWsbInstallerUtils::$sCheckFunction(ch_ltrim_str($this->_sModulePath . $sFile, CH_DIRECTORY_PATH_ROOT)))
                    $aResult[] = array('path' => $this->_sModulePath . $sFile, 'permissions' => $sCptPermissions);
        }

        return empty($aResult) ? CH_WSB_INSTALLER_SUCCESS : array('code' => CH_WSB_INSTALLER_FAILED, 'content' => $aResult);
    }
	...
	function actionChangePermissions($bInstall = true)
    {
        ...
        foreach($aPermissions as $sPermissions => $aFiles) {
            $sCheckFunction = 'is' . ucfirst($sPermissions);
            foreach($aFiles as $sFile) {
            	$sPath = ch_ltrim_str($this->_sModulePath . $sFile, CH_DIRECTORY_PATH_ROOT);
            	if(ChWsbInstallerUtils::$sCheckFunction($sPath))
            		continue;
		...
		ch_import('ChWsbFtp');
		$oFile = new ChWsbFtp($sFtpHost, getParam('sys_ftp_login'), getParam('sys_ftp_password'), getParam('sys_ftp_dir'));

		if(!$oFile->connect())
			return array('code' => CH_WSB_INSTALLER_FAILED, 'content_msg' => '_adm_txt_modules_wrong_permissions_change_cannot_connect_to_ftp', 'content_data' => $aResult);

		if(!$oFile->isCheetah())
			return array('code' => CH_WSB_INSTALLER_FAILED, 'content_msg' => '_adm_txt_modules_wrong_permissions_change_destination_not_valid', 'content_data' => $aResult);
        ...
        return empty($aResult) ? CH_WSB_INSTALLER_SUCCESS : array('code' => CH_WSB_INSTALLER_FAILED, 'content_msg' => '_adm_txt_modules_wrong_permissions_change', 'content_data' => $aResult);
    }
    ...
    function actionExecuteSql($bInstall = true)
    {
        ...
        if(!file_exists($sPath) || !($rHandler = fopen($sPath, "r")))
            return CH_WSB_INSTALLER_FAILED;
    ...
    function actionRecompileGlobalParamaters($bInstall = true)
    {
        ...
        return $bResult ? CH_WSB_INSTALLER_SUCCESS : CH_WSB_INSTALLER_FAILED;
    }
    function actionRecompileMainMenu($bInstall = true)
    {
        ob_start();
        $oChWsbMenu = new ChWsbMenu();
        $bResult = $oChWsbMenu->compile();
        ob_get_clean();
    ...
    function actionRecompileMemberMenu($bInstall = true)
    {
        ...
        $oMemberMenu = new ChWsbMemberMenu();
    ...
    {
        ob_start();
        $oPVCacher = new ChWsbPageViewCacher('sys_page_compose', 'sys_page_compose.inc');
    ...
    {
        ob_start();
        $oChWsbPFMCacher = new ChWsbPFMCacher();
        $bResult = $oChWsbPFMCacher -> createCache();
        ob_get_clean();
    ...
    function _addLanguage($aLanguage, $aLangInfo)
    {
        ...
        foreach ($aLanguage as $sKey => $sValue) {
            ...
            if (!$iExistedKey) { // key is missing, insert new key
                if (!$oDb->res("INSERT INTO `sys_localization_keys` VALUES (NULL, ?, ?)", [CH_WSB_LANGUAGE_CATEGORY_SYSTEM, $sDbKey]))
                    continue;
    ...
    function _recompileLanguageForAllModules($iLangId)
    {
        ...
        foreach ($aModules as $a) {
            $aConfig = false;
            $bInclude = @include(CH_DIRECTORY_PATH_MODULES . $a['path'] . 'install/config.php');
            if (!$bInclude || !$aConfig)
                continue;
