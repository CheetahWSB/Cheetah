 ...
 */
class ChWsbFormMedia extends ChTemplFormView
{
    var $_aMedia = array();
    ...
    function uploadPhotos ($sTag, $sCat, $sName = 'images', $sTitle = 'images_titles', $sTitleAlt = 'title')
    {
        ...
        foreach ($_FILES[$sName]['tmp_name'] as $i => $sUploadedFile) {
            ...
            $sTmpFile = CH_DIRECTORY_PATH_ROOT . 'tmp/i' . time() . $i . getLoggedId() . '.' . $aPathInfo['extension'];
    ...
    function uploadVideos ($sTag, $sCat, $sName = 'videos', $sTitle = 'videos_titles', $sTitleAlt = 'title')
    {
        ...
        foreach ($_FILES[$sName]['tmp_name'] as $i => $sUploadedFile) {
            ...
            $aPathInfo = pathinfo ($_FILES[$sName]['name'][$i]);
            $sTmpFile = CH_DIRECTORY_PATH_ROOT . 'tmp/v' . time() . $i . getLoggedId() . '.' . $aPathInfo['extension'];
            if (move_uploaded_file($sUploadedFile,  $sTmpFile)) {
                $iRet = ChWsbService::call('videos', 'perform_video_upload', array($sTmpFile, $aFileInfo, false), 'Uploader');
                @unlink ($sTmpFile);
                if ($iRet)
                    $aRet[] = $iRet;
            }
    ...
    function uploadSounds ($sTag, $sCat, $sName = 'sounds', $sTitle = 'sounds_titles', $sTitleAlt = 'title')
    {
        ...
        foreach ($_FILES[$sName]['tmp_name'] as $i => $sUploadedFile) {
            ...
            $aPathInfo = pathinfo ($_FILES[$sName]['name'][$i]);
            $sTmpFile = CH_DIRECTORY_PATH_ROOT . 'tmp/s' . time() . $i . getLoggedId() . '.' . $aPathInfo['extension'];
            if (move_uploaded_file($sUploadedFile,  $sTmpFile)) {
                $iRet = ChWsbService::call('sounds', 'perform_music_upload', array($sTmpFile, $aFileInfo, false), 'Uploader');
                @unlink ($sTmpFile);
                if ($iRet)
                    $aRet[] = $iRet;
            }
    ...
    function _getFilesInEntry ($sModuleName, $sServiceMethod, $sName, $sMediaType, $iIdProfile, $iEntryId)
    {
        ...
        foreach ($aReadyMedia as $iMediaId) {
            switch ($sModuleName) {
            case 'photos':
                $aRow = ChWsbService::call($sModuleName, $sServiceMethod, array($iMediaId, 'icon'), 'Search');
                break;
            case 'sounds':
                $aRow = ChWsbService::call($sModuleName, $sServiceMethod, array($iMediaId, 'browse'), 'Search');
                break;
            default:
                $aRow = ChWsbService::call($sModuleName, $sServiceMethod, array($iMediaId), 'Search');
            }
            ...
            if ($aDeletedMedia) {
                foreach ($aDeletedMedia as $iMediaId) {
                    if (!$this->_oDb->isMediaInUse($iMediaId, $sName))
                        ChWsbService::call($a['module'], 'remove_object', array($iMediaId));
                }
            }
        }
    ...
    function processMembershipChecksForMediaUploads (&$aInputs)
    {
        ...
        if (defined('CH_PHOTOS_ADD'))
            $aCheck = checkAction(getLoggedId(), CH_PHOTOS_ADD);
        if (!defined('CH_PHOTOS_ADD') || ($aCheck[CHECK_ACTION_RESULT] != CHECK_ACTION_RESULT_ALLOWED && !$isAdmin)) {
            unset($aInputs['thumb']);
        }

        $a = array ('images' => 'PHOTOS', 'videos' => 'VIDEOS', 'sounds' => 'SOUNDS', 'files' => 'FILES');
        foreach ($a as $k => $v) {
            if (defined("CH_{$v}_ADD"))
                $aCheck = checkAction(getLoggedId(), constant("CH_{$v}_ADD"));
            if ((!defined("CH_{$v}_ADD") || $aCheck[CHECK_ACTION_RESULT] != CHECK_ACTION_RESULT_ALLOWED) && !$isAdmin) {
