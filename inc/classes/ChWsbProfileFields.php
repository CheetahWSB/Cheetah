...
require_once( CH_DIRECTORY_PATH_CLASSES . 'ChWsbPrivacy.php' );

class ChWsbProfileFields extends Thing
{
    var $iAreaID;
    ...
    {
        ch_import('ChWsbForm');
        return (new ChWsbFormCheckerHelper)->checkCaptcha($mValue);
    }
        ...
        switch( $aItem['Type'] ) {
            ...
            case 'system':
                switch( $aItem['Name'] ) {
                    ...
                    case 'DateLastLogin':
                    case 'DateLastNav':
                        return $this -> getViewableDate($sValue, CH_WSB_LOCALE_DATE);
     ...
     * @return string
     */
    function getViewableDate($sDate, $iFormat = CH_WSB_LOCALE_DATE_SHORT)
    {
    ...
    function getFormCode($aParams = null)
    {
        ...
        $sCustomHtmlBefore = '';
        $sCustomHtmlAfter = '';
        $oAlert = new ChWsbAlerts('profile', 'show_profile_form', 0, 0, array('oProfileFields' => $this, 'oForm' => $oForm, 'sCustomHtmlBefore' => &$sCustomHtmlBefore, 'sCustomHtmlAfter' => &$sCustomHtmlAfter));
        $oAlert->alert();
                            ...
                            case 'Location':
                                ...
                                $aFormInput['content'] = <<<EOF
                                    <div class="location_wrapper">
                                        <div>
                                            <input type="text" name="distance" class="form_input_distance ch-def-round-corners-with-border ch-def-font" />
                                        ...
                                        <div>
                                            $sFromZipC
                                            <input type="text" name="zip" class="form_input_zip ch-def-round-corners-with-border ch-def-font" />
                                        </div>
                                    </div>
EOF;
                            break;
    ...
    function getFormJoin($aParams)
    {
        ...
        */
        ...
        $aForm = array(
            'form_attrs' => array(
                'name'     => 'join_form',
                'action'   => CH_WSB_URL_ROOT . 'join.php',
                'method'   => 'post',
    ...
    function getFormEdit($aParams)
    {
        ...
        */
        ...
        $aForm = array(
            'form_attrs' => array(
                'name'     => 'edit_form',
                'action'   => CH_WSB_URL_ROOT . 'pedit.php?ID=' . $iProfileID,
                'method'   => 'post',
                ...
                switch ($aItem['Name']) {
					...
					case 'Agree':
                        $aInput = array(
                            'type' => 'custom',
                            'colspan' => true,
                            'content' => _t('_join_form_note', CH_WSB_URL_ROOT) . '<input type="hidden" name="Agree" />',
                        );
                        $aItem['Caption'] = '';
                    break;
                }
