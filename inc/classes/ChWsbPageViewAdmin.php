...
class ChWsbPageViewAdmin
{
    ...
    function __construct( $sDBTable, $sCacheFile )
    {
        ...
        if (strlen($sPage) && in_array($sPage, $this->aPages)) {
            $this->oPage = new ChWsbPVAPage( $sPage, $this );
        }
        ...
        $_page = array(
            ...
            'js_name' => array('jquery.ui.core.min.js', 'jquery.ui.widget.min.js', 'jquery.ui.mouse.min.js', 'jquery.ui.sortable.min.js', 'jquery.ui.slider.min.js', 'jquery.cookie.min.js', 'ChWsbPageBuilder.js'),
    ...
    {
        $sPageWidth = process_db_input( $sPageWidth, CH_TAGS_STRIP );
    ...
    {
        $oCacher = new ChWsbPageViewCacher( $this -> sDBTable, $this -> sCacheFile );
        $oCacher -> createCache();
    ...
    {
        $sPageName = process_db_input($sPageName, CH_TAGS_STRIP, CH_SLASHES_AUTO);
        ...
        $oZ = new ChWsbAlerts('page_builder', 'page_delete', 0, 0, array('uri' => $sPageName));
        $oZ->alert();
    ...
    function deleteBlock( $iBlockID )
    {
        ...
        $oZ = new ChWsbAlerts('page_builder', 'block_delete', $iBlockID, 0, array('page_uri' => $aBlock['Page']));
        $oZ->alert();
    ...
    function resetPage()
    {
        if( $this -> oPage -> bResetable ) {
            ...
            $oZ = new ChWsbAlerts('page_builder', 'page_reset', 0, 0, array('uri' => $this -> sPage_db));
            $oZ->alert();
        }
    ...
    function saveItem( $aData )
    {
        ...
        $sCaption = process_db_input($aData['Caption'], CH_TAGS_STRIP);
        ...
        $oZ = new ChWsbAlerts('page_builder', 'block_form_save', $iID, 0, array('data' => $aData, 'caption_key' => &$sCaption));
        $oZ->alert();
    ...
    function showBuildZone()
    {
        ...
        if($oEditor) {
            ...
            $sEditorCode = $oEditor->attachEditor('#' . $sEditorId, CH_EDITOR_FULL);
        }
        ...
        return $GLOBALS['oAdmTemplate']->parseHtmlByName('pbuilder_content.html', array(
            ...
            'ch_if:page' => array(
                ...
                'content' => array(
                    ...
                    'parser_url' => ch_html_attribute($_SERVER['PHP_SELF']),
    ...
    function getPageSelector()
    {
        ...
        return $GLOBALS['oAdmTemplate']->parseHtmlByName('pbuilder_cpanel.html', array(
            'ch_repeat:pages' => $aPages,
            'ch_if:show_add_column' => array(
                'condition' => $bPage,
                'content' => array()
            ),
            'url' => ch_html_attribute($_SERVER['PHP_SELF'])
    ...
    function showPropForm($iBlockID)
    {
        ...
        if( $aItem['Func'] == 'Echo' ) {
        	$sMceEditorKey = 'ch_mce_editor_disabled';
        ...
        $oZ = new ChWsbAlerts('page_builder', 'block_form_display', $iBlockID, 0, array('form' => &$aForm));
        $oZ->alert();
        ...
        $oForm = new ChTemplFormView(array(
            ...
            'inputs' => array(
                ...
                array(
                    ...
                    'info' => _t('_adm_pbuilder_uri_info', CH_WSB_URL_ROOT . 'page/newpage'),
                ),
                array(
                    'type' => 'text',
                    'name' => 'title',
...
class ChWsbPVAPage
{
    ...
    function __construct( $sPage, &$oParent )
    {
        ...
        /* @var $this->oParent ChWsbPageViewAdmin */
        $this -> oParent = &$oParent;

        $this -> sDefaultSqlFile = CH_DIRECTORY_PATH_ROOT . "{$admin_dir}/default_builders/{$this -> oParent -> sDBTable}_{$this -> sName}.sql";
    ...
    function getJSON()
    {
        $oPVAPageJSON = new ChWsbPVAPageJSON( $this );
...
class ChWsbPageViewCacher
{
    ...
    function getBlocksCacheObject ()
    {
        if ($this->oBlocksCacheObject != null) {
            ...
            $this->oBlocksCacheObject = ch_instance ('ChWsbCache'.$sEngine);
            if (!$this->oBlocksCacheObject->isAvailable())
                $this->oBlocksCacheObject = ch_instance ('ChWsbCacheFile');
            return $this->oBlocksCacheObject;
        }
    }
