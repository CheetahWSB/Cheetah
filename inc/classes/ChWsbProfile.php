...
class ChWsbProfile
{
    ...
    function getProfileData()
    {
        ...
        $oPDb = new ChWsbProfileQuery();
        $sProfileCache = CH_DIRECTORY_PATH_CACHE . 'user' . $this -> _iProfileID . '.php';
        ...
        if( $this -> _aProfile['Couple'] ) {
            ...
            $sProfileCache = CH_DIRECTORY_PATH_CACHE . 'user' . $this -> _iCoupleID . '.php';
            if( $bUseCacheSystem && file_exists( $sProfileCache ) && is_file( $sProfileCache ) ) {
                require_once($sProfileCache);
    ...
    {
        $oProfileQuery = new ChWsbProfileQuery();
    ...
    function getID( $vID = false, $bWithEmail = 1 )
    {
        ...
        if ( $bWithEmail ) {
            ch_import('ChWsbForm');
            if (ChWsbFormCheckerHelper::checkEmail($vID)) {
