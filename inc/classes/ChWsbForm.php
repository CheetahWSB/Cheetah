...
define('CH_WSB_FORM_METHOD_GET', 'get');
define('CH_WSB_FORM_METHOD_POST', 'post');
...
class ChWsbForm
{
    ...
    function __construct ($aInfo)
    {
        ...
        if (!isset($this->aFormAttrs['method']))
            $this->aFormAttrs['method'] = CH_WSB_FORM_METHOD_GET;
        ...
        $oZ = new ChWsbAlerts('form', 'init', 0, 0, array(
            'form_object' => $this,
    ...
    {
        $oChecker = new ChWsbFormChecker($this->_sCheckerHelper);
        ...
        $oZ = new ChWsbAlerts('form', 'init_checker', 0, 0, array(
            'values' => $aValues,
    ...
    function insert ($aValsToAdd = array())
    {
        ...
        $oZ = new ChWsbAlerts('form', 'insert_data', 0, 0, array(
    ...
    function update ($val, $aValsToAdd = array())
    {
        ...
        $oZ = new ChWsbAlerts('form', 'update_data', 0, 0, array(
            'val' => $val,
            'vals_to_add' => $aValsToAdd,
            'checker_object' => $oChecker,
            'form_object' => $this,
    ...
    {
        return ChWsbForm::getSubmittedValue($this->aParams['db']['submit_name'], $this->aFormAttrs['method']) ? true : false;
    }
    ...
    {
        $aData = array();
        if($sMethod == CH_WSB_FORM_METHOD_GET)
            $aData = &$_GET;
        else if($sMethod == CH_WSB_FORM_METHOD_POST)
            $aData = &$_POST;
...
class ChWsbFormChecker
{
    ...
    {
        $this->_sFormMethod = CH_WSB_FORM_METHOD_GET;
        $this->_bFormCsrfChecking = true;

        $sCheckerName = !empty($sHelper) ? $sHelper : 'ChWsbFormCheckerHelper';
    ...
    function check (&$aInputs)
    {
        ...
        if($GLOBALS['MySQL']->getParam('sys_security_form_token_enable') == 'on' && !defined('CH_WSB_CRON_EXECUTE') && $this->_bFormCsrfChecking === true && ($mixedCsrfTokenSys = ChWsbForm::getCsrfToken()) !== false) {
            $mixedCsrfTokenUsr = ChWsbForm::getSubmittedValue('csrf_token', $this->_sFormMethod);
        ...
        foreach ($aInputs as $k => $a) {
            ...
            $val = ChWsbForm::getSubmittedValue($a['name'], $this->_sFormMethod);
            if($val === false)
        ...
        if (!$iErrors && ('on' == getParam('sys_uridnsbl_enable') || 'on' == getParam('sys_akismet_enable'))) {
            ...
            foreach ($aInputs as $k => $a) {
                ...
                ++$iErrors;
                $aInputs[$k]['error'] = sprintf(_t("_sys_spam_detected"), CH_WSB_URL_ROOT . 'contact.php');
    ...
    function get ($sName, $sPass = 'Xss', $aParams = array(), $sType = '')
    {
        if (!$sPass)
            $sPass = 'Xss';
        $this->_oChecker;
        $val = ChWsbForm::getSubmittedValue($sName, $this->_sFormMethod);
...
class ChWsbFormCheckerHelper
{
    ...
    public static function passXss ($s)
    {
        if (is_array($s)) {
            $a = array ();
            foreach ($s as $k => $v) {
                $a[$k] = process_db_input ($v, CH_TAGS_STRIP);
            }
            return $a;
        }
        return process_db_input ($s, CH_TAGS_STRIP);
    }
    ...
    public static function passXssHtml ($s)
    {
        if (is_array($s)) {
            $a = array ();
            foreach ($s as $k => $v) {
                $a[$k] = process_db_input ($v, CH_TAGS_VALIDATE);
            }
            return $a;
        }
        return process_db_input ($s, CH_TAGS_VALIDATE);
    }
    ...
    public static function passAll ($s)
    {
        if (is_array($s)) {
            $a = array ();
            foreach ($s as $k => $v) {
                $a[$k] = process_db_input ($v, CH_TAGS_NO_ACTION);
            }
            return $a;
        }
        return process_db_input ($s, CH_TAGS_NO_ACTION);
    }
