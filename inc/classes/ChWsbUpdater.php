...
class ChWsbUpdater extends ChWsbInstaller
{
    ...
    function update($aParams)
    {
        ...
        if($aResult['result']) {
            ...
            $aFiles = array();
            $this->_hash(CH_DIRECTORY_PATH_ROOT . 'modules/' . $this->_aConfig['module_dir'], $aFiles);
            foreach($aFiles as $aFile)
    ...
    function actionUpdateFiles($bInstall = true)
    {
        ...
        return $oFtp->copy($sPath . '*', 'modules/' . $this->_aConfig['module_dir']) ? CH_WSB_INSTALLER_SUCCESS : CH_WSB_INSTALLER_FAILED;
    }
    function actionUpdateLanguages($bInstall = true)
    {
        ...
        if(!file_exists($sModuleConfig))
            return array('code' => CH_WSB_INSTALLER_FAILED, 'content' => '_adm_txt_modules_module_config_not_found');
