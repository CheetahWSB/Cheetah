 ...
 */
class ChWsbTwigModuleDb extends ChWsbModuleDb
{
    ...
    {
        return $this->getOne ("SELECT `{$this->_sFieldId}` FROM `" . $this->_sPrefix . $this->_sTableMain . "` WHERE `{$this->_sFieldStatus}` = 'approved' AND `{$this->_sFieldAllowViewTo}` = '" . CH_WSB_PG_ALL . "' LIMIT 1");
    }
    ...
    function getLatestFeaturedItem ()
    {
        ...
        WHERE `{$this->_sFieldStatus}` = ? AND `{$this->_sFieldAllowViewTo}` = ? $sWhere ORDER BY `{$this->_sFieldCreated}` DESC LIMIT 1", ['approved', CH_WSB_PG_ALL]);
    }
    ...
    function deleteEntryMediaAll ($iEntryId, $sMediaType)
    {
        ...
        foreach ($a as $iMediaId)
            ChWsbService::call(('images' == $sMediaType ? 'photos' : $sMediaType), 'remove_object', array($iMediaId));
    ...
    {
        $sForumTitle = process_db_input($aDataEntry[$this->_sFieldTitle], CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION);
        $sUsername = process_db_input($sUsername, CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION);
    ...
    function deleteForum ($iEntryId)
    {
        ...
        require_once (CH_DIRECTORY_PATH_CLASSES . 'Thing.php');
        if (!class_exists('ThingPage'))
            require_once (CH_DIRECTORY_PATH_MODULES . 'cheetah/forum/classes/ThingPage.php');
        if (!class_exists('Mistake'))
            require_once (CH_DIRECTORY_PATH_MODULES . 'cheetah/forum/classes/Mistake.php');
        if (!class_exists('ChDb'))
            require_once (CH_DIRECTORY_PATH_MODULES . 'cheetah/forum/classes/ChDb.php');
        if (!class_exists('DbAdmin'))
            require_once (CH_DIRECTORY_PATH_MODULES . 'cheetah/forum/classes/DbAdmin.php');
    ...
    {
        if ($isProcessDbInput)
            $sNick = process_db_input ($sNick, CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION);
