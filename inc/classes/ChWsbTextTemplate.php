...
class ChWsbTextTemplate extends ChWsbModuleTemplate
{
    var $_oModule;
    ...
    function setModule(&$oModule)
    {
        ...
        $this->oPaginate = new ChWsbPaginate(array(
    ...
    function displayBlockInfo($aEntry, $sFields = '')
    {
        ...
        return $this->parseHtmlByName('entry_info.html', array (
            ...
            'date' => getLocaleDate($aEntry['date'], CH_WSB_LOCALE_DATE_SHORT),
    ...
    function displayItem($aParams, &$aEntry)
    {
        ...
        $aTmplVars = array(
            ...
            'ch_if:status' => array(
                'condition' => $iViewerType == CH_TD_VIEWER_TYPE_ADMIN,
                'content' => array(
                    'status' => _t('_' . $sModuleUri . '_status_' . $aEntry['status'])
                ),
            ),
            'ch_if:featured' => array(
                'condition' => $iViewerType == CH_TD_VIEWER_TYPE_ADMIN && (int)$aEntry['featured'] == 1,
                'content' => array(),
            ),
            'ch_if:edit_link' => array (
                'condition' => $iViewerType == CH_TD_VIEWER_TYPE_ADMIN,
                'content' => array(
                    'edit_link_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'admin/' . $aEntry['uri'],
...
<?php
    ...
    {
        return $this->_parseAnything ($s, ',', CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'tag/');
    }

    function parseCategories ($s)
    {
        ch_import ('ChWsbCategories');
        return $this->_parseAnything ($s, CATEGORIES_DIVIDER, CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'category/');
    }
