...
define('CH_WSB_UPLOADER_EP_PREFIX', 'extra_param_');

class ChWsbFilesUploader extends ChWsbTemplate
{
    var $_iOwnerId;
    var $_sJsPostObject;
    var $sWorkingFile;
    ...
    function __construct($sUploadTypeNC = 'Common')
    {
        ...
        $GLOBALS['oSysTemplate']->addJsTranslation(array(
            '_ch_' . $this->sUploadTypeLC . 's_val_title_err',
            '_ch_' . $this->sUploadTypeLC . 's_val_descr_err'
        ));
    ...
    function _addHidden($sPostType = "photo", $sContentType = "upload", $sAction = "post", $iIndex = 1)
    {
        ...
        foreach ($this->_aExtras as $sKey => $mixedValue) {
            $aResult[CH_WSB_UPLOADER_EP_PREFIX . $sKey] = array(
                'type'  => 'hidden',
                'name'  => CH_WSB_UPLOADER_EP_PREFIX . $sKey,
                'value' => $mixedValue
            );
        }
    ...
    function _getExtraParams(&$aRequest)
    {
        ...
        foreach ($aRequest as $sKey => $sValue) {
            if (strpos($sKey, CH_WSB_UPLOADER_EP_PREFIX) !== false) {
                $aParams[str_replace(CH_WSB_UPLOADER_EP_PREFIX, '', $sKey)] = $sValue;
            }
        }
    ...
    function _updateExtraParams($aExtra, $iFileId, $iAuthorId)
    {
        ...
        $oAlbums = new ChWsbAlbums('ch_' . $this->sUploadTypeLC . 's', $iAuthorId);
    ...
    function _GenMainAddCommonForm($aExtras = array(), $aUploaders = array())
    {
        ...
        if (array_key_exists($sMode, $aUplMethods)) {
            if (is_array($aUplMethods[$sMode])) {
                ...
                $sForm = ChWsbService::callArray($aUplMethods[$sMode]);
        ...
        <script src="__modules_url__cheetah/__upload_type__s/js/upload.js" type="text/javascript"
                language="javascript"></script>
        <script type="text/javascript">
            var __js_post_object__ = new Ch__upload_type_nc__Upload({
    ...
    function _getEmbedFormFile()
    {
        $aForm = array(
            ...
            'inputs'     => array(
                'header1'       => array(
                    'type'    => 'block_header',
                    'caption' => _t('_ch_' . $this->sUploadTypeLC . 's_embed')
                ),
                'embed'         => array(
                    'type'     => 'text',
                    'name'     => 'embed',
                ...
                'example'       => array(
                    'type'    => 'custom',
                    'name'    => 'example',
                    'content' => _t('_ch_' . $this->sUploadTypeLC . 's_Embed_example'),
                ),
                'hidden_action' => array(
                    'type'  => 'hidden',
                    'name'  => 'action',
        ...
        //--- Process Extras ---//
        foreach ($this->_aExtras as $sKey => $mixedValue) {
            $aForm['inputs'][CH_WSB_UPLOADER_EP_PREFIX . $sKey] = array(
                'type'  => 'hidden',
                'name'  => CH_WSB_UPLOADER_EP_PREFIX . $sKey,
                'value' => $mixedValue
            );
    ...
    function _getRecordFormFile($sCustomRecorderObject = '', $aExtras = array())
    {
        $aForm = array(
            ...
            'inputs'     => array(
                'header1'       => array(
                    'type'    => 'block_header',
                    'caption' => _t('_ch_' . $this->sUploadTypeLC . 's_record')
                ),
                'record'        => array(
                    'type'    => 'custom',
                    'name'    => 'file',
    ...
    {
        return '<script src="' . CH_WSB_URL_ROOT . 'plugins/jquery/jquery.js" type="text/javascript" language="javascript"></script>';
    }

    function embedReadUrl($sUrl)
    {
        return ch_file_get_contents($sUrl);
    }
    ...
    {
        if (!$this->_iOwnerId) {
            return $this->_getAuthorId() ? "" : '<script type="text/javascript">alert("' . ch_js_string(_t('_LOGIN_REQUIRED_AE1')) . '");</script>';
        }
    }
    ...
    function getUploadFormHtml5Files()
    {
        $aForm = array(
            'form_attrs' => array(
                'id'      => $this->sUploadTypeLC . '_upload_form',
                'name'    => 'upload',
                'action'  => ch_append_url_params($this->sWorkingFile, array('action' => 'accept_multi_html5')),
                'method'  => 'post',
                'enctype' => 'multipart/form-data',
                'target'  => $this->sUploadTypeLC . '_upload_frame'
            ),
    ...
    function getUploadHtml5File($aExtras)
    {
        ...
        foreach ($this->_aExtras as $sKey => $mixedValue) {
            $aCustomFormData[CH_WSB_UPLOADER_EP_PREFIX . $sKey] = $mixedValue;
        }

        $aVars = array(
            'upload_type'            => $this->sUploadTypeLC,
            'form'                   => $this->getUploadFormHtml5Files(),
            'static_path'            => parse_url(CH_WSB_URL_PLUGINS, PHP_URL_PATH),
            'plugins_url'            => CH_WSB_URL_PLUGINS,
            'preview_size'           => 240,
            'action_url'             => ch_append_url_params($this->sWorkingFile,
            ...
            'max_file_size_exceeded' => ch_js_string(_t('_sys_txt_upload_size_error',
                _t_format_size($this->iMaxFilesize)), CH_ESCAPE_STR_APOS),
        );
    ...
    function serviceAcceptHtml5File()
    {
        ...
        require_once(CH_DIRECTORY_PATH_PLUGINS . 'file-api/server/FileAPI.class.php');
    ...
    function serviceAcceptHtml5FilesInfo()
    {
        ...
        $sPattern = 'title-';
        $sOutput  = "<script>parent.$(parent.document).trigger('ch-files-cleanup');</script>";
    ...
    function getUploadFormFile($aExtras)
    {
        ...
        $aForm      = array(
            'form_attrs' => array(
                'id'      => $this->sUploadTypeLC . '_upload_form',
                'name'    => 'upload',
                'action'  => ch_append_url_params($this->sWorkingFile,
            ...
            'inputs'     => array(
                'header1'   => array(
                    'type'    => 'block_header',
                    'caption' => _t('_ch_' . $this->sUploadTypeLC . 's_upload')
                ),
                'browse'    => array(
                    'type'     => 'file',
                    'name'     => 'file[]',
                    'caption'  => _t('_ch_' . $this->sUploadTypeLC . 's_browse'),
                    'required' => true,
                    'attrs'    => array(
    ...
    function _GenSendFileInfoForm(
        ...
        $aFormCategories['categories']             = $oCategories->getGroupChooser('ch_' . $this->sUploadTypeLC . 's',
            $this->_iOwnerId, true);
        $aFormCategories['categories']['required'] = false;
        $sKey                                      = 'album';
        $aAlbums                                   = array();
        if ($this->_aExtras[$sKey] != '') {
            $aAlbums[CH_WSB_UPLOADER_EP_PREFIX . $sKey] = array(
                'type'  => 'hidden',
                'name'  => CH_WSB_UPLOADER_EP_PREFIX . $sKey,
            ...
            $oAlbum     = new ChWsbAlbums('ch_' . $this->sUploadTypeLC . 's');
        ...
        $aForm['inputs'][] = array(
            ...
            1         => array(
                'type'    => 'button',
                'name'    => 'delete',
                'value'   => _t('_ch_' . $this->sUploadTypeLC . 's_admin_delete'),
                'colspan' => true,
                'attrs'   => array(
        ...
        return "<script src='" . CH_WSB_URL_ROOT . "inc/js/jquery.webForms.js' type='text/javascript' language='javascript'></script><script type='text/javascript'>parent." . $this->_sJsPostObject . ".genSendFileInfoForm('" . $iFileID . "', '" . $sForm . "'); parent." . $this->_sJsPostObject . "._loading(false);</script>";
    }
    ...
    function serviceAcceptUpload($sAction)
    {
        ...
        foreach ($aUploaders as $k => $r) {
            ...
            if (is_array($r['handle'])) {
                $sCode = ChWsbService::callArray($r['handle']);
    ...
    function initFile($iMedID, $sTitle, $sCategories = '', $sTags = '', $sDesc = '', $aCustom = array())
    {
        ...
        // If title is empty, generate a uri from microtime.
        if($sTitle == '') {
    ...
    function alertAdd($iMedID, $bCheckPrivacy = false)
    {
        ...
        if (!$bCheckPrivacy || !isset($aExtra['privacy_view']) || (int)$aExtra['privacy_view'] != (int)CH_WSB_PG_HIDDEN) {
            $oZ = new ChWsbAlerts($this->oModule->_oConfig->sPrefix, 'add', $iMedID, $this->_iOwnerId, $aExtra);
            $oZ->alert();
        }
    }
    ...
    function getFileAddError($sMessage = '')
    {
        ...
        return '<script type="text/javascript">alert("' . ch_js_string($sMessage) . '"); parent.' . $this->_sJsPostObject . '._loading(false);</script>';
    }
