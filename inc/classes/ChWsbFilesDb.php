...
require_once('ChWsbModuleDb.php');

class ChWsbFilesDb extends ChWsbModuleDb
{
    var $_oConfig;
    ...
    function getFilesByMonth ($iYear, $iMonth, $iNextYear, $iNextMonth, $sStatus = 'approved')
    {
        ...
        $sStatus = process_db_input($sStatus, CH_TAGS_STRIP);
        ...
        if ($this->iViewer)
            $sqlQuery .= "<> " . CH_WSB_PG_HIDDEN;
        else
            $sqlQuery .= "= " . CH_WSB_PG_ALL;
    ...
    function _setData ($aData, $iFileId = 0)
    {
        ...
        foreach ($aData as $sKey => $sValue) {
            if (array_key_exists($sKey, $this->aFileFields))
               $sqlQuery .= "`{$this->aFileFields[$sKey]}`='" . process_db_input($sValue, CH_TAGS_STRIP) . "', ";
        }
    ...
    function getFilesCountByParams (&$aParams)
    {
        ...
        foreach ($aParams as $sField => $mixedParam) {
            $sParam = process_db_input($sParam, CH_TAGS_STRIP);
            switch ($sField) {
                case 'albumID':
                    $oAlbum = new ChWsbAlbums($this->_oConfig->getMainPrefix(), $this->iViever);
    ...
    function getFileInfo ($aIdent, $bSimple = false, $aFields = array())
    {
        ...
        if (isset($aIdent['fileUri']))
            $sqlCondition = "`{$this->sFileTable}`.`{$this->aFileFields['medUri']}`='" . process_db_input($aIdent['fileUri'], CH_TAGS_STRIP) . "'";
        ...
        if (!$bSimple) {
            // album joins
            $oAlbum = new ChWsbAlbums($this->_oConfig->getMainPrefix());
            $sqlAlbumJoin = "
    ...
    {
        $sArg = process_db_input($sArg, CH_TAGS_STRIP);
