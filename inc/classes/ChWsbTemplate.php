...
define('CH_WSB_TEMPLATE_DEFAULT_CODE', 'uni');
define('CH_WSB_TEMPLATE_FOLDER_ROOT', 'templates');
define('CH_WSB_TEMPLATE_FOLDER_BASE', 'base');

define('CH_WSB_TEMPLATE_INJECTIONS_CACHE', 'sys_injections.inc');

define('CH_WSB_TEMPLATE_CHECK_IN_BOTH', 'both');
 ...
 *  8. <ch_image_url:image_file_name /> - image with 'image_file_name' file name will be searched in the images folder of current template.
 ...
 *  9. <ch_icon_url:icon_file_name /> - the same with <ch_image_url:image_file_name />, but icons will be searched in the images/icons/ folders.
 *  10. <ch_injection:injection_name /> - will be replaced with injections registered with the page and injection_name in the `sys_injections`/`sys_injections_admin`/ tables.
 *  11. <ch_if:tag_name>some_HTML</ch_if:tag_name> - will be replaced with provided content if the condition is true, and with empty string otherwise.
 *  12. <ch_repeat:cycle_name>some_HTML</ch_repeat:cycle_name> - an inner HTML content will be repeated in accordance with received data.
 *
 *
 * Related classes:
 *  ChWsbTemplateAdmin - for processing admin templates.
...
class ChWsbTemplate
{
    ...
    function __construct($sRootPath = CH_DIRECTORY_PATH_ROOT, $sRootUrl = CH_WSB_URL_ROOT)
    {
        ...
        $this->addLocationJs('system_inc_js', CH_DIRECTORY_PATH_INC . 'js/' , CH_WSB_URL_ROOT . 'inc/js/');
        $this->addLocationJs('system_inc_js_classes', CH_DIRECTORY_PATH_INC . 'js/classes/' , CH_WSB_URL_ROOT . 'inc/js/classes/');
        $this->addLocationJs('system_plugins', CH_DIRECTORY_PATH_PLUGINS, CH_WSB_URL_PLUGINS);
        $this->addLocationJs('system_plugins_jquery', CH_DIRECTORY_PATH_PLUGINS . 'jquery/' , CH_WSB_URL_PLUGINS . 'jquery/');
        $this->addLocationJs('system_plugins_tinymce', CH_DIRECTORY_PATH_PLUGINS . 'tiny_mce/' , CH_WSB_URL_PLUGINS . 'tiny_mce/');

        $this->_bCacheEnable = !defined('CH_WSB_CRON_EXECUTE') && getParam('sys_template_cache_enable') == 'on';
        $this->_sCacheFolderUrl = '';
        $this->_sCachePublicFolderUrl = CH_WSB_URL_CACHE_PUBLIC;
        $this->_sCachePublicFolderPath = CH_DIRECTORY_PATH_CACHE_PUBLIC;
        $this->_sCacheFilePrefix = "ch_templ_";
        ...
        $this->_bCssCache = !defined('CH_WSB_CRON_EXECUTE') && getParam('sys_template_cache_css_enable') == 'on';
        $this->_bCssArchive = $this->_bCssCache && $bArchive;
        $this->_sCssCachePrefix = $this->_sCacheFilePrefix . 'css_';

        $this->_bJsCache = !defined('CH_WSB_CRON_EXECUTE') && getParam('sys_template_cache_js_enable') == 'on';
     ...
     * @param string $sLocationPath - location's path. For modules: '[path_to_cheetah]/modules/[vendor_name]/[module_name]/'
     * @param string $sLocationUrl  - location's url. For modules: '[url_to_cheetah]/modules/[vendor_name]/[module_name]/'
     */
    function addLocation($sKey, $sLocationPath, $sLocationUrl)
    {
        $this->_aLocations[$sKey] = array(
            'path' => $sLocationPath . CH_WSB_TEMPLATE_FOLDER_ROOT . DIRECTORY_SEPARATOR,
            'url' => $sLocationUrl . CH_WSB_TEMPLATE_FOLDER_ROOT . '/'
        );
    }
    /**
     ...
     *
     * @param string $sKey          - location's	unique key.
     * @param string $sLocationPath - location's path. For modules: '[path_to_cheetah]/modules/[vendor_name]/[module_name]/js/'
     * @param string $sLocationUrl  - location's url. For modules: '[url_to_cheetah]/modules/[vendor_name]/[module_name]/js/'
     */
    ...
    function getCode()
    {
        return isset($GLOBALS['iAdminPage']) && (int)$GLOBALS['iAdminPage'] == 1 ? CH_WSB_TEMPLATE_DEFAULT_CODE : $this->_sCode;
    }
    /**
     * Get page width.
     *
    ...
    function addJsOption($mixedName)
    {
        ...
        foreach($mixedName as $sName)
            $GLOBALS['ChWsbTemplateJsOptions'][$sName] = $GLOBALS['MySQL']->getParam($sName);
    }
    /**
    ...
    function addJsTranslation($mixedKey)
    {
        ...
        foreach($mixedKey as $sKey)
            $GLOBALS['ChWsbTemplateJsTranslations'][$sKey] = _t($sKey, '{0}', '{1}');
    }
    /**
    ...
    function addJsImage($aImages)
    {
        ...
        foreach($aImages as $sKey => $sFile) {
            ...
            $GLOBALS['ChWsbTemplateJsImages'][$sKey] = $sUrl;
        }
    }
    /**
     ...
     */
    function getIconUrl($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
        $sContent = "";
     ...
     */
    function getIconPath($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
     ...
     * @return string full URL.
     */
    function getImageUrl($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
        $sContent = "";
     ...
     * @return string absolute path.
     */
    function getImagePath($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
     ...
     * @return string full URL.
     */
    function getCssUrl($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
     ...
     * @return string full URL.
     */
    function getCssPath($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
     ...
     */
    function getHtml($sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
    ...
    {
        if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->beginTemplate($sName, $sRand = time().rand());

        if (($sContent = $this->getCached($sName, $aVariables, $mixedKeyWrapperHtml, $sCheckIn)) !== false) {
            if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->endTemplate($sName, $sRand, $sRet, true);
            return $sContent;
        }
        ...
        if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->endTemplate($sName, $sRand, $sRet, false);
    ...
    {
        if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->beginPage($sName);

        // add facebook meta tag
        if ($sFbId = getParam('ch_facebook_connect_api_key'))
            $this->setOpenGraphInfo(array('app_id' => $sFbId), 'fb');

        $sContent = $this->parseHtmlByName($sName, $aVariables, $this->_sKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_BOTH);
        if(empty($sContent))
            $sContent = $this->parseHtmlByName('default.html', $aVariables, $this->_sKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_BOTH);

        //--- Add CSS and JS at the very last ---//
        if(strpos($sContent, '<ch_include_css_styles />') !== false)
			$sContent = str_replace('<ch_include_css_styles />', $this->includeCssStyles(), $sContent);

        if(strpos($sContent , '<ch_include_css />') !== false) {
            if (!empty($GLOBALS['_page']['css_name'])) {
                $this->addCss($GLOBALS['_page']['css_name']);
            }
            $sContent = str_replace('<ch_include_css />', $this->includeFiles('css', true) . $this->includeFiles('css'), $sContent);
        }

        if(strpos($sContent , '<ch_include_js />') !== false) {
            if (!empty($GLOBALS['_page']['js_name'])) {
                $this->addJs($GLOBALS['_page']['js_name']);
            }
            $sContent = str_replace('<ch_include_js />', $this->includeFiles('js', true) . $this->includeFiles('js') . $this->includeCssAsync(), $sContent);
        }

        if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->endPage($sContent);
    ...
    function parseSystemKey($sKey, $mixedKeyWrapperHtml = null)
    {
        ...
        switch( $sKey ) {
            ...
            case 'show_breadcrumb':
                $sShow = getParam('ext_breadcrumb_menu_enabled');
                if($sShow) {
                  $sRet = '';
                } else {
                  $sRet = 'display: none; visibility: hidden;';
                }
                break;
            case 'footer_style_breadcrumb':
                $sShow = getParam('ext_breadcrumb_menu_enabled');
                if($sShow) {
                  $sRet = '';
                } else {
                  $sRet = 'position: relative;';
                }
                break;
            ...
            case 'extra_top_menu':
                ...
                if ($iProfileId && getParam('ext_nav_menu_enabled')) {
                    ch_import('ChTemplMemberMenu');
                    $oMemberMenu = new ChTemplMemberMenu();
        ...
        $sRet = ChWsbTemplate::processInjection($_page['name_index'], $sKey, $sRet);
        return $sRet;
    }
    /**
    ...
    function getTemplatesCacheObject ()
    {
        ...
        $oCacheEngine = ch_instance('ChWsbCache' . $sCacheEngine);
        if(!$oCacheEngine->isAvailable())
            $oCacheEngine = ch_instance('ChWsbCacheFileHtml');
        return $oCacheEngine;
    }
    /**
     ...
     */
    function getCached($sName, &$aVariables, $mixedKeyWrapperHtml = null, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH, $bEvaluate = true)
    {
        // initialization
    ...
    function _compileJs($sAbsolutePath, &$aIncluded)
    {
        ...
        if($bExternal) {
            ...
            $sContent = ch_file_get_contents($sAbsolutePath);
        } else {
        ...
        $sUrl = ch_ltrim_str($sPath, realpath(CH_DIRECTORY_PATH_ROOT), CH_WSB_URL_ROOT);
    ...
    function includeCssAsync ()
    {
        ...
        foreach ($GLOBALS[$this->_sPrefix . 'CssAsync'] as $sUrl)
            $sList .= 'loadCSS("' . $sUrl . '", document.getElementById("ch_css_async"));';

        // don't load css for mobile devices
        return '
            <script id="ch_css_async">
        ...
        if($bExternal) {
            ...
            $sContent = preg_replace_callback(
                ...
                function ($aMatches) use ($sPath) {
                    return ChWsbTemplate::_callbackParseUrl(addslashes($sPath), $aMatches);
                },
                $sContent
            );
        }
    ...
    {
        require_once(CH_DIRECTORY_PATH_PLUGINS . 'minify/lib/Minify/CSS/Compressor.php');
    ...
    public static function _callbackParseUrl($sPath, $aMatches)
    {
        ...
        $sRootPath = realpath(CH_DIRECTORY_PATH_ROOT);
        ...
        return 'url(' . ch_ltrim_str($sAbsolutePath, $sRootPath, CH_WSB_URL_ROOT) . ')';
    }
    ...
    function _processFiles($sType, $sAction, $mixedFiles, $bDynamic = false, $bSystem = false)
    {
        ...
        foreach($mixedFiles as $sFile) {
            ...
            //--- Process Custom CSS/JS file ---//
                ...
                if (!isset($aParts[0]))
                    $aParts[0] = '';
                $sUrl = CH_WSB_URL_ROOT . (isset($aParts[1]) ? $aParts[1] : $aParts[0]) . $sFile;
                $sPath = realpath(CH_DIRECTORY_PATH_ROOT . $aParts[0] . $sFile);
            }
    ...
    function _parseContent($sContent, $aVariables, $mixedKeyWrapperHtml = null)
    {
        ...
        for ($i = 0; $i < $iCountKeys; $i++) {
            if (strncmp($aKeys[$i], 'ch_repeat:', 10) === 0) {
            ...
            } else if (strncmp($aKeys[$i], 'ch_if:', 6) === 0) {
                $sKey = "'<" . $aKeys[$i] . ">(.*)<\/" . $aKeys[$i] . ">'s";
        ...
        $aKeys = array_merge($aKeys, array(
            ...
            "'<ch_image_url:([^\s]+) \/>'s",
            "'<ch_icon_url:([^\s]+) \/>'s",
            "'<ch_text:([_\{\}\w\d\s]+[^\s]{1}) \/>'s",
        	"'<ch_text_js:([^\s]+) \/>'s",
			"'<ch_text_attribute:([^\s]+) \/>'s",
            "'<ch_url_root />'",
            "'<ch_url_admin />'"
        ));

        $aValues = array_merge($aValues, array(
            function($matches) use ($aVariables, $mixedKeyWrapperHtml) { return $this->parseHtmlByName($matches[1], $aVariables, $mixedKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_BOTH); },
            function($matches) use ($aVariables, $mixedKeyWrapperHtml) { return $this->parseHtmlByName($matches[1], $aVariables, $mixedKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_TMPL); },
            function($matches) use ($aVariables, $mixedKeyWrapperHtml) { return $this->parseHtmlByName($matches[1], $aVariables, $mixedKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_BASE); },
        	...
        	function($matches) { return ch_js_string(_t($matches[1])); },
        	function($matches) { return ch_html_attribute(_t($matches[1])); },
            CH_WSB_URL_ROOT,
            CH_WSB_URL_ADMIN
        ));
    ...
    function _compileContent($sContent, $aVarName, $iVarDepth, $aVarValues, $mixedKeyWrapperHtml = null)
    {
        ...
        for($i = 0; $i < count($aKeys); $i++) {
            ...
            } else if(strpos($aKeys[$i], 'ch_if:') === 0) {
                $sKey = "'<" . $aKeys[$i] . ">(.*)<\/" . $aKeys[$i] . ">'s";
        ...
        $aValues = array_merge($aValues, array(
            function($matches) use ($aVarValues, $mixedKeyWrapperHtml) { return $this->getCached($matches[1], $aVarValues, $mixedKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_BOTH, false); },
            function($matches) use ($aVarValues, $mixedKeyWrapperHtml) { return $this->getCached($matches[1], $aVarValues, $mixedKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_BASE, false); },
            function($matches) use ($aVarValues, $mixedKeyWrapperHtml) { return $this->getCached($matches[1], $aVarValues, $mixedKeyWrapperHtml, CH_WSB_TEMPLATE_CHECK_IN_TMPL, false); },
            function($matches) { return '<?=$this->processInjection($GLOBALS[\'_page\'][\'name_index\'], "'.$matches[1].'")?>'; },
            function($matches) { return $this->getImageUrl($matches[1]); },
            function($matches) { return $this->getIconUrl($matches[1]); },
            function($matches) { return _t($matches[1]); },
     ...
     */
    function _getAbsoluteLocation($sType, $sFolder, $sName, $sCheckIn = CH_WSB_TEMPLATE_CHECK_IN_BOTH)
    {
        if($sType == 'path') {
            $sDivider = DIRECTORY_SEPARATOR;
            $sRoot = CH_DIRECTORY_PATH_ROOT;
        } else if($sType == 'url') {
            $sDivider = '/';
            $sRoot = CH_WSB_URL_ROOT;
        }
        ...
        if(strpos($sName,'|') !== false) {
            ...
            $sName = $aParts[1];
            $sLocationKey = $this->addDynamicLocation(CH_DIRECTORY_PATH_ROOT . $aParts[0], CH_WSB_URL_ROOT . $aParts[0]);
        }
        ...
        foreach($aLocations as $sKey => $aLocation) {
            ...
            if(($sCheckIn == CH_WSB_TEMPLATE_CHECK_IN_BOTH || $sCheckIn == CH_WSB_TEMPLATE_CHECK_IN_TMPL) && extFileExists($aLocation['path'] . 'tmpl_' . $sCode . DIRECTORY_SEPARATOR . $sFolder . $sName))
                $sResult = $aLocation[$sType] . 'tmpl_' . $sCode . $sDivider . $sFolder . $sName;
            else if(($sCheckIn == CH_WSB_TEMPLATE_CHECK_IN_BOTH || $sCheckIn == CH_WSB_TEMPLATE_CHECK_IN_BASE) && extFileExists($aLocation['path'] . CH_WSB_TEMPLATE_FOLDER_BASE . DIRECTORY_SEPARATOR . $sFolder . $sName))
                $sResult = $aLocation[$sType] . CH_WSB_TEMPLATE_FOLDER_BASE . $sDivider . $sFolder . $sName;
            else
                continue;
            break;
        }
         ...
         */
        if(!$sResult && @is_file(CH_DIRECTORY_PATH_ROOT . $aParts[0] . DIRECTORY_SEPARATOR . $aParts[1])) {
    ...
    function _processJsTranslations()
    {
        ...
        $sReturn = '';
        foreach($GLOBALS['ChWsbTemplateJsTranslations'] as $sKey => $sString) {
        ...
        return '<script type="text/javascript" language="javascript">var aWsbLang = {' . substr($sReturn, 0, -1) . '};</script>';
    }
    /**
    ...
    {
        $sReturn = '';
        foreach($GLOBALS['ChWsbTemplateJsOptions'] as $sName => $mixedValue)
            $sReturn .= "'" .  $sName . "': '" . addslashes($mixedValue) . "',";

        return '<script type="text/javascript" language="javascript">var aWsbOptions = {' . substr($sReturn, 0, -1) . '};</script>';
    }
    /**
    ...
    {
        $sReturn = '';
        foreach($GLOBALS['ChWsbTemplateJsImages'] as $sKey => $sUrl)
            $sReturn .= "'" .  $sKey . "': '" . $sUrl . "',";

        return '<script type="text/javascript" language="javascript">var aWsbImages = {' . substr($sReturn, 0, -1) . '};</script>';
    }
    ...
    {
        return CH_WSB_URL_ROOT . 'gzip_loader.php?file=' . $sName . '.' . $sType;
    }
    ...
    function processInjection($iPageIndex, $sKey, $sValue = "")
    {
        ...
        if(is_array($aSelection))
            foreach($aSelection as $aInjection) {
                ...
                if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->beginInjection($sRand = time().rand());
                ...
                if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->endInjection($sRand, $aInjection['name'], $aInjection['key'], (int)$aInjection['replace'] == 1);
