    ...
    function response ()
    {
        foreach ($this->aChoice as $sKey => $aValue) {
            if (!class_exists($aValue['class'])) {
                ...
                require_once(CH_DIRECTORY_PATH_ROOT . $sClassPath);
            }
 ...
 *
 * 1. Extends your own search class from this one (or from ChBaseSearchResult or ChBaseSearchResultSharedMedia classes)
 * 2. Set necessary fields of class (using as example ChFilesSearch):
...
class ChWsbSearchResult
{
    ...
    function setConditionParams()
    {
        ...
        if ($sKeyword !== false)
            $this->aCurrent['restriction']['keyword'] = array(
                'value' => process_db_input($sKeyword, CH_TAGS_STRIP),
                'field' => '',
                'operator' => 'against'
            );

        if (isset($_GET['ownerName'])) {
            $sName = process_db_input($_GET['ownerName'], CH_TAGS_STRIP);
    ...
    */
            ...
            foreach ($this->aCurrent['restriction'] as $sKey => $aValue) {
                ...
                if (isset($aValue['operator']) && !empty($aValue['value'])) {
                   ...
                   if (!isset($aValue['no_quote_value']))
                       $aValue['value'] = process_db_input($aValue['value'], CH_TAGS_STRIP);
    ...
    {
        if ($this->iRate == 1)
           $this->oRate = new ChTemplVotingView($this->aCurrent['name'], 0, 0);
    }
