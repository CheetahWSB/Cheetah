...
class ChWsbMenu
{
    ...
    function load()
    {
        ...
        if (ChWsbRequest::serviceExists('pageac', 'menu_items_filter')) {
            ChWsbService::call('pageac', 'menu_items_filter', array('top', &$this->aTopMenu));
        }
    ...
    function getMenuInfo()
    {
        ...
        $aSiteUrl = parse_url( CH_WSB_URL_ROOT );
        $this->sRequestUriFile = substr($_SERVER['REQUEST_URI'], strlen($aSiteUrl['path']));
        $this->sSelfFile       = substr(ch_html_attribute($_SERVER['PHP_SELF']), strlen($aSiteUrl['path']));
        ...
        foreach( $this->aTopMenu as $iItemID => $aItem ) {
            ...
            if((!isset($this->aMenuInfo['profileID']) || !$this->aMenuInfo['profileID']) && CH_PROFILE_MENU_ID == $aItem['Parent'] )
                continue;
