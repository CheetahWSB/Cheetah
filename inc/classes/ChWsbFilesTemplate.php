...
class ChWsbFilesTemplate extends ChWsbModuleTemplate
{
    /**
     * Constructor
     */
    ...
    function pageCode ($aPage = array(), $aPageCont = array(), $aCss = array(), $aJs = array(), $bAdminMode = false, $isSubActions = true)
    {
        ...
        if ($isSubActions) {
            $aVars = array ('BaseUri' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri());
    ...
    function getBasicFileInfoForm (&$aInfo, $sUrlPref = '')
    {
        $aForm = array(
            'date' => array(
                'type' => 'value',
                'value' => getLocaleDate($aInfo['medDate'], CH_WSB_LOCALE_DATE_SHORT) . ' (' . defineTimeInterval($aInfo['medDate'], false) . ')',
                'caption' => _t('_Date'),
            ),
    ...
    function getRate ($iFile)
    {
        ...
        $oVotingView = new ChTemplVotingView($this->_oConfig->getMainPrefix(), $iFile);
    ...
    {
        $iNumber     = (int)$iNumber;
        $sAlbumUri   = process_db_input($sAlbumUri, CH_TAGS_STRIP);
    ...
    function getSitesSetBox ($sLink)
    {
        ...
        return $this->parseHtmlByName('popup_share.html', array('ch_repeat:sites' => $aUnits));
    }
    ...
    {
        $aUnit = array(
            'site_admin_url' => CH_WSB_URL_ADMIN,
            'site_plugins' => CH_WSB_URL_PLUGINS,
            'users_processing' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'administration/',
    ...
    function getExtraTopMenu ($aMenu = array(), $sUrlPrefix = CH_WSB_URL_ROOT)
    {
        $aUnits = array();
        foreach ($aMenu as $aValue) {
            ...
            $aUnits[] = array(
                ...
                'ch_if:not_active' => array(
                    'condition' => $aValue['active'] == false,
                    'content' => $aValue
                )
            );
        }
        $aUnit['ch_repeat:menu'] = $aUnits;
    ...
    function getAlbumFeed (&$aUnits)
    {
        ...
        if (is_array($aUnits) && !empty($aUnits)) {
            foreach ($aUnits as $aData) {
                $aData['link'] = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aData['uri'];
                if (!isset($aData['main']))
	...
	function getAlbumInfo (&$aInfo)
    {
        $aForm = array(
            ...
            'inputs' => array(
				'location' => array(
	                'type' => 'value',
	                'value' => ch_linkify(process_text_output($aInfo['Location'])),
	                'caption' => _t('_Location'),
	            ),
        		'description' => array(
	                'type' => 'value',
	                'value' => ch_linkify(process_html_output($aInfo['Description'])),
	                'caption' => _t('_Description'),
	            ),
        		'date' => array(
	                'type' => 'value',
	                'value' => getLocaleDate($aInfo['Date'], CH_WSB_LOCALE_DATE_SHORT) . ' (' . defineTimeInterval($aInfo['Date'], false) . ')',
	                'caption' => _t('_Date'),
	            ),
        	)
        );
    ...
    function getJsInclude($sType = 'main', $bWrap = true)
    {
        ...
        switch($sType) {
            case 'main':
                $sConten = "var oChWsbFiles = new ChWsbFiles({sBaseUrl: '" . $sBaseUrl . "'});";
            break;
        }
