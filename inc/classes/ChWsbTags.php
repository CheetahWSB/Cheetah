...
class ChWsbTags
{
    ...
    var $sTagsDivider = CH_WSB_TAGS_DIVIDER;
    var $bToLower = true;
    ...
    function _getSelectCondition($aParam)
    {
        ...
        if (isset($this->aTagFields['owner'])) {
            ...
            if (isset($aParam['admin'])) {
                if ($aParam['admin'])
                    $sCondition .= '(`profiles`.`Role` & ' . CH_WSB_ROLE_ADMIN . ')';
                else
                    $sCondition .= 'NOT (`profiles`.`Role` & ' . CH_WSB_ROLE_ADMIN . ')';
            } else
    ...
    function _insertTags ($aTagsSet)
    {
        ...
        foreach( $aTags as $iKey => $sTag ) {
            ...
            $sTag = process_db_input($sTag, CH_TAGS_STRIP, CH_SLASHES_NO_ACTION);
