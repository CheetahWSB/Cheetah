...
ch_import('ChWsbSubscriptionQuery');

define('CH_WSB_SBS_TYPE_VISITOR', 0);
define('CH_WSB_SBS_TYPE_MEMBER', 1);
 ...
 *
 * Related classes:
 *  ChWsbSubscriptionQuery - database queries.
 *
 * Example of usage:
 ...
 *
 *    $oSubscription = new ChWsbSubscription();
 *    $oSubscription->getButton($iUserId, $sUnit, $sAction, $iObjectId);
 *
 * @see an example of integration in the default Cheetah's modules(feedback, news, etc)
 *
 *
 * Memberships/ACL:
...
class ChWsbSubscription
{
    ...
    {
        $this->_oDb = new ChWsbSubscriptionQuery($this);
        $this->_bDataAdded = false;
        $this->_sJsObject = 'oChWsbSubscription';
    ...
    {
        if(!isset($GLOBALS['chWsbClasses']['ChWsbSubscription']))
        	$GLOBALS['chWsbClasses']['ChWsbSubscription'] = new ChWsbSubscription();

		return $GLOBALS['chWsbClasses']['ChWsbSubscription'];
    }
    ...
    function getMySubscriptions()
    {
        ...
        if((int)$aUserInfo['EmailNotify'] == 0)
            $sContent .= MsgBox(_t('_sbs_wrn_email_notify_disabled', CH_WSB_URL_ROOT . 'pedit.php?ID=' . (int)$aUserInfo['ID']));
        ...
        $oSysTemplate->addJs(array('ChWsbSubscription.js'));
    ...
    function getData($bDynamic = false)
    {
        ...
        $sCssJs .= $oSysTemplate->addJs(array('ChWsbSubscription.js'), $bDynamic);
    ...
    function unsubscribe($aParams)
    {
        ...
        switch($aParams['type']) {
            ...
            case 'visitor':
                $aRequest = array(
                    'type' => CH_WSB_SBS_TYPE_VISITOR,
                    'user_id' => $aParams['id']
                );
                break;
            case 'member':
                $aRequest = array(
                    'type' => CH_WSB_SBS_TYPE_MEMBER,
                    'user_id' => $aParams['id']
                );
                break;
        }
    ...
    function getSubscribersCount($iType = CH_WSB_SBS_TYPE_VISITOR)
    {
        return $this->_oDb->getSubscribersCount($iType);
    }
    function getSubscribers($iType = CH_WSB_SBS_TYPE_VISITOR, $iStart = 0, $iCount = 1)
    {
    ...
    function _getJsCode()
    {
        ob_start();
        ?>
            var <?=$this->_sJsObject; ?> = new ChWsbSubscription({
