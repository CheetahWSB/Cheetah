 ...
 */
class ChWsbTwigModule extends ChWsbModule
{
    var $_iProfileId;
    var $_sPrefix;
    var $_sFilterName;
    ...
    function _actionFiles ($sUri, $sTitle)
    {
        ...
        echo ChWsbService::call ('files', 'get_browse_block', array($aRestriction, $this->_oConfig->getBaseUri() . 'files/' . $sUri), 'Search');
    ...
    function _actionVideos ($sUri, $sTitle)
    {
        ...
        echo ChWsbService::call ('videos', 'get_browse_block', array($aRestriction, $this->_oConfig->getBaseUri() . 'videos/' . $sUri), 'Search');
    ...
    function _actionSounds ($sUri, $sTitle)
    {
        ...
        echo ChWsbService::call ('sounds', 'get_browse_block', array($aRestriction, $this->_oConfig->getBaseUri() . 'sounds/' . $sUri), 'Search');
    ...
    function _actionPhotos ($sUri, $sTitle)
    {
        ...
        echo ChWsbService::call ('photos', 'get_browse_block', array($aRestriction, $this->_oConfig->getBaseUri() . 'photos/' . $sUri), 'Search');
    ...
    function _actionBrowseFans ($sUri, $sFuncAllowed, $sFuncDbGetFans, $iPerPage, $sUrlBrowse, $sTitle)
    {
        ...
        foreach ($aProfiles as $aProfile) {
            $sMainContent .= $oChTemplSearchProfile->displaySearchUnit($aProfile);
        }
        ...
        $sUrlStart = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . $sUrlBrowse . $aDataEntry[$this->_oDb->_sFieldUri];
    ...
    function _actionView ($sUri, $sMsgPendingApproval)
    {
        ...
        new ChWsbViews($this->_sPrefix, $aDataEntry[$this->_oDb->_sFieldId]);
    }
    ...
    function _actionUploadMedia ($sUri, $sIsAllowedFuncName, $sMedia, $aMediaFields, $sTitle)
    {
        ...
        ch_import ('FormUploadMedia', $this->_aModule);
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            header ('Location:' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri]);
            exit;
    ...
    function _actionBroadcast ($iEntryId, $sTitle, $sMsgNoRecipients, $sMsgSent)
    {
        ...
        $GLOBALS['oTopMenu']->setCustomSubHeaderUrl(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri]);
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_'.$this->_sPrefix) => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
            $aDataEntry[$this->_oDb->_sFieldTitle] => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
            $sTitle => '',
        ));
        ...
        ch_import ('FormBroadcast', $this->_aModule);
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            $aTemplateVars = array (
                ...
                'EntryUrl' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
            );
            $iSentMailsCounter = 0;
    ...
    function _actionInvite ($iEntryId, $sEmailTemplate, $iMaxEmailInvitations, $sMsgInvitationSent, $sMsgNoUsers, $sTitle)
    {
        ...
        ch_import('ChWsbTwigFormInviter');
        $oForm = new ChWsbTwigFormInviter ($this, $sMsgNoUsers);
        $oForm->initChecker();
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            if (false !== ch_get('inviter_users') && is_array(ch_get('inviter_users'))) {
                $aInviteUsers = ch_get('inviter_users');
            ...
            $aEmails = preg_split ("#[,\s\\b]+#", ch_get('inviter_emails'));
    ...
    function _actionCalendar ($iYear, $iMonth, $sTitle)
    {
        ...
        $oCalendar = ch_instance ($this->_aModule['class_prefix'] . 'Calendar', array ($iYear, $iMonth, $this->_oDb, $this->_oConfig, $this->_oTemplate));
    ...
    function actionBrowse ($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        ...
        ch_import ('SearchResult', $this->_aModule);
        $sClass = $this->_aModule['class_prefix'] . 'SearchResult';
        $o = new $sClass(process_db_input($sMode, CH_TAGS_STRIP), process_db_input($sValue, CH_TAGS_STRIP), process_db_input($sValue2, CH_TAGS_STRIP), process_db_input($sValue3, CH_TAGS_STRIP));
    ...
    function _actionSearch ($sKeyword, $sCategory, $sTitle)
    {
        ...
        ch_import ('FormSearch', $this->_aModule);
    ...
    function _actionEdit ($iEntryId, $sTitle)
    {
        ...
        ch_import ('FormEdit', $this->_aModule);
    ...
    function _actionDelete ($iEntryId, $sMsgSuccess)
    {
        ...
        if ($this->_oDb->deleteEntryByIdAndOwner($iEntryId, $this->_iProfileId, $this->isAdmin())) {
            ...
            $sRedirect = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'browse/' . ($this->_iProfileId ? 'user/' . $this->_oDb->getProfileNickNameById($this->_iProfileId) : '');
    ...
    function _actionMarkFeatured ($iEntryId, $sMsgSuccessAdd, $sMsgSuccessRemove)
    {
        ...
        if ($this->_oDb->markAsFeatured($iEntryId)) {
            ...
            $sRedirect = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri];
            $sJQueryJS = genAjaxyPopupJS($iEntryId, 'ajaxy_popup_result_div', $sRedirect);
    ...
    function _actionManageFansPopup ($iEntryId, $sTitle, $sFuncGetFans = 'getFans', $sFuncIsAllowedManageFans = 'isAllowedManageFans', $sFuncIsAllowedManageAdmins = 'isAllowedManageAdmins', $iMaxFans = 1000)
    {
        ...
        $sActionsUrl = ch_append_url_params(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . "view/" . $aDataEntry[$this->_oDb->_sFieldUri],  'ajax_action=');
        $aButtons = array (
            array (
                'type' => 'submit',
                'name' => 'fans_remove',
        ...
        $sControl = ChTemplSearchResult::showAdminActionsPanel('sys_manage_items_manage_fans', $aButtons, 'sys_fan_unit');
    ...
    function _actionSharePopup ($iEntryId, $sTitle, $bAddTempleateExt = false)
    {
        ...
        $sEntryUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri];
    ...
    function _actionTags($sTitle, $sTitleAllTags = '')
    {
        ...
        $oTags = new ChTemplTagsModule($aParam, $sTitleAllTags, CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'tags');
        $this->_oTemplate->pageStart();
        echo $oTags->getCode();
        $this->_oTemplate->pageCode($sTitle, false, false);
    }
    ...
    function _actionCategories($sTitle)
    {
        ...
        $oCateg = new ChTemplCategoriesModule($aParam, _t('_categ_users'), CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories');
        $this->_oTemplate->pageStart();
        echo $oCateg->getCode();
        $this->_oTemplate->pageCode($sTitle, false, false);
    }
    ...
    {
        $aFile = ChWsbService::call('files', 'get_file_array', array($aFileInfo[$sFieldMediaId]), 'Search');
        if (!$aFile['date']) {
            $this->_oTemplate->displayPageNotFound ();
            exit;
        }
    ...
    function _serviceGetMemberMenuItem ($sTitle, $sAlt, $sIcon, $isExtraCount = true, $sLinkAppend = '')
    {
        ...
        $aLinkInfo = array(
            'item_img_src'  => $sIcon,
            'item_img_alt'  => $sAlt,
            'item_link'     => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'browse/my/' . $sLinkAppend,
            'item_title'    => $sTitle,
    ...
    function _serviceGetWallPost ($aEvent, &$aParams)
    {
        ...
        ch_import('Voting', $this->_aModule);
        ...
        if($iItems > 1) {
            ...
            $sTmplName = isset($aParams['templates']['grouped']) ? $aParams['templates']['grouped'] : 'modules/cheetah/wall/|timeline_post_twig_grouped.html';
            return array(
            	'owner_id' => $iOwner,
        ...
        $sTmplName = isset($aParams['templates']['single']) ? $aParams['templates']['single'] : 'modules/cheetah/wall/|timeline_post_twig.html';
        return array(
        	'owner_id' => $iOwner,
    ...
    function _serviceGetWallPostOutline($aEvent, $sIcon, $aParams = array())
    {
        ...
        foreach($aObjectIds as $iId) {
            ...
            if(empty($aItem))
                ...
                if($aItem[$this->_oDb->_sFieldThumb]) {
                    ...
                    $aImage = ChWsbService::call('photos', 'get_entry', array($aItem[$this->_oDb->_sFieldThumb], 'browse2x'), 'Search');
        ...
        $iItems = count($aItems);
        if($iItems > 1) {
            $sTmplName = isset($aParams['templates']['grouped']) ? $aParams['templates']['grouped'] : 'modules/cheetah/wall/|outline_item_image_grouped';
        ...
        $sTmplName = isset($aParams['templates']['single']) ? $aParams['templates']['single'] : 'modules/cheetah/wall/|outline_item_image';
    ...
    function _serviceGetWallAddComment($aEvent, $aParams)
    {
        ...
        $sTextWallObject = _t('_ch_' . $sUri . '_wall_object');

        $sTmplName = isset($aParams['templates']['main']) ? $aParams['templates']['main'] : 'modules/cheetah/wall/|timeline_comment.html';
        return array(
            'title' => _t('_ch_' . $sUri . '_wall_added_new_title_comment', $sOwner, $sTextWallObject),
            'description' => $aComment['cmt_text'],
            'content' => $sCss . $this->_oTemplate->parseHtmlByName($sTmplName, array(
        		'mod_prefix' => $sCssPrefix,
	            'cpt_user_name' => $sOwner,
	            'cpt_added_new' => _t('_ch_' . $sUri . '_wall_added_new_comment'),
	            'cpt_object' => $sTextWallObject,
	            'cpt_item_url' => $sBaseUrl . $aItem[$this->_oDb->_sFieldUri],
    ...
    function _serviceGetSpyPost($sAction, $iObjectId, $iSenderId, $aExtraParams, $aLangKeys)
    {
        ...
        return array(
            ...
            'params' => array(
                ...
                'entry_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
    ...
    function serviceGetShoutboxData ()
    {
        ...
        return array(
            array(
                ...
                'code_allow_use' => method_exists($this, 'isFan') ? 'return ChWsbService::call("' . $this->_aModule['uri'] . '", "is_fan", array($iHandler));' : '',
                'code_allow_delete' => method_exists($this, 'isEntryAdmin') ? 'return ChWsbService::call("' . $this->_aModule['uri'] . '", "is_entry_admin", array($iHandler));' : '',
                'code_allow_block' => '',
            ),
        );
    }
    ...
    function _serviceGetSubscriptionParams ($sAction, $iEntryId, $aAction2Name)
    {
        ...
        return array (
            ...
            'template' => array (
                ...
                'ViewLink' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
            ),
        );
    }
    ...
    function _actionAdministrationManage ($isAdminEntries, $sKeyBtnDelete = '', $sKeyBtnActivate = '', $sUrl = false)
    {
        ...
        if ($isAdminEntries) {
            $sContent = $this->_manageEntries ('admin', '', true, 'ch_twig_admin_form', array(
                'action_delete' => $sKeyBtnDelete,
            ), '', true, 0, $sUrl);
        } else {
            $sContent = $this->_manageEntries ('pending', '', true, 'ch_twig_admin_form', array(
    ...
    function actionAdministrationCreateEntry ()
    {
        ...
        ob_start();
        $this->_addForm(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'administration/admin_entries');
        $aVars = array (
    ...
    function onEventCreate ($iEntryId, $sStatus, $aDataEntry = array())
    {
        ...
        if (ChWsbModule::getInstance('ChWmapModule'))
            ChWsbService::call('wmap', 'response_entry_add', array($this->_oConfig->getUri(), $iEntryId));

        $this->_oDb->createForum ($aDataEntry, $this->_oDb->getProfileNickNameById($this->_iProfileId));
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'add', $iEntryId, $this->_iProfileId, array('Status' => $sStatus));
        $oAlert->alert();
    }
    ...
    function onEventChanged ($iEntryId, $sStatus)
    {
        ...
        if (ChWsbModule::getInstance('ChWmapModule'))
            ChWsbService::call('wmap', 'response_entry_change', array($this->_oConfig->getUri(), $iEntryId));

        $oAlert = new ChWsbAlerts($this->_sPrefix, 'change', $iEntryId, $this->_iProfileId, array('Status' => $sStatus));
        $oAlert->alert();
    }
    ...
    function onEventDeleted ($iEntryId, $aDataEntry = array())
    {
        ...
        // delete views
        ch_import ('ChWsbViews');
        $oViews = new ChWsbViews($this->_sPrefix, $iEntryId, false);
        ...
        if (ChWsbModule::getInstance('ChWmapModule'))
            ChWsbService::call('wmap', 'response_entry_delete', array($this->_oConfig->getUri(), $iEntryId));
        ...
        // arise alert
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'delete', $iEntryId, $this->_iProfileId);
        $oAlert->alert();
    }
    ...
    {
        // arise alert
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'mark_as_featured', $iEntryId, $this->_iProfileId, array('Featured' => $aDataEntry[$this->_oDb->_sFieldFeatured]));
        $oAlert->alert();
    }
    ...
    function onEventJoin ($iEntryId, $iProfileId, $aDataEntry)
    {
        ...
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'join', $iEntryId, $iProfileId);
        $oAlert->alert();
    ...
    function _onEventJoinRequest ($iEntryId, $iProfileId, $aDataEntry, $sEmailTemplate, $iMaxFans = 1000)
    {
        ...
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'join_request', $iEntryId, $iProfileId);
        $oAlert->alert();
    }
    ...
    function _onEventJoinReject ($iEntryId, $iProfileId, $aDataEntry, $sEmailTemplate)
    {
        ...
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'join_reject', $iEntryId, $iProfileId);
        $oAlert->alert();
    }
    ...
    function _onEventFanRemove ($iEntryId, $iProfileId, $aDataEntry, $sEmailTemplate)
    {
        ...
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'fan_remove', $iEntryId, $iProfileId);
        $oAlert->alert();
    ...
    {
        $this->_notifyEmail ($sEmailTemplate, $iProfileId, $aDataEntry);
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'fan_become_admin', $iEntryId, $iProfileId);
        $oAlert->alert();
    }
    ...
    {
        $this->_notifyEmail ($sEmailTemplate, $iProfileId, $aDataEntry);
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'admin_become_fan', $iEntryId, $iProfileId);
        $oAlert->alert();
    }
    ...
    {
        $this->_notifyEmail ($sEmailTemplate, $iProfileId, $aDataEntry);
        $oAlert = new ChWsbAlerts($this->_sPrefix, 'join_confirm', $iEntryId, $iProfileId);
        $oAlert->alert();
    }
    ...
    function _addForm ($sRedirectUrl)
    {
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            if ($iEntryId) {
                ...
                if (!$sRedirectUrl)
                    $sRedirectUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri];
    ...
    function _manageEntries ($sMode, $sValue, $isFilter, $sFormName, $aButtons, $sAjaxPaginationBlockId = '', $isMsgBoxIfEmpty = true, $iPerPage = 0, $sUrlAdmin = false)
    {
        ...
        $aVars = array (
            ...
            'filter_panel' => $isFilter ? $o->showAdminFilterPanel(false !== ch_get($this->_sFilterName) ? ch_get($this->_sFilterName) : '', 'filter_input_id', 'filter_checkbox_id', $this->_sFilterName) : '',
    ...
    function _preProductTabs ($sUri, $sSubTab = '')
    {
        ...
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_'.$this->_sPrefix) => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
            $aDataEntry[$this->_oDb->_sFieldTitle] => $sSubTab ? CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri] : '',
            $sSubTab => '',
        ));
    ...
    function _processFansActions ($aDataEntry, $iMaxFans = 1000)
    {
        ...
        if (false !== ch_get('ajax_action') && $this->isAllowedManageFans($aDataEntry) && 0 === strcasecmp($_SERVER['REQUEST_METHOD'], 'POST')) {

            $iEntryId = $aDataEntry[$this->_oDb->_sFieldId];
            $aIds = array ();
            if (false !== ch_get('ids'))
                $aIds = $this->_getCleanIdsArray (ch_get('ids'));

            $isShowConfirmedFansOnly = false;
            switch (ch_get('ajax_action')) {
                case 'remove':
                ...
                case 'confirm':
                    if ($this->_oDb->confirmFans($iEntryId, $aIds)) {
                        echo '<script type="text/javascript" language="javascript">
                            document.location = "' . CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . "view/" . $aDataEntry[$this->_oDb->_sFieldUri] . '";
                        </script>';
                        $aProfiles = array ();
    ...
    function _browseMy (&$aProfile, $sTitle = null)
    {
        ...
        if (ch_get('action_delete') && is_array(ch_get('entry'))) {
            $aEntries = ch_get('entry');
        ...
        if ($bAjaxMode && ($this->_sPrefix . '_my_active') == ch_get('block')) {
            header('Content-type:text/html;charset=utf-8');
        ...
        if ($bAjaxMode && ($this->_sPrefix . '_my_pending') == ch_get('block')) {
            header('Content-type:text/html;charset=utf-8');
    ...
    {
        return $this->_isMembershipEnabledFor ('CH_PHOTOS_ADD');
    }

    function isMembershipEnabledForVideos ()
    {
        return $this->_isMembershipEnabledFor ('CH_VIDEOS_ADD');
    }

    function isMembershipEnabledForSounds ()
    {
        return $this->_isMembershipEnabledFor ('CH_SOUNDS_ADD');
    }

    function isMembershipEnabledForFiles ()
    {
        return $this->_isMembershipEnabledFor ('CH_FILES_ADD');
    }
