...
ch_import('ChWsbPageViewAdmin'); // for caching abilities
 ...
 *
 * class ChExamplePageView extends ChWsbPageView {
 *     function ChExamplePageView() {
 *         parent::ChWsbPageView('example'); // Attention! Here should be name which you will insert into sys_page_compose_pages in next step.
 *     }
 *
...
class ChWsbPageView
{
    ...
    function __construct( $sPageName )
    {
        ...
        $oZ = new ChWsbAlerts('system', 'page_init', 0, 0, array(
    ...
    function createCache()
    {
        if ($this->oCacher == null)
            $this->oCacher = new ChWsbPageViewCacher($this->sTableName, $this->sCacheFile);
    ...
    function load()
    {
        ...
        if (ChWsbRequest::serviceExists('pageac', 'page_blocks_filter')) {
            ChWsbService::call('pageac', 'page_blocks_filter', array(&$this));
        }
    ...
    function getCode()
    {
        if( !($this -> bAjaxMode and (int)$_REQUEST['pageBlock']) )
            ...
            $oZ = new ChWsbAlerts('system', 'page_output_block', 0, 0, array(
                'page_name' => $this -> sPageName,
                'page_data' => &$this -> aPage,
        ...
        $oZ = new ChWsbAlerts('system', 'page_output', 0, 0, array(
            'page_name' => $this -> sPageName,
            'page_data' => &$this -> aPage,
    ...
    */
    function genBlock( $iBlockID, $aBlock, $bStatic = true, $sDynamicType = 'tab' )
    {
        ...
        if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->beginPageBlock(_t($aBlock['Caption']), $iBlockID);
        ...
        if ($aBlock['Cache'] > 0 && getParam('sys_pb_cache_enable') == 'on') {
            ...
            if ($sBlock !== null) {
                if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->endPageBlock($iBlockID, $sBlock ? false : true, true);
                $this->sCode .= $sBlock;
                return;
            }
        }
        ...
        if ($sBlockCode) {
            ...
            if ($bStatic) {
                $sBlock .= '<div class="page_block_container ch-def-margin-sec-leftright" id="page_block_' . $iBlockID . '">' . $sCodeDB . '</div>';
            } else {
                if ($sDynamicType == 'tab')
                    $sBlock .= $sCodeDB;
        ...
        if (isset($GLOBALS['ch_profiler'])) $GLOBALS['ch_profiler']->endPageBlock($iBlockID, $sBlockCode ? false : true, false );
    ...
    public static function getBlockCaptionItemCode( $iBlockID, $aLinks )
    {
        ...
        foreach( $aLinks as $sTitle => $aLink ) {
            ...
            $aItems[] = array(
                'ch_if:item_act' => array(
                    'condition' => $bActive,
                    'content' => array(
                        'ch_if:icon_act' => array(
                ...
                'ch_if:item_pas' => array(
                    'condition' => !$bActive,
                    'content' => array(
                        'ch_if:icon_pas' => array(
                            'condition' => isset($aLink['icon']) && !$isTextIcon,
                            'content' => array(
                                'class' => $sClass,
                                'src' => empty($aLink['icon']) ? '' : $aLink['icon']
...
BLAH;
    ...
    {
        $sApplication = ChWsbService::call('open_social', 'gen_application', array($sContent));
        return <<<EOF
<div class="dbContent ch-def-bc-margin">
...
<div class="logged_member_block">
    {$sMemberIcon}
    <div class="hello_member ch-def-margin-sec-left">
        {$sHelloMemberC}<br />
...
EOF;
        ...
        return '<div class="dbContent ch-def-bc-margin">'.$ret.'</div>';
    }
    ...
    function GenFormWrap($sMainContent, $sPage, $sFunctionName, $iMaxThumbWidth, $iThumbsCnt)
    {
        ...
        return <<<EOF
<div class="block_rel_100 ch-def-bc-margin">
