...
class ChWsbSubscriptionQuery extends ChWsbDb
{
    var $_sPrefix;
    var $_oSubscription;
    ...
    function getSubscriptionsByUser($iUserId)
    {
        $sSql = "SELECT
            ...
            WHERE `tt`.`action`='' AND `te`.`subscriber_id`= ? AND `te`.`subscriber_type`='" . CH_WSB_SBS_TYPE_MEMBER . "'
    ...
    {
        switch($aParams['type']) {
            case CH_WSB_SBS_TYPE_VISITOR:
                $sUserName = process_db_input($aParams['user_name'], CH_TAGS_STRIP);
                $sUserEmail = process_db_input($aParams['user_email'], CH_TAGS_STRIP);
    ...
    function deleteSubscription($aParams)
    {
        switch($aParams['type']) {
            case CH_WSB_SBS_TYPE_VISITOR:
                if(isset($aParams['user_id']))
                    ...
                    $iUserId = (int)$this->getOne("SELECT `id` FROM `" . $this->_sPrefix . "users` WHERE `name`='" . process_db_input($aParams['user_name'], CH_TAGS_STRIP) . "' AND `email`='" . process_db_input($aParams['user_email'], CH_TAGS_STRIP) . "' LIMIT 1");

                $iUserType = CH_WSB_SBS_TYPE_VISITOR;
                break;
            case CH_WSB_SBS_TYPE_MEMBER:
                $iUserId = (int)$aParams['user_id'];
                $iUserType = CH_WSB_SBS_TYPE_MEMBER;
                break;
        }
        ...
        if($iUserType == CH_WSB_SBS_TYPE_VISITOR || (isset($aParams['unit']) && isset($aParams['object_id']))) {
            $iSbsEntries = (int)$this->getOne("SELECT COUNT(`id`) FROM `" . $this->_sPrefix . "entries` WHERE `subscriber_id`='" . $iUserId . "' AND `subscriber_type`='" . CH_WSB_SBS_TYPE_VISITOR . "' LIMIT 1");
            if($iSbsEntries == 0)
    ...
    function sendDelivery($aParams)
    {
        ...
        foreach($aSubscribers as $aSubscriber) {
            ...
            $iQueued += (int)$this->query("INSERT INTO `" . $this->_sPrefix . "queue`(`email`, `subject`, `body`) VALUES('" . $aUser['email'] . "', '" . process_db_input($aMail['subject'], CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION) . "', '" . process_db_input($aMail['body'], CH_TAGS_NO_ACTION, CH_SLASHES_NO_ACTION) . "')");
        }
        ...
        switch($iType) {
            case CH_WSB_SBS_TYPE_VISITOR:
                $sSql = "SELECT
                        ...
                        INNER JOIN `" . $this->_sPrefix . "entries` AS `tse` ON `tsu`.`id`=`tse`.`subscriber_id` AND `tse`.`subscriber_type`='" . CH_WSB_SBS_TYPE_VISITOR . "'
                        WHERE 1
                        LIMIT 1";
                break;
            case CH_WSB_SBS_TYPE_MEMBER:
                $sSql = "SELECT
                        ...
                        INNER JOIN `" . $this->_sPrefix . "entries` AS `tse` ON `tsu`.`ID`=`tse`.`subscriber_id` AND `tse`.`subscriber_type`='" . CH_WSB_SBS_TYPE_MEMBER . "'
                        WHERE 1
                        LIMIT 1";
                break;
        }
