...
class ChWsbAdminSettings
{
    ...
    function _saveCatWatermark(&$aData)
    {
        global $dir;
        $bResult = false;
        $iImgWidth = (int)getParam('ch_photos_file_width');
        if(empty($iImgWidth))
            $iImgWidth = 100;
        $iImgHeight = (int)getParam('ch_photos_file_height');
        if(empty($iImgHeight))
            $iImgHeight = 100;
        ...
        if(!empty($aData['enable_watermark']))
            $sValue = process_db_input($aData['enable_watermark'], CH_TAGS_STRIP);
        else
            $sValue = '';
