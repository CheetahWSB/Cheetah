...
class ChWsbCronCmd extends ChWsbCron
{
    // - Functions -
    ...
    public function clean_database()
    {
        ...
        $iIps = $oChWsbAdminIpBlockList->deleteExpired();

        // clean old views
        ch_import('ChWsbViews');
        $oChViews = new ChWsbViews('', 0);
        $iDeletedViews = $oChViews->maintenance();

        // clean old votes
        ch_import('ChWsbVoting');
        $oChVotes = new ChWsbVoting('', 0);
        $iDeletedVotes = $oChVotes->maintenance();

        // clean comments ratings
        ch_import('ChWsbCmts');
        $oChCmts = new ChWsbCmts('', 0);
        $iDeletedCommentVotes = $oChCmts->maintenance();
    ...
    public function del_old_all_files()
    {
        global $dir;
        ...
        $dirToClean[CH_DIRECTORY_PATH_CACHE_PUBLIC] = "/^export/";
    ...
    public function processAdminStats()
    {
        $iCounter = 0;
        $sMessage = "";
    ...
    public function processing()
    {
        ...
        while (!empty($aRow)) {
            ...
            elseif ($aCurrentMem['ID'] != MEMBERSHIP_ID_STANDARD) {
