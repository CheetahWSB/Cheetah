...
class ChWsbAdminIpBlockList
{
    ...
    function __construct($sActionUrl = '')
    {
        ...
        $this->_sActionUrl = !empty($sActionUrl) ? $sActionUrl : ch_html_attribute($_SERVER['PHP_SELF']) . (!empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '');
    }
    ...
    function GenStoredMemIPs()
    {
        ...
        while( $aIPList =  $rIPList ->fetch() ) {
            ...
            $sLastDT = getLocaleDate($aIPList['DateTimeTS'], CH_WSB_LOCALE_DATE);
    ...
    function GenIPBlackListTable()
    {
        ...
        while( $aIPList =  $rIPList ->fetch() ) {
            ...
            $sLastDT_Formatted = getLocaleDate($aIPList['LastDT'], CH_WSB_LOCALE_DATE);
            $sLastDT = preg_replace('/([\d]{2}):([\d]{2}):([\d]{2})/', '$1:$2', $aIPList['LastDT_U']);
            $sDesc = process_html_output($aIPList['Desc']);
            $sDescAttr = ch_html_attribute(ch_js_string($aIPList['Desc'],  CH_ESCAPE_STR_APOS));
            ...
            $aTmplVarsItems[] = array(
                ...
                'delete_action_url' => ch_append_url_params($this->_sActionUrl, array('action' => 'apply_delete', 'id' => $iID)),
            );
        }
