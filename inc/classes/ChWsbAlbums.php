...
class ChWsbAlbums
{
    ...
    function __construct($sType, $iOwnerId = 0)
    {
        ...
        $this->sType              = process_db_input($sType, CH_TAGS_STRIP);
    ...
    {
        return uriFilter(ChWsbAlbums::getAbumName($sName, $iUserId));
    }
        ...
        foreach ($aFields as $sKey => $sValue) {
            if (in_array($sKey, $this->aAlbumFields) && ($bUseEmptyValues || strlen($sValue))) {
                switch ($sKey) {
                    case 'description':
                    case 'Description':
                        $sValue = process_db_input($sValue, CH_TAGS_STRIP_AND_NL2BR);
                        break;
    ...
    function _getSqlSpec($aData)
    {
        ...
        if (isset($aData['hide_default']) && $aData['hide_default'] === true) {
            $aRes['def'] = " AND `{$this->sAlbumTable}`.`AllowAlbumView` <> " . CH_WSB_PG_HIDDEN;
        }
    ...
    function addAlbum($aData = array(), $bCheck = true)
    {
        ...
        if (isset($aData['AllowAlbumView'])) {
            ...
            $iAllowAlbumView = $oPrivacy->getDefaultValueModule(str_replace('ch_', '', $this->sType), 'album_view');
            if (!$iAllowAlbumView) {
                $iAllowAlbumView = CH_WSB_PG_ALL;
            }
        } else {
            $iAllowAlbumView = CH_WSB_PG_NOBODY;
        }
        ...
        if($iResult > 0) {
        	$oAlert = new ChWsbAlerts('album', 'add', $iResult, $iOwner, array('Type' => $this->sType, 'Uri' => $sUri));
			$oAlert->alert();

			$oAlert = new ChWsbAlerts($this->sType, 'addAlbum', $iResult, $iOwner, array('Uri' => $sUri));
			$oAlert->alert();
        }
    ...
    {
        $sUri     = process_db_input($sUri, CH_TAGS_STRIP);
    ...
    {
    	$sUri = process_db_input($mixedIdent, CH_TAGS_STRIP);
	...
	function _updateAlbum ($aAlbum, $aData)
    {
        ...
        if((int)$mixedResult > 0) {
        	...
        	$oAlert = new ChWsbAlerts('album', 'change', $aAlbum['ID'], $iUserId, array('Type' => $aAlbum['Type'], 'Uri' => $aAlbum['Uri']));
			$oAlert->alert();

			$oAlert = new ChWsbAlerts($aAlbum['Type'], 'changeAlbum', $aAlbum['ID'], $iUserId, array('Uri' => $aAlbum['Uri']));
			$oAlert->alert();
        }
    ...
    function removeAlbum ($iAlbumId)
    {
        ...
        if((int)$mixedResult > 0) {
        	...
        	$oAlert = new ChWsbAlerts('album', 'delete', $iAlbumId, $iUserId, array('Type' => $aAlbum['Type'], 'Uri' => $aAlbum['Uri']));
			$oAlert->alert();

			$oAlert = new ChWsbAlerts($aAlbum['Type'], 'deleteAlbum', $iAlbumId, $iUserId, array('Uri' => $aAlbum['Uri']));
			$oAlert->alert();
        }
    ...
    function getAlbumObjList($mixedAlbum)
    {
        ...
        if ((int)$mixedAlbum > 0) {
            ...
            $sqlWhere = "`sys_albums`.`Uri` = '" . process_db_input($mixedAlbum, CH_TAGS_STRIP) . "'";
        }
    ...
    function addObject($iAlbumId, $mixedObj, $bUpdateCount = true)
    {
        ...
        $sqlFields  = "`id_album`, `id_object`, `obj_order`";
        $sqlBody    = "";
        ...
        if (is_array($mixedObj)) {
            foreach ($mixedObj as $iValue) {
                $iValue = (int)$iValue;
                $sqlBody .= "('$iAlbumId', '$iValue', '$iValue'), ";
                $iCount++;
            }
        } else {
            $iValue  = (int)$mixedObj;
