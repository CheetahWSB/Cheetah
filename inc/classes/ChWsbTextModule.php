...
class ChWsbTextModule extends ChWsbModule
{
    var $_oTextData;
    var $_oPrivacy;
    ...
    function getBlockView($sUri)
    {
        ...
        $oView = new ChWsbViews($this->_oConfig->getViewsSystemName(), $aEntry['id']);
        $oView->makeView();

        $this->_oTemplate->setPageTitle($aEntry['caption']);
        $GLOBALS['oTopMenu']->setCustomSubHeader($aEntry['caption']);
        $GLOBALS['oTopMenu']->setCustomSubHeaderUrl(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aEntry['uri']);
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_' . $sModuleUri . '_top_menu_item') => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'index/',
            $aEntry['caption'] => '')
        );
    ...
    function getBlockComment($sUri)
    {
        ...
        $sModuleUri = $this->_oConfig->getUri();
        if($aEntry['status'] != CH_TD_STATUS_ACTIVE)
    ...
    {
        $aEntry = $this->_oDb->getEntries(array('sample_type' => 'uri', 'uri' => $sUri));
        if(!$aEntry || $aEntry['status'] != CH_TD_STATUS_ACTIVE)
            return '';

        $sUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aEntry['uri'];
        $sTitle = $aEntry['caption'];

        ch_import('ChTemplSocialSharing');
        $s = ChTemplSocialSharing::getInstance()->getCode($sUrl, $sTitle);
    ...
    function getSearchContent($aRestrictions, $sUri, $iPage = 1, $iPerPage = 0)
    {
        ...
        if(!empty($sCode)) {
            $oPaginate = new ChWsbPaginate(array(
                'page_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . $sUri . '{page}/{per_page}',
    ...
    {
        $sCategory = uri2title(process_db_input($sCategory, CH_TAGS_STRIP));
        $iPage = (int)$iPage;
    ...
    {
        $sTag = uri2title(process_db_input($sTag, CH_TAGS_STRIP));
        $iPage = (int)$iPage;
        $iPerPage = (int)$iPerPage;
    ...
    {
        $sUri = $this->_oConfig->getUri();
        $oCategories = new ChTemplCategoriesModule(array('type' => $this->_oConfig->getCategoriesSystemName()), _t('_categ_users'), CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'category');
    ...
    {
        $sUri = $this->_oConfig->getUri();
        $oTags = new ChTemplTagsModule(array('type' => $this->_oConfig->getTagsSystemName(), 'orderby' => 'popular'), _t('_' . $sUri . '_bcaption_all_tags'), CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'tags');
    ...
    function serviceGetCalendarBlock($iBlockID, $aParams = array())
    {
        ...
        $sDynamicUrl = isset($aParams['dynamic_url']) ? $aParams['dynamic_url'] : CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'index/';
    ...
    function serviceGetSubscriptionParams($sUnit, $sAction, $iObjectId)
    {
        ...
        return array(
            'template' => array(
                ...
                'ViewLink' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri()  . 'view/' . $aItem['uri']
            )
        );
    }
     ...
     * Action methods
     */
    function actionRss($iLength = 0, $sType = 'archive')
    {
        ...
        $aRssData = array();
        $sRssViewUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/';
        ...
        if($sType == 'archive') {
          ...
          return $oRss->GenRssByData($aRssData, _t('_news_rss_caption'), $this->_oConfig->getBaseUri() . 'act_rss/' . $iLength . '/' . $sType . '/');
        }
    }
    function actionGetEntries($sSampleType = 'all', $iStart = 0, $iPerPage = 0)
    {
        ...
        return $this->_oTemplate->displayList(array(
            ...
            'start' => $iStart,
            'count' => $iPerPage,
            'filter_value' => isset($_POST['filter_value']) ? process_db_input($_POST['filter_value'], CH_TAGS_STRIP) : ''
        ));
    }
	...
	function actionSharePopup($iEntry)
    {
        ...
        $sEntryUrl = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aEntry['uri'];
    ...
    function actionCalendar($iYear = 0, $iMonth = 0, $iDay = 0, $iStart = 1, $iPerPage = 0)
    {
        ...
        $sBlockTitle = !$iDay
            ...
            : _t('_' . $sUri . '_bcaption_calendar_browse') . ': ' . getLocaleDate(
                strtotime("{$iYear}-{$iMonth}-{$iDay}"), CH_WSB_LOCALE_DATE_SHORT);
    ...
    {
        $sUri = $this->_oConfig->getUri();
        $oCategories = new ChTemplCategoriesModule(array('type' => $this->_oConfig->getCategoriesSystemName()), _t('_categ_users'), CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories');
    ...
    {
        $sUri = $this->_oConfig->getUri();
        $sBaseUri = CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri();
    ...
    {
        $sUri = $this->_oConfig->getUri();
        $oPage = ch_instance($this->_oConfig->getClassPrefix() . 'PageMain', array($this), $this->_aModule);
    ...
    {
        $sUri = $this->_oConfig->getUri();
        $oPage = ch_instance($this->_oConfig->getClassPrefix() . 'PageView', array($sName, $this), $this->_aModule);
    ...
    function actionAdmin($sName = '')
    {
        ...
        $sPostForm = '';
        if(!empty($sName))
            $sPostForm = $this->serviceEditBlock(process_db_input($sName, CH_TAGS_STRIP));
        ...
        if(isset($_GET[$sUri . '-filter']))
            $sFilterValue = process_db_input($_GET[$sUri . '-filter'], CH_TAGS_STRIP);
    ...
    function actionMobileLatestEntries($iPage = 1)
    {
        ...
        foreach ($aEntries as $aEntry) {
            $aVars = array (
                'content' => '<h2>' . $aEntry['caption'] . '</h2>' . getLocaleDate($aEntry['when_uts'], CH_WSB_LOCALE_DATE),
                'url' => ch_js_string(CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'mobile_entry/' . $aEntry['id']),
            );
        ...
        ch_import('ChWsbPaginate');
        $oPaginate = new ChWsbPaginate(array(
            'page_url' => CH_WSB_URL_ROOT . $this->_oConfig->getBaseUri() . 'mobile_latest_entries/{page}',
            'count' => $iTotalCount,
            'per_page' => $iPerPage,
            'page' => $iPage,
        ));
    ...
    function actionMobileEntry($iId)
    {
        ...
        echo getLocaleDate($aEntry['when_uts'], CH_WSB_LOCALE_DATE);
        echo $aEntry['content'];
    ...
    function _actFeatured($aIds, $bPositive = true)
    {
        ...
        if($bResult)
            foreach($aIds as $iId) {
                ...
                ch_import('ChWsbAlerts');
                $oAlert = new ChWsbAlerts($this->_oConfig->getAlertsSystemName(), 'featured', $iId, ChWsbTextData::getAuthorId());
                $oAlert->alert();
                //--- Entry -> Featured for Alerts Engine ---//
            }
    ...
    function _actPublish($aIds, $bPositive = true)
    {
        ...
        $bResult = $this->_oDb->updateEntry($aIds, array('status' => ($bPositive ? CH_TD_STATUS_ACTIVE : CH_TD_STATUS_INACTIVE)));
        if($bResult)
            foreach($aIds as $iId) {
                //--- Entry -> Publish/Unpublish for Alerts Engine ---//
                $oAlert = new ChWsbAlerts($this->_oConfig->getAlertsSystemName(), ($bPositive ? 'publish' : 'unpublish'), $iId, ChWsbTextData::getAuthorId());
                $oAlert->alert();
                //--- Entry -> Publish/Unpublish for Alerts Engine ---//
    ...
    function _actDelete($aIds)
    {
        ...
        if($bResult) {
            ...
            foreach($aIds as $iId) {
                ...
                $oAlert = new ChWsbAlerts($this->_oConfig->getAlertsSystemName(), 'delete', $iId, ChWsbTextData::getAuthorId());
                $oAlert->alert();
                //--- Entry -> Delete for Alerts Engine ---//
