...
function ActionChangeStatusMessage($iMemberIdForce = 0)
{
    ...
    if( $iMemberID && isset($_POST['status_message']) ) {
        ...
        if( db_res($sQuery, 0) ) {
            ...
            $oZ = new ChWsbAlerts('profile', 'edit_status_message'
...
function ActionChangeStatus($iProfileId, $sStatus = '')
{
    ...
    if ( $oUserStatus -> getRegisteredStatus($sStatus) ) {
        //process status
        $sStatus = process_db_input($sStatus, CH_TAGS_STRIP, CH_SLASHES_AUTO);
        ...
        if( db_res($sQuery, 0) ) {
            // send system event
            ch_import('ChWsbAlerts');
            $oZ = new ChWsbAlerts('profile', 'edit_status', $iProfileId, $iProfileId);
            $oZ -> alert();
...
function PageListFriendRemove($iProfileId, $iMemberId = 0)
{
    ...
    $oCommunicator = new ChTemplCommunicator(array('member_id' => $iProfileId));
...
function PageListFriend($iProfileId, $iMemberId = 0)
{
    ...
    if($aFriendsInfo) {
        if( isset($aFriendsInfo['Check']) && $aFriendsInfo['Check'] == 1) {
            ...
            if ($iProfileId == $aFriendsInfo['ID'] && $aFriendsInfo['Check'] == 0) {
                ...
                if ( db_res($sQuery, 0) ) {
                    ...
                    //send system alert
                    ch_import('ChWsbAlerts');
                    $oZ = new ChWsbAlerts('friend', 'accept', $iMemberId, $iProfileId);
                    $oZ->alert();
                } else {
        ...
        if ( db_res($sQuery, 0) ) {
            ...
            //send system alert
            ch_import('ChWsbAlerts');
            $oZ = new ChWsbAlerts('friend', 'request', $iMemberId, $iProfileId);
            $oZ -> alert();
...
function PageListHot($iId, $iProfileId = 0)
{
    ...
    if((int)$GLOBALS['MySQL']->query($sQuery) > 0) {
        ...
        //send system alert
        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('fave', 'add', $iProfileId, $iId);
        $oZ -> alert();
    } else
        $sOutputCode = MsgBox(_t('_Failed to apply changes'));
...
function PageListHotRemove($iId, $iProfileId = 0)
{
    ...
    if((int)$GLOBALS['MySQL']->query($sQuery) > 0) {
        ...
        //send system alert
        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('fave', 'remove', $iProfileId, $iId);
        $oZ -> alert();
    } else
        $sOutputCode = MsgBox(_t('_Failed to apply changes'));
...
function PageListUnBlock($iProfileId, $iMemberId = 0)
{
    ...
    ch_import('ChWsbCommunicator');
    $oCommunicator = new ChWsbCommunicator(array('member_id' => $iMemberId));
...
function PageListBlock($iProfileId, $iMemberId = 0)
{
    ...
    if( db_res($sQuery, 0) ) {
        ...
        // send system alert
        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('block', 'add', $iMemberId, $iProfileId);
        $oZ->alert();
...
function PageListMemberMenuSettings($iProfileId, $sAction)
{
    ...
    $aForm = array (
        'form_attrs' => array (
            'action' => CH_WSB_URL_ROOT . 'list_pop.php?action=' . clear_xss($sAction),
            'method' => 'post',
    ...
    if (isAdmin($iViewerId) OR (isModerator($iViewerId) AND $iViewerId != $iTargetId))
    {
        switch ($sAction)
        {
            ...
            case 'deactivate':
                ...
                break;
            case 'ban':
                if (ch_admin_profile_ban_control($iTargetId))
                    $sMsg = '_Success';
                $mixedRes = MsgBox(_t($sMsg));
                break;
            case 'unban':
                if (ch_admin_profile_ban_control($iTargetId, FALSE))
                    $sMsg = '_Success';
                $mixedRes = MsgBox(_t($sMsg));
                break;
            case 'featured':
            ...
            case 'delete':
                ...
                $mixedRes = MsgBox(_t('_Success')) . genAjaxyPopupJS($iTargetId, 'ajaxy_popup_result_div', CH_WSB_URL_ROOT . 'browse.php');
                break;
            case 'delete_spam':
...
function _setStatus($iTargetId, $sAction)
{
    ...
    if (ch_admin_profile_change_status($iTargetId, $sStatus, $bSendActMail))
        $sMsg = '_Success';
...
function _setFeature($iTargetId, $sType)
{
    ...
    $sMsg = '_Error';
    if (ch_admin_profile_featured_control($iTargetId, $bFeature))
        $sMsg = '_Success';
    return MsgBox(_t($sMsg));
}
