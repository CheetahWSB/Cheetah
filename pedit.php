...
class ChWsbPEditProcessor extends ChWsbPageView
{
    ...
    function __construct()
    {
        ...
        $this -> bForceAjaxSave = ch_get('force_ajax_save');
    ...
    function getBlockCode_Info()
    {
        ...
        /* @var $this->oPC ChWsbProfilesController */
        $this -> oPC = new ChWsbProfilesController();
        ...
        /* @var $this->oPF ChWsbProfileFields */
        $this -> oPF = new ChWsbProfileFields( $this -> iArea );
        if( !$this -> oPF -> aArea )
    ...
    function getBlockCode_Privacy()
    {
        ...
        $this->aFormPrivacy = array(
            'form_attrs' => array(
                ...
                'action' => CH_WSB_URL_ROOT . 'pedit.php?ID=' . $this->iProfileID,
                'method' => 'post',
        ...
        $oForm = new ChTemplFormView($this->aFormPrivacy);
        $oForm->initChecker();
        ...
        if($oForm->isSubmittedAndValid()) {
            ...
            if ((int)db_res("UPDATE `Profiles` SET `allow_view_to`='" . $iAllowViewTo . "' WHERE `ID`='" . $iProfileId . "' LIMIT 1") > 0) {
                ...
                if (ChWsbModule::getInstance('ChWmapModule'))
                    ChWsbService::call('wmap', 'response_entry_change', array('profiles', $iProfileId));

                // create system event
                ch_import('ChWsbAlerts');
                $oZ = new ChWsbAlerts('profile', 'edit', $iProfileId, 0, array('OldProfileInfo' => $aProfileInfo, 'privacy' => $iAllowViewTo));
                $oZ->alert();
            }
        }
    ...
    function getBlockCode_Membership()
    {
        ...
        $oForm = new ChTemplFormView($this->aFormMembership);
        $oForm->initChecker();
...
<?php
    ...
    function saveProfile()
    {
        ...
        if($bResult && $this -> iProfileID == getLoggedId() && isset($aUpd['Password']))
        	ch_login($this -> iProfileID, false, false);

        if (ChWsbModule::getInstance('ChWmapModule'))
            ChWsbService::call('wmap', 'response_entry_change', array('profiles', $this->iProfileID));

        // create system event
        ch_import('ChWsbAlerts');
        $oZ = new ChWsbAlerts('profile', 'edit',  $this -> iProfileID, 0, array('OldProfileInfo' => $aProfileInfo) );
        $oZ->alert();
...
if (!(isAdmin() || isModerator() || (isLogged() && getLoggedId() == ch_get('ID')))) {
...
$oEditProc = new ChWsbPEditProcessor();
