...
if (isset($_GET['action']) && $_GET['action']=='get_prof_status_mess') {
    if ($iLoggedID) {
        ...
        echo ChWsbUserStatusView::getStatusPageLight($iLoggedID);
    }
    exit;
}
...
if ($sAction!== false && $sAction=='get_prof_comment_block') {
    ...
    if ($iProfileID) {
        ...
        $oCmtsView = new ChTemplCmtsView ('profile', $iProfileID);
...
function PageCompPageMainCode($iLoggedID)
{
    ...
    if ( $_POST['CHANGE_STATUS'] ) {
        ...
        if (!empty($sStatus))
            ch_admin_profile_change_status ($member['ID'], $sStatus);
    ...
    $aForm = array(
        'form_attrs' => array (
            'action' =>  CH_WSB_URL_ROOT . 'change_status.php',
            'method' => 'post',
