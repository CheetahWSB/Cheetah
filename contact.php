...
function PageCompPageMainCodeWithForm()
{
    ...
    $aForm = array(
        'form_attrs' => array(
            'id' => 'post_us_form',
            'action' => CH_WSB_URL_ROOT . 'contact.php',
            'method' => 'post',
        ),
        'params' => array (
            'db' => array(
    ...
    if ( $oForm->isSubmittedAndValid() ) {
        $sSenderName	= process_pass_data($_POST['name'], CH_TAGS_STRIP);
        $sSenderEmail	= process_pass_data($_POST['email'], CH_TAGS_STRIP);
        $sLetterSubject = process_pass_data($_POST['subject'], CH_TAGS_STRIP);
        $sLetterBody	= process_pass_data($_POST['body'], CH_TAGS_STRIP);
